"""
Embedded Entelechy client with automatic daemon lifecycle management.

This module provides EntelechyEmbedded, a client that uses the same daemon
management interface as entelechy-embed CLI, ensuring full compatibility.
...
Using context manager:
    ...
    with EntelechyEmbedded(profile="myapp") as client:
        client.retain(bank_id="alice", content="Alice loves AI")
...
class EntelechyEmbedded:
    """
    Entelechy client with automatic daemon lifecycle management.

    This client uses the same daemon management interface as entelechy-embed CLI,
    ...
    Profile data is stored in: ~/.pg0/instances/entelechy-embed-{profile}/

    All methods from EntelechyClient are available:
    - retain(), retain_batch()
    - recall()
    - reflect()
    ...
    def __init__(
        ...
        self.config = {
            ...
            "ENTELECHY_API_LLM_API_KEY": llm_api_key,
            "ENTELECHY_API_LLM_MODEL": llm_model,
            "ENTELECHY_API_LOG_LEVEL": log_level,
            "ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT": str(idle_timeout),
        }

        if llm_base_url:
            self.config["ENTELECHY_API_LLM_BASE_URL"] = llm_base_url

        if database_url:
            self.config["ENTELECHY_EMBED_API_DATABASE_URL"] = database_url

        self._ui = ui
        self._ui_port = ui_port
        self._ui_hostname = ui_hostname

        self._client: Optional[Entelechy] = None
    ...
    def _ensure_started(self):
        ...
        with self._lock:
            ...
            if self._closed:
                raise RuntimeError(
                    "Cannot use EntelechyEmbedded after it has been closed"
                )
            ...
            self._client = Entelechy(base_url=daemon_url)
            self._started = True
    ...
    def __getattr__(self, name: str):
        """
        Proxy all method calls to the underlying Entelechy client.

        This allows EntelechyEmbedded to expose all EntelechyClient methods
    ...
    def banks(self) -> BanksAPI:
        ...
        Example:
            ...
            embedded = EntelechyEmbedded(profile="myapp", ...)
    ...
    @property
    def client(self) -> Entelechy:
        """
        Get the underlying Entelechy client for direct access.

        Ensures daemon is started (and restarts it if it has crashed) before
        returning the client.

        Returns:
            Entelechy: The underlying client instance
