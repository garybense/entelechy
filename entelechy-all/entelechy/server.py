"""
Server module for running Entelechy in a background thread.

Provides a simple way to start and stop the Entelechy HTTP API server
...
class Server:
    """
    Entelechy server that runs in a background thread.

    Example:
        ```python
        from entelechy import Server
    ...
    def __init__(
        ...
        """
        Initialize the Entelechy server.
        ...
        while time.time() - start_time < timeout:
            try:
                with socket.create_connection((self.host, self.port), timeout=1):
                    logger.info(f"Entelechy server started at {self.url}")
                    return self
    ...
    def stop(self, timeout: float = 10.0) -> None:
        ...
        self._server = None
        self._thread = None
        logger.info("Entelechy server stopped")
...
def start_server(
    ...
    """
    Start a Entelechy server in a background thread.
    ...
    Example:
        ```python
        from entelechy import start_server, Client
