"""
Integration test for Entelechy server with context manager.
...
def llm_config():
    ...
    provider = os.getenv("ENTELECHY_LLM_PROVIDER", "groq")
    api_key = os.getenv("ENTELECHY_LLM_API_KEY", "")
    model = os.getenv("ENTELECHY_LLM_MODEL", "openai/gpt-oss-120b")

    # vertexai uses GCP service account credentials (ENTELECHY_API_LLM_VERTEXAI_*),
    ...
    if not api_key and provider not in providers_without_api_key:
        raise Exception("LLM API key not configured. Set ENTELECHY_LLM_API_KEY environment variable.")
...
def shared_server(llm_config):
    ...
    """
    server = EntelechyServer(db_url="pg0", **llm_config)
    server.start()
    yield server
    server.stop()
...
def client(shared_server):
    ...
    return EntelechyClient(base_url=shared_server.url)
