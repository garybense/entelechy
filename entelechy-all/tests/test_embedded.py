"""
Integration tests for EntelechyEmbedded client.
...
4. Method proxying to underlying EntelechyClient
5. Proper cleanup
...
def llm_config():
    ...
    provider = os.getenv("ENTELECHY_API_LLM_PROVIDER") or os.getenv(
        "ENTELECHY_LLM_PROVIDER", "groq"
    )
    api_key = os.getenv("ENTELECHY_API_LLM_API_KEY") or os.getenv(
        "ENTELECHY_LLM_API_KEY", ""
    )
    model = os.getenv("ENTELECHY_API_LLM_MODEL") or os.getenv(
        "ENTELECHY_LLM_MODEL", "openai/gpt-oss-120b"
    )

    if not api_key:
        pytest.skip(
            "LLM API key not configured. Set ENTELECHY_API_LLM_API_KEY or ENTELECHY_LLM_API_KEY."
        )
...
def test_embedded_lazy_start(llm_config):
    """
    Test that EntelechyEmbedded starts server lazily on first use.
    """
    profile = f"test_lazy_{uuid.uuid4().hex[:8]}"
    bank_id = f"bank_{uuid.uuid4().hex[:8]}"

    # Create client - should NOT start server yet
    client = EntelechyEmbedded(profile=profile, log_level="info", **llm_config)
...
def test_embedded_context_manager(llm_config):
    """
    Test EntelechyEmbedded with context manager.
    """
    profile = f"test_ctx_{uuid.uuid4().hex[:8]}"
    bank_id = f"bank_{uuid.uuid4().hex[:8]}"

    # Use context manager
    with EntelechyEmbedded(profile=profile, log_level="info", **llm_config) as client:
...
def test_embedded_complete_workflow(llm_config):
    """
    Test complete workflow with EntelechyEmbedded.
...
def test_embedded_method_proxying(llm_config):
    """
    Test that all EntelechyClient methods are properly proxied.
...
def test_embedded_multiple_banks(llm_config):
    """
    Test that EntelechyEmbedded can work with multiple banks.
    """
...
def test_embedded_profile_isolation(llm_config):
    ...
    client1 = EntelechyEmbedded(profile=profile1, log_level="info", **llm_config)
    client2 = EntelechyEmbedded(profile=profile2, log_level="info", **llm_config)
...
def test_embedded_error_after_close(llm_config):
    """
    Test that using EntelechyEmbedded after close() raises an error.
    """
    ...
    with pytest.raises(
        RuntimeError, match="Cannot use EntelechyEmbedded after it has been closed"
    ):
...
def test_embedded_ui_flag(llm_config):
    ...
    client = EntelechyEmbedded(profile=profile, log_level="info", ui=True, **llm_config)
...
def test_embedded_daemon_crash_recovery(llm_config):
    """
    Test that EntelechyEmbedded recovers when the daemon crashes.
