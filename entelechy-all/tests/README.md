...
This directory contains integration tests for the Entelechy all-in-one package.
...
1. **`test_server_context_manager_basic_workflow`**: Main integration test that:
   - Starts the Entelechy server using a context manager
...
1. Install the entelechy package with test dependencies:
   ```bash
   cd entelechy
...
2. Set up your LLM credentials in the `.env` file at the project root:
   ...
   ENTELECHY_API_LLM_MODEL=openai/gpt-oss-20b
   ```
...
cd entelechy
source ../.env
export ENTELECHY_LLM_PROVIDER=$ENTELECHY_API_LLM_PROVIDER
export ENTELECHY_LLM_API_KEY=$ENTELECHY_API_LLM_API_KEY
export ENTELECHY_LLM_MODEL=$ENTELECHY_API_LLM_MODEL
pytest tests/ -v
```
...
- `ENTELECHY_LLM_API_KEY` is not set
