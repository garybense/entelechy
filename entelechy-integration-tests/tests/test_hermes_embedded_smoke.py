"""Smoke test for the Hermes Agent ↔ Entelechy integration in embedded mode.

Drives the `EntelechyMemoryProvider` plugin shipped with Hermes Agent against
a locally-spawned Entelechy Embedded daemon, exercising the full retain →
recall roundtrip end-to-end.

Run on demand via the installed Hermes venv (it already has every dep —
hermes-agent's plugin code, entelechy_embed, entelechy_client, pytest):

    ENTELECHY_LLM_API_KEY=... \
        ~/.hermes/hermes-agent/venv/bin/python -m pytest \
        entelechy-integration-tests/tests/test_hermes_embedded_smoke.py -v -s

Skipped automatically if `ENTELECHY_LLM_API_KEY` (or `OPENAI_API_KEY`) is not
set, since embedded mode needs an LLM to extract facts during retain.

Defaults: openai / gpt-4o-mini. Override via `ENTELECHY_LLM_PROVIDER` and
`ENTELECHY_LLM_MODEL`.
"""
...
LLM_API_KEY = os.environ.get("ENTELECHY_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY", "")
...
pytestmark = [
    pytest.mark.skipif(
        not LLM_API_KEY,
        reason="ENTELECHY_LLM_API_KEY (or OPENAI_API_KEY) not set",
    ),
    pytest.mark.skipif(
        not (HERMES_VENV_SITE / "plugins" / "memory" / "entelechy" / "__init__.py").exists(),
...
def embedded_provider(tmp_path, monkeypatch, hermes_path):
    """Spin up a EntelechyMemoryProvider in local_embedded mode.

    Uses a temp HERMES_HOME so we never touch the user's real ~/.hermes.
    The Entelechy daemon stores its data under that temp dir too.
    """
    profile_name = f"hermes-smoke-{uuid.uuid4().hex[:8]}"
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    monkeypatch.setenv("ENTELECHY_LLM_API_KEY", LLM_API_KEY)

    config_dir = tmp_path / "entelechy"
    ...
    from plugins.memory.entelechy import EntelechyMemoryProvider

    provider = EntelechyMemoryProvider()
    ...
    while time.time() < deadline:
        ...
        provider.shutdown()
        pytest.fail(f"Entelechy embedded daemon did not start within 240s (last error: {last_err!r})")
