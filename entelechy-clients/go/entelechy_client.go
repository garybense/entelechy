...
func defaultUserAgent() string {
	...
	if info, ok := debug.ReadBuildInfo(); ok {
		for _, dep := range info.Deps {
			if dep.Path == "github.com/vectorize-io/entelechy/entelechy-clients/go" {
				version = dep.Version
				break
			}
		}
	}
	return "entelechy-client-go/" + version
}
...
// Example:
//
//	client := entelechy.NewAPIClientWithToken("https://api.example.com", "your-api-token")
...
// Example:
//
//	client := entelechy.NewAPIClientWithTimeout("https://api.example.com", "your-api-token", 30*time.Second)
//	resp, _, err := client.MemoryAPI.RetainMemories(ctx, bankID).RetainRequest(req).Execute()
