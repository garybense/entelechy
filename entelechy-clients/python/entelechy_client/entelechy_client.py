"""
Clean, pythonic wrapper for the Entelechy API client.
...
try:
    _CLIENT_VERSION = metadata.version("entelechy-client")
except metadata.PackageNotFoundError:
    _CLIENT_VERSION = "0.0.0"

DEFAULT_USER_AGENT = f"entelechy-client-python/{_CLIENT_VERSION}"
from entelechy_client_api.api import (
    banks_api,
    directives_api,
    documents_api,
    entities_api,
...
class Entelechy:
    """
    High-level, easy-to-use Entelechy API client.
    ...
    **Prefer the async variants** (``aretain``, ``arecall``, ``areflect``, etc.)
    ...
    Example — async (preferred)::
        ...
        client = Entelechy(base_url="http://localhost:8888", api_key="your-api-key")
    ...
    def __init__(
        ...
        """
        Initialize the Entelechy client.

        Args:
            base_url: The base URL of the Entelechy API server
            ...
            user_agent: Override the default ``User-Agent`` header. Integrations
                ...
                ``"entelechy-crewai/1.2.0"``). Defaults to
                ``entelechy-client-python/<version>``.
        """
        config = entelechy_client_api.Configuration(host=base_url, access_token=api_key)
        self._api_client = entelechy_client_api.ApiClient(config)
    ...
    async def arecall(
        ...
        if tag_groups is not None:
            from entelechy_client_api.models.recall_request_tag_groups_inner import RecallRequestTagGroupsInner
    ...
    def create_mental_model(
        ...
        """
        from entelechy_client_api.models import create_mental_model_request, mental_model_trigger_input
    ...
    def update_mental_model(
        ...
        Returns:
            MentalModelResponse
        """
        from entelechy_client_api.models import mental_model_trigger_input, update_mental_model_request
    ...
    def create_directive(
        ...
        Returns:
            DirectiveResponse
        """
        from entelechy_client_api.models import create_directive_request
    ...
    def update_directive(
        ...
        Returns:
            DirectiveResponse
        """
        from entelechy_client_api.models import update_directive_request
    ...
    def get_bank_config(self, bank_id: str) -> dict[str, Any]:
        ...
        Can be disabled on the server by setting ``ENTELECHY_API_ENABLE_BANK_CONFIG_API=false``.
