//! Entelechy API Client
//!
//! A Rust client library for the Entelechy semantic memory system API.
//!
//! # Example
//!
//! ```rust,no_run
//! use entelechy_client::Client;
//!
//! #[tokio::main]
...
include!(concat!(env!("OUT_DIR"), "/entelechy_client_generated.rs"));
...
pub const DEFAULT_USER_AGENT: &str = concat!("entelechy-client-rust/", env!("CARGO_PKG_VERSION"));

/// Build a [`reqwest::Client`] with the given `User-Agent` header.
///
/// Integrations should use this to identify themselves (e.g.
/// `"entelechy-cli/0.6.2"`) so self-hosted deployments behind Cloudflare or
...
/// the bare `Client::new` when pointing at a hosted Entelechy deployment.
...
/// Construct a [`Client`] with the default Entelechy `User-Agent`.
///
...
mod tests {
    ...
    async fn test_memory_lifecycle() {
        let api_url = std::env::var("ENTELECHY_API_URL")
