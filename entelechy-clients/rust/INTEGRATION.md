...
pub struct ApiClient {
    client: entelechy_client::Client,
...
impl ApiClient {
    pub fn new(base_url: String) -> Result<Self> {
        ...
        let client = entelechy_client::Client::new(&base_url);
        Ok(ApiClient { client, runtime })
    }

    pub fn list_agents(&self) -> Result<Vec<entelechy_client::types::AgentListItem>> {
...
async fn main() -> Result<()> {
    let cli = Cli::parse();
    let client = entelechy_client::Client::new(&config.api_url);
...
The generated types are in `entelechy_client::types::*`. Here's the mapping:
