/**
 * Entelechy Client - Clean, TypeScript SDK for the Entelechy API.
 *
 * Example:
 * ```typescript
...
export const DEFAULT_USER_AGENT = `entelechy-client-typescript/${CLIENT_VERSION}`;

export interface EntelechyClientOptions {
  baseUrl: string;
  /**
   ...
   * identify themselves (e.g. `"entelechy-ai-sdk/1.2.0"`). Browsers ignore
   * attempts to set `User-Agent`; this only takes effect in Node.js / Bun /
   * Deno runtimes. Defaults to `entelechy-client-typescript/<version>`.
   */
  userAgent?: string;
}

/**
 * Error thrown by the Entelechy client when an API request fails.
 * Includes the HTTP status code and error details from the API.
 */
export class EntelechyError extends Error {
  public statusCode?: number;
  public details?: unknown;

  constructor(message: string, statusCode?: number, details?: unknown) {
    super(message);
    this.name = "EntelechyError";
...
export class EntelechyClient {
  private client: Client;

  constructor(options: EntelechyClientOptions) {
 ...
 *
 * Mirrors the format used internally by Entelechy's reflect operation.
 */
