//! End-to-end tests for the `entelechy profile` CRUD subcommands and the
//! global `-p/--profile` flag.
//!
//! These tests do not require a running Entelechy API server: they exercise
...
//! API is the `-p` precedence test, which uses `entelechy version` pointed at
...
fn unique_tempdir(tag: &str) -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("entelechy-profile-test-{}-{}-{}-{}", tag, pid, nanos, n));
    std::fs::create_dir_all(&dir).unwrap();
    dir
}

fn entelechy_binary() -> String {
    std::env::var("CARGO_BIN_EXE_entelechy").unwrap_or_else(|_| {
        let debug = "./target/debug/entelechy";
        let release = "./target/release/entelechy";
...
fn run_with_home(home: &std::path::Path, args: &[&str]) -> Output {
    Command::new(entelechy_binary())
        ...
        .env_remove("ENTELECHY_API_URL")
        .env_remove("ENTELECHY_API_KEY")
        .env_remove("ENTELECHY_PROFILE")
        .args(args)
        .output()
        .expect("failed to spawn entelechy binary")
}
...
fn profile_create_writes_toml_and_json_output() {
    ...
    let path = home.join(".entelechy/cli-profiles/prod.toml");
...
fn profile_show_missing_profile_errors_with_hint() {
    ...
    assert!(err.contains("entelechy profile create nope"));
}
...
fn p_flag_overrides_config_file_api_url() {
    // Build a HOME that contains BOTH a legacy ~/.entelechy/config pointing
    // at URL_A and a named profile pointing at URL_B. Running `entelechy -p`
    ...
    let entelechy_dir = home.join(".entelechy");
    std::fs::create_dir_all(&entelechy_dir).unwrap();
    std::fs::write(
        entelechy_dir.join("config"),
...
#[test]
fn entelechy_profile_env_var_is_honored() {
    ...
    // Re-run without `-p` but with ENTELECHY_PROFILE set.
    let out = Command::new(entelechy_binary())
        .env("HOME", &home)
        .env_remove("ENTELECHY_API_URL")
        .env_remove("ENTELECHY_API_KEY")
        .env("ENTELECHY_PROFILE", "staging")
        .args(["version"])
        .output()
        .unwrap();
    assert!(!out.status.success());
