//! Integration tests for the entelechy CLI commands.
//!
//! These tests require a running entelechy API server.
//! Set ENTELECHY_API_URL environment variable to point to the server.
...
fn server_available() -> bool {
    let api_url = env::var("ENTELECHY_API_URL").unwrap_or_else(|_| "http://localhost:8080".to_string());
...
/// Get the path to the entelechy binary
fn entelechy_binary() -> String {
    env::var("CARGO_BIN_EXE_entelechy")
        .unwrap_or_else(|_| {
            // Try common locations
            let target_debug = "./target/debug/entelechy";
            let target_release = "./target/release/entelechy";
...
/// Run a entelechy CLI command
fn run_entelechy(args: &[&str]) -> std::process::Output {
    let api_url = env::var("ENTELECHY_API_URL").unwrap_or_else(|_| "http://localhost:8080".to_string());

    Command::new(entelechy_binary())
        .env("ENTELECHY_API_URL", &api_url)
        .args(args)
        .output()
        .expect("Failed to execute entelechy command")
}
...
fn test_health_check() {
    ...
    let output = run_entelechy(&["health"]);
...
fn test_health_check_json_output() {
    ...
    let output = run_entelechy(&["health", "-o", "json"]);
...
fn test_bank_list() {
    ...
    let output = run_entelechy(&["bank", "list"]);
...
fn test_bank_list_json_output() {
    ...
    let output = run_entelechy(&["bank", "list", "-o", "json"]);
...
fn test_bank_create_and_delete() {
    ...
    let output = run_entelechy(&["bank", "disposition", &bank_id]);
    if created {
        assert!(
            output.status.success(),
    ...
    let output = run_entelechy(&["bank", "delete", &bank_id, "-y"]);
...
fn test_memory_list() {
    ...
    let _ = run_entelechy(&["bank", "create", &bank_id, "--name", "Test Bank"]);

    // List memories (should be empty for new bank)
    let output = run_entelechy(&["memory", "list", &bank_id]);
    ...
    // Clean up
    let _ = run_entelechy(&["bank", "delete", &bank_id, "-y"]);
}
...
fn test_mental_model_list() {
    ...
    // List mental models
    let output = run_entelechy(&["mental-model", "list", &bank_id]);
...
fn test_mental_model_create_and_delete() {
    ...
    let output = run_entelechy(&["mental-model", "list", &bank_id, "-o", "json"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_tag_list() {
    ...
    // List tags
    let output = run_entelechy(&["tag", "list", &bank_id]);
...
fn test_entity_list() {
    ...
    // List entities
    let output = run_entelechy(&["entity", "list", &bank_id]);
...
fn test_operation_list() {
    ...
    // List operations
    let output = run_entelechy(&["operation", "list", &bank_id]);
...
fn test_bank_stats() {
    ...
    // Get stats
    let output = run_entelechy(&["bank", "stats", &bank_id]);
...
fn test_bank_graph() {
    ...
    // Get graph
    let output = run_entelechy(&["bank", "graph", &bank_id]);
...
fn test_bank_update() {
    ...
    // Create the bank first
    let output = run_entelechy(&["bank", "create", &bank_id, "--name", "Test Bank"]);
    ...
    if output.status.success() {
        ...
        // Verify the update
        let output = run_entelechy(&["bank", "disposition", &bank_id, "-o", "json"]);
        if output.status.success() {
            let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_json_yaml_output_formats() {
    ...
    let output = run_entelechy(&["bank", "list", "-o", "yaml"]);
    if output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_directive_list() {
    ...
    // List directives
    let output = run_entelechy(&["directive", "list", &bank_id]);
...
fn test_directive_create_get_update_delete() {
    ...
    let output = run_entelechy(&["directive", "list", &bank_id, "-o", "json"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    if let Some(id) = directive_id {
        // Get the directive
        let output = run_entelechy(&["directive", "get", &bank_id, &id]);
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        ...
        let output = run_entelechy(&["directive", "get", &bank_id, &id, "-o", "json"]);
        if output.status.success() {
            let stdout = String::from_utf8_lossy(&output.stdout);
            let result: serde_json::Value = serde_json::from_str(&stdout).unwrap();
            assert_eq!(
        ...
        let output = run_entelechy(&["directive", "delete", &bank_id, &id, "-y"]);
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
    ...
    if output.status.success() {
        ...
        if let Ok(result) = serde_json::from_str::<serde_json::Value>(&stdout) {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                let output = run_entelechy(&["mental-model", "get", &bank_id, id]);
                let stdout = String::from_utf8_lossy(&output.stdout);
                let stderr = String::from_utf8_lossy(&output.stderr);
    ...
    if output.status.success() {
        ...
        if let Ok(result) = serde_json::from_str::<serde_json::Value>(&stdout) {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                // Verify update
                let output = run_entelechy(&["mental-model", "get", &bank_id, id, "-o", "json"]);
                if output.status.success() {
                    let stdout = String::from_utf8_lossy(&output.stdout);
                    let result: serde_json::Value = serde_json::from_str(&stdout).unwrap();
                    assert_eq!(
    ...
    if output.status.success() {
        ...
        if let Ok(result) = serde_json::from_str::<serde_json::Value>(&stdout) {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                let output = run_entelechy(&["mental-model", "refresh", &bank_id, id]);
                let stdout = String::from_utf8_lossy(&output.stdout);
                let stderr = String::from_utf8_lossy(&output.stderr);
...
fn test_bank_consolidate() {
    ...
    let output = run_entelechy(&["bank", "consolidate", &bank_id]);
...
fn test_bank_clear_observations() {
    ...
    // Clear observations
    let output = run_entelechy(&["bank", "clear-observations", &bank_id, "-y"]);
...
fn test_version() {
    ...
    let output = run_entelechy(&["version"]);
...
fn test_version_json() {
    ...
    let output = run_entelechy(&["version", "-o", "json"]);
