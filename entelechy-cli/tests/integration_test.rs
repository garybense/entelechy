...
fn test_cli_help() {
    ...
    assert!(stdout.contains("Entelechy CLI"));
}
...
fn test_cli_version() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("entelechy"));
}
...
fn test_ui_command_without_config() {
    ...
    let temp_dir = std::env::temp_dir().join(format!("entelechy-test-ui-{}", std::process::id()));
    ...
    assert!(
        ...
        || stdout.contains("Launching Entelechy Control Plane UI")
        || stderr.contains("Configuration error")
        || stderr.contains("ENTELECHY_API_URL"),
...
fn test_ui_command_with_config() {
    ...
    // 1. Build the control plane: cd entelechy-control-plane && npm run build
    // 2. Run: cargo test test_ui_command_with_config -- --ignored

    // Just verify that the ui command accepts the configuration
    let temp_dir = std::env::temp_dir().join(format!("entelechy-test-ui-valid-{}", std::process::id()));
    std::fs::create_dir_all(&temp_dir).expect("Failed to create temp dir");

    // Write a minimal config
    let config_dir = temp_dir.join(".config").join("entelechy");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Entelechy CLI") || output.status.success());
...
fn test_configure_command() {
    ...
    let temp_dir = std::env::temp_dir().join(format!("entelechy-test-{}", std::process::id()));
    std::fs::create_dir_all(&temp_dir).expect("Failed to create temp dir");
