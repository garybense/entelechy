...
#[derive(Parser)]
#[command(name = "entelechy")]
#[command(about = "Entelechy CLI - Semantic memory system", long_about = None)]
#[command(version)]
...
struct Cli {
    ...
    /// Named profile to load from ~/.entelechy/cli-profiles/<name>.toml
    /// (env var ENTELECHY_PROFILE is used if this flag is omitted).
    /// Environment variables (ENTELECHY_API_URL / ENTELECHY_API_KEY) still override profile values.
    #[arg(short = 'p', long, global = true, env = "ENTELECHY_PROFILE")]
...
fn get_after_help() -> String {
    ...
    format!(
        "Current API URL: {} (from {})\n\nRun 'entelechy configure' to change the API URL.",
        api_url, source
    )
}
...
enum Commands {
    ...
    #[command(
        after_help = "Configuration priority:\n  1. Environment variables (ENTELECHY_API_URL, ENTELECHY_API_KEY) - highest priority\n  2. Named profile (-p / ENTELECHY_PROFILE, see 'entelechy profile')\n  3. Config file (~/.entelechy/config)\n  4. Default (http://localhost:8888)"
    )]
    Configure {
    ...
    /// Manage named connection profiles (~/.entelechy/cli-profiles/<name>.toml)
    #[command(subcommand)]
...
enum ProfileCommands {
    ...
    Create {
        /// Profile name (used with -p/--profile or $ENTELECHY_PROFILE)
        name: String,
        /// API URL (required)
        #[arg(long)]
        api_url: String,
...
fn handle_configure(
    ...
    if output_format == OutputFormat::Pretty {
        ui::print_info("Entelechy CLI Configuration");
        println!();
    ...
    if output_format == OutputFormat::Pretty {
        ...
        println!();
        println!("Note: Environment variables ENTELECHY_API_URL and ENTELECHY_API_KEY will override these settings.");
    } else {
...
fn handle_ui(profile: Option<&str>, output_format: OutputFormat) -> Result<()> {
    ...
    if output_format == OutputFormat::Pretty {
        ui::print_info("Launching Entelechy Control Plane UI...");
        println!();
        println!("  API URL: {}", api_url);
        println!();
    }

    // Run npx @vectorize-io/entelechy-control-plane --api-url {api_url}
    let status = Command::new("npx")
        .arg("@vectorize-io/entelechy-control-plane")
        .arg("--api-url")
        .arg(api_url)
        .status();
    ...
    match status {
        ...
        Err(e) => {
            ...
            ui::print_info("You can also install the control plane globally: npm install -g @vectorize-io/entelechy-control-plane");
            std::process::exit(1);
        }
    }
    ...
    match cmd {
        ProfileCommands::Create {
            ...
            if output_format == OutputFormat::Pretty {
                ...
                println!();
                println!(
                    "Use with: entelechy -p {} <command>  (or export ENTELECHY_PROFILE={})",
                    name, name
                );
            } else {
                let result = serde_json::json!({
        ...
        ProfileCommands::List => {
            ...
            if output_format == OutputFormat::Pretty {
                if names.is_empty() {
                    ...
                    println!();
                    println!("Create one with: entelechy profile create <name> --api-url <url>");
                } else {
