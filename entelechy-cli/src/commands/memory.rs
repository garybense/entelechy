    ...
    let mapped_fact_types = fact_types.as_ref().map(|types| {
        types
            ...
            .filter_map(|t| match t.to_lowercase().as_str() {
                "world" => Some(entelechy_client::types::FactTypesItem::World),
                "experience" => Some(entelechy_client::types::FactTypesItem::Experience),
                "observation" => Some(entelechy_client::types::FactTypesItem::Observation),
                _ => None,
            })
            .collect::<Vec<_>>()
    });
