...
use entelechy_client::types::{BankListItem, RecallResult, EntityListItem, Budget, TagsMatch};
...
fn render_header(f: &mut Frame, app: &App, area: Rect) {
    ...
    let title = format!("Entelechy Explorer - {}{}", app.view.title(), bank_info);
...
fn render_help(f: &mut Frame, area: Rect) {
    let help_text = vec![
        Line::from(Span::styled("Entelechy Explorer - Keyboard Shortcuts", Style::default().fg(BRAND_START).add_modifier(Modifier::BOLD))),
        Line::from(""),
        Line::from(vec![
