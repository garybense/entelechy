...
fn format_error_message(err: &anyhow::Error, api_url: &str) -> String {
    ...
    if err_str.contains("Connection refused") || err_str.contains("tcp connect error") || err_str.contains("error sending request") {
        return format!(
            "{} {}\n\n{}\n  {}\n\n{}\n  • {}\n  • {}\n  • {}\n\n{}\n  {}",
            "✗".bright_red().bold(),
            "Cannot connect to Entelechy API".bright_red().bold(),
            "API URL:".bright_yellow(),
            api_url.bright_white(),
            "Possible causes:".bright_yellow(),
            "The Entelechy API server is not running".bright_white(),
            ...
            "Try:".bright_green(),
            "Start the Entelechy API server and ensure it's accessible".bright_white()
        );
    }
    ...
    if err_str.contains("dns") || err_str.contains("DNS") || err_str.contains("failed to lookup") {
        return format!(
            ...
            "Try:".bright_green(),
            "Check the ENTELECHY_API_URL environment variable".bright_white()
        );
    }
    ...
    if err_str.contains("404") {
        if err_str.contains("Bank configuration API is disabled") {
            return format!(
                ...
                "To enable, set ENTELECHY_API_ENABLE_BANK_CONFIG_API=true on the API server".bright_white(),
                "Note:".bright_cyan(),
        ...
        return format!(
            ...
            "Try:".bright_green(),
            "Check that you're using the correct Entelechy API version".bright_white()
        );
    }
    ...
    if err_str.contains("invalid URL") || err_str.contains("InvalidUri") {
        return format!(
            ...
            "export ENTELECHY_API_URL=http://localhost:8888".bright_white()
        );
    }
    ...
    if err_str.contains("Failed to parse") || err_str.contains("error decoding") {
        ...
        return format!(
            ...
            "Ensure you're using a compatible Entelechy API version".bright_white()
        );
    }
    ...
    format!(
        ...
        "Check that ENTELECHY_API_URL is set correctly".bright_white(),
        "Ensure the Entelechy API server is running".bright_white(),
...
pub fn print_config_help() {
    ...
    println!("  {}", "entelechy configure".bright_white());
    println!();
    println!("  Or set it directly:");
    println!("  {}", "entelechy configure --api-url http://your-api:8888".bright_white());
    println!();
    println!("  {}", "Configuration priority:".bright_yellow());
    println!("    1. Environment variable (ENTELECHY_API_URL) - highest priority");
    println!("    2. Config file (~/.entelechy/config)");
