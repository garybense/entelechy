...
const CONFIG_DIR_NAME: &str = ".entelechy";
const PROFILE_DIR_NAME: &str = "cli-profiles";
const PROFILE_ENV_VAR: &str = "ENTELECHY_PROFILE";
...
impl Config {
    ...
    /// 1. Environment variable (ENTELECHY_API_URL/ENTELECHY_API_KEY) - highest priority
    /// 2. Named profile (from `profile_name` arg, else `$ENTELECHY_PROFILE`)
    ///    at `~/.entelechy/cli-profiles/<name>.toml`
    /// 3. Local config file (`~/.entelechy/config`)
    /// 4. Default (http://localhost:8888)
    pub fn load_with_profile(profile_name: Option<&str>) -> Result<Self> {
        let env_api_key = env::var("ENTELECHY_API_KEY").ok();

        // 1. Environment variable takes highest priority
        if let Ok(api_url) = env::var("ENTELECHY_API_URL") {
    ...
    /// Save a named profile to `~/.entelechy/cli-profiles/<name>.toml`.
...
fn load_profile_from_dir(dir: &std::path::Path, name: &str) -> Result<(String, Option<String>)> {
    ...
    if !path.exists() {
        anyhow::bail!(
            "profile '{}' not found at {}; create with: entelechy profile create {} --api-url <url>",
            name,
            path.display(),
            name
        );
...
mod tests {
    ...
    fn tempdir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("entelechy-cli-test-{}-{}-{}", pid, nanos, n));
