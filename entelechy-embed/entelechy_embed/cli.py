"""
Entelechy Embedded CLI.

A wrapper CLI that manages a local daemon and forwards commands to entelechy-cli.
No external server required - runs everything locally with automatic daemon management.

Usage:
    entelechy-embed configure              # Interactive setup
    entelechy-embed retain "User prefers dark mode"
    entelechy-embed recall "What are user preferences?"
    entelechy-embed daemon status          # Check daemon status
...
Environment variables:
    ...
    ENTELECHY_EMBED_BANK_ID: Optional. Memory bank ID (default: "default").
    ENTELECHY_EMBED_API_URL: Optional. Use external API server instead of starting local daemon.
    ENTELECHY_EMBED_API_TOKEN: Optional. Authentication token for external API (sent as Bearer token).
    ENTELECHY_EMBED_API_DATABASE_URL: Optional. Database URL for daemon (default: "pg0://entelechy-embed").
    ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT: Optional. Seconds before daemon auto-exits when idle (default: 300).
    ENTELECHY_EMBED_API_VERSION: Optional. entelechy-api version to use (default: matches embed version).
                                 Note: Only applies when starting daemon. To change version, stop daemon first.
    ENTELECHY_EMBED_CLI_VERSION: Optional. entelechy CLI version to install (default: {embed_version}).
"""
...
CONFIG_DIR = Path.home() / ".entelechy"
...
def setup_logging(verbose: bool = False):
    """Configure logging."""
    level_str = os.environ.get("ENTELECHY_EMBED_LOG_LEVEL", "info").lower()
    if verbose:
        level_str = "debug"
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    Delegates to entelechy_api.config when available (same Python environment),
    with a minimal fallback for standalone use.
    """
    try:
        from entelechy_api.config import PROVIDER_DEFAULT_MODELS
...
def get_config():
    ...
    load_config_file()
    provider = os.environ.get("ENTELECHY_API_LLM_PROVIDER", "openai")
    default_model = get_default_model_for_provider(provider)
    return {
        "llm_api_key": os.environ.get("ENTELECHY_API_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY"),
        "llm_provider": provider,
        "llm_model": os.environ.get("ENTELECHY_API_LLM_MODEL", default_model),
        "bank_id": os.environ.get("ENTELECHY_EMBED_BANK_ID", "default"),
    }
...
def _has_non_interactive_env() -> bool:
    ...
    """
    if os.environ.get("ENTELECHY_API_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY"):
        return True
    return os.environ.get("ENTELECHY_API_LLM_PROVIDER") in ("ollama", "vertexai")


def _do_configure_from_env():
    """Non-interactive configuration from environment variables (for CI)."""
    # Check for required environment variables
    api_key = os.environ.get("ENTELECHY_API_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY")
    provider = os.environ.get("ENTELECHY_API_LLM_PROVIDER", "openai")
    ...
    if not api_key and provider not in ("ollama", "vertexai"):
        ...
        print("  ENTELECHY_API_LLM_API_KEY=<your-api-key>", file=sys.stderr)
        print(f"  ENTELECHY_API_LLM_PROVIDER={provider}  # optional, default: openai", file=sys.stderr)
        print(f"  ENTELECHY_API_LLM_MODEL=<model>  # optional, default: {default_model}", file=sys.stderr)
        return 1

    model = os.environ.get("ENTELECHY_API_LLM_MODEL", default_model)
    bank_id = os.environ.get("ENTELECHY_EMBED_BANK_ID", "default")

    print()
    print("\033[1m\033[36m  Entelechy Embed - Non-interactive Configuration\033[0m")
    print()
    ...
    with open(CONFIG_FILE, "w") as f:
        f.write("# Entelechy Embed Configuration\n")
        f.write("# Generated by entelechy-embed configure (non-interactive)\n\n")
        f.write(f"ENTELECHY_API_LLM_PROVIDER={provider}\n")
        f.write(f"ENTELECHY_API_LLM_MODEL={model}\n")
        f.write(f"ENTELECHY_EMBED_BANK_ID={bank_id}\n")
        if api_key:
            f.write(f"ENTELECHY_API_LLM_API_KEY={api_key}\n")
        ...
        if platform.system() == "Darwin":  # macOS
            ...
            f.write("ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU=1\n")
            f.write("ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU=1\n")
...
def _do_configure_interactive(profile_name: str | None = None, port: int | None = None):
    ...
    if profile_name:
        ...
        print("\033[1m\033[36m  │   Entelechy Embed Configuration    │\033[0m")
        print("\033[1m\033[36m  ╰─────────────────────────────────────╯\033[0m")
    print()
    ...
    # Prepare config dict
    config_dict = {
        "ENTELECHY_API_LLM_PROVIDER": provider,
        "ENTELECHY_API_LLM_MODEL": model,
        "ENTELECHY_EMBED_BANK_ID": bank_id,
    }
    if api_key:
        config_dict["ENTELECHY_API_LLM_API_KEY"] = api_key

    # Force CPU mode for embeddings/reranker on macOS to avoid MPS/XPC crashes in daemon mode
    import platform

    if platform.system() == "Darwin":  # macOS
        config_dict["ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU"] = "1"
        config_dict["ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU"] = "1"
    ...
    if profile_name:
        ...
        with open(CONFIG_FILE, "w") as f:
            f.write("# Entelechy Embed Configuration\n")
            f.write("# Generated by entelechy-embed configure\n\n")
    ...
    print('    \033[36mentelechy-embed retain "Alice works at Google as a software engineer"\033[0m')
    print('    \033[36mentelechy-embed recall "Alice"\033[0m')
    print()
...
def do_daemon(args, config: dict, logger):
    ...
    if args.daemon_command == "start":
        ...
        if daemon_client.is_daemon_running(profile):
            ...
            database_url = os.getenv("ENTELECHY_EMBED_API_DATABASE_URL")
            if not database_url:
        ...
        print("Usage: entelechy-embed daemon {start|stop|status|logs}", file=sys.stderr)
        return 1
...
def do_ui(args, config: dict, logger):
    ...
    if args.ui_command == "start":
        ...
        print("Usage: entelechy-embed ui {start|stop|status|logs}", file=sys.stderr)
        return 1
...
def _do_configure_profile_with_env(profile_name: str, port: int | None, env_vars: list[str]) -> int:
    ...
    for env_str in env_vars:
        ...
        # Validate key format
        if not key.startswith("ENTELECHY_EMBED_") and not key.startswith("ENTELECHY_API_"):
            print(
                f"Warning: Key '{key}' doesn't start with ENTELECHY_EMBED_ or ENTELECHY_API_",
                file=sys.stderr,
            )
    ...
    print(f"    \033[36mentelechy-embed daemon start --profile {profile_name}\033[0m")
    print(f'    \033[36mentelechy-embed --profile {profile_name} memory recall default "query"\033[0m')
    print()
...
def do_profile_command(args: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(prog="entelechy-embed profile")
    ...
    if parsed_args.profile_command == "list":
        ...
        if not profiles:
            ...
            print("  entelechy-embed configure --profile my-app --port 9100 --env ENTELECHY_API_LLM_PROVIDER=...")
            return 0
        ...
        if profile_exists and not merge:
            ...
            print(f"  entelechy-embed profile delete {display_name}", file=sys.stderr)
            return 1
        ...
        if not active_profile:
            source = "default"
        elif os.getenv("ENTELECHY_EMBED_PROFILE"):
            source = "ENTELECHY_EMBED_PROFILE"
        ...
        elif source == "ENTELECHY_EMBED_PROFILE":
            print("  \033[2mSource:\033[0m ENTELECHY_EMBED_PROFILE environment variable")
...
def main():
    ...
    if remaining_args:
        ...
        if command == "configure":
            ...
            parser = argparse.ArgumentParser(prog="entelechy-embed configure")
        ...
        if command == "daemon":
            ...
            parser = argparse.ArgumentParser(prog="entelechy-embed daemon")
        ...
        # Handle UI subcommands
        if command == "ui":
            parser = argparse.ArgumentParser(prog="entelechy-embed ui")
        ...
        if command in ("start", "stop", "status", "logs"):
            ...
            print(f"\nDid you mean: entelechy-embed daemon {command}", file=sys.stderr)
            if global_profile:
                print(f"              (with --profile {global_profile})", file=sys.stderr)
            sys.exit(1)

        # Forward all other commands to entelechy-cli
        config = get_config()
        ...
        if not config["llm_api_key"] and llm_provider not in providers_without_api_key:
            ...
            print("Run 'entelechy-embed configure' to set up.", file=sys.stderr)
            sys.exit(1)

        from . import daemon_client

        # Forward to entelechy-cli (handles daemon startup and CLI installation)
...
def print_help():
    """Print help message."""
    print("""Entelechy Embedded CLI - local memory operations with automatic daemon management.

Usage: entelechy-embed [-p PROFILE] <command> [options]
...
CLI commands (forwarded to entelechy-cli):
    ...
    ...                              Run 'entelechy --help' for all commands
...
Examples:
    # Create a profile
    entelechy-embed profile create my-app --port 9100 --env ENTELECHY_API_LLM_PROVIDER=openai

    # Manage environment variables
    entelechy-embed profile set-env my-app ENTELECHY_API_LLM_MODEL gpt-4
    entelechy-embed profile remove-env my-app ENTELECHY_API_LLM_MODEL

    # Use profile with commands
    entelechy-embed -p my-app daemon start
    entelechy-embed -p my-app memory retain default "User prefers dark mode"
    entelechy-embed --profile my-app bank list
