"""Profile management for entelechy-embed.
...
class ProfileManager:
    """Manages configuration profiles for entelechy-embed."""
    ...
    def load_profile_config(self, name: str) -> dict[str, str]:
        ...
        Returns:
            ...
            Also includes simple key aliases (e.g., 'idle_timeout' for 'ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT').
        """
        ...
        key_aliases = {
            ...
            "ENTELECHY_API_LLM_BASE_URL": "llm_base_url",
            "ENTELECHY_API_LOG_LEVEL": "log_level",
            "ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT": "idle_timeout",
        }
...
def resolve_active_profile() -> str:
    ...
    1. ENTELECHY_EMBED_PROFILE environment variable
    ...
    if env_profile := os.getenv("ENTELECHY_EMBED_PROFILE"):
        return env_profile
...
def validate_profile_exists(profile: str):
    ...
    if not pm.profile_exists(profile):
        ...
        print(
            f"Create it with: entelechy-embed configure --profile {profile}",
            file=sys.stderr,
        )
        sys.exit(1)
