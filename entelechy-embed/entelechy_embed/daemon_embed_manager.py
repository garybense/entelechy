...
# runners than POSIX.
DAEMON_STARTUP_TIMEOUT = int(os.getenv("ENTELECHY_EMBED_DAEMON_STARTUP_TIMEOUT", "180"))
...
class DaemonEmbedManager(EmbedManager):
    ...
    def get_database_url(self, profile: str, db_url: Optional[str] = None) -> str:
        ...
        return f"pg0://entelechy-embed-{safe_profile}"
    ...
    def _find_api_command(self) -> list[str]:
        """Find the command to run entelechy-api."""
        # Check if we're in development mode
        dev_api_path = Path(__file__).parent.parent.parent / "entelechy-api-slim"
        if dev_api_path.exists() and (dev_api_path / "pyproject.toml").exists():
            return ["uv", "run", "--project", str(dev_api_path), "entelechy-api"]

        # Prefer a entelechy-api entry point installed alongside entelechy-embed
        # (e.g. `uv pip install entelechy-all` or `--target`). Falling through
        ...
        binary_name = "entelechy-api.exe" if platform.system() == "Windows" else "entelechy-api"
        ...
        api_version = os.getenv("ENTELECHY_EMBED_API_VERSION", __version__)
        return ["uvx", f"entelechy-api@{api_version}"]
    ...
    def _clear_port(self, port: int) -> bool:
        ...
        Behavior:
          ...
          * Port occupied by a *healthy* entelechy daemon → True without killing.
          ...
          * Port occupied but /health is unreachable / non-200 → treat as a stale
            entelechy daemon (or foreign process) and attempt to reclaim by killing
            the PID listening on the port. This preserves the original intent of
            clearing stale daemons from version upgrades.
          * Kill failed, or non-entelechy process occupying the port → False.
        """
        if not self._is_port_in_use(port):
            return True

        # Port is occupied — check if it's a healthy entelechy daemon.
        health_ok = False
        try:
            with httpx.Client(timeout=2) as client:
        ...
        if health_ok:
            logger.debug(f"Port {port} already serving a healthy entelechy daemon; reusing it")
            return True
    ...
    def _start_daemon_locked(
        ...
        if not self._clear_port(port):
            logger.error(f"Cannot start daemon: port {port} is in use by a non-entelechy process")
            return False

        # _clear_port returns True without killing when a healthy entelechy daemon
        ...
        # Support both formats: simple keys ("llm_api_key") and env var format ("ENTELECHY_API_LLM_API_KEY")
        env = os.environ.copy()

        # Map of simple key -> env var key
        key_mapping = {
            "llm_api_key": "ENTELECHY_API_LLM_API_KEY",
            "llm_provider": "ENTELECHY_API_LLM_PROVIDER",
            "llm_model": "ENTELECHY_API_LLM_MODEL",
            "llm_base_url": "ENTELECHY_API_LLM_BASE_URL",
            "log_level": "ENTELECHY_API_LOG_LEVEL",
            "idle_timeout": "ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT",
        }
        ...
        # Propagate any other ENTELECHY_* keys from the merged profile/explicit
        # config into the daemon env. Without this, arbitrary settings in the
        # profile's .env file (e.g. ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU,
        # ENTELECHY_API_EMBEDDINGS_PROVIDER) are silently dropped because the
        # whitelist above only covers LLM/log/idle_timeout keys.
        for key, value in config.items():
            if key.startswith("ENTELECHY_") and value is not None:
                env[key] = str(value)

        # Use profile-specific database (check config for override)
        db_override = config.get("ENTELECHY_EMBED_API_DATABASE_URL") or env.get("ENTELECHY_EMBED_API_DATABASE_URL")
        if db_override:
            env["ENTELECHY_API_DATABASE_URL"] = db_override
        else:
            env["ENTELECHY_API_DATABASE_URL"] = self.get_database_url(profile)

        database_url = env["ENTELECHY_API_DATABASE_URL"]
        is_pg0 = database_url.startswith("pg0://")

        # Set defaults if not provided
        if "ENTELECHY_API_LOG_LEVEL" not in env:
            env["ENTELECHY_API_LOG_LEVEL"] = "info"
        if "ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT" not in env:
            env["ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT"] = str(DEFAULT_DAEMON_IDLE_TIMEOUT)
        ...
        if platform.system() == "Darwin":
            if "ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU" not in env:
                env["ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU"] = "1"
            if "ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU" not in env:
                env["ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU"] = "1"

        # Get idle timeout from env
        idle_timeout = int(env.get("ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT", str(DEFAULT_DAEMON_IDLE_TIMEOUT)))

        # Create log directory
        daemon_log.parent.mkdir(parents=True, exist_ok=True)
        env["ENTELECHY_API_DAEMON_LOG"] = str(daemon_log)
        ...
        try:
            ...
            # own logging (via ENTELECHY_API_DAEMON_LOG) will append to the
            ...
            error_msg = (
                ...
                "Install entelechy-api with: pip install entelechy-api"
            )
            error_panel = Panel(
    ...
    def _register_profile(self, profile: str, port: int, config: dict) -> None:
        ...
        Only saves ENTELECHY_API_* config keys (not internal daemon keys).
        Silently ignores errors to avoid blocking daemon startup.
        """
        try:
            api_config = {k: v for k, v in config.items() if k.startswith("ENTELECHY_API_")}
            if not api_config:
                return
    ...
    def _find_ui_command(self) -> list[str]:
        ...
        dev_cp_path = Path(__file__).parent.parent.parent / "entelechy-control-plane"
        ...
        cp_version = os.getenv("ENTELECHY_EMBED_CP_VERSION", __version__)
    ...
    def start_ui(self, profile: str, ui_port: int | None = None, hostname: str = "0.0.0.0") -> bool:
        ...
        env["ENTELECHY_CP_DATAPLANE_API_URL"] = api_url
    ...
    def ensure_running(self, config: dict, profile: str, extra_args: list[str] | None = None) -> bool:
        ...
        Args:
            config: Environment configuration dict (ENTELECHY_API_* vars)
            profile: Profile name for isolation
            extra_args: Extra CLI arguments to pass to entelechy-api (e.g. ["--offline"])
