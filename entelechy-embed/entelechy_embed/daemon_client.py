...
and the entelechy Rust CLI binary.
"""
...
def ensure_daemon_running(config: dict, profile: str | None = None, extra_args: list[str] | None = None) -> bool:
    ...
    Args:
        config: Configuration dict with LLM settings (accepts both simple keys
                like "llm_api_key" and env var format like "ENTELECHY_API_LLM_API_KEY").
        profile: Profile name (None = resolve from priority).
...
def find_cli_binary() -> Path | None:
    """Find the entelechy CLI binary in known locations or PATH."""
    import shutil
    import sys

    binary_names = ("entelechy.exe", "entelechy") if sys.platform == "win32" else ("entelechy",)

    # Check standard install locations. On Windows we look for both the
    # `.exe` (produced by `cargo build`) and the bare `entelechy` in case a
    ...
    # "entelechy" finds entelechy.exe automatically.
    path_binary = shutil.which("entelechy")
    if path_binary:
...
def is_cli_installed() -> bool:
    """Check if the entelechy CLI is installed."""
    return find_cli_binary() is not None


def install_cli() -> bool:
    """
    Install the entelechy CLI using the official installer.
    ...
    cli_version = os.getenv("ENTELECHY_EMBED_CLI_VERSION", __version__)

    print(f"Installing entelechy CLI (version {cli_version})...")
    print(f"  Installer URL: {CLI_INSTALLER_URL}")

    try:
        # Download and run installer with version env var
        env = os.environ.copy()
        env["ENTELECHY_CLI_VERSION"] = cli_version
...
def run_cli(args: list[str], config: dict, profile: str | None = None) -> int:
    """
    Run the entelechy CLI with the given arguments.

    Ensures daemon is running (unless ENTELECHY_API_URL is already set) and passes the API URL.
    ...
    cli_binary = find_cli_binary()
    if not cli_binary:
        print("Error: entelechy CLI not found", file=sys.stderr)
        return 1
    ...
    api_url = env.get("ENTELECHY_EMBED_API_URL")
    ...
    # Set the API URL for the CLI (using the standard ENTELECHY_API_URL var that the CLI expects)
    env["ENTELECHY_API_URL"] = api_url

    # Pass through API token if set (using the standard ENTELECHY_API_KEY var that the CLI expects)
    api_token = env.get("ENTELECHY_EMBED_API_TOKEN")
    if api_token:
        env["ENTELECHY_API_KEY"] = api_token
