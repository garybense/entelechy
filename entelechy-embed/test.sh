#!/bin/bash
#
# Smoke test for entelechy-embed CLI with daemon mode
...
API_DIR="$(cd "$SCRIPT_DIR/../entelechy-api" && pwd)"

echo "=== Entelechy Embed Smoke Test (Daemon Mode) ==="

# Check required environment (load from config if not set)
if [ -f ~/.entelechy/config.env ]; then
    source ~/.entelechy/config.env
fi

# vertexai uses GCP service account credentials instead of an API key
if [ -z "$ENTELECHY_API_LLM_API_KEY" ] && [ -z "$OPENAI_API_KEY" ]; then
    if [ "${ENTELECHY_API_LLM_PROVIDER}" != "vertexai" ]; then
        echo "Error: ENTELECHY_API_LLM_API_KEY or OPENAI_API_KEY is required"
        exit 1
    fi
fi
...
echo "  ENTELECHY_API_LLM_API_KEY: ${ENTELECHY_API_LLM_API_KEY:+set (hidden)}"
...
uv run --project "$SCRIPT_DIR" entelechy-embed daemon stop 2>/dev/null || true
sleep 1
...
OUTPUT=$(uv run --project "$SCRIPT_DIR" entelechy-embed memory retain "$BANK_ID" "The user's favorite color is blue" 2>&1)
EXIT_CODE=$?
set -e
...
if [ $EXIT_CODE -ne 0 ]; then
    ...
    if [ -f ~/.entelechy/daemon.log ]; then
        echo "=== daemon.log ==="
        tail -50 ~/.entelechy/daemon.log
    else
        echo "No daemon.log found"
    fi
    if [ -f ~/.entelechy/daemon.stderr ]; then
        echo ""
        echo "=== daemon.stderr ==="
        cat ~/.entelechy/daemon.stderr
    else
        echo "No daemon.stderr found"
    fi
    echo ""
    echo "Checking for entelechy-api..."
    which entelechy-api 2>/dev/null || echo "entelechy-api not in PATH"
...
START_TIME=$("$PYTHON" -c "import time; print(time.time())")
set +e
OUTPUT=$(uv run --project "$SCRIPT_DIR" entelechy-embed memory recall "$BANK_ID" "What is the user's favorite color?" 2>&1)
EXIT_CODE=$?
set -e
END_TIME=$("$PYTHON" -c "import time; print(time.time())")
DURATION=$("$PYTHON" -c "print(f'{$END_TIME - $START_TIME:.2f}')")
...
START_TIME=$("$PYTHON" -c "import time; print(time.time())")
set +e
OUTPUT=$(uv run --project "$SCRIPT_DIR" entelechy-embed memory retain "$BANK_ID" "User prefers Python over JavaScript" --context work 2>&1)
EXIT_CODE=$?
set -e
END_TIME=$("$PYTHON" -c "import time; print(time.time())")
DURATION=$("$PYTHON" -c "print(f'{$END_TIME - $START_TIME:.2f}')")
...
START_TIME=$("$PYTHON" -c "import time; print(time.time())")
set +e
JSON_OUTPUT=$(uv run --project "$SCRIPT_DIR" entelechy-embed memory recall "$BANK_ID" "programming preferences" -o json 2>&1)
EXIT_CODE=$?
set -e
END_TIME=$("$PYTHON" -c "import time; print(time.time())")
DURATION=$("$PYTHON" -c "print(f'{$END_TIME - $START_TIME:.2f}')")
...
STATUS_OUTPUT=$(uv run --project "$SCRIPT_DIR" entelechy-embed daemon status 2>&1)
echo "$STATUS_OUTPUT"
