...
from entelechy_embed.profile_manager import (
    ProfileInfo,
    ProfileManager,
    ProfilePaths,
...
@pytest.fixture
def temp_entelechy_dir(tmp_path, monkeypatch):
    """Create a temporary entelechy directory for tests.
    ...
    # Clear ENTELECHY_EMBED_PROFILE env var
    ...
    temp_config = temp_home / ".entelechy"
    temp_config.mkdir()
    return temp_config


@pytest.fixture
def profile_manager(temp_entelechy_dir):
...
class TestProfileManager:
    ...
    def test_create_profile_success(self, profile_manager, temp_entelechy_dir):
        """Test creating a new profile."""
        config = {
            "ENTELECHY_EMBED_LLM_PROVIDER": "openai",
            "ENTELECHY_EMBED_LLM_API_KEY": "sk-test",
        ...
        config_path = temp_entelechy_dir / "profiles" / "test-profile.env"
        assert config_path.exists()

        # Verify config contents
        config_content = config_path.read_text()
        assert "ENTELECHY_EMBED_LLM_PROVIDER=openai" in config_content
        assert "ENTELECHY_EMBED_LLM_API_KEY=sk-test" in config_content
        assert "ENTELECHY_EMBED_LLM_MODEL=gpt-4o-mini" in config_content

        # Verify metadata was created
        metadata_path = temp_entelechy_dir / "profiles" / "metadata.json"
    ...
    def test_delete_profile(self, profile_manager, temp_entelechy_dir):
        ...
        profiles_dir = temp_entelechy_dir / "profiles"
    ...
    def test_set_active_profile(self, profile_manager, temp_entelechy_dir):
        ...
        active_file = temp_entelechy_dir / "active_profile"
    ...
    def test_clear_active_profile(self, profile_manager, temp_entelechy_dir):
    ...
    def test_list_profiles(self, profile_manager, temp_entelechy_dir):
    ...
    def test_list_profiles_with_active(self, profile_manager, temp_entelechy_dir):
    ...
    def test_resolve_profile_paths_default(self, profile_manager, temp_entelechy_dir):
        """Test resolving paths for default profile."""
        paths = profile_manager.resolve_profile_paths("")

        assert paths.config == temp_entelechy_dir / "embed"
        assert paths.lock == temp_entelechy_dir / "daemon.lock"
        assert paths.log == temp_entelechy_dir / "daemon.log"
        assert paths.port == 8888

    def test_resolve_profile_paths_named(self, profile_manager, temp_entelechy_dir):
        ...
        assert paths.config == temp_entelechy_dir / "profiles" / "test-profile.env"
        assert paths.lock == temp_entelechy_dir / "profiles" / "test-profile.lock"
        assert paths.log == temp_entelechy_dir / "profiles" / "test-profile.log"
    ...
    def test_update_existing_profile(self, profile_manager, temp_entelechy_dir):
        ...
        config_path = temp_entelechy_dir / "profiles" / "test.env"
        config_content = config_path.read_text()
        assert "KEY=new_value" in config_content
        assert "KEY=old_value" not in config_content

    def test_metadata_persistence(self, profile_manager, temp_entelechy_dir):
    ...
    def test_delete_active_profile_clears_active_file(self, profile_manager, temp_entelechy_dir):
...
class TestValidateProfileExists:
    ...
    def test_validate_nonexistent_profile_fails(self, profile_manager, capsys):
        ...
        assert "entelechy-embed configure --profile nonexistent" in captured.err
