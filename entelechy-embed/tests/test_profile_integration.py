...
@pytest.fixture
def entelechy_embed_cmd():
    """Get the entelechy-embed command."""
    return ["uv", "run", "entelechy-embed"]


class TestProfileIntegration:
    """Integration tests for profile workflows."""

    def test_create_and_list_profile(self, temp_home, entelechy_embed_cmd):
        ...
        profile_path = temp_home / ".entelechy" / "profiles" / "test-app.env"
        ...
        # List profiles
        result = subprocess.run(
            entelechy_embed_cmd + ["profile", "list"],
            capture_output=True,
            text=True,
            env={**os.environ, "HOME": str(temp_home)},
        )
    ...
    def test_profile_show_default(self, temp_home, entelechy_embed_cmd):
        """Test showing the default profile."""
        # Create default config
        config_dir = temp_home / ".entelechy"
        config_dir.mkdir(parents=True, exist_ok=True)
        (config_dir / "embed").write_text("KEY=value")

        result = subprocess.run(
            entelechy_embed_cmd + ["profile", "show"],
            capture_output=True,
            text=True,
            env={**os.environ, "HOME": str(temp_home)},
        )
    ...
    def test_profile_show_with_env_var(self, temp_home, entelechy_embed_cmd):
        """Test profile resolution with ENTELECHY_EMBED_PROFILE env var."""
        # Create profile
        subprocess.run(
            entelechy_embed_cmd
            + [
        ...
        env = {**os.environ, "HOME": str(temp_home), "ENTELECHY_EMBED_PROFILE": "test-app"}
        result = subprocess.run(
            entelechy_embed_cmd + ["profile", "show"], capture_output=True, text=True, env=env
        )

        assert result.returncode == 0
        output = strip_ansi(result.stdout)
        assert "Active profile: test-app" in output
        assert "Source: ENTELECHY_EMBED_PROFILE environment variable" in output

    def test_set_active_profile(self, temp_home, entelechy_embed_cmd):
        ...
        # Set as active
        result = subprocess.run(
            entelechy_embed_cmd + ["profile", "set-active", "test-app"],
            capture_output=True,
            text=True,
            env={**os.environ, "HOME": str(temp_home)},
        )
        ...
        active_file = temp_home / ".entelechy" / "active_profile"
    ...
    def test_delete_profile(self, temp_home, entelechy_embed_cmd):
        ...
        result = subprocess.run(
            entelechy_embed_cmd + ["profile", "delete", "test-app"],
            input="y\n",
            capture_output=True,
            text=True,
        )
    ...
    def test_multiple_profiles_different_ports(self, temp_home, entelechy_embed_cmd):
        ...
        # Read metadata
        metadata_path = temp_home / ".entelechy" / "profiles" / "metadata.json"
    ...
    def test_profile_validation_fails_for_nonexistent(self, temp_home, entelechy_embed_cmd):
        """Test that using non-existent profile fails."""
        # Try to use non-existent profile
        env = {**os.environ, "HOME": str(temp_home), "ENTELECHY_EMBED_PROFILE": "nonexistent"}
        result = subprocess.run(
            entelechy_embed_cmd + ["profile", "show"], capture_output=True, text=True, env=env
        )
    ...
    def test_backward_compatibility_default_profile(self, temp_home, entelechy_embed_cmd):
    ...
    def test_configure_without_profile_flag(self, temp_home, entelechy_embed_cmd):
        ...
        result = subprocess.run(
            entelechy_embed_cmd + ["configure"], capture_output=True, text=True, env=env
        )
        ...
        config_path = temp_home / ".entelechy" / "embed"
        assert config_path.exists()

    def test_clear_active_profile(self, temp_home, entelechy_embed_cmd):
        ...
        result = subprocess.run(
            entelechy_embed_cmd + ["profile", "set-active", "--none"],
            capture_output=True,
            text=True,
        )
    ...
    def test_profile_port_persistence(self, temp_home, entelechy_embed_cmd):
        ...
        subprocess.run(
            ...
            + [
                ...
                "test-app",
                "--env",
                "ENTELECHY_EMBED_LLM_PROVIDER=groq",
                "--env",
                "ENTELECHY_EMBED_LLM_API_KEY=gsk-test",
            ],
            capture_output=True,
        )
