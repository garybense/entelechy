...
from entelechy_embed import daemon_client
...
class TestRunCli:
    ...
    def test_run_cli_with_external_api_url(self, config, mock_cli_binary, monkeypatch):
        """Test that external ENTELECHY_EMBED_API_URL skips daemon startup."""
        # Set up environment with external API URL
        external_api_url = "http://external-api:8000"
        monkeypatch.setenv("ENTELECHY_EMBED_API_URL", external_api_url)
        ...
        with (
            ...
            assert call_args.kwargs["env"]["ENTELECHY_API_URL"] == external_api_url
    ...
    def test_run_cli_without_external_api_url(self, config, mock_cli_binary, monkeypatch):
        ...
        # Ensure ENTELECHY_EMBED_API_URL is not set
        monkeypatch.delenv("ENTELECHY_EMBED_API_URL", raising=False)
        ...
        with (
            ...
            assert call_args.kwargs["env"]["ENTELECHY_API_URL"] == daemon_client.get_daemon_url()
    ...
    def test_run_cli_with_api_token(self, config, mock_cli_binary, monkeypatch):
        """Test that ENTELECHY_EMBED_API_TOKEN is passed through to the CLI."""
        # Set up environment with external API URL and token
        external_api_url = "http://external-api:8000"
        api_token = "test-bearer-token-12345"
        monkeypatch.setenv("ENTELECHY_EMBED_API_URL", external_api_url)
        monkeypatch.setenv("ENTELECHY_EMBED_API_TOKEN", api_token)
        ...
        with (
            ...
            assert call_args.kwargs["env"]["ENTELECHY_API_URL"] == external_api_url
            assert call_args.kwargs["env"]["ENTELECHY_API_KEY"] == api_token
...
class TestClearPort:
    ...
    def test_port_occupied_by_healthy_entelechy_is_reused(self):
        """Port occupied by a healthy entelechy daemon — reused without killing.

        Regression: previously _clear_port killed any entelechy daemon on the
    ...
    def test_port_occupied_by_non_entelechy_returns_false(self):
        """Port occupied by non-entelechy process — returns False."""
        manager = DaemonEmbedManager()
        with (
            patch.object(DaemonEmbedManager, "_is_port_in_use", return_value=True),
            patch("httpx.Client") as mock_httpx_cls,
    ...
    def test_port_occupied_health_non_200_returns_false(self):
        """Port responds but not with 200 — treated as non-entelechy."""
        manager = DaemonEmbedManager()
        with (
            patch.object(DaemonEmbedManager, "_is_port_in_use", return_value=True),
            patch("httpx.Client") as mock_httpx_cls,
    ...
    def test_unhealthy_daemon_kill_succeeds_returns_true(self):
        """Unhealthy entelechy daemon (stale from version upgrade) reclaimed via kill."""
        manager = DaemonEmbedManager()
        with (
            patch.object(DaemonEmbedManager, "_is_port_in_use", return_value=True),
            patch("httpx.Client") as mock_httpx_cls,
...
class TestStartDaemonSerialization:
    ...
    def test_start_daemon_locked_skips_spawn_when_port_already_healthy(self, tmp_path, monkeypatch):
        ...
        subprocess.Popen.
        """
        from entelechy_embed.profile_manager import ProfilePaths
