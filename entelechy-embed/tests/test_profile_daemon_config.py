...
def test_profile_config_is_loaded_for_daemon(temp_home):
    ...
    using only os.environ or hardcoded defaults. This caused ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT
    and other profile-specific settings to be silently ignored.
    """
    # Create a profile with custom configuration
    profile_dir = temp_home / ".entelechy" / "profiles"
    ...
    from entelechy_embed.profile_manager import ProfileManager
    ...
    assert profile_config["ENTELECHY_API_LLM_PROVIDER"] == "openai"
    assert profile_config["ENTELECHY_API_LLM_API_KEY"] == "sk-test-fake-key"
    assert profile_config["ENTELECHY_API_LLM_MODEL"] == "gpt-4o-mini"
    assert profile_config["ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT"] == "0"
    assert profile_config["ENTELECHY_API_LOG_LEVEL"] == "debug"
...
def test_load_config_file_uses_correct_profile(temp_home, monkeypatch):
    ...
    from entelechy_embed.cli import set_cli_profile_override

    set_cli_profile_override(None)

    # Must clear ENTELECHY_EMBED_PROFILE env var to ensure test isolation
    monkeypatch.delenv("ENTELECHY_EMBED_PROFILE", raising=False)

    from entelechy_embed.cli import load_config_file
    from entelechy_embed.profile_manager import ProfileManager

    # Create default profile with one provider
    default_config_dir = temp_home / ".entelechy"
    ...
    profile_name = "myapp"
    profile_env_path = profile_dir / f"{profile_name}.env"
    profile_env_path.write_text("ENTELECHY_API_LLM_PROVIDER=groq\n" "ENTELECHY_API_LLM_MODEL=llama-3.1-70b\n")
    ...
    monkeypatch.delenv("ENTELECHY_API_LLM_PROVIDER", raising=False)
    monkeypatch.delenv("ENTELECHY_API_LLM_MODEL", raising=False)

    # Test 1: Load default profile (no profile specified)
    set_cli_profile_override(None)
    load_config_file()

    assert os.environ.get("ENTELECHY_API_LLM_PROVIDER") == "openai"
    assert os.environ.get("ENTELECHY_API_LLM_MODEL") == "gpt-4o-mini"

    # Clear env
    monkeypatch.delenv("ENTELECHY_API_LLM_PROVIDER")
    monkeypatch.delenv("ENTELECHY_API_LLM_MODEL")
    ...
    assert os.environ.get("ENTELECHY_API_LLM_PROVIDER") == "groq", "Should load from named profile, not default"
    assert os.environ.get("ENTELECHY_API_LLM_MODEL") == "llama-3.1-70b", "Should load from named profile, not default"
...
def test_get_config_respects_profile(temp_home, monkeypatch):
    ...
    from entelechy_embed.cli import get_config, set_cli_profile_override
    from entelechy_embed.profile_manager import ProfileManager
    ...
    (default_config_dir / "embed").write_text(
        "ENTELECHY_API_LLM_PROVIDER=openai\n"
        "ENTELECHY_API_LLM_MODEL=gpt-4o-mini\n"
        "ENTELECHY_API_LLM_API_KEY=sk-default-key\n"
        "ENTELECHY_EMBED_BANK_ID=default-bank\n"
    )
    ...
    profile_env_path.write_text(
        ...
        "ENTELECHY_API_LLM_API_KEY=sk-ant-production\n"
        "ENTELECHY_EMBED_BANK_ID=production-bank\n"
    )
    ...
    monkeypatch.delenv("ENTELECHY_API_LLM_PROVIDER", raising=False)
    monkeypatch.delenv("ENTELECHY_API_LLM_MODEL", raising=False)
    monkeypatch.delenv("ENTELECHY_API_LLM_API_KEY", raising=False)
    monkeypatch.delenv("ENTELECHY_EMBED_BANK_ID", raising=False)
...
def test_macos_forces_cpu_for_local_embeddings_and_reranker(temp_home, monkeypatch):
    ...
    from entelechy_embed.daemon_embed_manager import DaemonEmbedManager

    # Clear any caller-supplied force-cpu env so we test the default behavior.
    monkeypatch.delenv("ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU", raising=False)
    monkeypatch.delenv("ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU", raising=False)
    ...
    with (
        patch("entelechy_embed.daemon_embed_manager.subprocess.Popen", side_effect=fake_popen),
        patch("entelechy_embed.daemon_embed_manager.time.sleep"),  # skip 2s stability wait
        patch.object(manager, "_clear_port", return_value=True),
        patch.object(manager, "_find_api_command", return_value=["entelechy-api"]),
        patch.object(manager, "is_running", side_effect=fake_is_running),
        patch("entelechy_embed.daemon_embed_manager.platform.system", return_value="Darwin"),
    ):
    ...
    env = captured["env"]
    assert env.get("ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU") == "1"
    assert env.get("ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU") == "1"


def test_profile_env_propagates_arbitrary_entelechy_keys_to_daemon(temp_home, monkeypatch):
    """Regression test: ENTELECHY_* keys in profile .env must reach the daemon subprocess env.
    ...
    in the profile's .env — e.g. ENTELECHY_API_EMBEDDINGS_PROVIDER or
    ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU on non-macOS — was silently dropped.
    """
    import json
    from unittest.mock import MagicMock, patch
    ...
    # Write a profile .env containing non-whitelisted ENTELECHY_API_* keys.
    profile_dir = temp_home / ".entelechy" / "profiles"
    profile_dir.mkdir(parents=True, exist_ok=True)
    profile_name = "forwarding"
    (profile_dir / f"{profile_name}.env").write_text(
        "ENTELECHY_API_LLM_PROVIDER=openai\n"
        "ENTELECHY_API_LLM_API_KEY=sk-x\n"
        "ENTELECHY_API_LLM_MODEL=gpt-4o-mini\n"
        "ENTELECHY_API_EMBEDDINGS_PROVIDER=tei\n"
        "ENTELECHY_API_EMBEDDINGS_TEI_URL=http://localhost:8080\n"
    ...
    for key in (
        "ENTELECHY_API_EMBEDDINGS_PROVIDER",
        "ENTELECHY_API_EMBEDDINGS_TEI_URL",
    ...
    with (
        patch("entelechy_embed.daemon_embed_manager.subprocess.Popen", side_effect=fake_popen),
        patch("entelechy_embed.daemon_embed_manager.time.sleep"),
        patch.object(manager, "_clear_port", return_value=True),
        patch.object(manager, "_find_api_command", return_value=["entelechy-api"]),
        patch.object(manager, "is_running", side_effect=fake_is_running),
        patch("entelechy_embed.daemon_embed_manager.platform.system", return_value="Linux"),
    ):
        manager._start_daemon(config={}, profile=profile_name)

    env = captured["env"]
    assert env.get("ENTELECHY_API_EMBEDDINGS_PROVIDER") == "tei"
    assert env.get("ENTELECHY_API_EMBEDDINGS_TEI_URL") == "http://localhost:8080"
    assert env.get("ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU") == "1"
...
def test_macos_force_cpu_respects_explicit_override(temp_home, monkeypatch):
    ...
    monkeypatch.setenv("ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU", "0")
    monkeypatch.setenv("ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU", "0")
    ...
    env = captured["env"]
    assert env.get("ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU") == "0"
    assert env.get("ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU") == "0"
...
def test_windows_popen_uses_detached_process_flags(temp_home, monkeypatch):
    ...
    with (
        ...
        patch.object(manager, "_find_api_command", return_value=["entelechy-api"]),
        patch.object(manager, "is_running", side_effect=fake_is_running),
        patch("entelechy_embed.daemon_embed_manager.platform.system", return_value="Windows"),
    ):
        manager._start_daemon(
            config={"llm_provider": "openai", "llm_api_key": "sk-x", "llm_model": "gpt-4o-mini"},
            profile="",
