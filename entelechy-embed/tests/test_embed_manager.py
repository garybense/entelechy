...
from entelechy_embed import get_embed_manager
...
def test_sanitize_profile_name_via_db_url():
    ...
    assert manager.get_database_url(None) == "pg0://entelechy-embed-default"

    # Test simple alphanumeric names
    assert manager.get_database_url("myapp") == "pg0://entelechy-embed-myapp"
    assert manager.get_database_url("my-app") == "pg0://entelechy-embed-my-app"
    assert manager.get_database_url("my_app") == "pg0://entelechy-embed-my_app"
    assert manager.get_database_url("app123") == "pg0://entelechy-embed-app123"

    # Test special characters get replaced with dashes
    assert manager.get_database_url("my app") == "pg0://entelechy-embed-my-app"
    assert manager.get_database_url("my.app") == "pg0://entelechy-embed-my-app"
    assert manager.get_database_url("my@app!") == "pg0://entelechy-embed-my-app-"
    assert manager.get_database_url("My App 2.0!") == "pg0://entelechy-embed-My-App-2-0-"
...
def test_get_database_url_default():
    ...
    assert manager.get_database_url("myapp") == "pg0://entelechy-embed-myapp"
    assert manager.get_database_url("myapp", None) == "pg0://entelechy-embed-myapp"
    assert manager.get_database_url("myapp", "pg0") == "pg0://entelechy-embed-myapp"
...
def test_register_profile_skips_when_no_api_keys():
    """
    When config contains only short keys (no ENTELECHY_API_* prefix),
...
def test_register_profile_calls_create_when_api_keys_present():
    """
    When config contains ENTELECHY_API_* keys, _register_profile should
    ...
    config = {
        ...
        "ENTELECHY_API_LLM_API_KEY": "sk-123",
    ...
    manager._profile_manager.create_profile.assert_called_once_with(
        "myprofile",
        8100,
        {"ENTELECHY_API_LLM_PROVIDER": "openai", "ENTELECHY_API_LLM_API_KEY": "sk-123"},
    )
...
def test_find_ui_command_uses_npx_yes_flag_for_published_control_plane(monkeypatch):
    ...
    manager = DaemonEmbedManager()
    monkeypatch.setenv("ENTELECHY_EMBED_CP_VERSION", "9.9.9")
...
def test_find_api_command_prefers_sibling_binary_over_uvx(tmp_path, monkeypatch):
    """
    When entelechy-api is installed alongside entelechy-embed (e.g. via
    `uv pip install entelechy-all`), _find_api_command should invoke that
    ...
    """
    package_root = tmp_path / "site-packages" / "entelechy_embed"
    package_root.mkdir(parents=True)
    fake_module = package_root / "daemon_embed_manager.py"
    fake_module.write_text("")
    sibling_bin = tmp_path / "site-packages" / "bin" / "entelechy-api"
    sibling_bin.parent.mkdir(parents=True)
    sibling_bin.touch()

    manager = DaemonEmbedManager()
    monkeypatch.setattr("entelechy_embed.daemon_embed_manager.__file__", str(fake_module))
    monkeypatch.setattr("entelechy_embed.daemon_embed_manager.platform.system", lambda: "Linux")
...
def test_find_api_command_falls_back_to_uvx_when_no_sibling_binary(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("entelechy_embed.daemon_embed_manager.__file__", str(fake_module))
    monkeypatch.setattr("entelechy_embed.daemon_embed_manager.platform.system", lambda: "Linux")
    monkeypatch.setenv("ENTELECHY_EMBED_API_VERSION", "1.2.3")

    assert manager._find_api_command() == ["uvx", "entelechy-api@1.2.3"]
...
def test_find_api_command_windows_uses_exe_suffix(tmp_path, monkeypatch):
    ...
    fake_module = package_root / "daemon_embed_manager.py"
    fake_module.write_text("")
    sibling_bin = tmp_path / "site-packages" / "Scripts" / "entelechy-api.exe"
    sibling_bin.parent.mkdir(parents=True)
    sibling_bin.touch()

    manager = DaemonEmbedManager()
    monkeypatch.setattr("entelechy_embed.daemon_embed_manager.__file__", str(fake_module))
    monkeypatch.setattr("entelechy_embed.daemon_embed_manager.platform.system", lambda: "Windows")
