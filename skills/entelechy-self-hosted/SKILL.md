---
name: entelechy-self-hosted
description: Store team knowledge, project conventions, and learnings from tasks. Use to remember what works and recall context before new tasks. Connects to a self-hosted Entelechy server. (user)
---

# Entelechy Memory Skill (Self-Hosted)

You have persistent memory via a **self-hosted Entelechy server**. This memory bank can be **shared with the team**, so knowledge stored here benefits everyone working on this codebase.

**Proactively store team knowledge and recall context** to provide better assistance.

## Setup Check (First-Time Only)

Before using memory commands, verify the Entelechy CLI is configured:
...
2. **Create the config file** - ask the user for:
   - **API URL**: Their self-hosted Entelechy server URL (e.g., `https://entelechy.mycompany.com`)
...
When you call `retain`, Entelechy does **not** store the string as-is. The server runs an internal pipeline that:
...
```bash
entelechy memory retain <bank-id> "The project uses ESLint configured with the Airbnb rule set and Prettier for formatting. Auto-fix on save is enabled in the editor config."
entelechy memory retain <bank-id> "Ran the test suite with NODE_ENV=test. Tests pass. Without NODE_ENV=test, the suite fails with a missing config error." --context procedures
entelechy memory retain <bank-id> "Build failed on Node 18 with error 'ERR_UNSUPPORTED_ESM_URL_SCHEME'. Switched to Node 20 and build succeeded." --context learnings
entelechy memory retain <bank-id> "Alice reviewed the PR and asked for verbose commit messages that explain the motivation, not just what changed." --context preferences
```

You can also pass a raw conversation transcript with timestamps:

```bash
entelechy memory retain <bank-id> "[2026-03-16T10:12:03] User: The auth tests keep failing on CI but pass locally. Any idea?
...
```bash
entelechy memory recall <bank-id> "project conventions and coding standards"
entelechy memory recall <bank-id> "Alice preferences for this project"
entelechy memory recall <bank-id> "what issues have we encountered before"
entelechy memory recall <bank-id> "how does the auth module work"
```
...
```bash
entelechy memory reflect <bank-id> "How should I approach this task based on past experience?"
```
