...
# Entelechy Memory Skill (Local)

You have persistent memory via the `entelechy-embed` CLI. **Proactively store learnings and recall context** to provide better assistance.

## Setup Check (First-Time Only)

Before using memory commands, verify Entelechy is configured:
...
```bash
uvx entelechy-embed memory retain default "User is working on a TypeScript project. They enabled strict mode and prefer explicit type annotations over inference."
uvx entelechy-embed memory retain default "Ran the test suite with NODE_ENV=test. Tests pass. Without NODE_ENV=test, the suite fails with a missing config error." --context procedures
uvx entelechy-embed memory retain default "Build failed on Node 18 with error 'ERR_UNSUPPORTED_ESM_URL_SCHEME'. Switched to Node 20 and build succeeded." --context learnings
```

You can also pass a raw conversation transcript with timestamps:

```bash
uvx entelechy-embed memory retain default "[2026-03-16T10:12:03] User: The auth tests keep failing on CI but pass locally. Any idea?
...
```bash
uvx entelechy-embed memory recall default "user preferences for this project"
uvx entelechy-embed memory recall default "what issues have we encountered before"
```
...
```bash
uvx entelechy-embed memory reflect default "How should I approach this task based on past experience?"
```
