...
Entelechy uses several machine learning models for different tasks.
...
> 
Entelechy works with any provider that exposes an OpenAI-compatible API (e.g., Azure OpenAI). Simply set `ENTELECHY_API_LLM_PROVIDER=openai` and configure `ENTELECHY_API_LLM_BASE_URL` to point to your provider's endpoint.

See [Configuration](./configuration#llm-provider) for setup examples.
> **💡 AWS Bedrock**
> 
Set `ENTELECHY_API_LLM_PROVIDER=bedrock` to use AWS Bedrock models directly. Model names use Bedrock model IDs (e.g., `us.amazon.nova-2-lite-v1:0`). No API key is required — authentication uses AWS credentials (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION_NAME`) or IAM roles.

See [Configuration](./configuration#llm-provider) for setup examples.
> **💡 Built-in llama.cpp (fully local, no API key)**
> 
Set `ENTELECHY_API_LLM_PROVIDER=llamacpp` to run a built-in llama.cpp server with no external dependencies. A Gemma 4 E2B GGUF model (~3.5 GB) is auto-downloaded on first run. Requires the `local-llm` extra: `pip install 'entelechy-api-slim[local-llm]'`.

See [Configuration](./configuration#built-in-llamacpp) for all options.
> **💡 LiteLLM Provider (Azure, Together AI, and more)**
> 
Set `ENTELECHY_API_LLM_PROVIDER=litellm` to use any model supported by [LiteLLM](https://docs.litellm.ai/docs/providers), including **Azure OpenAI**, **Together AI**, **Fireworks AI**, and many more. Model names use LiteLLM's provider prefix format (e.g., `azure/gpt-4o`).

See [Configuration](./configuration#llm-provider) for setup examples.
### Benchmarks

Not sure which model to use? The **[Model Leaderboard](https://benchmarks.entelechy.vectorize.io/)** benchmarks models across accuracy, speed, cost, and reliability for retain, reflect, and observation consolidation so you can pick the right trade-off for your use case.

[](https://benchmarks.entelechy.vectorize.io/)

### Tested Models

The following models have been tested and verified to work correctly with Entelechy:
...
Each provider has a recommended default model that's used when `ENTELECHY_API_LLM_MODEL` is not explicitly set. This makes configuration simpler - just specify the provider and get a sensible default:
...
You can override the default by explicitly setting `ENTELECHY_API_LLM_MODEL`:
```bash
...
export ENTELECHY_API_RETAIN_LLM_PROVIDER=anthropic
```

### Using Other Models

Other LLM models not listed above may work with Entelechy, but they must support **at least 65,000 output tokens** to ensure reliable fact extraction. If you need support for a specific model that doesn't meet this requirement, please [open an issue](https://github.com/entelechy-ai/entelechy/issues) to request an exception.
...
export ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS=32000

# For models that support 16k output tokens
export ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS=16000
```

**Important:** `ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS` must be greater than `ENTELECHY_API_RETAIN_CHUNK_SIZE` (default: 3000). The system will validate this on startup and provide an error message if the configuration is invalid.
### Configuration
...
export ENTELECHY_API_LLM_PROVIDER=minimax
export ENTELECHY_API_LLM_API_KEY=your-minimax-api-key
export ENTELECHY_API_LLM_MODEL=MiniMax-M2.7

# DeepSeek (https://api.deepseek.com)
export ENTELECHY_API_LLM_PROVIDER=deepseek
export ENTELECHY_API_LLM_API_KEY=sk-xxxxxxxxxxxx
export ENTELECHY_API_LLM_MODEL=deepseek-v4-flash  # or deepseek-v4-pro / deepseek-chat / deepseek-reasoner
...
# export ENTELECHY_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY=/path/to/key.json
```
...
Use your ChatGPT Plus or Pro subscription for Entelechy without separate OpenAI Platform API costs.
...
4. **Configure Entelechy:**
   ```bash
   export ENTELECHY_API_LLM_PROVIDER=openai-codex
   # export ENTELECHY_API_LLM_MODEL=gpt-5.1-codex  # defaults to gpt-5.2-codex
...
Use your Claude Pro or Max subscription for Entelechy without separate Anthropic API costs.
...
- Anthropic's [Agent SDK documentation](https://docs.claude.com/en/api/agent-sdk/overview)
  ...
  their products. Entelechy does **not** perform any login on your behalf — it uses
...
- In January 2026, Anthropic [enforced restrictions](https://paddo.dev/blog/anthropic-walled-garden-crackdown/)
  ...
  targeted tools that **spoofed the Claude Code client identity** — Entelechy uses the
...
For production or team use, we recommend using `ENTELECHY_API_LLM_PROVIDER=anthropic` with
...
|----------|-------------|----------|
| `ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID` | Your GCP project ID | Yes |
| `ENTELECHY_API_LLM_VERTEXAI_REGION` | GCP region (e.g., `us-central1`) | No (default: `us-central1`) |
| `ENTELECHY_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY` | Path to service account JSON key file | No (uses ADC if not set) |
...
1. **Application Default Credentials (ADC)** - Recommended for development
   ...
   export ENTELECHY_API_LLM_PROVIDER=vertexai
   export ENTELECHY_API_LLM_MODEL=gemini-2.0-flash-001
   export ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID=your-project-id
   ```

2. **Service Account Key** - Recommended for production
   ```bash
   # Create service account and download key
   gcloud iam service-accounts create entelechy-api
   gcloud projects add-iam-policy-binding your-project-id \
     --member="serviceAccount:entelechy-api@your-project-id.iam.gserviceaccount.com" \
     --role="roles/aiplatform.user"
   gcloud iam service-accounts keys create key.json \
     --iam-account=entelechy-api@your-project-id.iam.gserviceaccount.com
...
Google's `gemini-embedding-001` supports configurable output dimensionality via truncation, google recommend using: 768, 1536, 3072, via `ENTELECHY_API_EMBEDDINGS_GEMINI_OUTPUT_DIMENSIONALITY`. Default is 768.
...
> 
Entelechy automatically detects the embedding dimension at startup and adjusts the database schema. Once memories are stored, you cannot change dimensions without losing data.
...
export ENTELECHY_API_EMBEDDINGS_OPENAI_API_KEY=sk-xxxxxxxxxxxx
...
export ENTELECHY_API_EMBEDDINGS_GEMINI_API_KEY=xxxxxxxxxxxx
...
export ENTELECHY_API_EMBEDDINGS_PROVIDER=google
export ENTELECHY_API_EMBEDDINGS_GEMINI_MODEL=gemini-embedding-001
export ENTELECHY_API_EMBEDDINGS_VERTEXAI_PROJECT_ID=your-gcp-project-id
...
export ENTELECHY_API_EMBEDDINGS_LITELLM_MODEL=text-embedding-3-small
```
...
export ENTELECHY_API_RERANKER_COHERE_MODEL=rerank-v3.5  # whatever model the endpoint serves
export ENTELECHY_API_RERANKER_COHERE_BASE_URL=https://your-endpoint.example/rerank
...
export ENTELECHY_API_RERANKER_ZEROENTROPY_MODEL=zerank-2  # default, can omit
...
export ENTELECHY_API_RERANKER_SILICONFLOW_MODEL=BAAI/bge-reranker-v2-m3  # default, can omit
...
export ENTELECHY_API_RERANKER_PROVIDER=flashrank

# LiteLLM proxy
export ENTELECHY_API_RERANKER_PROVIDER=litellm
export ENTELECHY_API_LITELLM_API_BASE=http://localhost:4000
export ENTELECHY_API_RERANKER_LITELLM_MODEL=cohere/rerank-english-v3.0

# RRF-only (no neural reranking)
export ENTELECHY_API_RERANKER_PROVIDER=rrf
```
