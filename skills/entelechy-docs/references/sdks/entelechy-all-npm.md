...
1. `server.start()` resolves the underlying `entelechy-embed` command (via `uvx` from PyPI, or `uv run --directory <path>` for a local checkout).
...
- **`uv` / `uvx`** on `PATH` — used to download and run the Entelechy daemon. Install via [docs.astral.sh/uv](https://docs.astral.sh/uv/).
...
For a remote Entelechy API, skip the server entirely and point `EntelechyClient` directly at the remote URL.

## `EntelechyServerOptions`
...
| `embedVersion` | `string` | `"latest"` | Version of the underlying `entelechy-embed` package to run via `uvx`. |
| `embedPackagePath` | `string` | — | Local checkout path — takes precedence over `embedVersion`. Uses `uv run --directory` instead of `uvx`. |
| `env` | `Record<string, string \| undefined>` | `{}` | Environment variables passed to the daemon process **and** written into the profile config via `--env KEY=VALUE`. The preferred way to surface any `ENTELECHY_API_*` / `ENTELECHY_EMBED_*` setting. |
...
| `platformCpuWorkaround` | `boolean` | `true` on macOS | Auto-set `ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU=1` and `ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU=1` to avoid Metal/MPS crashes. Caller-supplied `env` values win over the auto-applied ones. |
