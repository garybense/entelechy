...
<PageHero title="FAQ" subtitle="Common questions and answers about Entelechy." />

**Contents**
- [What is Entelechy and how does it differ from RAG?](#what-is-entelechy-and-how-does-it-differ-from-rag)
- [Why use Entelechy instead of other solutions?](#why-use-entelechy-instead-of-other-solutions)
- [Supported clients, integrations, and LLM providers](#which-clients-and-languages-are-supported)
- [Which model should I use?](#which-model-should-i-use-with-entelechy)
...
- [Tags, metadata, and entity labels](#does-entelechy-support-metadata-filtering)
...
### What is Entelechy and how does it differ from RAG?

Entelechy is an agent memory system that provides long-term memory for AI agents using biomimetic data structures. Unlike traditional RAG (Retrieval-Augmented Generation), Entelechy:
...
For a detailed comparison, see [RAG vs Memory](developer/rag-vs-entelechy.md).

---

### Why use Entelechy instead of other solutions?

Entelechy is purpose-built for agent memory with unique advantages:

- **State-of-the-art accuracy**: Ranked #1 LongMemEval benchmarks for agent memory (see [details](https://benchmarks.entelechy.vectorize.io/))
...
- **Flexible deployment**: Self-host or use Entelechy Cloud - works with any LLM provider
...
Unlike vector databases (just search) or RAG systems (document retrieval), Entelechy provides **living memory** that evolves with your users.
...
### Which model should I use with Entelechy?

The **[Model Leaderboard](https://benchmarks.entelechy.vectorize.io/)** benchmarks models across accuracy, speed, cost, and reliability for retain, reflect, and observation consolidation — it's the best place to find the right trade-off for your use case.
...
1. **Entelechy Cloud** - Fully managed service at [ui.entelechy.vectorize.io](https://ui.entelechy.vectorize.io)
...
For running the Entelechy API server locally:
- Python 3.11+
...
Entelechy has three core operations:
...
### Does Entelechy support metadata filtering?
...
Pass the **entire conversation as a single document** and upsert it as the conversation grows — Entelechy chunks it automatically, so you don't need to split it yourself.
...
Entelechy has internal chunking optimizations for the JSON array format, since it's the most common conversation shape.
...
**Don't pre-summarize or pre-extract facts.** Entelechy does this automatically and needs the full conversation for context — a message like "yes, exactly" or "I'll go with option 2" is meaningless without the surrounding exchange.
