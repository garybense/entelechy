...
from entelechy_api.engine.task_backend import BrokerTaskBackend, SyncTaskBackend, WorkerTaskBackend
...
async def backend(pg0_db_url):
    ...
    from entelechy_api.engine.db import create_database_backend
    from entelechy_api.pg0 import resolve_database_url
...
def test_all_operation_types_have_slot_reservation_config():
    ...
    from entelechy_api.config import WORKER_SLOT_RESERVATION_TYPES

    # Parse memory_engine.py and extract all operation_type="..." string values
    engine_path = pathlib.Path(__file__).parent.parent / "entelechy_api" / "engine" / "memory_engine.py"
...
class TestWorkerPoller:
    ...
    async def test_execute_task_executor_marks_completed(self, pool, backend, clean_operations):
        ...
        """
        from entelechy_api.worker import WorkerPoller
        from entelechy_api.worker.poller import ClaimedTask
    ...
    async def test_executor_exception_triggers_retry(self, pool, backend, clean_operations):
        ...
        from entelechy_api.worker.exceptions import RetryTaskAt
    ...
    async def test_defer_operation_exported_from_extensions(self):
        """DeferOperation must be importable from entelechy_api.extensions for extension authors."""
        from entelechy_api.extensions import DeferOperation as DeferFromExtensions
        from entelechy_api.worker.exceptions import DeferOperation as DeferFromWorker
    ...
    async def test_memory_engine_execute_task_passes_through_defer_operation(self, memory):
        ...
        from entelechy_api.worker.exceptions import DeferOperation as DeferOpFromExceptions
        from entelechy_api.worker.exceptions import RetryTaskAt
...
class TestDynamicTenantDiscovery:
    ...
    async def test_poller_discovers_tenants_dynamically(self, pool, backend, clean_operations):
        ...
        from entelechy_api.extensions.tenant import Tenant, TenantExtension
        from entelechy_api.worker import WorkerPoller
...
async def test_pending_breakdown_explains_unclaimable_rows(pool, backend, clean_operations, caplog):
    ...
    # always logs.
    with caplog.at_level(logging.INFO, logger="entelechy_api.worker.poller"):
