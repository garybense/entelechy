...
from entelechy_api.engine.llm_wrapper import create_llm_provider
...
@pytest.mark.asyncio
async def test_batch_api_normal_flow(mock_llm_config, test_contents, entelechy_config, memory, request_context):
...
@pytest.mark.asyncio
async def test_batch_api_crash_recovery(mock_llm_config, test_contents, entelechy_config, memory, request_context):
    ...
    try:
        ...
        from entelechy_api.engine.task_backend import fq_table
...
@pytest.mark.asyncio
async def test_batch_api_fallback_unsupported_provider(mock_llm_config, test_contents, entelechy_config):
    ...
    with patch(
        "entelechy_api.engine.retain.fact_extraction.extract_facts_from_contents"
...
async def test_worker_batch_recovery(memory, request_context):
    ...
    try:
        ...
        # Create WorkerPoller
        from entelechy_api.extensions.builtin.tenant import DefaultTenantExtension
...
async def test_batch_api_via_extract_facts_from_contents(
    mock_llm_config, test_contents, entelechy_config, memory, request_context
):
    ...
    try:
        ...
        entelechy_config.retain_batch_enabled = True
