"""Tests for ``entelechy_api.db_url.to_libpq_url``.
...
class TestSslParamRename:
    ...
    def test_ssl_param_preserved_among_other_params(self) -> None:
        result = to_libpq_url(
            "postgresql+asyncpg://h/d?ssl=require&application_name=entelechy&connect_timeout=10"
        )
        assert result.startswith("postgresql://h/d?")
        # Query order should be preserved; ssl renamed, others untouched.
        assert "sslmode=require" in result
        assert "application_name=entelechy" in result
...
class TestProductionConfigs:
    ...
    These are the exact shapes currently set for ENTELECHY_API_DATABASE_URL,
    ENTELECHY_API_CONTROL_DATABASE_URL and ENTELECHY_API_MIGRATION_DATABASE_URL
