...
from entelechy_api.extensions.builtin.supabase_tenant import (
    JWKS_CACHE_TTL_SECONDS,
    JWKS_MIN_REFRESH_INTERVAL_SECONDS,
    MIN_TOKEN_LENGTH,
    SupabaseTenantExtension,
)
from entelechy_api.extensions.context import ExtensionContext
from entelechy_api.extensions.loader import load_extension
from entelechy_api.extensions.tenant import AuthenticationError, Tenant, TenantContext, TenantExtension
...
class TestSupabaseTenantExtensionInit:
    ...
    def test_init_missing_supabase_url(self):
        with pytest.raises(ValueError, match="ENTELECHY_API_TENANT_SUPABASE_URL is required"):
            SupabaseTenantExtension({})
...
class TestSupabaseTenantExtensionStartup:
    ...
    async def test_on_startup_creates_http_client(self):
        ...
        with patch("entelechy_api.extensions.builtin.supabase_tenant.httpx.AsyncClient", return_value=mock_client):
            with patch("entelechy_api.extensions.builtin.supabase_tenant.PyJWK"):
    ...
    async def test_on_startup_fetches_jwks(self):
        ...
        with patch("entelechy_api.extensions.builtin.supabase_tenant.httpx.AsyncClient", return_value=mock_client):
            with patch("entelechy_api.extensions.builtin.supabase_tenant.PyJWK") as mock_pyjwk:
    ...
    async def test_on_startup_raises_if_no_jwks_and_no_service_key(self):
        ...
        with patch("entelechy_api.extensions.builtin.supabase_tenant.httpx.AsyncClient", return_value=mock_client):
            with pytest.raises(ValueError, match="ENTELECHY_API_TENANT_SUPABASE_SERVICE_KEY is required"):
                await ext.on_startup()
...
class TestJWKSCacheManagement:
    ...
    async def test_get_signing_key_from_cache(self):
        ...
        with patch("entelechy_api.extensions.builtin.supabase_tenant.pyjwt.get_unverified_header") as mock_header:
    ...
    async def test_get_signing_key_refreshes_stale_cache(self):
        ...
        with (
            patch("entelechy_api.extensions.builtin.supabase_tenant.pyjwt.get_unverified_header") as mock_header,
            patch("entelechy_api.extensions.builtin.supabase_tenant.PyJWK", return_value=new_key),
        ):
            mock_header.return_value = {"kid": "test-key-1", "alg": "RS256"}
            key = await ext._get_signing_key("fake-token")
    ...
    async def test_get_signing_key_handles_key_rotation(self):
        ...
        with (
            patch("entelechy_api.extensions.builtin.supabase_tenant.pyjwt.get_unverified_header") as mock_header,
            patch("entelechy_api.extensions.builtin.supabase_tenant.PyJWK", return_value=rotated_key),
        ):
...
class TestAuthenticateJWKS:
    ...
    async def test_authenticate_valid_token(self):
        ...
        with (
            patch("entelechy_api.extensions.builtin.supabase_tenant.pyjwt.get_unverified_header") as mock_header,
            patch("entelechy_api.extensions.builtin.supabase_tenant.pyjwt.decode") as mock_decode,
        ):
            mock_header.return_value = {"kid": "test-key-1", "alg": "RS256"}
    ...
    async def test_authenticate_expired_token(self):
        ...
        with (
            patch("entelechy_api.extensions.builtin.supabase_tenant.pyjwt.get_unverified_header") as mock_header,
            patch(
                "entelechy_api.extensions.builtin.supabase_tenant.pyjwt.decode",
...
class TestSupabaseTenantExtensionLoader:
    ...
    def test_load_via_extension_loader(self, monkeypatch):
        monkeypatch.setenv(
            "ENTELECHY_API_TENANT_EXTENSION",
            "entelechy_api.extensions.builtin.supabase_tenant:SupabaseTenantExtension",
        )
        monkeypatch.setenv("ENTELECHY_API_TENANT_SUPABASE_URL", "https://test.supabase.co")
        monkeypatch.setenv("ENTELECHY_API_TENANT_SUPABASE_SERVICE_KEY", "test-key")
        monkeypatch.setenv("ENTELECHY_API_TENANT_SCHEMA_PREFIX", "custom")
    ...
    def test_load_without_service_key(self, monkeypatch):
        ...
        monkeypatch.setenv("ENTELECHY_API_TENANT_SUPABASE_URL", "https://test.supabase.co")
        monkeypatch.delenv("ENTELECHY_API_TENANT_SUPABASE_SERVICE_KEY", raising=False)
