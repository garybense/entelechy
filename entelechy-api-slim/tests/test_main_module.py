"""
Tests for entelechy_api.main module (single-worker code path).

The main.py module is used when running with a single worker:
    entelechy-api  (or entelechy-api --workers 1)
...
class TestMainModuleExtensionLoading:
    ...
    def test_main_loads_tenant_extension_when_configured(self, monkeypatch):
        """
        Verify that main.py loads tenant extension from ENTELECHY_API_TENANT_EXTENSION.
        ...
        monkeypatch.setenv("ENTELECHY_API_WORKERS", "1")

        # Track what extensions were loaded via load_extension
        loaded_extensions = {}

        # Get the real load_extension function
        from entelechy_api.extensions.loader import load_extension as real_load_extension
        ...
        with patch("entelechy_api.main.MemoryEngine") as mock_engine, \
             patch("entelechy_api.main.create_app") as mock_create_app, \
             patch("entelechy_api.main._get_raw_config") as mock_get_config, \
             patch("entelechy_api.main.load_extension", side_effect=tracking_load_extension), \
             patch("entelechy_api.main.DefaultExtensionContext"), \
             patch("entelechy_api.main.print_banner"), \
            ...
            with patch.object(sys, 'argv', ['entelechy-api']):
                from entelechy_api.main import main
                main()
    ...
    def test_main_loads_operation_validator_when_configured(self, monkeypatch):
        """
        Verify that main.py loads operation validator from ENTELECHY_API_OPERATION_VALIDATOR_EXTENSION.
        """
        monkeypatch.setenv(
            "ENTELECHY_API_OPERATION_VALIDATOR_EXTENSION",
    ...
    def test_main_passes_extensions_to_memory_engine(self, monkeypatch):
        ...
        with patch("entelechy_api.main.MemoryEngine", side_effect=capture_memory_engine), \
             patch("entelechy_api.main.create_app") as mock_create_app, \
             patch("entelechy_api.main._get_raw_config") as mock_get_config, \
             patch("entelechy_api.main.DefaultExtensionContext"), \
             patch("entelechy_api.main.print_banner"), \
    ...
    def test_main_sets_extension_context_on_tenant_extension(self, monkeypatch):
        ...
        with patch("entelechy_api.main.MemoryEngine", side_effect=capture_memory_engine), \
             patch("entelechy_api.main.create_app") as mock_create_app, \
             patch("entelechy_api.main._get_raw_config") as mock_get_config, \
             patch("entelechy_api.main.DefaultExtensionContext", side_effect=capture_context), \
             patch("entelechy_api.main.print_banner"), \
             patch("uvicorn.run"):
    ...
    def test_main_works_without_extensions(self, monkeypatch):
        ...
        monkeypatch.delenv("ENTELECHY_API_TENANT_EXTENSION", raising=False)
        monkeypatch.delenv("ENTELECHY_API_OPERATION_VALIDATOR_EXTENSION", raising=False)
        monkeypatch.setenv("ENTELECHY_API_WORKERS", "1")
    ...
    def test_main_uses_app_object_for_single_worker(self, monkeypatch):
        ...
        with patch("entelechy_api.main.MemoryEngine") as mock_engine, \
             patch("entelechy_api.main.create_app", return_value=mock_app), \
             patch("entelechy_api.main._get_raw_config") as mock_get_config, \
             patch("entelechy_api.main.print_banner"), \
            ...
            with patch.object(sys, 'argv', ['entelechy-api', '--workers', '1']):
                from entelechy_api.main import main
                main()
    ...
    def test_main_uses_import_string_for_multiple_workers(self, monkeypatch):
        ...
        """
        monkeypatch.setenv("ENTELECHY_API_WORKERS", "2")
        monkeypatch.delenv("ENTELECHY_API_TENANT_EXTENSION", raising=False)
        ...
        with patch("entelechy_api.main.MemoryEngine") as mock_engine, \
            ...
            with patch.object(sys, 'argv', ['entelechy-api', '--workers', '2']):
                from entelechy_api.main import main
                main()

        assert len(uvicorn_calls) == 1
        # With workers > 1, should use import string
        assert uvicorn_calls[0]["app"] == "entelechy_api.server:app", \
...
class MockTenantExtension(TenantExtension):
    ...
    async def list_tenants(self) -> list:
        from entelechy_api.extensions.tenant import Tenant
