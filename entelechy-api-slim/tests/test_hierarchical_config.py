...
from entelechy_api.config import EntelechyConfig, normalize_config_dict, normalize_config_key
from entelechy_api.config_resolver import ConfigResolver
from entelechy_api.extensions.tenant import TenantExtension
...
class MockTenantExtension(TenantExtension):
    ...
    async def authenticate(self, context):
        from entelechy_api.extensions.tenant import TenantContext
...
async def test_config_key_normalization():
    ...
    assert normalize_config_key("ENTELECHY_API_LLM_PROVIDER") == "llm_provider"
    assert normalize_config_key("ENTELECHY_API_LLM_MODEL") == "llm_model"
    assert normalize_config_key("ENTELECHY_API_RETAIN_LLM_PROVIDER") == "retain_llm_provider"
...
async def test_hierarchical_fields_categorization():
    ...
    configurable = EntelechyConfig.get_configurable_fields()
    credentials = EntelechyConfig.get_credential_fields()
    static = EntelechyConfig.get_static_fields()
...
async def test_config_supports_both_key_formats(memory, request_context):
    ...
    try:
        ...
        await resolver.update_bank_config(bank_id, {"ENTELECHY_API_RETAIN_CHUNK_SIZE": 8000})
        ...
        await resolver.update_bank_config(
            bank_id,
            {
                ...
                "ENTELECHY_API_RETAIN_EXTRACTION_MODE": "verbose",  # Env format
            },
        )
...
async def test_config_only_configurable_fields_stored(memory, request_context):
    ...
    try:
        ...
        for key in bank_overrides.keys():
            assert key in EntelechyConfig.get_configurable_fields(), f"Non-configurable field {key} in bank config"
...
async def test_config_get_bank_config_no_static_or_credential_fields_leak(memory, request_context):
    ...
    try:
        ...
        configurable_fields = EntelechyConfig.get_configurable_fields()
        credential_fields = EntelechyConfig.get_credential_fields()
        static_fields = EntelechyConfig.get_static_fields()
