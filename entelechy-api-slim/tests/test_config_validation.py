...
def setup_test_env():
    ...
    env_vars_to_save = [
        "ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS",
        "ENTELECHY_API_RETAIN_CHUNK_SIZE",
        "ENTELECHY_API_LLM_PROVIDER",
        "ENTELECHY_API_LLM_MODEL",
        "ENTELECHY_API_DATABASE_URL",
        "ENTELECHY_API_MIGRATION_DATABASE_URL",
    ]
...
def test_retain_max_completion_tokens_must_be_greater_than_chunk_size():
    ...
    os.environ["ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS"] = "1000"
    os.environ["ENTELECHY_API_RETAIN_CHUNK_SIZE"] = "2000"
    ...
    assert "ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS" in error_message
    assert "1000" in error_message
    assert "ENTELECHY_API_RETAIN_CHUNK_SIZE" in error_message
    ...
    assert "Increase ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS" in error_message
...
def test_retain_max_completion_tokens_equal_to_chunk_size_fails():
    ...
    # Set invalid config: max_completion_tokens == chunk_size
    os.environ["ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS"] = "3000"
    os.environ["ENTELECHY_API_RETAIN_CHUNK_SIZE"] = "3000"
    os.environ["ENTELECHY_API_LLM_PROVIDER"] = "mock"
...
def test_valid_retain_config_succeeds():
    ...
    os.environ["ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS"] = "64000"
    os.environ["ENTELECHY_API_RETAIN_CHUNK_SIZE"] = "3000"
    os.environ["ENTELECHY_API_LLM_PROVIDER"] = "mock"
...
def test_log_config_masks_database_urls(caplog):
    ...
    os.environ["ENTELECHY_API_DATABASE_URL"] = "postgresql://entelechy_user:plain-password@db:5432/entelechy_db"
    os.environ["ENTELECHY_API_MIGRATION_DATABASE_URL"] = (
        "postgresql://migration_user:migration-password@db-admin:5432/entelechy_db"
    )
    os.environ["ENTELECHY_API_RETAIN_MAX_COMPLETION_TOKENS"] = "64000"
    os.environ["ENTELECHY_API_RETAIN_CHUNK_SIZE"] = "3000"
    os.environ["ENTELECHY_API_LLM_PROVIDER"] = "mock"

    caplog.set_level(logging.INFO, logger="entelechy_api.config")

    config = EntelechyConfig.from_env()
    config.log_config()

    log_output = "\n".join(record.getMessage() for record in caplog.records)
    assert "entelechy_user" not in log_output
    ...
    assert "postgresql://***:***@db:5432/entelechy_db" in log_output
    assert "postgresql://***:***@db-admin:5432/entelechy_db" in log_output
