...
async def test_recall_span_hierarchy(mock_tracer, memory, request_context):
    ...
    try:
        ...
        assert parent_call[0][0] == "entelechy.recall"
        ...
        assert "entelechy.bank_id" in recall_attrs
        assert recall_attrs["entelechy.bank_id"] == bank_id
        assert "entelechy.query" in recall_attrs
        assert "entelechy.fact_types" in recall_attrs
        assert "entelechy.thinking_budget" in recall_attrs
        assert "entelechy.max_tokens" in recall_attrs
        ...
        if mock_tracer.start_span.called:
            ...
            assert "entelechy.recall_embedding" in child_spans
            assert "entelechy.recall_retrieval" in child_spans
            assert "entelechy.recall_fusion" in child_spans
            assert "entelechy.recall_rerank" in child_spans
...
async def test_recall_span_attributes(mock_tracer, memory, request_context):
    ...
    try:
        ...
        assert "entelechy.bank_id" in attrs
        assert "entelechy.query" in attrs
        assert "entelechy.fact_types" in attrs
        assert "entelechy.max_tokens" in attrs
        assert "entelechy.thinking_budget" in attrs

        # Verify attribute values
        assert attrs["entelechy.bank_id"] == bank_id
        assert "test query" in attrs["entelechy.query"]
        assert attrs["entelechy.max_tokens"] == 2048
