...
from entelechy_api import RequestContext
from entelechy_api.engine.memory_engine import Budget
...
def test_recall_result_model_empty_construction():
    ...
    """
    from entelechy_api.engine.response_models import RecallResult
...
async def test_custom_extraction_mode():
    ...
    This test verifies that when ENTELECHY_API_RETAIN_EXTRACTION_MODE=custom and
    ENTELECHY_API_RETAIN_CUSTOM_INSTRUCTIONS is set, the fact extraction uses the
    custom guidelines while keeping structural parts intact.
    """
    import os

    from entelechy_api import LLMConfig
    from entelechy_api.config import _get_raw_config, clear_config_cache
    from entelechy_api.engine.retain.fact_extraction import extract_facts_from_text

    # Save original env vars
    original_mode = os.getenv("ENTELECHY_API_RETAIN_EXTRACTION_MODE")
    original_instructions = os.getenv("ENTELECHY_API_RETAIN_CUSTOM_INSTRUCTIONS")

    try:
        # Set custom extraction mode with challenging language-specific guidelines
        os.environ["ENTELECHY_API_RETAIN_EXTRACTION_MODE"] = "custom"
        os.environ["ENTELECHY_API_RETAIN_CUSTOM_INSTRUCTIONS"] = """ONLY extract facts that are in ITALIAN language.
...
If the text contains both Italian and English content, extract ONLY the Italian facts."""
        ...
        if original_mode is not None:
            os.environ["ENTELECHY_API_RETAIN_EXTRACTION_MODE"] = original_mode
        else:
            os.environ.pop("ENTELECHY_API_RETAIN_EXTRACTION_MODE", None)

        if original_instructions is not None:
            os.environ["ENTELECHY_API_RETAIN_CUSTOM_INSTRUCTIONS"] = original_instructions
        else:
            os.environ.pop("ENTELECHY_API_RETAIN_CUSTOM_INSTRUCTIONS", None)
...
def test_collapse_to_verbatim_single_fact_per_chunk():
    ...
    """
    from entelechy_api.engine.retain.fact_extraction import _collapse_to_verbatim
    from entelechy_api.engine.retain.types import ChunkMetadata, ExtractedFact
...
def test_chunks_extraction_mode():
    ...
    from entelechy_api.config import _get_raw_config, clear_config_cache
    from entelechy_api.engine.retain.fact_extraction import extract_facts_from_contents
    from entelechy_api.engine.retain.types import RetainContent

    original_mode = os.getenv("ENTELECHY_API_RETAIN_EXTRACTION_MODE")

    try:
        os.environ["ENTELECHY_API_RETAIN_EXTRACTION_MODE"] = "chunks"
        clear_config_cache()
...
async def test_verbatim_extraction_mode():
    ...
    try:
        os.environ["ENTELECHY_API_RETAIN_EXTRACTION_MODE"] = "verbatim"
        clear_config_cache()
...
def test_retain_mission_injected_into_prompt():
    ...
    from entelechy_api.engine.retain.fact_extraction import _build_extraction_prompt_and_schema
...
def test_retain_mission_config_loaded_from_env():
    ...
    from entelechy_api.config import EntelechyConfig, _get_raw_config, clear_config_cache

    original = os.getenv("ENTELECHY_API_RETAIN_MISSION")
    try:
        os.environ["ENTELECHY_API_RETAIN_MISSION"] = "Only technical decisions."
        clear_config_cache()
        config = _get_raw_config()
        assert config.retain_mission == "Only technical decisions."
        assert "retain_mission" in EntelechyConfig.get_configurable_fields()
    finally:
        if original is None:
            os.environ.pop("ENTELECHY_API_RETAIN_MISSION", None)
        else:
            os.environ["ENTELECHY_API_RETAIN_MISSION"] = original
        clear_config_cache()
...
def test_retain_request_per_item_strategy_field():
    ...
    """
    from entelechy_api.api.http import RetainRequest
...
from entelechy_api.engine.llm_wrapper import TokenUsage
from entelechy_api.engine.memory_engine import MemoryEngine
from entelechy_api.engine.task_backend import SyncTaskBackend
...
def _make_mock_llm_call():
    ...
    async def mock_llm_call(*args, **kwargs):
        from entelechy_api.engine.consolidation.consolidator import _ConsolidationBatchResponse
...
async def test_streaming_chunk_batching_produces_same_facts(memory_mock_llm, request_context):
    ...
    try:
        with patch("entelechy_api.engine.llm_wrapper.LLMProvider.call", new=mock_llm_call):
