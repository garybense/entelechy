...
from entelechy_api.engine.db import DatabaseBackend, DatabaseConnection, create_database_backend
from entelechy_api.engine.db.postgresql import PostgreSQLBackend
from entelechy_api.engine.db.result import DictResultRow as ResultRow
from entelechy_api.engine.sql import SQLDialect, create_sql_dialect
from entelechy_api.engine.sql.postgresql import PostgreSQLDialect
...
class TestOracleDialect:
    @pytest.fixture()
    def d(self):
        from entelechy_api.engine.sql.oracle import OracleDialect
...
class TestOracleQueryRewriter:
    ...
    def test_param_rewrite(self):
        from entelechy_api.engine.db.oracle import _rewrite_pg_to_oracle
...
class TestConfig:
    def test_database_backend_field_exists(self):
        ...
        field_names = {f.name for f in dataclasses.fields(EntelechyConfig)}
        assert "database_backend" in field_names

    def test_default_database_backend(self):
        from entelechy_api.config import DEFAULT_DATABASE_BACKEND
...
class TestOracleOpsInsertFactsBatch:
    ...
    @pytest.fixture()
    def ops(self):
        from entelechy_api.engine.db.ops_oracle import OracleOps
...
class TestNormalizeSchema:
    ...
    def test_oracle_maps_public_to_none(self):
        from entelechy_api.engine.db.oracle import OracleBackend
