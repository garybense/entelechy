...
def test_llm_wrapper_vertexai_missing_dependency():
    ...
    from entelechy_api.engine import llm_wrapper
    ...
    try:
        ...
        with patch.dict(
            os.environ,
            {
                "ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID": "test-project",
                "ENTELECHY_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY": "/path/to/key.json",
            },
            clear=False,
        ):
...
def test_llm_wrapper_vertexai_adc_auth():
    ...
    with patch.dict(
        ...
        {
            "ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID": "test-project",
            "ENTELECHY_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY": "",  # Clear SA key to test ADC path
        },
        clear=False,
    ):
        from entelechy_api.config import clear_config_cache
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    not os.getenv("ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID"),
    reason="Vertex AI integration tests require ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID",
)
async def test_vertexai_integration_actual_api():
    ...
    Requires:
    - ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID
    - ADC or ENTELECHY_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY
    """
    from entelechy_api.engine.llm_wrapper import LLMProvider
