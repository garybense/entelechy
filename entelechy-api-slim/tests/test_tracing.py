...
from entelechy_api.tracing import (
...
@patch("entelechy_api.tracing.time")
def test_llm_span_recorder_record_success(mock_time):
    ...
    # Verify span was created with correct name (entelechy.{scope})
    mock_tracer.start_as_current_span.assert_called_once()
    call_args = mock_tracer.start_as_current_span.call_args
    assert call_args[0][0] == "entelechy.test"
    ...
    assert attribute_calls["entelechy.scope"] == "test"
...
@patch("entelechy_api.tracing._tracing_enabled", False)
...
@patch("entelechy_api.tracing._tracer")
@patch("entelechy_api.tracing._tracing_enabled", True)
def test_create_operation_span_enabled(mock_tracer):
    ...
    mock_tracer.start_as_current_span.assert_called_once_with("entelechy.retain")

    # Verify attributes were set
    mock_span.set_attribute.assert_any_call("entelechy.operation", "retain")
    mock_span.set_attribute.assert_any_call("entelechy.bank_id", "bank123")
...
def test_create_operation_span_no_bank_id(mock_tracer):
    ...
    mock_tracer.start_as_current_span.assert_called_once_with("entelechy.consolidation")

    # Verify only operation attribute was set (not bank_id)
    assert mock_span.set_attribute.call_count == 1
    mock_span.set_attribute.assert_called_once_with("entelechy.operation", "consolidation")
...
def test_create_operation_span_all_operations(mock_tracer):
    ...
    for operation in operations:
        ...
        # Verify span was created with correct name
        mock_tracer.start_as_current_span.assert_called_once_with(f"entelechy.{operation}")

        # Verify attributes
        mock_span.set_attribute.assert_any_call("entelechy.operation", operation)
        mock_span.set_attribute.assert_any_call("entelechy.bank_id", "test_bank")
...
def test_parent_child_span_hierarchy(mock_tracer, mock_time):
    ...
    assert first_call[0][0] == "entelechy.retain"

    # Verify child span was created second (entelechy.{scope})
    second_call = mock_tracer.start_as_current_span.call_args_list[1]
    assert second_call[0][0] == "entelechy.retain_extract_facts"
