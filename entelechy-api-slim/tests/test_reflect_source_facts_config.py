...
from entelechy_api.engine.reflect.tools import tool_search_observations
...
class TestReflectSourceFactsConfig:
    """Test that reflect_source_facts_max_tokens is properly wired in EntelechyConfig."""
    ...
    def test_config_is_configurable(self):
        ...
        assert "reflect_source_facts_max_tokens" in EntelechyConfig.get_configurable_fields()

    def test_default_value_is_disabled(self):
        """Default should be -1 (disabled)."""
        from entelechy_api.config import DEFAULT_REFLECT_SOURCE_FACTS_MAX_TOKENS

        assert DEFAULT_REFLECT_SOURCE_FACTS_MAX_TOKENS == -1

    def test_env_var_constant_exists(self):
        """Env var constant should be defined."""
        from entelechy_api.config import ENV_REFLECT_SOURCE_FACTS_MAX_TOKENS

        assert ENV_REFLECT_SOURCE_FACTS_MAX_TOKENS == "ENTELECHY_API_REFLECT_SOURCE_FACTS_MAX_TOKENS"

    @patch.dict("os.environ", {"ENTELECHY_API_REFLECT_SOURCE_FACTS_MAX_TOKENS": "8000"})
