...
def test_gemini_safety_settings_parsed_from_env():
    """Safety settings JSON from env var is parsed into EntelechyConfig."""
    import json

    from entelechy_api.config import ENV_LLM_GEMINI_SAFETY_SETTINGS, EntelechyConfig, clear_config_cache
...
def test_gemini_safety_settings_is_configurable_field():
    ...
    assert "llm_gemini_safety_settings" in EntelechyConfig.get_configurable_fields()
...
def test_gemini_safety_settings_not_in_credential_fields():
    ...
    assert "llm_gemini_safety_settings" not in EntelechyConfig.get_credential_fields()
...
def _make_gemini_provider(safety_settings=None):
    ...
    with patch("google.genai.Client") as mock_client_cls:
        ...
        from entelechy_api.engine.providers.gemini_llm import GeminiLLM
...
async def test_with_config_resets_after_call():
    ...
    from entelechy_api.engine.providers.gemini_llm import _safety_settings_ctx
...
def test_llm_provider_reads_safety_settings_from_config():
    ...
    from entelechy_api.config import ENV_LLM_GEMINI_SAFETY_SETTINGS, clear_config_cache

    settings_json = json.dumps(SAMPLE_SAFETY_SETTINGS)
    env_overrides = {
        "ENTELECHY_API_LLM_PROVIDER": "gemini",
        "ENTELECHY_API_LLM_API_KEY": "fake-key",
