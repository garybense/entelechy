...
from entelechy_api import LLMConfig, LocalSTEmbeddings, MemoryEngine, RequestContext
from entelechy_api.engine.cross_encoder import LocalSTCrossEncoder
from entelechy_api.engine.query_analyzer import DateparserQueryAnalyzer
from entelechy_api.engine.task_backend import SyncTaskBackend
from entelechy_api.pg0 import EmbeddedPostgres

# Default pg0 instance configuration for tests
DEFAULT_PG0_INSTANCE_NAME = "entelechy-test"
DEFAULT_PG0_PORT = int(os.environ.get("ENTELECHY_TEST_PG_PORT", "5556"))
...
def db_url():
    ...
    If ENTELECHY_API_DATABASE_URL is set, use it directly.
    Otherwise, return None to indicate pg0 should be used (managed by pg0_instance fixture).
    """
    return os.getenv("ENTELECHY_API_DATABASE_URL")
...
def pg0_db_url(db_url, tmp_path_factory, worker_id):
    ...
    If ENTELECHY_API_DATABASE_URL is a plain postgresql:// URL, uses it directly.
    If ENTELECHY_API_DATABASE_URL is a pg0:// URL, resolves it to a real URL first.
    ...
    """
    from entelechy_api.pg0 import parse_pg0_url as _parse_pg0_url
    ...
    if db_url and not _parse_pg0_url(db_url)[0]:
        ...
        from entelechy_api.migrations import run_migrations
...
def oracle_db_url(_oracle_admin_dsn):
    ...
    This fixture creates a ``ENTELECHY_TEST`` user (idempotent) with the USERS
    ...
    test_user = "ENTELECHY_TEST"
    test_pass = "entelechy_test"
    ...
    from entelechy_api.migrations_oracle import run_oracle_migrations
...
async def oracle_memory(oracle_db_url, embeddings, cross_encoder, query_analyzer):
    ...
    old_backend = os.environ.get("ENTELECHY_API_DATABASE_BACKEND")
    os.environ["ENTELECHY_API_DATABASE_BACKEND"] = "oracle"
    clear_config_cache()
    ...
    try:
        mem = MemoryEngine(
            ...
            memory_llm_provider=os.getenv("ENTELECHY_API_LLM_PROVIDER", "openai"),
            memory_llm_api_key=os.getenv("ENTELECHY_API_LLM_API_KEY"),
            memory_llm_model=os.getenv("ENTELECHY_API_LLM_MODEL", "gpt-4o-mini"),
        ...
        if old_backend is None:
            os.environ.pop("ENTELECHY_API_DATABASE_BACKEND", None)
        else:
            os.environ["ENTELECHY_API_DATABASE_BACKEND"] = old_backend
        clear_config_cache()
