...
a PostgreSQL pattern used in the Entelechy engine so that passing here means the
...
def setup_tables(sync_pool, test_schema):
    """Create tables in the test schema matching Entelechy's schema."""
...
class TestOracleConnectionMethods:
    ...
    async def test_fetch_returns_result_rows(self, oracle_dsn, test_schema, setup_tables):
        from entelechy_api.engine.db.oracle import OracleBackend
        from entelechy_api.engine.db.result import ResultRow
