...
2. TestDeltaRefreshGeminiEval: real-LLM behavioral evals against Gemini. These are
   gated on ENTELECHY_RUN_GEMINI_EVALS=1 (plus a Gemini API key) because they cost
...
def patch_llm_call(monkeypatch):
    ...
    """
    from entelechy_api.engine.reflect.delta_ops import DeltaOperationList
...
_GEMINI_API_KEY = (
    os.getenv("ENTELECHY_GEMINI_API_KEY")
    or os.getenv("GEMINI_API_KEY")
    or os.getenv("GOOGLE_API_KEY")
)
_OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
_RUN_LLM_EVAL = os.getenv("ENTELECHY_RUN_GEMINI_EVALS") == "1" and (
...
pytestmark_gemini = pytest.mark.skipif(
    not _RUN_LLM_EVAL,
    reason=(
        "Real-LLM delta evals are gated. Set ENTELECHY_RUN_GEMINI_EVALS=1 and provide "
...
async def gemini_memory(memory_no_llm_verify: MemoryEngine):
    ...
    if _GEMINI_API_KEY:
        provider = "gemini"
        model = os.getenv("ENTELECHY_GEMINI_EVAL_MODEL", "gemini-2.0-flash")
        cfg = LLMConfig(provider=provider, api_key=_GEMINI_API_KEY, base_url="", model=model)
    else:
        provider = "openai"
        model = os.getenv("ENTELECHY_OPENAI_EVAL_MODEL", "gpt-4o-mini")
...
class TestDeltaRefreshGeminiEval:
    ...
    async def test_new_observation_is_merged_surgically(
        ...
        """
        from entelechy_api.engine.reflect.structured_doc import (
            StructuredDocument,
            render_section,
        )
        ...
        from entelechy_api.engine.reflect.structured_doc import parse_markdown
