"""
Tests for entelechy_api.server module (multi-worker code path).

The server.py module is used when running with multiple workers:
    uvicorn entelechy_api.server:app --workers 2
...
def _clean_server_module():
    """Remove entelechy_api.server from sys.modules for fresh import."""
    modules_to_remove = [k for k in sys.modules.keys() if k.startswith("entelechy_api.server")]
...
class TestServerModuleExtensionLoading:
    ...
    def test_server_loads_tenant_extension_when_configured(self, monkeypatch):
        """
        Verify that server.py loads tenant extension from ENTELECHY_API_TENANT_EXTENSION.
        ...
        # Note: We patch the entire entelechy_api module namespace
        with patch("entelechy_api.MemoryEngine") as mock_engine, \
             patch("entelechy_api.api.create_app") as mock_create_app, \
             patch("entelechy_api.config.get_config") as mock_get_config, \
             patch("entelechy_api.extensions.load_extension", side_effect=tracking_load_extension), \
             patch("entelechy_api.extensions.DefaultExtensionContext"):
    ...
    def test_server_loads_operation_validator_when_configured(self, monkeypatch):
        """
        Verify that server.py loads operation validator from ENTELECHY_API_OPERATION_VALIDATOR_EXTENSION.
        """
        monkeypatch.setenv(
    ...
    def test_server_passes_extensions_to_memory_engine(self, monkeypatch):
        ...
        with patch("entelechy_api.MemoryEngine", side_effect=capture_memory_engine), \
             patch("entelechy_api.api.create_app") as mock_create_app, \
             patch("entelechy_api.config.get_config") as mock_get_config, \
             patch("entelechy_api.extensions.DefaultExtensionContext"):
    ...
    def test_server_sets_extension_context_on_tenant_extension(self, monkeypatch):
        ...
        with patch("entelechy_api.MemoryEngine", side_effect=capture_memory_engine), \
             patch("entelechy_api.api.create_app") as mock_create_app, \
             patch("entelechy_api.config.get_config") as mock_get_config, \
             patch("entelechy_api.extensions.DefaultExtensionContext", side_effect=capture_context):
