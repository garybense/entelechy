...
def _make_config(llm_max_retries: int = 3, retain_llm_max_retries: int | None = None):
    """Build a minimal EntelechyConfig for fact extraction tests."""
    from entelechy_api.config import EntelechyConfig

    cfg = MagicMock(spec=EntelechyConfig)
...
async def test_non_dict_json_all_retries_returns_empty():
    ...
    with patch(
        "entelechy_api.engine.retain.fact_extraction._build_extraction_prompt_and_schema",
