...
from entelechy_api.engine.reflect.agent import (
    _clean_answer_text,
    _clean_done_answer,
...
class TestContextOverflowIntegration:
    ...
    async def test_reflect_completes_with_tiny_context_budget(self, memory, request_context):
        ...
        try:
            ...
            from entelechy_api.config import get_config as _real_get_config
            ...
            with patch("entelechy_api.engine.memory_engine.get_config", return_value=_TinyContextProxy()):
                result = await memory.reflect_async(
                    bank_id=bank_id,
