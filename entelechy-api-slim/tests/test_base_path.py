...
Tests that the API works correctly when deployed with a base path (e.g., /entelechy)
...
async def api_client_with_base_path(memory):
    ...
    base_path = "/entelechy"
    os.environ["ENTELECHY_API_BASE_PATH"] = base_path
    ...
    os.environ.pop("ENTELECHY_API_BASE_PATH", None)
    clear_config_cache()
...
async def test_base_path_health_endpoint(api_client_with_base_path):
    ...
    # With base path set to /entelechy, health should be at /entelechy/health
    # But since our client base_url is already http://test/entelechy, we request /health
...
async def test_base_path_openapi_schema(api_client_with_base_path):
    ...
    assert servers[0]["url"] == "/entelechy"
...
async def test_without_base_path_still_works(api_client_without_base_path):
    ...
    This test verifies that when ENTELECHY_API_BASE_PATH is not set,
