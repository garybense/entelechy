...
from entelechy_api.engine.consolidation.consolidator import (
...
from entelechy_api.engine.reflect.tools import (
    tool_recall,
...
class TestConsolidationTagRouting:
    ...
    async def test_consolidation_inherits_dates_from_source_memory(self, memory: MemoryEngine, request_context):
        ...
        from entelechy_api.engine.consolidation.consolidator import run_consolidation_job
...
class TestObservationDrillDown:
    ...
    async def test_search_observations_returns_source_memory_ids(self, memory: MemoryEngine, request_context):
        ...
        """
        from entelechy_api.engine.reflect.tools import tool_expand, tool_search_observations
...
def test_consolidation_prompt_default():
    ...
    from entelechy_api.engine.consolidation.prompts import build_batch_consolidation_prompt
...
def test_observations_mission_config():
    ...
    original = os.getenv("ENTELECHY_API_OBSERVATIONS_MISSION")
    try:
        os.environ["ENTELECHY_API_OBSERVATIONS_MISSION"] = "Weekly sprint summaries only."
        clear_config_cache()
        config = _get_raw_config()
        assert config.observations_mission == "Weekly sprint summaries only."
        assert "observations_mission" in EntelechyConfig.get_configurable_fields()
    finally:
        if original is None:
            os.environ.pop("ENTELECHY_API_OBSERVATIONS_MISSION", None)
        else:
            os.environ["ENTELECHY_API_OBSERVATIONS_MISSION"] = original
        clear_config_cache()
...
async def test_consolidation_with_observations_mission(memory: "MemoryEngine", request_context):
    ...
    original = os.getenv("ENTELECHY_API_OBSERVATIONS_MISSION")
    try:
        os.environ["ENTELECHY_API_OBSERVATIONS_MISSION"] = (
...
def test_max_observations_per_scope_config():
    ...
    from entelechy_api.config import EntelechyConfig, clear_config_cache

    original = os.getenv("ENTELECHY_API_MAX_OBSERVATIONS_PER_SCOPE")
    try:
        os.environ["ENTELECHY_API_MAX_OBSERVATIONS_PER_SCOPE"] = "42"
        clear_config_cache()
        config = _get_raw_config()
        assert config.max_observations_per_scope == 42
        assert "max_observations_per_scope" in EntelechyConfig.get_configurable_fields()
    finally:
        if original is None:
            os.environ.pop("ENTELECHY_API_MAX_OBSERVATIONS_PER_SCOPE", None)
        else:
            os.environ["ENTELECHY_API_MAX_OBSERVATIONS_PER_SCOPE"] = original
        clear_config_cache()
...
def _make_mock_llm_one_obs_per_fact():
    ...
    from entelechy_api.engine.providers.mock_llm import MockLLM
