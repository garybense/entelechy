...
from ..config import DEFAULT_DATABASE_SCHEMA, EntelechyConfig
...
app = typer.Typer(name="entelechy-admin", help="Entelechy administrative commands")
...
def backup(
    ...
    """Backup the Entelechy database to a zip file."""
    config = EntelechyConfig.from_env()

    if not config.database_url:
        typer.echo("Error: Database URL not configured.", err=True)
        typer.echo("Set ENTELECHY_API_DATABASE_URL environment variable.", err=True)
        raise typer.Exit(1)
