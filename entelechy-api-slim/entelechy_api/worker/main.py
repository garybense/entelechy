"""
Command-line interface for Entelechy Worker.
...
def create_worker_app(poller: WorkerPoller, memory):
    ...
    app = FastAPI(
        title="Entelechy Worker",
        description="Worker process for distributed task execution",
    )

    # Initialize OpenTelemetry metrics
    try:
        prometheus_reader = initialize_metrics(service_name="entelechy-worker", service_version="1.0.0")
...
def main():
    """Main entry point for the entelechy-worker CLI."""
    # Load configuration from environment
    config = get_config()

    parser = argparse.ArgumentParser(
        prog="entelechy-worker",
        description="Entelechy Worker - distributed task processor",
    )
    ...
    parser.add_argument(
        ...
        help="Worker identifier (default: hostname, env: ENTELECHY_API_WORKER_ID)",
    )
    parser.add_argument(
        "--poll-interval",
        type=int,
        default=config.worker_poll_interval_ms,
        help=f"Poll interval in milliseconds (default: {config.worker_poll_interval_ms}, env: ENTELECHY_API_WORKER_POLL_INTERVAL_MS)",
    )
    parser.add_argument(
        "--max-retries",
        type=int,
        default=config.worker_max_retries,
        help=f"Max retries before marking failed (default: {config.worker_max_retries}, env: ENTELECHY_API_WORKER_MAX_RETRIES)",
    )
    ...
    parser.add_argument(
        ...
        help=f"HTTP port for metrics/health endpoints (default: {config.worker_http_port}, env: ENTELECHY_API_WORKER_HTTP_PORT)",
    )
    parser.add_argument(
        "--http-host",
        default="0.0.0.0",
    ...
    parser.add_argument(
        ...
        help=f"Log level (default: {config.log_level}, env: ENTELECHY_API_LOG_LEVEL)",
    )
    ...
    print(f"Starting Entelechy Worker: {args.worker_id}")
    ...
    async def run():
        ...
        from entelechy_api.config import DEFAULT_DATABASE_SCHEMA
