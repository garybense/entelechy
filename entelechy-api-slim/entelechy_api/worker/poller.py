...
class WorkerPoller:
    ...
    async def _scan_active_schemas(self, schemas: list[str | None]) -> set[str | None]:
        ...
        In entelechy-cloud deployments this is installed by a Helm hook
        ...
        async with self._backend.acquire() as conn:
            ...
            # entelechy-cloud. Skip on non-PG backends to avoid constant
    ...
    async def _log_progress_if_due(self):
        ...
        Emits four kinds of lines:
          ...
          * [DB_WAITS]      - any non-idle entelechy session waiting on a lock
        """
        now = time.time()
    ...
    async def _log_db_waits(self) -> None:
        """Log any non-idle entelechy session that's waiting on a lock or other resource.
