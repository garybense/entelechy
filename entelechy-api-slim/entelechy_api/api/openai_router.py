...
async def list_models(raw_request: Request, request_context: RequestContext = Depends(get_request_context)):
...
async def openai_chat_completions_get():
    return {
        "status": "ok",
        "message": "Entelechy OpenAI-compatible endpoint is active. Please use POST for chat completions.",
    }
...
async def openai_chat_completions(
    request: ChatCompletionRequest, raw_request: Request, request_context: RequestContext = Depends(get_request_context)
):
    memory = raw_request.app.state.memory
    ...
    try:
        ...
        response = ChatCompletionResponse(
            ...
            choices=[
                ChatCompletionChoice(
                    index=0,
                    message=ChatCompletionMessage(role="assistant", content=core_result.text),
                    finish_reason="stop",
                )
            ],
    ...
    try:
        ...
        for i in range(0, len(content), chunk_size):
            ...
            chunk_resp = ChatCompletionStreamResponse(
                ...
                choices=[
                    ChatCompletionStreamChoice(
                        index=0, delta=ChatCompletionMessage(role="assistant", content=chunk_text), finish_reason=None
                    )
                ],
            )
        ...
        final_chunk = ChatCompletionStreamResponse(
            ...
            choices=[
                ChatCompletionStreamChoice(
                    index=0, delta=ChatCompletionMessage(role="assistant", content=""), finish_reason="stop"
                )
            ],
        )
        ...
        error_resp = {"error": {"message": str(e), "type": "server_error", "param": None, "code": None}}
