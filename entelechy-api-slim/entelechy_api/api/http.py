...
from entelechy_api.engine.audit import AuditEntry, AuditLogger
...
from entelechy_api.engine.memory_engine import Budget, _current_schema, _get_tiktoken_encoding, fq_table
from entelechy_api.engine.providers.none_llm import LLMNotAvailableError
from entelechy_api.engine.response_models import VALID_RECALL_FACT_TYPES, MemoryFact, TokenUsage
from entelechy_api.engine.search.tags import TagGroup, TagsMatch
from entelechy_api.extensions import HttpExtension, OperationValidationError, load_extension
from entelechy_api.metrics import create_metrics_collector, get_metrics_collector, initialize_metrics
...
class BankConfigUpdate(BaseModel):
    ...
    updates: dict[str, Any] = Field(
        ...
        "or environment variable format (ENTELECHY_API_LLM_PROVIDER). "
    ...
    def validate_version(cls, v: str) -> str:
        ...
        if ver > int(BANK_TEMPLATE_CURRENT_VERSION):
            raise ValueError(
                ...
                f"(max supported: {BANK_TEMPLATE_CURRENT_VERSION}). Please upgrade Entelechy."
            )
        return v
...
async def apply_bank_template_manifest(
    ...
    driven by ENTELECHY_API_DEFAULT_BANK_TEMPLATE. The bank MUST already
...
from entelechy_api.webhooks.models import WebhookHttpConfig
...
def create_app(
    ...
    Args:
        ...
        http_extension: Optional HTTP extension to mount custom endpoints under /extension/.
                       If None, attempts to load from ENTELECHY_API_HTTP_EXTENSION env var.
    ...
    async def lifespan(app: FastAPI):
        ...
        from entelechy_api.config import get_config
        from entelechy_api.worker import WorkerPoller
        ...
        try:
            prometheus_reader = initialize_metrics(service_name="entelechy-api", service_version="1.0.0")
        ...
        if config.otel_traces_enabled:
            if not config.otel_exporter_otlp_endpoint:
                ...
                from entelechy_api.tracing import create_span_recorder, initialize_tracing
    ...
    from entelechy_api import __version__
    from entelechy_api.config import get_config

    config = get_config()

    app = FastAPI(
        title="Entelechy HTTP API",
        version=__version__,
        description="HTTP API for Entelechy",
        contact={
            "name": "Memory System",
        },
        license_info={
...
def _register_routes(app: FastAPI):
    ...
    async def api_get_bank_config(bank_id: str, request_context: RequestContext = Depends(get_request_context)):
        ...
        if not get_config().enable_bank_config_api:
            raise HTTPException(
                status_code=404,
                detail="Bank configuration API is disabled. Set ENTELECHY_API_ENABLE_BANK_CONFIG_API=true to re-enable.",
            )
        try:
    ...
    @app.patch(
        ...
        "Keys can be provided in Python field format (llm_provider) or environment variable format (ENTELECHY_API_LLM_PROVIDER).",
    ...
    async def api_create_webhook(
        ...
        try:
            ...
            from entelechy_api.engine.db_utils import acquire_with_retry
            from entelechy_api.engine.memory_engine import fq_table
            from entelechy_api.engine.retain import bank_utils
            ...
            if request.async_:
                ...
                if config.retain_batch_enabled:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            "Batch API is enabled (ENTELECHY_API_RETAIN_BATCH_ENABLED=true) but async=false. "
                            ...
                            "by setting ENTELECHY_API_RETAIN_BATCH_ENABLED=false in your environment."
                        ),
                    )
    ...
    @app.post(
        ...
        "- Falls back to the server default (`ENTELECHY_API_FILE_PARSER`) if not specified.\n"
    ...
    async def api_file_retain(
        ...
        if not config.enable_file_upload_api:
            raise HTTPException(
                status_code=404,
                detail="File upload API is disabled. Set ENTELECHY_API_ENABLE_FILE_UPLOAD_API=true to enable.",
            )
