"""
Supabase Tenant Extension for Entelechy
...
Features:
    ...
    - Production Ready: Includes health checks, timeouts, key rotation handling,
      and error handling
    - Built-in: Ships with Entelechy, no extra installation needed
...
Configuration via environment variables:
    ...
    # Optional
    ENTELECHY_API_TENANT_SCHEMA_PREFIX=user  # Default: "user" (creates user_<uuid> schemas)

Usage:
    Clients pass their Supabase JWT in the Authorization header:

    curl -H "Authorization: Bearer <supabase_jwt>" \\
        https://your-entelechy-server/v1/default/banks/my-bank/memories/recall
...
class SupabaseTenantExtension(TenantExtension):
    ...
    def __init__(self, config: dict[str, str]) -> None:
        ...
        Config keys are derived from ENTELECHY_API_TENANT_* env vars:
        - ENTELECHY_API_TENANT_SUPABASE_URL -> config["supabase_url"] (required)
        - ENTELECHY_API_TENANT_SUPABASE_SERVICE_KEY -> config["supabase_service_key"] (optional)
        - ENTELECHY_API_TENANT_SCHEMA_PREFIX -> config["schema_prefix"] (optional)
        ...
        if not self.supabase_url:
            raise ValueError(
                "ENTELECHY_API_TENANT_SUPABASE_URL is required. "
    ...
    async def on_shutdown(self) -> None:
        """Called when Entelechy shuts down. Closes the HTTP client."""
    ...
    async def _try_init_jwks(self) -> None:
        ...
        if not self.supabase_service_key:
            raise ValueError(
                "ENTELECHY_API_TENANT_SUPABASE_SERVICE_KEY is required when JWKS "
