"""
Command-line interface for Entelechy API.

Run the server with:
    entelechy-api

Run as background daemon:
    entelechy-api --daemon
...
from .config import DEFAULT_WORKERS, ENV_WORKERS, EntelechyConfig, _get_raw_config
from .daemon import (
    DEFAULT_DAEMON_PORT,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="entelechy-api",
        description="Entelechy API Server",
    )

    # Server options
    parser.add_argument(
        "--host", default=config.host, help=f"Host to bind to (default: {config.host}, env: ENTELECHY_API_HOST)"
    )
    parser.add_argument(
        "--port",
        type=int,
        default=config.port,
        help=f"Port to bind to (default: {config.port}, env: ENTELECHY_API_PORT)",
    )
    parser.add_argument(
        "--log-level",
    ...
    uvicorn_config = {
        "app": "entelechy_api.server:app" if use_import_string else app,
        "host": args.host,
        "port": args.port,
