...
class GeminiEmbeddings(Embeddings):
    ...
    def _init_vertexai(self, genai) -> None:
        ...
        if not self.vertexai_project_id:
            raise ValueError(
                "ENTELECHY_API_EMBEDDINGS_VERTEXAI_PROJECT_ID (or ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID) "
...
def create_embeddings_from_env() -> Embeddings:
    ...
    if provider == "tei":
        ...
        if not api_key:
            raise ValueError(
                "ENTELECHY_API_EMBEDDINGS_OPENROUTER_API_KEY, ENTELECHY_API_OPENROUTER_API_KEY, "
        ...
        if not api_key:
            if "GITHUB_ACTIONS" in os.environ or os.environ.get("PYTEST_CURRENT_TEST"):
                ...
                raise ValueError(
                    f"{ENV_EMBEDDINGS_COHERE_API_KEY} is required when {ENV_EMBEDDINGS_PROVIDER} is 'cohere'"
                )
