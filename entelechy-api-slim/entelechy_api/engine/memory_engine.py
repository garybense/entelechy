...
if TYPE_CHECKING:
    from entelechy_api.extensions import OperationValidatorExtension, TenantExtension
...
class MemoryEngine(MemoryEngineInterface):
    ...
    def __init__(
        ...
        See entelechy_api.config for environment variable names and defaults.

        Args:
            db_url: PostgreSQL connection URL. Defaults to ENTELECHY_API_DATABASE_URL env var or "pg0".
                    Also supports pg0 URLs: "pg0" or "pg0://instance-name" or "pg0://instance-name:port"
            memory_llm_provider: LLM provider. Defaults to ENTELECHY_API_LLM_PROVIDER env var or "groq".
            memory_llm_api_key: API key for the LLM provider. Defaults to ENTELECHY_API_LLM_API_KEY env var.
            memory_llm_model: Model name. Defaults to ENTELECHY_API_LLM_MODEL env var.
            ...
            pool_min_size: Minimum number of connections in the pool. Defaults to ENTELECHY_API_DB_POOL_MIN_SIZE.
            pool_max_size: Maximum number of connections in the pool. Defaults to ENTELECHY_API_DB_POOL_MAX_SIZE.
            db_command_timeout: PostgreSQL command timeout in seconds. Defaults to ENTELECHY_API_DB_COMMAND_TIMEOUT.
            db_acquire_timeout: Connection acquisition timeout in seconds. Defaults to ENTELECHY_API_DB_ACQUIRE_TIMEOUT.
            ...
            skip_llm_verification: Skip LLM connection verification during initialization.
                                  Defaults to ENTELECHY_API_SKIP_LLM_VERIFICATION env var or False.
            lazy_reranker: Delay reranker initialization until first use. Useful for retain-only
                          ...
                          ENTELECHY_API_LAZY_RERANKER env var or False.
        """
        ...
        if not memory_llm_api_key and requires_api_key(memory_llm_provider):
            raise ValueError("LLM API key is required. Set ENTELECHY_API_LLM_API_KEY environment variable.")
        ...
        # Configurable via ENTELECHY_API_RECALL_MAX_CONCURRENT (default: 50)
        ...
        # Configurable via ENTELECHY_API_RETAIN_MAX_CONCURRENT (default: 4).
    ...
    async def _validate_operation(self, validation_coro) -> "ValidationResult | None":
        ...
        from entelechy_api.extensions import OperationValidationError, ValidationResult
    ...
    async def _authenticate_tenant(self, request_context: "RequestContext | None") -> str:
        ...
        """
        from entelechy_api.extensions import AuthenticationError
    ...
    async def _handle_file_convert_retain(self, task_dict: dict[str, Any]):
        ...
        if self._operation_validator:
            try:
                from entelechy_api.extensions.operation_validator import FileConvertResult
                from entelechy_api.models import RequestContext
    ...
    async def _handle_refresh_mental_model(self, task_dict: dict[str, Any]):
        ...
        if self._operation_validator:
            from entelechy_api.extensions.operation_validator import MentalModelRefreshResult
    ...
    async def _handle_webhook_delivery(self, task_dict: dict[str, Any]) -> None:
        ...
        headers: dict[str, str] = {
            ...
            "X-Entelechy-Event": event_type,
            **http_config.headers,
        }
        if secret and self._webhook_manager:
            headers["X-Entelechy-Signature"] = self._webhook_manager._sign_payload(secret, payload_bytes)
    ...
    async def initialize(self):
        ...
        if llama_parse_key:
            ...
            logger.debug("LlamaParse parser not registered (ENTELECHY_API_FILE_PARSER_LLAMA_PARSE_API_KEY not set)")
    ...
    def retain(
        ...
        from entelechy_api.models import RequestContext as RC
    ...
    async def retain_batch_async(
        ...
        *,
        ...
        if self._operation_validator:
            from entelechy_api.extensions import RetainContext
        ...
        # Call post-operation hook if validator is configured
        if self._operation_validator:
            from entelechy_api.extensions import RetainResult
    ...
    async def _retain_batch_async_internal(
        ...
        from entelechy_api.config_resolver import apply_strategy
    ...
    async def recall_async(
        ...
        *,
        ...
        # Validate operation if validator is configured
        if self._operation_validator:
            from entelechy_api.extensions import RecallContext
        ...
        recall_span_context = tracer.start_as_current_span("entelechy.recall")
        recall_span = recall_span_context.__enter__()
        recall_span.set_attribute("entelechy.bank_id", bank_id)
        recall_span.set_attribute("entelechy.query", query[:100])
        recall_span.set_attribute("entelechy.fact_types", ",".join(fact_type))
        recall_span.set_attribute("entelechy.thinking_budget", thinking_budget)
        recall_span.set_attribute("entelechy.max_tokens", max_tokens)
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        if is_connection_error and attempt < max_retries:
                            ...
                            error_msg = str(e)
                            if self._operation_validator:
                                from entelechy_api.extensions.operation_validator import RecallResult
    ...
    async def _search_with_retries(
        ...
        try:
            ...
            embedding_span = tracer_otel.start_span("entelechy.recall_embedding")
            embedding_span.set_attribute("entelechy.bank_id", bank_id)
            embedding_span.set_attribute("entelechy.query", query[:100])
            ...
            retrieval_span = tracer_otel.start_span("entelechy.recall_retrieval")
            retrieval_span.set_attribute("entelechy.bank_id", bank_id)
            retrieval_span.set_attribute("entelechy.fact_types", ",".join(fact_type))
            retrieval_span.set_attribute("entelechy.thinking_budget", thinking_budget)
            ...
            fusion_span = tracer_otel.start_span("entelechy.recall_fusion")
            fusion_span.set_attribute("entelechy.bank_id", bank_id)
            fusion_span.set_attribute("entelechy.semantic_count", len(semantic_results))
            fusion_span.set_attribute("entelechy.bm25_count", len(bm25_results))
            fusion_span.set_attribute("entelechy.graph_count", len(graph_results))
            fusion_span.set_attribute("entelechy.temporal_count", len(temporal_results) if temporal_results else 0)
            ...
            try:
                ...
                fusion_span.set_attribute("entelechy.merged_count", len(merged_candidates))
                fusion_span.end()
            ...
            rerank_span = tracer_otel.start_span("entelechy.recall_rerank")
            rerank_span.set_attribute("entelechy.bank_id", bank_id)
            rerank_span.set_attribute("entelechy.candidates_count", len(merged_candidates))
            ...
            try:
                ...
                rerank_span.set_attribute("entelechy.scored_count", len(scored_results))
                if pre_filtered_count > 0:
                    rerank_span.set_attribute("entelechy.pre_filtered_count", pre_filtered_count)
                rerank_span.end()
    ...
    async def get_document(
        ...
        *,
        ...
        await self._authenticate_tenant(request_context)
        if self._operation_validator:
            from entelechy_api.extensions import BankReadContext
    ...
    async def delete_document(
        ...
        *,
        ...
        await self._authenticate_tenant(request_context)
        if self._operation_validator:
            from entelechy_api.extensions import BankWriteContext
    ...
    async def get_bank_profile(
        ...
        *,
        ...
        # Apply ENTELECHY_API_DEFAULT_BANK_TEMPLATE to freshly-created banks. Done
    ...
    async def _apply_default_bank_template(
        ...
        """Apply ENTELECHY_API_DEFAULT_BANK_TEMPLATE to a freshly-created bank.
        ...
        from entelechy_api.api.http import (
        ...
        try:
            ...
            logger.error(
                "ENTELECHY_API_DEFAULT_BANK_TEMPLATE failed schema validation "
        ...
        if semantic_errors:
            logger.error(
                "ENTELECHY_API_DEFAULT_BANK_TEMPLATE failed semantic validation "
        ...
        try:
            ...
            logger.info(f"Applied ENTELECHY_API_DEFAULT_BANK_TEMPLATE to newly-created bank '{bank_id}'")
        except Exception as e:
            logger.error(f"Failed to apply ENTELECHY_API_DEFAULT_BANK_TEMPLATE to bank '{bank_id}': {e}")
    ...
    async def list_banks(
        ...
        *,
        ...
        if self._operation_validator:
            from entelechy_api.extensions import BankListContext
    ...
    async def reflect_async(
        ...
        *,
        ...
        if self._reflect_llm_config is None:
            raise ValueError("Memory LLM API key not set. Set ENTELECHY_API_LLM_API_KEY environment variable.")
        ...
        if self._reflect_llm_config.provider == "none":
            ...
            raise LLMNotAvailableError(
                ...
                "Set ENTELECHY_API_LLM_PROVIDER to a real provider (e.g., openai, anthropic, gemini)."
            )
        ...
        # Validate operation if validator is configured
        if self._operation_validator:
            from entelechy_api.extensions import ReflectContext
        ...
        try:
            ...
            from entelechy_api.engine.response_models import DirectiveRef
            ...
            from entelechy_api.engine.response_models import TokenUsage
            ...
            # Call post-operation hook if validator is configured
            if self._operation_validator:
                from entelechy_api.extensions.operation_validator import ReflectResultContext
    ...
    async def get_mental_model(
        ...
        *,
        ...
        if self._operation_validator:
            from entelechy_api.extensions.operation_validator import MentalModelGetContext
        ...
        if result and self._operation_validator:
            from entelechy_api.extensions.operation_validator import MentalModelGetResult
    ...
    async def refresh_mental_model(
        ...
        *,
        ...
        with create_operation_span("mental_model_refresh", bank_id):
            ...
            # Skip creating a nested "entelechy.reflect" span since we already have "entelechy.mental_model_refresh"
    ...
    async def compute_mental_model_is_stale(
        ...
        """
        from entelechy_api.engine.search.tags import _parse_tags_match
    ...
    async def submit_async_refresh_mental_model(
        ...
        *,
        ...
        if self._operation_validator:
            from entelechy_api.extensions.operation_validator import MentalModelRefreshContext
