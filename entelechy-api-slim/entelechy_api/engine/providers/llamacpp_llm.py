...
Usage:
    ENTELECHY_API_LLM_PROVIDER=llamacpp
    ENTELECHY_API_LLAMACPP_MODEL_PATH=~/.entelechy/models/gemma-4-E2B-it-Q4_K_M.gguf
    ENTELECHY_API_LLAMACPP_GPU_LAYERS=-1  # -1 = all layers on GPU
    ENTELECHY_API_LLAMACPP_CONTEXT_SIZE=8192
"""
...
from entelechy_api.engine.llm_interface import LLMInterface
from entelechy_api.engine.response_models import LLMToolCallResult
...
MODELS_DIR = Path.home() / ".entelechy" / "models"
...
def _download_default_model() -> Path:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install with: pip install 'entelechy-api-slim[local-llm]'"
        )
    ...
    if model_path:
        ...
        if not p.exists():
            raise FileNotFoundError(
                ...
                f"Set ENTELECHY_API_LLAMACPP_MODEL_PATH to a valid .gguf file, "
...
class LlamaCppLLM(LLMInterface):
    ...
    async def _ensure_initialized(self) -> None:
        ...
        if self._no_grammar:
            logger.info("Grammar enforcement disabled (ENTELECHY_API_LLAMACPP_NO_GRAMMAR=true)")
