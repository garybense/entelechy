...
from entelechy_api.engine.llm_interface import LLMInterface, OutputTooLongError
from entelechy_api.engine.response_models import LLMToolCall, LLMToolCallResult, TokenUsage
from entelechy_api.metrics import get_metrics_collector
...
class ClaudeCodeLLM(LLMInterface):
    ...
    async def call(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Record trace span
                try:
                    from entelechy_api.tracing import get_span_recorder
    ...
    async def call_with_tools(
        ...
        format to SDK MCP tools, and tool names are formatted as mcp__entelechy_tools__{name}.
        ...
        allowed_tool_names = [f"mcp__entelechy_tools__{name}" for name in tool_names]
        mcp_servers_config = {"entelechy_tools": mcp_server} if sdk_tools else {}
        ...
        if isinstance(tool_choice, dict) and tool_choice.get("type") == "function":
            ...
            if forced_name:
                ...
                forced_tool_mcp_name = f"mcp__entelechy_tools__{forced_name}"
                        ...
                        if isinstance(message, AssistantMessage):
                            for block in message.content:
                                if isinstance(block, TextBlock):
                                    ...
                                    # SDK returns tool names with MCP prefix (mcp__entelechy_tools__{name})
                                    # Strip the prefix to return original tool name expected by caller
                                    tool_name = block.name
                                    if tool_name.startswith("mcp__entelechy_tools__"):
                                        tool_name = tool_name.replace("mcp__entelechy_tools__", "", 1)
