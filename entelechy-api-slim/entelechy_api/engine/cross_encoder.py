...
Configuration via environment variables - see entelechy_api.config for all env var names.
"""
...
class LocalSTCrossEncoder(CrossEncoderModel):
    ...
    async def initialize(self) -> None:
        ...
        if self.force_cpu:
            device = "cpu"
            logger.info("Reranker: forcing CPU mode (ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU=1)")
        else:
...
def create_cross_encoder_from_env() -> CrossEncoderModel:
    ...
    if provider == "tei":
        ...
        if not api_key:
            raise ValueError(
                "ENTELECHY_API_RERANKER_OPENROUTER_API_KEY, ENTELECHY_API_OPENROUTER_API_KEY, "
                f"or ENTELECHY_API_LLM_API_KEY is required when {ENV_RERANKER_PROVIDER} is 'openrouter'"
            )
        return CohereCrossEncoder(
            api_key=api_key,
        ...
        if not project_id:
            raise ValueError(
                f"{ENV_RERANKER_GOOGLE_PROJECT_ID} (or ENTELECHY_API_LLM_VERTEXAI_PROJECT_ID) "
