...
def create_file_storage(
    ...
    **kwargs,
    ...
    if storage_type == "native":
        ...
        if not bucket:
            raise ValueError("ENTELECHY_API_FILE_STORAGE_S3_BUCKET is required for S3 storage")
        ...
        if not bucket:
            raise ValueError("ENTELECHY_API_FILE_STORAGE_GCS_BUCKET is required for GCS storage")
        ...
        if not container:
            raise ValueError("ENTELECHY_API_FILE_STORAGE_AZURE_CONTAINER is required for Azure storage")
