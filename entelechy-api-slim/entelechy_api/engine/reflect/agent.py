...
async def run_reflect_agent(
    ...
    for iteration in range(max_iterations):
        ...
        if done_call:
            ...
            from entelechy_api.tracing import get_tracer

            tracer = get_tracer()
            span_name = "entelechy.reflect_tool_call"
            with tracer.start_as_current_span(span_name) as span:
                span.set_attribute("entelechy.scope", "reflect_tool_call")
                span.set_attribute("entelechy.operation", "reflect_tool_call")
...
async def _execute_tool_with_timing(
    ...
    span_name = f"entelechy.reflect_tool_exec.{normalized_name}"
    ...
    with tracer.start_as_current_span(
        ...
        # Set attributes
        span.set_attribute("entelechy.tool.name", normalized_name)
        span.set_attribute("entelechy.tool.id", tc.id)
        span.set_attribute("entelechy.tool.arguments", json.dumps(tc.arguments, ensure_ascii=False))
        ...
        try:
            ...
            span.set_attribute("entelechy.tool.duration_ms", duration_ms)
