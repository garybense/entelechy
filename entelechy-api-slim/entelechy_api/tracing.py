"""
OpenTelemetry distributed tracing instrumentation for Entelechy API.
...
# Provider name mapping (Entelechy internal -> GenAI semantic convention)
...
def create_operation_span(operation: str, bank_id: str | None = None):
    """
    Create a parent span for a Entelechy operation (retain, reflect, consolidation, etc.).

    This creates the span hierarchy:
    - entelechy.{operation} (parent)
    ...
    span_name = f"entelechy.{operation}"
    span = _tracer.start_as_current_span(span_name)

    # Add operation-specific attributes
    if span and hasattr(span, "set_attribute"):
        span.set_attribute("entelechy.operation", operation)
        if bank_id:
            span.set_attribute("entelechy.bank_id", bank_id)
...
class LLMSpanRecorder:
    ...
    def record_llm_call(
        ...
        Args:
            provider: Entelechy provider name
            model: Model name
        ...
        try:
            ...
            # Create span name: "entelechy.{scope}" for consistency with parent spans
            # Model info is available in span attributes (gen_ai.request.model)
            if scope:
                span_name = f"entelechy.{scope}"
            else:
            ...
            with self.tracer.start_as_current_span(
                ...
                # Add custom attributes for Entelechy context
                span.set_attribute("entelechy.scope", scope)
                span.set_attribute("entelechy.provider.internal", provider)
