"""
Daemon mode support for Entelechy API.
...
DEFAULT_IDLE_TIMEOUT = 0  # 0 = no auto-exit (entelechy-embed passes its own timeout)

# Allow override via environment variable for profile-specific logs
DAEMON_LOG_PATH = Path(os.getenv("ENTELECHY_API_DAEMON_LOG", str(Path.home() / ".entelechy" / "daemon.log")))
...
def daemonize():
    ...
    redirect stdout/stderr to ENTELECHY_API_DAEMON_LOG before exec. We
