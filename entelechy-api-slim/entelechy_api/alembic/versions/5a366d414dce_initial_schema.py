...
def _detect_vector_extension() -> str:
    ...
    Respects ENTELECHY_API_VECTOR_EXTENSION env var if set.
    """
    conn = op.get_bind()
    ...
    if vector_extension == "pgvectorscale":
        ...
        raise ValueError(
            f"Invalid ENTELECHY_API_VECTOR_EXTENSION: {vector_extension}. Must be 'pgvector', 'vchord', or 'pgvectorscale'"
        )
...
def _detect_text_search_extension() -> str:
    ...
    Respects ENTELECHY_API_TEXT_SEARCH_EXTENSION env var.
    Creates the extension if needed.
    """
    text_search_extension = os.getenv("ENTELECHY_API_TEXT_SEARCH_EXTENSION", "native").lower()
    ...
    if text_search_extension == "vchord":
        ...
        raise ValueError(
            f"Invalid ENTELECHY_API_TEXT_SEARCH_EXTENSION: {text_search_extension}. Must be 'native', 'vchord', or 'pg_textsearch'"
        )
