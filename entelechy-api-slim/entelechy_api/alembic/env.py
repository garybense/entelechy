...
from entelechy_api.db_url import to_libpq_url
from entelechy_api.models import Base


# Load environment variables based on ENTELECHY_API_DATABASE_URL env var or default to local
def load_env():
    """Load environment variables from .env"""
    # Check if ENTELECHY_API_DATABASE_URL is already set (e.g., by CI/CD)
    if os.getenv("ENTELECHY_API_DATABASE_URL"):
        return
    ...
    if not database_url:
        ...
        if not database_url:
            raise ValueError(
                ...
                "Set ENTELECHY_API_DATABASE_URL environment variable or pass database_url to run_migrations()."
            )
