...
DEFAULT_USERNAME = "entelechy"
DEFAULT_PASSWORD = "entelechy"
DEFAULT_DATABASE = "entelechy"
        ...
        if self._pg0 is None:
            try:
                ...
                raise ImportError(
                    ...
                    "Install it with: pip install 'entelechy-api-slim[embedded-db]'"
                )
            kwargs = {
                "name": self.name,
...
def parse_pg0_url(db_url: str) -> tuple[bool, str | None, int | None]:
    ...
    Supports:
    - "pg0" -> default instance "entelechy"
    ...
    """
    if db_url == "pg0":
        return True, "entelechy", None

    if db_url.startswith("pg0://"):
        url_part = db_url[6:]  # Remove "pg0://"
        if ":" in url_part:
            instance_name, port_str = url_part.rsplit(":", 1)
            return True, instance_name or "entelechy", int(port_str)
        else:
            return True, url_part or "entelechy", None
