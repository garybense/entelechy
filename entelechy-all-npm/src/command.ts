/**
 * Resolve the command that invokes the `entelechy-embed` Python CLI.
 *
 * - If `embedPackagePath` is set, runs the package from a local checkout via
 *   `uv run --directory <path> entelechy-embed`. Used for in-repo development.
 * - Otherwise runs it via `uvx entelechy-embed@<version>` so no global install
 *   is required.
 *
...
export function getEmbedCommand(opts: EmbedCommandOptions = {}): string[] {
  if (opts.embedPackagePath) {
    return ["uv", "run", "--directory", opts.embedPackagePath, "entelechy-embed"];
  }
  const version = opts.embedVersion && opts.embedVersion.length > 0 ? opts.embedVersion : "latest";
  return ["uvx", `entelechy-embed@${version}`];
}
