...
/**
 * Options for {@link EntelechyServer}.
 *
 ...
 * a new dedicated option. Use {@link env} for `ENTELECHY_*` / `OPENAI_API_KEY` /
 ...
 * client.
 */
export interface EntelechyServerOptions {
  ...
  host?: string;
  /** Version of the underlying `entelechy-embed` PyPI package to run via `uvx`. Default: `"latest"`. */
  embedVersion?: string;
  /** Local path to a `entelechy-embed` checkout — takes precedence over `embedVersion`. */
   ...
   * to surface any `ENTELECHY_API_*` / `ENTELECHY_EMBED_*` setting — adding a
   * new daemon env var never requires a wrapper update.
   *
   * Values of `undefined` are dropped (so you can spread conditionally).
   */
  env?: Record<string, string | undefined>;
  /** Extra args appended verbatim to `entelechy-embed profile create <name> --merge ...`. */
  extraProfileCreateArgs?: string[];
  /** Extra args appended verbatim to `entelechy-embed daemon --profile <name> start ...`. */
  extraDaemonStartArgs?: string[];
  /**
   * On macOS, automatically set
   * `ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU=1` and
   * `ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU=1` to avoid Metal/MPS crashes in
