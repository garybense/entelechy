...
Store documents, conversations, and raw content into Entelechy to automatically extract and create memories.

When you **retain** content, Entelechy doesn't just store the raw text—it intelligently analyzes the content to extract meaningful facts, identify entities, and build a connected knowledge graph. This process transforms unstructured information into structured, queryable memories.
...
A single retain call accepts one or more **items**. Each item is a piece of raw content — a conversation, a document, a note — that Entelechy will analyze and decompose into one or many memories. The content itself is never stored verbatim; what gets stored are the structured facts the LLM extracts from it.
...
entelechy memory retain my-bank "$CONVERSATION" \
    --context "team chat" \
...
When the conversation grows — a new message arrives — just retain again with the full updated content and the same `document_id`. Entelechy will delete the previous version and reprocess from scratch, so memories always reflect the latest state of the conversation.
...
The raw text to store. This is the only required field. Entelechy chunks the content, sends each chunk to the LLM for fact extraction, and stores the resulting structured facts — not the original text. A single `content` value can produce many memories depending on how much information it contains.
...
```bash
entelechy memory retain my-bank "Alice got promoted" \
    --context "career update"
```
...
When you provide a `document_id`, Entelechy upserts the document: if a document with that ID already exists in the bank, it and all its associated memories are deleted before the new content is processed and inserted. This means you can safely re-run retain on updated content — for example, a chat thread that grew since last time — without accumulating duplicate memories.

If you omit `document_id`, Entelechy assigns a random UUID per request, so re-ingesting the same content will create duplicate memories.

### update_mode

Controls how Entelechy handles an existing document when you retain with a `document_id` that already exists.
...
**When to use append**: Use `"append"` for content that grows incrementally — for example, a log file, a journal, or a chat transcript where you receive new messages one at a time. Instead of re-sending the entire history on each update, send only the new content with `update_mode: "append"` and Entelechy will efficiently merge it with what it already has.
...
Multiple items can be submitted in a single request. Batch ingestion is the recommended approach — it reduces network overhead and lets Entelechy optimize extraction across related content.
...
entelechy memory retain my-bank "Alice works at Google" \
    --context "career" --doc-id "conversation_001_msg_1"
entelechy memory retain my-bank "Bob is a data scientist at Meta" \
    --context "career" --doc-id "conversation_001_msg_2"
entelechy memory retain my-bank "Alice and Bob are friends" \
...
Upload files directly — Entelechy converts them to text and extracts memories automatically. File processing always runs asynchronously and returns operation IDs for tracking.
...
entelechy memory retain-files my-bank "$SAMPLE_FILE"

# Upload a directory of files
entelechy memory retain-files my-bank "$SCRIPT_DIR/"

# Queue files for background processing (returns immediately)
entelechy memory retain-files my-bank "$SCRIPT_DIR/" --async
```
...
:::info File Storage
Uploaded files are stored server-side (PostgreSQL by default, or S3/GCS/Azure for production). Configure storage via `ENTELECHY_API_FILE_STORAGE_TYPE`. See [Configuration](../configuration#file-processing) for details.
---
...
```bash
entelechy memory retain my-bank "Meeting notes" --async
```
...
```bash
export ENTELECHY_API_RETAIN_BATCH_ENABLED=true
```

Entelechy submits fact extraction calls as a batch job to the provider, polls for completion, and processes results automatically. No changes to your API calls are needed.
