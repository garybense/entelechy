...
Entelechy provides three core operations: **retain**, **recall**, and **reflect**.

{/* Import raw source files */}

> **💡 Prerequisites**
> 
Make sure you've [installed Entelechy](../installation) and completed the [Quick Start](./quickstart).
...
```bash
# Store a single fact
entelechy memory retain my-bank "Alice joined Google in March 2024 as a Senior ML Engineer"

# Store from a file
entelechy memory retain-files my-bank conversation.txt --context "Daily standup"
...
```bash
# Basic search
entelechy memory recall my-bank "What does Alice do at Google?"

# Search with options
entelechy memory recall my-bank "What happened last spring?" \
    --budget high \
    --max-tokens 8192 \
    --fact-type world,experience

# Verbose output
entelechy memory recall my-bank "Tell me about Alice" -v
```
...
```bash
# Basic reflect
entelechy memory reflect my-bank "Should we adopt TypeScript for our backend?"

# With higher reasoning budget
entelechy memory reflect my-bank "Analyze our tech stack" --budget high
```
