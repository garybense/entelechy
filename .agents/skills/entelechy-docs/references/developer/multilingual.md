...
Entelechy automatically detects the language of your input and responds in the same language. This means facts, entities, and reflect responses are preserved in their original language without translation to English.
...
When you retain content or reflect on a query, Entelechy:
...
When you retain content in any language, Entelechy extracts and stores facts in that same language.

### Example: Chinese Content

```python
from entelechy import Entelechy
...
Entelechy handles mixed-language content gracefully, preserving both languages where appropriate.
...
**Entelechy's multilingual support depends entirely on your LLM's language capabilities.** Entelechy instructs the LLM to detect the input language and respond in that same language. If your LLM supports a language, Entelechy will work with it.
...
**To verify support for your target language**, test your LLM directly with content in that language. If the LLM can understand and generate text in the language, Entelechy will preserve it correctly.
...
```bash
# In your .env file
ENTELECHY_API_EMBEDDINGS_LOCAL_MODEL=BAAI/bge-m3
```
...
```bash
# In your .env file
ENTELECHY_API_RERANKER_LOCAL_MODEL=BAAI/bge-reranker-v2-m3
```
