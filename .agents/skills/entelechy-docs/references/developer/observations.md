...
After memories are retained, Entelechy automatically consolidates related facts into **observations** — deduplicated, evidence-grounded beliefs the bank has built up from multiple memories. Each observation tracks its supporting evidence (with exact quotes), a proof count, and a computed freshness trend, and is refined rather than overwritten when new evidence arrives.
...
Set `observations_mission` via the [bank config API](api/memory-banks.md#observations-configuration) or the [`ENTELECHY_API_OBSERVATIONS_MISSION`](configuration.md#observations) environment variable.
...
Observations are derived from source memories. When source memories are removed, Entelechy automatically keeps observations consistent:
