...
Traditional RAG (Retrieval-Augmented Generation) retrieves documents similar to a query. Entelechy provides structured memory with temporal reasoning, entity understanding, and belief formation.

## Capability Comparison

| Capability | RAG | Entelechy |
|------------|-----|-----------|
...
| Entelechy | Traverses Alice → Project Atlas → Kubernetes → outage via entity links |
...
| Entelechy | Parses "last spring" → March-May, filters to that range |
...
| Entelechy | Returns connected facts via entity graph: subscription status, billing, known issues |
...
| Entelechy | Consolidates mental model "user prefers sync" → refines to "user growing comfortable with async" |
...
| AI assistants with persistent memory | Entelechy |
| Applications requiring entity tracking | Entelechy |
| Systems needing consistent disposition | Entelechy |
| Temporal queries ("last month", "in 2023") | Entelechy |
