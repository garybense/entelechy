 ...
 * and forwards to the Entelechy origin.
 */

import type { Env } from "./env";

type ProxyEnv = Pick<Env, "ENTELECHY_ORIGIN" | "PROXY_SECRET" | "ENTELECHY_API_TOKEN">;
...
export async function proxyRequest(
  ...
  const originUrl = new URL(env.ENTELECHY_ORIGIN);
  ...
  headers.set("Authorization", `Bearer ${env.ENTELECHY_API_TOKEN}`);
  ...
  try {
    ...
    console.error("Failed to proxy to Entelechy:", err);
