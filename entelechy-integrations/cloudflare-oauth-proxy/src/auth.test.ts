...
function createEnv(overrides: Partial<Env> = {}): Env {
  return {
    ...
    ENTELECHY_ORIGIN: "https://entelechy-origin.example.com",
    ALLOWED_EMAIL: "test@example.com",
    SESSION_SECRET: "correct-horse-battery-staple",
    PROXY_SECRET: "proxy-secret",
    ENTELECHY_API_TOKEN: "server-token",
    ...overrides,
  };
}
...
describe("handleDefaultRequest", () => {
  ...
  describe("/health", () => {
    it("returns { status: 'ok' }", async () => {
      const response = await handleDefaultRequest(
        new Request("https://entelechy.mydomain.com/health"),
        env
      );
  ...
  describe("/ (service root)", () => {
    it("returns the service identifier", async () => {
      const response = await handleDefaultRequest(
        new Request("https://entelechy.mydomain.com/"),
        env
      );
      expect(response.status).toBe(200);
      expect(await response.json()).toEqual({ service: "Entelechy MCP OAuth Proxy" });
    });
  });
  ...
  describe("unknown paths", () => {
    it("returns 404", async () => {
      const response = await handleDefaultRequest(
        new Request("https://entelechy.mydomain.com/nope"),
        env
      );
  ...
  describe("GET /authorize", () => {
    it("stores OAuth state in KV and renders the login page", async () => {
      const response = await handleDefaultRequest(
        new Request("https://entelechy.mydomain.com/authorize?client_id=x"),
        env
      );
      ...
      expect(html).toContain("Entelechy MCP");
  ...
  describe("POST /authorize", () => {
    async function postForm(form: Record<string, string>, e: Env = env): Promise<Response> {
      ...
      return handleDefaultRequest(
        new Request("https://entelechy.mydomain.com/authorize", {
          method: "POST",
