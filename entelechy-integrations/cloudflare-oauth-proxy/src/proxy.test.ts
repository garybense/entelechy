...
const baseEnv = {
  ...
  ENTELECHY_API_TOKEN: "server-api-token",
};

describe("proxyRequest", () => {
  it("rewrites the target URL to the Entelechy origin", async () => {
    let calledUrl: string | undefined;
    const fetchImpl = vi.fn<typeof fetch>(async (url) => {
      calledUrl = url.toString();
      return new Response("ok");
    });
    const request = new Request("https://entelechy.mydomain.com/mcp", {
      method: "POST",
    ...
    expect(calledUrl).toBe("https://entelechy-origin.example.com/mcp");
  });
  ...
  it("drops upstream CORS and Set-Cookie headers from the response", async () => {
    ...
    const request = new Request("https://entelechy.mydomain.com/mcp", { method: "GET" });
  ...
  it("does not forward the Host header to the upstream", async () => {
    ...
    const request = new Request("https://entelechy.mydomain.com/mcp", {
      method: "GET",
      headers: { Host: "entelechy.mydomain.com" },
    });
