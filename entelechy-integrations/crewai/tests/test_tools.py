"""Unit tests for EntelechyReflectTool."""

from unittest.mock import MagicMock, patch

from entelechy_crewai import EntelechyReflectTool, configure, reset_config
...
class TestReflectTool:
    ...
    @patch("entelechy_crewai.tools.call_sync", side_effect=_passthrough)
    def test_reflect_returns_text(self, _mock_cs):
        tool = EntelechyReflectTool(bank_id="test-bank")
    ...
    def test_reflect_passes_budget(self, _mock_cs):
        tool = EntelechyReflectTool(bank_id="test-bank", budget="high")
        mock_client = MagicMock()
        mock_result = MagicMock()
    ...
    @patch("entelechy_crewai.tools.call_sync", side_effect=_passthrough)
    def test_reflect_raises_entelechy_error_on_failure(self, _mock_cs):
        tool = EntelechyReflectTool(bank_id="test-bank")
        mock_client = MagicMock()
