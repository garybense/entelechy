"""Unit tests for EntelechyStorage."""

from unittest.mock import MagicMock, patch

from entelechy_crewai import EntelechyStorage, configure, reset_config
from entelechy_crewai.errors import EntelechyError
...
class TestEntelechyStorage:
    ...
    def _make_storage(self, **kwargs):
        ...
        storage = EntelechyStorage(bank_id="test-bank", **kwargs)
    ...
    @patch("entelechy_crewai.storage.call_sync", side_effect=_passthrough)
    ...
    @patch("entelechy_crewai.storage.call_sync", side_effect=_passthrough)
    def test_save_raises_entelechy_error_on_failure(self, _mock_cs):
        storage, mock_client = self._make_storage()
        mock_client.retain.side_effect = RuntimeError("connection refused")

        with pytest.raises(EntelechyError, match="Failed to store memory"):
            storage.save("text")
    ...
    @patch("entelechy_crewai.storage.call_sync", side_effect=_passthrough)
    def test_search_raises_entelechy_error_on_failure(self, _mock_cs):
        storage, mock_client = self._make_storage()
        mock_client.recall.side_effect = RuntimeError("timeout")

        with pytest.raises(EntelechyError, match="Failed to search memories"):
            storage.search("test")
    ...
    def test_per_agent_banks_resolves_bank_id(self):
        storage = EntelechyStorage(bank_id="crew", per_agent_banks=True)
    ...
    def test_custom_bank_resolver(self):
        ...
        storage = EntelechyStorage(bank_id="crew", bank_resolver=resolver)
