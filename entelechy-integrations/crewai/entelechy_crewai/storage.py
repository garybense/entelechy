"""CrewAI Storage backend powered by Entelechy.

Implements CrewAI's Storage interface (save/search/reset) using
Entelechy's retain/recall APIs for persistent agent memory.
"""
...
try:
    _VERSION = metadata.version("entelechy-crewai")
except metadata.PackageNotFoundError:
    _VERSION = "0.0.0"
_USER_AGENT = f"entelechy-crewai/{_VERSION}"


class EntelechyStorage(Storage):
    """CrewAI Storage backend that persists memories to Entelechy.

    Maps CrewAI's storage interface to Entelechy's memory API:
    ...
    Args:
        bank_id: The Entelechy memory bank ID for this crew.
    ...
    def __init__(
        ...
        config = get_config()
        self._api_url = entelechy_api_url or (config.entelechy_api_url if config else "http://localhost:8888")
    ...
    def _get_client(self) -> Any:
        """Get or create a thread-local Entelechy client.
    ...
    def save(
        ...
        Raises:
            EntelechyError: If the retain operation fails.
        """
        bank_id = self._resolve_bank_id(agent)
        self._ensure_bank(bank_id)

        # Build retain metadata — Entelechy requires dict[str, str]
        ...
        try:
            ...
            raise EntelechyError(f"Failed to store memory: {e}") from e
    ...
    def search(
        ...
        """Search memories in Entelechy via recall.
        ...
        Raises:
            EntelechyError: If the recall operation fails.
        """
        ...
        try:
            ...
            for i, r in enumerate(recall_results[:limit]):
                # Entelechy returns results ordered by relevance.
            ...
            raise EntelechyError(f"Failed to search memories: {e}") from e
