...
CrewAI runs inside an async event loop. The Entelechy client's sync
...
event loop for all Entelechy API calls, ensuring the aiohttp session
...
def call_sync(fn: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    """Call a sync Entelechy client method safely.
