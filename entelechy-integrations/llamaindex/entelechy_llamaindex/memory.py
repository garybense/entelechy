"""Entelechy BaseMemory implementation for LlamaIndex.

Provides automatic memory for LlamaIndex agents:
- ``put()`` retains messages to Entelechy for long-term storage
...
class EntelechyMemory(BaseMemory):
    """Automatic long-term memory for LlamaIndex agents via Entelechy.

    On ``put()``, user and assistant messages are automatically retained
    to Entelechy. On ``get()``, relevant memories are recalled and
    ...
    a local chat buffer for the current session, with Entelechy
    providing cross-session long-term memory.

    Args:
        bank_id: Entelechy memory bank to operate on.
    ...
    bank_id: str = Field(description="Entelechy memory bank ID")
    ...
    def __init__(self, client: Entelechy, mission: Optional[str] = None, **kwargs: Any):
    ...
    def class_name(cls) -> str:
        return "EntelechyMemory"

    @classmethod
    def from_defaults(cls, **kwargs: Any) -> "EntelechyMemory":
        """Create from defaults. Prefer ``from_client()`` instead."""
        raise NotImplementedError("Use EntelechyMemory.from_client() or EntelechyMemory.from_url() instead.")
    ...
    def from_client(
        ...
        *,
        ...
        """Create a EntelechyMemory with a pre-configured client.

        Args:
            client: Entelechy client instance.
    ...
    def from_url(
        ...
        **kwargs: Any,
        ...
        """Create a EntelechyMemory from an API URL.
        ...
        """
        client_kwargs: dict[str, Any] = {"base_url": entelechy_api_url, "timeout": 30.0}
        if api_key:
            client_kwargs["api_key"] = api_key
        client = Entelechy(**client_kwargs)
    ...
    def _retain_message(self, message: ChatMessage) -> None:
        """Retain a message to Entelechy (sync)."""
    ...
    async def _aretain_message(self, message: ChatMessage) -> None:
        """Retain a message to Entelechy (async)."""
        if message.role not in (MessageRole.USER, MessageRole.ASSISTANT):
            return
        content = str(message.content) if message.content else ""
        content = self._extract_clean_content(content, message.role)
    ...
    def get(self, input: Optional[str] = None, **kwargs: Any) -> list[ChatMessage]:
        """Get chat history, enriched with recalled Entelechy memories.
    ...
    def put(self, message: ChatMessage) -> None:
        """Store a message in local buffer and retain to Entelechy."""
    ...
    def set(self, messages: list[ChatMessage]) -> None:
        """Set the chat history, retaining new messages to Entelechy."""
    ...
    def reset(self) -> None:
        """Reset the local chat buffer. Does not clear Entelechy memories."""
        self._chat_history = []
