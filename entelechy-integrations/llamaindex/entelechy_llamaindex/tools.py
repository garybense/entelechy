"""LlamaIndex tool spec for Entelechy memory operations.

Provides a ``BaseToolSpec`` subclass and a convenience factory that create
LlamaIndex-compatible tools backed by Entelechy's retain/recall/reflect APIs.
"""
...
class EntelechyToolSpec(BaseToolSpec):
    """LlamaIndex tool spec providing Entelechy memory tools.
    ...
    Example::
        ...
        spec = EntelechyToolSpec(client=client, bank_id="user-123")
...
def create_entelechy_tools(
    *,
    ...
    """Create Entelechy memory tools for a LlamaIndex agent.

    Convenience factory that creates a ``EntelechyToolSpec`` and returns
