"""Manual integration test for Entelechy LlamaIndex tools.

Requires a running Entelechy server at http://localhost:8888.
...
@pytest.fixture
def client():
    return Entelechy(base_url=ENTELECHY_URL, timeout=30.0)
...
@pytest.mark.skip(reason="Requires running Entelechy server")
class TestManualToolSpec:
    def test_retain_and_recall_round_trip(self, client, bank_id):
        spec = EntelechyToolSpec(client=client, bank_id=bank_id)
    ...
    def test_create_entelechy_tools_factory(self, client, bank_id):
