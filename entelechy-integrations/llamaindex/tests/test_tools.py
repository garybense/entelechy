"""Unit tests for Entelechy LlamaIndex tools."""

from unittest.mock import AsyncMock, MagicMock, patch

import pytest
from entelechy_llamaindex import (
    EntelechyToolSpec,
    configure,
    create_entelechy_tools,
    reset_config,
)
from entelechy_llamaindex.errors import EntelechyError


def _mock_client():
    """Create a mock Entelechy client with sync and async methods."""
    client = MagicMock()
...
class TestEntelechyToolSpec:
    def test_spec_functions_list(self):
        assert EntelechyToolSpec.spec_functions == [
    ...
    def test_to_tool_list_returns_three_tools(self):
        client = _mock_client()
        spec = EntelechyToolSpec(bank_id="test", client=client)
...
class TestCreateEntelechyTools:
    ...
    def test_falls_back_to_global_config(self):
        ...
        with patch("entelechy_llamaindex._client.Entelechy") as mock_cls:
...
class TestRetainTool:
    def test_retain_stores_memory(self):
        ...
        spec = EntelechyToolSpec(bank_id="test-bank", client=client)
    ...
    def test_retain_passes_context_label(self):
        client = _mock_client()
        client.retain.return_value = _mock_retain_response()
        spec = EntelechyToolSpec(bank_id="test", client=client, retain_context="my-app")
        spec.retain_memory("content")
        call_kwargs = client.retain.call_args[1]
...
class TestReflectTool:
    ...
    def test_reflect_passes_budget(self):
        ...
        spec = EntelechyToolSpec(bank_id="test", client=client, budget="high")
...
class TestBankMission:
    ...
    def test_mission_from_config(self):
        ...
        with patch("entelechy_llamaindex._client.Entelechy") as mock_cls:
            ...
            spec = EntelechyToolSpec(bank_id="test")
            spec.retain_memory("content")
...
class TestConfigFallback:
    ...
    def test_budget_falls_back_to_config(self):
        ...
        with patch("entelechy_llamaindex._client.Entelechy") as mock_cls:
            ...
            reset_config()
            configure(entelechy_api_url="http://localhost:8888", budget="high")
    ...
    def test_explicit_budget_overrides_config(self):
        ...
        with patch("entelechy_llamaindex._client.Entelechy") as mock_cls:
            ...
            spec = EntelechyToolSpec(bank_id="test", budget="low")
            spec.recall_memory("query")
            call_kwargs = mock_instance.recall.call_args[1]
    ...
    def test_context_falls_back_to_config(self):
        configure(entelechy_api_url="http://localhost:8888", context="my-app")
        with patch("entelechy_llamaindex._client.Entelechy") as mock_cls:
            mock_instance = _mock_client()
            mock_cls.return_value = mock_instance
            mock_instance.retain.return_value = _mock_retain_response()
