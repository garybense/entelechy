/**
 * Tests for paperclip-plugin-entelechy.
 *
 ...
 *
 * Entelechy API calls are intercepted via global fetch mocking.
 */
...
describe("agent.run.started", () => {
  ...
  it("does not throw when Entelechy is unreachable", async () => {
    vi.stubGlobal(
      "fetch",
...
// ---------------------------------------------------------------------------
// entelechy_recall tool
// ---------------------------------------------------------------------------

describe("entelechy_recall tool", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });
...
// ---------------------------------------------------------------------------
// entelechy_retain tool
// ---------------------------------------------------------------------------

describe("entelechy_retain tool", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it("stores content via Entelechy retain endpoint", async () => {
...
describe("onValidateConfig", () => {
  it("fails when entelechyApiUrl is missing", async () => {
    const result = await plugin.definition.onValidateConfig!({ entelechyApiUrl: "" });
    expect(result.ok).toBe(false);
    expect(result.errors?.some((e) => e.includes("entelechyApiUrl"))).toBe(true);
  });

  it("fails when Entelechy is unreachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response("", { status: 503 }))
    );
  ...
  it("passes with a reachable Entelechy instance", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response("{}", { status: 200 }))
    );
