"""LLM provider detection for Entelechy's fact extraction.

Port of: detectLLMConfig() in index.js

When running entelechy-embed locally (daemon mode), it needs an LLM to
extract facts from retained conversations. This module detects the LLM
config using the same priority chain as Openclaw:

  1. ENTELECHY_API_LLM_* environment variables (highest priority)
...
def detect_llm_config(config: dict) -> dict:
    ...
    """
    override_provider = os.environ.get("ENTELECHY_API_LLM_PROVIDER")
    override_model = os.environ.get("ENTELECHY_API_LLM_MODEL")
    override_key = os.environ.get("ENTELECHY_API_LLM_API_KEY")
    override_base_url = os.environ.get("ENTELECHY_API_LLM_BASE_URL")

    # Priority 1: ENTELECHY_API_LLM_PROVIDER env var
    if override_provider:
        if not override_key and override_provider not in NO_KEY_REQUIRED:
            raise RuntimeError(
                f'ENTELECHY_API_LLM_PROVIDER is set to "{override_provider}" but ENTELECHY_API_LLM_API_KEY is not set.'
            )
        ...
        return {
            ...
            "source": "ENTELECHY_API_LLM_PROVIDER override",
        }
    ...
    raise RuntimeError(
        "No LLM configuration found for Entelechy.\n\n"
        "Option 1: Set a standard provider API key (auto-detect):\n"
        "  export OPENAI_API_KEY=sk-your-key\n"
        "  export ANTHROPIC_API_KEY=your-key\n\n"
        "Option 2: Override with Entelechy-specific env vars:\n"
        "  export ENTELECHY_API_LLM_PROVIDER=openai\n"
        "  export ENTELECHY_API_LLM_API_KEY=sk-your-key\n\n"
        "Option 3: Use an external Entelechy API (server-side LLM):\n"
        "  Set entelechyApiUrl in settings.json or ENTELECHY_API_URL env var\n\n"
        "The model will be selected automatically by Entelechy. To override: export ENTELECHY_API_LLM_MODEL=your-model"
    )


def get_llm_env_vars(llm_config: dict) -> dict:
    """Build environment variables for entelechy-embed daemon from LLM config.
    ...
    if llm_config.get("provider"):
        env["ENTELECHY_API_LLM_PROVIDER"] = llm_config["provider"]
    if llm_config.get("api_key"):
        env["ENTELECHY_API_LLM_API_KEY"] = llm_config["api_key"]
    if llm_config.get("model"):
        env["ENTELECHY_API_LLM_MODEL"] = llm_config["model"]
    if llm_config.get("base_url"):
        env["ENTELECHY_API_LLM_BASE_URL"] = llm_config["base_url"]
    return env
