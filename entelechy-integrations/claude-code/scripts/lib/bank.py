...
Claude Code equivalent dimensions:
  - agent   → configured name or "claude-code" (ENTELECHY_AGENT_NAME)
  - project → derived from cwd (working directory basename)
  - session → session_id from hook input
  - channel → from env var ENTELECHY_CHANNEL_ID (for Telegram/Discord agents)
  - user    → from env var ENTELECHY_USER_ID (for multi-user agents)
...
Telegram/Discord agents set ENTELECHY_CHANNEL_ID and ENTELECHY_USER_ID in
...
def derive_bank_id(hook_input: dict, config: dict) -> str:
    ...
    for f in fields:
        if f not in VALID_FIELDS:
            print(
                f'[Entelechy] Unknown dynamicBankGranularity field "{f}" — '
    ...
    # (e.g. Telegram bot sets ENTELECHY_CHANNEL_ID=telegram-group-12345)
    channel_id = os.environ.get("ENTELECHY_CHANNEL_ID", "")
