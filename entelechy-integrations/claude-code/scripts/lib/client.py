"""Entelechy REST API client.

Communicates with a Entelechy server via HTTP. Mirrors the HTTP mode of the
Openclaw EntelechyClient (client.js), adapted for Python stdlib.
"""
...
USER_AGENT = f"entelechy-claude-code/{_plugin_version()}"
...
def _validate_api_url(url: str) -> str:
    ...
    if parsed.scheme not in ("http", "https"):
        raise ValueError(f"Entelechy API URL must use http or https, got: {parsed.scheme!r}")
    if not parsed.hostname:
        raise ValueError(f"Entelechy API URL has no hostname: {url!r}")
    return url.rstrip("/")


class EntelechyClient:
    """HTTP client for the Entelechy API."""
    ...
    def health_check(self, timeout: int = 5) -> bool:
        """Check if the Entelechy server is reachable.
    ...
    def retain(
        ...
        The context field helps Entelechy cluster memories by provenance
