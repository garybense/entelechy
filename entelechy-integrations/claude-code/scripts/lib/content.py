...
def strip_channel_envelope(content: str) -> str:
    ...
    transport metadata that Entelechy doesn't need.
    """
...
def strip_memory_tags(content: str) -> str:
    """Remove <entelechy_memories> and <relevant_memories> blocks.
    ...
    """
    content = re.sub(r"<entelechy_memories>[\s\S]*?</entelechy_memories>", "", content)
    ...
    for block in content:
        ...
        if block_type == "text":
            ...
            if _is_channel_message_tool(block):
                ...
                inp = block.get("input", {})
                # Skip Entelechy MCP tools to avoid feedback loops
...
def _is_channel_message_tool(block: dict) -> bool:
    ...
    purposes (entelechy recall, search) are excluded by pattern and by
