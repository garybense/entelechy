...
auto-started a entelechy-embed daemon, this is where we stop it.
...
def main():
    ...
    if config.get("autoRetain") and hook_input.get("transcript_path"):
        try:
            ...
            print(f"[Entelechy] SessionEnd final retain error: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    try:
        ...
        print(f"[Entelechy] SessionEnd error: {e}", file=sys.stderr)
        sys.exit(0)
