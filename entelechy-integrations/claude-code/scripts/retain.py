...
Flow:
  ...
  7. POST to Entelechy retain API (async)
...
from lib.client import EntelechyClient
...
def run_retain(hook_input: dict, force: bool = False) -> None:
    ...
    try:
        ...
        print(f"[Entelechy] {e}", file=sys.stderr)
        return

    api_token = config.get("entelechyApiToken")
    try:
        client = EntelechyClient(api_url, api_token)
    except ValueError as e:
        print(f"[Entelechy] Invalid API URL: {e}", file=sys.stderr)
        return
    ...
    template_vars = {
        ...
        "user_id": os.environ.get("ENTELECHY_USER_ID", ""),
    }
    ...
    # when ENTELECHY_USER_ID is unset). Tags without ':' are preserved as-is.
    ...
    try:
        ...
        print(f"[Entelechy] Retain failed: {e}", file=sys.stderr)
...
def main():
    try:
        ...
        print("[Entelechy] Failed to read hook input", file=sys.stderr)
        return
...
if __name__ == "__main__":
    try:
        ...
        print(f"[Entelechy] Unexpected error in retain: {e}", file=sys.stderr)
        try:
