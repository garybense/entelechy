#!/usr/bin/env python3
"""Register entelechy-memory hooks into ~/.claude/settings.json.
...
Or via the /entelechy:setup skill inside a Claude Code session.
"""
...
def find_plugin_root() -> str:
    """Locate the installed entelechy-memory plugin cache directory."""
    cache_base = os.path.expanduser("~/.claude/plugins/cache/entelechy/entelechy-memory")
    if not os.path.isdir(cache_base):
        raise RuntimeError(
            "entelechy-memory plugin not found. "
            "Run /plugin install entelechy-memory inside Claude Code first."
        )
...
def main():
    ...
    if existing_hooks:
        print("Existing hooks found — merging (entelechy-memory hooks take precedence).")
    ...
    print(f"entelechy-memory hooks registered successfully.")
