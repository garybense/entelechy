...
- `{user_id}` template variable for `retainTags` and `retainMetadata`, resolved
  from the `ENTELECHY_USER_ID` env var (empty string if unset). Enables
...
- Tags that resolve to an empty namespace content (e.g. `"user:"` when
  `ENTELECHY_USER_ID` is unset) are now dropped from retain requests. Previously
  such tags were sent as-is. Tags without `:` are unaffected.

## [0.1.0] - 2025-03-23

### Added
- Initial release: Claude Code plugin for Entelechy long-term memory
...
- Three connection modes: external API, auto-managed local daemon (`uvx entelechy-embed`), existing local server
