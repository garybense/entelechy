...
class TestLoadConfig:
    ...
    def test_api_url_env_override(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CLAUDE_PLUGIN_ROOT", str(tmp_path))
        monkeypatch.setenv("ENTELECHY_API_URL", "http://myserver:8080")
        cfg = load_config()
        assert cfg["entelechyApiUrl"] == "http://myserver:8080"
    ...
    def test_user_config_overrides_plugin_settings(self, tmp_path, monkeypatch):
        ...
        # User overrides to "high" via ~/.entelechy/claude-code.json
        user_cfg = tmp_path / ".entelechy" / "claude-code.json"
    ...
    def test_user_config_missing_falls_back_gracefully(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CLAUDE_PLUGIN_ROOT", str(tmp_path))
        # HOME points to tmp_path where no .entelechy/claude-code.json exists
        monkeypatch.setenv("HOME", str(tmp_path))
        cfg = load_config()
    ...
    def test_env_var_wins_over_user_config(self, tmp_path, monkeypatch):
        plugin_root = tmp_path / "plugin"
        plugin_root.mkdir()
        user_cfg_dir = tmp_path / ".entelechy"
        user_cfg_dir.mkdir()
