...
class TestDeriveBankIdDynamic:
    def test_dynamic_agent_project(self):
        ...
        result = derive_bank_id(_hook(cwd="/home/user/entelechy"), cfg)
        assert result == "mybot::entelechy"
    ...
    def test_dynamic_channel_from_env(self, monkeypatch):
        monkeypatch.setenv("ENTELECHY_CHANNEL_ID", "telegram-123")
    ...
    def test_dynamic_user_from_env(self, monkeypatch):
        monkeypatch.setenv("ENTELECHY_USER_ID", "user-456")
    ...
    def test_dynamic_missing_env_uses_defaults(self, monkeypatch):
        monkeypatch.delenv("ENTELECHY_CHANNEL_ID", raising=False)
        monkeypatch.delenv("ENTELECHY_USER_ID", raising=False)
