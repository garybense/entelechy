"""Tests for lib/client.py — Entelechy REST API client."""
...
from lib.client import USER_AGENT, EntelechyClient, _validate_api_url
...
class TestEntelechyClientInit:
    def test_rejects_non_http_url(self):
        with pytest.raises(ValueError):
            EntelechyClient("ftp://bad")

    def test_stores_token(self):
        c = EntelechyClient("http://localhost:9077", api_token="tok123")
        assert c.api_token == "tok123"

    def test_no_token(self):
        c = EntelechyClient("http://localhost:9077")
        assert c.api_token is None


class TestEntelechyClientRecall:
    ...
    def test_includes_auth_header_when_token_set(self):
        c = EntelechyClient("http://localhost:9077", api_token="mytoken")
        captured = {}
    ...
    def test_sends_user_agent_header(self):
        ...
        assert captured["ua"].startswith("entelechy-claude-code/")
...
class TestEntelechyClientRetain:
...
class TestEntelechyClientHealthCheck:
...
class TestEntelechyClientSetBankMission:
