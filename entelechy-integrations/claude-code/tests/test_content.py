...
class TestStripMemoryTags:
    def test_strips_entelechy_memories_block(self):
        raw = "before\n<entelechy_memories>secret</entelechy_memories>\nafter"
        assert "entelechy_memories" not in strip_memory_tags(raw)
...
class TestComposeRecallQuery:
    ...
    def test_strips_memory_tags_from_context(self):
        msgs = _msgs(
            ("user", "<entelechy_memories>secret</entelechy_memories> actual question"),
        )
        result = compose_recall_query("now", msgs, recall_context_turns=2)
        assert "entelechy_memories" not in result
...
class TestIsChannelMessageTool:
    ...
    def test_operational_recall_tool_excluded(self):
        block = {"type": "tool_use", "name": "mcp__entelechy__recall", "input": {"query": "test"}}
...
class TestPrepareRetentionTranscript:
    ...
    def test_strips_memory_tags(self):
        msgs = _msgs(("user", "<entelechy_memories>leaked</entelechy_memories> actual question"))
    ...
    def test_json_format_excludes_entelechy_mcp_tools(self):
        """Entelechy MCP tools should be excluded even in JSON mode."""
        import json
        ...
        msgs = [
            {"role": "user", "content": "recall something"},
            {
                ...
                "content": [
                    ...
                    {"type": "tool_use", "name": "mcp__entelechy__recall", "input": {"query": "test"}},
                ],
            },
        ]
