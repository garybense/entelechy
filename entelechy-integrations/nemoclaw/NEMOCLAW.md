...
openclaw plugins install /path/to/entelechy-integrations/openclaw
```
...
Add the plugin config to `~/.openclaw/openclaw.json` under `plugins.entries.entelechy-openclaw`:
...
| Field                                   | Value                  | Why                                                                                                                       |
| --------------------------------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `entelechyApiUrl` + `entelechyApiToken` | External API URL + key | Skips the local daemon; no `uvx`/`uv` required inside the sandbox                                                         |
...
grep Entelechy ~/.openclaw/logs/gateway.log | tail -20
```
...
# Should see: [Entelechy] Retained N messages to bank my-sandbox-openclaw for session ...
```
...
```bash
openclaw plugins uninstall entelechy-openclaw
openclaw plugins install /path/to/entelechy-integrations/openclaw  # no --link
```

**`[Entelechy] Failed to retain memory (HTTP 403)`**

The sandbox network policy is blocking the outbound call. Check that:

1. The `entelechy` network policy block is present in your policy YAML
