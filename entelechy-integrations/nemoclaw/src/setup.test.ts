...
vi.mock("./policy-writer.js", () => ({
  hasEntelechyPolicy: vi.fn(),
  mergeEntelechyPolicy: vi.fn(),
...
describe("runSetup", () => {
  beforeEach(async () => {
    ...
    const { hasEntelechyPolicy, mergeEntelechyPolicy, serializePolicy } =
      await import("./policy-writer.js");
    vi.mocked(hasEntelechyPolicy).mockReturnValue(false);
    vi.mocked(mergeEntelechyPolicy).mockImplementation((p) => ({
      ...p,
      network_policies: { ...p.network_policies, entelechy: { name: "entelechy", endpoints: [] } },
    }));
  ...
  it("skips policy set when Entelechy policy already exists", async () => {
    const { hasEntelechyPolicy } = await import("./policy-writer.js");
    vi.mocked(hasEntelechyPolicy).mockReturnValue(true);
