...
export async function runSetup(args: CliArgs): Promise<void> {
  ...
  const pluginConfig = {
    entelechyApiUrl: args.apiUrl,
    entelechyApiToken: args.apiToken,
  ...
  if (!args.skipPolicy) {
    step(3, `Applying Entelechy network policy to sandbox "${args.sandbox}"...`);

    const currentPolicy = await readSandboxPolicy(args.sandbox);

    if (hasEntelechyPolicy(currentPolicy)) {
      log("  ✓ Entelechy policy already present — skipping");
    } else {
      const updatedPolicy = mergeEntelechyPolicy(currentPolicy);
      ...
      if (args.dryRun) {
        ...
        const tmpFile = join(tmpdir(), `entelechy-policy-${randomBytes(6).toString("hex")}.yaml`);
        try {
  ...
  log("    grep Entelechy ~/.openclaw/logs/gateway.log | tail -5");
  log("  Expected: [Entelechy] ✓ Ready (external API mode)");
  log("");
