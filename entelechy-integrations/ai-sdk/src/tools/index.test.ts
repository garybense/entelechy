...
import { createEntelechyTools, type EntelechyClient } from "./index.js";

describe("createEntelechyTools", () => {
  let mockClient: EntelechyClient;
  ...
  describe("tool creation", () => {
    it("should create all tools", () => {
      const tools = createEntelechyTools({ client: mockClient, bankId: "test-bank" });
  ...
  describe("bankId enforcement", () => {
    it("should always use the bankId from constructor options", async () => {
      const tools = createEntelechyTools({ client: mockClient, bankId: "forced-bank" });
      vi.mocked(mockClient.retain).mockResolvedValue({
        success: true,
        bank_id: "forced-bank",
        items_count: 1,
