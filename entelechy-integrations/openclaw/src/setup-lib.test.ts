...
describe("isValidEnvVarName", () => {
  it("accepts UPPER_SNAKE_CASE", () => {
    ...
    expect(isValidEnvVarName("ENTELECHY_CLOUD_TOKEN")).toBe(true);
...
describe("ensurePluginConfig", () => {
  it("initializes the entelechy-openclaw entry on an empty config", () => {
...
describe("applyCloudMode — direct token value", () => {
  it("stores the token inline when a literal value is provided", () => {
    ...
    expect(pc.entelechyApiUrl).toBe(ENTELECHY_CLOUD_URL);
    expect(pc.entelechyApiToken).toBe("hsk_literal_value");
  ...
  it("trims whitespace around an inline token", () => {
    ...
    expect(pc.entelechyApiToken).toBe("hsk_padded");
  });
...
describe("applyCloudMode", () => {
  it("writes the default URL and a SecretRef, stripping local LLM state", () => {
    ...
    applyCloudMode(pc, { tokenEnvVar: "ENTELECHY_CLOUD_TOKEN" });
    expect(pc.entelechyApiUrl).toBe(ENTELECHY_CLOUD_URL);
  ...
  it("honours an overridden apiUrl", () => {
    ...
    expect(pc.entelechyApiUrl).toBe("https://cloud.example.com");
    expect((pc.entelechyApiToken as { id: string }).id).toBe("CLOUD_TOKEN");
  });
});
...
describe("applyApiMode — direct token value", () => {
  it("stores the token inline when a literal value is provided", () => {
    ...
    expect(pc.entelechyApiUrl).toBe("https://mcp.example.com");
    expect(pc.entelechyApiToken).toBe("api_literal");
  });
...
describe("applyApiMode", () => {
  it("writes the URL without a token when none is provided", () => {
    const pc: Record<string, unknown> = {
      llmProvider: "openai",
      entelechyApiToken: { source: "env", provider: "default", id: "STALE_TOKEN" },
    };
    applyApiMode(pc, { apiUrl: "https://mcp.example.com" });
    expect(pc.entelechyApiUrl).toBe("https://mcp.example.com");
    expect(pc.entelechyApiToken).toBeUndefined();
    expect(pc.llmProvider).toBeUndefined();
  });
...
describe("applyEmbeddedMode", () => {
  it("writes llmProvider + SecretRef for providers that require a key", () => {
    const pc: Record<string, unknown> = {
      entelechyApiUrl: "https://stale.example.com",
      entelechyApiToken: { source: "env", provider: "default", id: "STALE" },
    };
    ...
    expect(pc.entelechyApiUrl).toBeUndefined();
    expect(pc.entelechyApiToken).toBeUndefined();
  });
...
describe("loadConfig / saveConfig", () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), "entelechy-openclaw-setup-"));
  });
