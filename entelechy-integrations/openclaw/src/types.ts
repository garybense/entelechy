...
export interface PluginConfig {
  ...
  embedVersion?: string; // entelechy-embed version (default: "latest")
  embedPackagePath?: string; // Local path to entelechy package (e.g. '/path/to/entelechy')
  ...
  entelechyApiUrl?: string; // External Entelechy API URL (skips local daemon when set)
  entelechyApiToken?: string; // API token for external Entelechy API. Configure via SecretRef.
  ...
  retainQueuePath?: string; // Path to JSONL file for buffering failed retains. Default: ~/.openclaw/data/entelechy-retain-queue.jsonl
...
// entelechy-client SDK. We alias MemoryResult → RecallResult so existing code
...
export interface RetainRequest {
   ...
   * (Entelechy ≥ 0.5 only — older versions silently ignore the field and
