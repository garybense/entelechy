#!/usr/bin/env node
/**
 * Setup wizard for the Entelechy OpenClaw plugin.
 *
 * Two modes of operation:
 *
 *   1. Interactive — no flags, just run `entelechy-openclaw-setup`. Walks the
...
function usage(): string {
  return [
    "Usage: entelechy-openclaw-setup [options] [config-path]",
    "",
    ...
    `  --api-url <url>         Override the Entelechy Cloud URL (default: ${ENTELECHY_CLOUD_URL})`,
    ...
    "",
    "Examples:",
    "  entelechy-openclaw-setup",
    "  entelechy-openclaw-setup --mode cloud --token hsk_...",
    "  entelechy-openclaw-setup --mode cloud --token-env ENTELECHY_CLOUD_TOKEN",
    "  entelechy-openclaw-setup --mode api --api-url https://mcp.entelechy.example.com --no-token",
    "  entelechy-openclaw-setup --mode embedded --provider openai --api-key-env OPENAI_API_KEY",
    "  entelechy-openclaw-setup --mode embedded --provider claude-code",
  ].join("\n");
}
...
async function promptCloud(pluginConfig: Record<string, unknown>): Promise<string> {
  const useDefaultUrl = await p.confirm({
    message: `Use the default Entelechy Cloud URL (${ENTELECHY_CLOUD_URL})?`,
    initialValue: true,
  });
  ...
  if (!useDefaultUrl) {
    const custom = await p.text({
      message: "Entelechy Cloud URL",
      placeholder: ENTELECHY_CLOUD_URL,
  ...
  const token = await p.password({
    message: "Entelechy Cloud API token (paste the value, it will be masked)",
...
async function promptApi(pluginConfig: Record<string, unknown>): Promise<string> {
  const apiUrl = await p.text({
    message: "Entelechy API URL",
    placeholder: "https://mcp.entelechy.example.com",
    validate: validateRequired("URL is required"),
  });
...
async function promptEmbedded(pluginConfig: Record<string, unknown>): Promise<string> {
  const provider = await p.select({
    message: "LLM provider used by the Entelechy memory daemon",
    options: [
...
async function runInteractive(
  ...
  p.intro("🦞 Entelechy Memory setup for OpenClaw");
  ...
  const mode = await p.select({
    message: "How do you want to run Entelechy?",
    options: [
      { value: "cloud", label: "Cloud", hint: "managed Entelechy, no local setup" },
      { value: "api", label: "External API", hint: "your own running Entelechy deployment" },
      {
        value: "embedded",
        label: "Embedded daemon",
        hint: "spawn a local entelechy daemon on this machine",
      },
    ],
  });
  ...
  p.note(
    [
      ...
      "      --ref-source env --ref-id ENTELECHY_CLOUD_TOKEN",
    ].join("\n"),
    "Entelechy Memory configured"
  );
  p.outro("Done.");
  return { summary, configPath };
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(`entelechy-openclaw-setup: ${err instanceof Error ? err.message : err}`);
    console.error();
    console.error(usage());
    process.exit(2);
  }
  ...
  if (args.mode) {
    ...
    try {
      ...
      console.log(`Entelechy Memory configured: ${summary}`);
 ...
 *
 * When invoked through `node_modules/.bin/entelechy-openclaw-setup` (npm-created
...
if (isDirectRun()) {
  main().catch((err: unknown) => {
    ...
    console.error(`entelechy-openclaw-setup failed: ${msg}`);
    process.exit(1);
  });
}
