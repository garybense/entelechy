...
describe("parseCliArgs", () => {
  ...
  it("parses cloud-mode flags (token env var)", () => {
    const args = parseCliArgs([
      ...
      "--token-env",
      "ENTELECHY_CLOUD_TOKEN",
    ]);
    expect(args).toMatchObject({
      mode: "cloud",
      apiUrl: "https://cloud.example.com",
      tokenEnv: "ENTELECHY_CLOUD_TOKEN",
    });
  });
...
describe("runNonInteractive", () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), "entelechy-openclaw-setup-cli-"));
  ...
  it("writes a cloud-mode config with the default URL", async () => {
    const args = parseCliArgs(["--mode", "cloud", "--token-env", "ENTELECHY_CLOUD_TOKEN"]);
  ...
  it("writes a cloud-mode config with a custom URL", async () => {
    const args = parseCliArgs([
      ...
      "cloud",
      "--api-url",
      "https://entelechy.custom.example.com",
      "--token-env",
      "MY_TOKEN",
    ]);
    ...
    const cfg = await readBack();
    const pc = cfg.plugins?.entries?.[PLUGIN_ID]?.config ?? {};
    expect(pc.entelechyApiUrl).toBe("https://entelechy.custom.example.com");
    expect((pc.entelechyApiToken as { id: string }).id).toBe("MY_TOKEN");
  });
  ...
  it("writes an api-mode config with token", async () => {
    ...
    const cfg = await readBack();
    const pc = cfg.plugins?.entries?.[PLUGIN_ID]?.config ?? {};
    expect((pc.entelechyApiToken as { id: string }).id).toBe("ENTELECHY_API_TOKEN");
  });
  ...
  it("clears stale fields when switching between modes", async () => {
    ...
    await runNonInteractive(
      parseCliArgs(["--mode", "cloud", "--token-env", "ENTELECHY_CLOUD_TOKEN"]),
      configPath
    );
    cfg = await readBack();
    const pc = cfg.plugins?.entries?.[PLUGIN_ID]?.config ?? {};
