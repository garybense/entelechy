...
function usage(): string {
  return [
    "Usage: entelechy-openclaw-backfill [options]",
    "",
    "Options:",
    ...
    "  --api-url <url>               Entelechy API base URL override",
    "  --api-token <token>           Entelechy API bearer token override",
...
function inferApiSettings(
  ...
  const apiUrl =
    explicitApiUrl ||
    pluginConfig.entelechyApiUrl ||
    `http://127.0.0.1:${pluginConfig.apiPort || 9077}`;
  const apiToken = explicitApiToken || pluginConfig.entelechyApiToken;
...
export async function createBackfillRuntime(
  ...
  const manager = new EntelechyServer({
    profile: "openclaw",
...
/**
 * Fetch stats for a single bank over HTTP. The high-level `EntelechyClient`
...
export async function runCli(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  // Single shared client — entelechy-client takes bankId as a parameter on
  // every call, so there's no reason to cache per-bank clients anymore.
  const client = new EntelechyClient({ baseUrl: runtime.apiUrl, apiKey: runtime.apiToken });
