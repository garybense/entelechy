...
const USER_AGENT = `entelechy-openclaw/${loadPackageVersion()}`;
...
const debug = (...args: unknown[]) => {
  if (debugEnabled)
    log.verbose(
      args
        .map((a) => (typeof a === "string" ? a.replace(/^\[Entelechy\]\s*/, "") : String(a)))
        .join(" ")
    );
};

// Module-level state
let entelechyServer: EntelechyServer | null = null;
let client: EntelechyClient | null = null;
let clientOptions: EntelechyClientOptions | null = null;
...
// client carried a mutable bankId. EntelechyClient takes bankId as a parameter on every
...
// Lightweight bank-scoped facade over EntelechyClient. Created per-request via
...
// at build time; EntelechyClient wants Record<string, string>).
...
function scopeClient(c: EntelechyClient, bankId: string): BankScopedClient {
  return {
    bankId,
    async retain(req) {
...
const skipEntelechyTurnBySession = new Map<string, IdentitySkipReason>();
...
async function flushRetainQueue(): Promise<void> {
  ...
  try {
    ...
    if (flushed > 0) {
      ...
      debug(`[Entelechy] Queue flush: API still unreachable, ${remaining} retains pending`);
    }
  } finally {
...
async function lazyReinit(configOverride?: PluginConfig): Promise<void> {
  ...
  if (!config) {
    debug("[Entelechy] lazyReinit skipped - no plugin config available");
    return;
  }
  ...
  debug("[Entelechy] Attempting lazy re-initialization...");
  try {
    ...
    client = new EntelechyClient(clientOptions);
    ...
    debug("[Entelechy] ✓ Lazy re-initialization succeeded");
  } catch (error) {
    log.warn(
...
if (typeof global !== "undefined") {
  (global as any).__entelechyClient = {
    getClient: () => client,
    waitForReady: async () => {
      if (isInitialized) {
        return;
      ...
      try {
        ...
        // Attempt lazy re-initialization so Entelechy recovers
    ...
    getClientForContext: async (
      ...
      if (config.bankMission && !banksWithMissionSet.has(bankId)) {
        try {
          ...
          banksWithMissionSet.add(bankId);
          debug(`[Entelechy] Set mission for new bank: ${bankId}`);
        } catch (error) {
 ...
 * Removes <entelechy_memories> and <relevant_memories> blocks that were injected
...
export function stripMemoryTags(content: string): string {
  content = content.replace(/<entelechy_memories>[\s\S]*?<\/entelechy_memories>/g, "");
 ...
 * - <entelechy_retain_tags>tag:a, tag:b</entelechy_retain_tags>
 */
export function extractInlineRetainTags(content: string): string[] {
  if (!content) return [];

  const tags: string[] = [];
  const blockRe = /<(?:entelechy_)?retain_tags>([\s\S]*?)<\/(?:entelechy_)?retain_tags>/gi;
...
export function stripInlineRetainTags(content: string): string {
  ...
  return content.replace(
    /<(?:entelechy_)?retain_tags>[\s\S]*?<\/(?:entelechy_)?retain_tags>/gi,
    ""
  );
}
...
function resolveAndCacheIdentity(options: ResolveAndCacheIdentityOptions): {
  ...
  if (
    ...
    if (sessionKey) {
      setCappedMapValue(skipEntelechyTurnBySession, sessionKey, skipReason);
    }
  ...
  if (sessionKey) {
    if (skipReason) {
      ...
      skipEntelechyTurnBySession.delete(sessionKey);
    }
  }
...
export function deriveBankId(
  ...
  if (fields.includes("user") && resolvedCtx && !resolvedCtx.senderId) {
    debug(
      '[Entelechy] senderId not available in context — bank ID will use "anonymous". Ensure your OpenClaw provider passes senderId.'
    );
  }
...
export function detectLLMConfig(pluginConfig?: PluginConfig): {
  ...
  if (!provider) {
    throw new Error(
      `No LLM provider configured for the Entelechy memory plugin.\n\n` +
        `Set the provider via 'openclaw config set':\n` +
        `  openclaw config set plugins.entries.entelechy-openclaw.config.llmProvider openai\n\n` +
        ...
        `  openclaw config set plugins.entries.entelechy-openclaw.config.llmApiKey \\\n` +
        ...
        `Or point the plugin at an external Entelechy API by setting entelechyApiUrl instead.`
    );
  }
...
/**
 * Detect external Entelechy API configuration from plugin config.
 */
export function detectExternalApi(pluginConfig?: PluginConfig): {
  ...
  return {
    apiUrl: pluginConfig?.entelechyApiUrl ?? null,
    apiToken: pluginConfig?.entelechyApiToken ?? null,
  };
}

/**
 * Build EntelechyClientOptions for the generated entelechy-client. In
...
/**
 * Health check for external Entelechy API.
 ...
 * Entelechy API version. Returns `null` (treated as "no append support") if
 ...
 */
async function fetchEntelechyApiVersion(
  apiUrl: string,
  apiToken?: string | null
): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      debug(`[Entelechy] /version returned HTTP ${response.status}; assuming legacy`);
      return null;
    }
    const data = (await response.json()) as { api_version?: unknown };
    const v = typeof data.api_version === "string" ? data.api_version : null;
    if (!v) {
      debug(`[Entelechy] /version payload missing api_version; assuming legacy`);
    }
    return v;
  } catch (error) {
    debug(`[Entelechy] /version probe failed: ${String(error)}; assuming legacy`);
    return null;
  }
}
...
async function detectAppendCapability(apiUrl: string, apiToken?: string | null): Promise<void> {
  const version = await fetchEntelechyApiVersion(apiUrl, apiToken);
  const supported =
  ...
  if (supported) {
    debug(`[Entelechy] API version ${version} supports update_mode=append`);
    return;
  }
  ...
  log.warn(
    `[Entelechy] ⚠️  API at ${apiUrl} reports version "${version ?? "unknown"}", which is older than ${MIN_VERSION_FOR_UPDATE_MODE_APPEND}. ` +
      ...
      `Upgrade Entelechy to ${MIN_VERSION_FOR_UPDATE_MODE_APPEND} or newer to enable session-scoped retention with update_mode=append.`
  );
}
...
async function checkExternalApiHealth(apiUrl: string, apiToken?: string | null): Promise<void> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      debug(
        `[Entelechy] Checking external API health at ${healthUrl}... (attempt ${attempt}/${maxRetries})`
      );
      const headers: Record<string, string> = { "User-Agent": USER_AGENT };
      if (apiToken) {
      ...
      debug(`[Entelechy] External API health: ${JSON.stringify(data)}`);
      return;
    } catch (error) {
      if (attempt < maxRetries) {
        debug(`[Entelechy] Health check attempt ${attempt} failed, retrying in ${retryDelay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, retryDelay));
      } else {
        throw new Error(`Cannot connect to external Entelechy API at ${apiUrl}: ${error}`, {
          cause: error,
        });
      }
    }
...
function getPluginConfig(api: MoltbotPluginAPI): PluginConfig {
  const config = api.config.plugins?.entries?.["entelechy-openclaw"]?.config || {};
  ...
  return {
    ...
    entelechyApiUrl: config.entelechyApiUrl,
    entelechyApiToken: config.entelechyApiToken,
...
export default function (api: MoltbotPluginAPI) {
  try {
    ...
    debug("[Entelechy] Plugin loading...");
    ...
    debug("[Entelechy] Plugin loaded successfully (deferred heavy init to gateway start)");
    ...
    debug("[Entelechy] Registering service...");
    log.info("registering plugin service");
    api.registerService({
      id: "entelechy-memory",
      async start() {
        log.info("service.start invoked");
        debug("[Entelechy] Service start called - beginning heavy initialization...");

        // Detect LLM configuration (env vars > plugin config > auto-detect)
        debug("[Entelechy] Detecting LLM config...");
        ...
        if (llmConfig.provider === "ollama") {
          debug(
            `[Entelechy] ✓ Using provider: ${llmConfig.provider}, model: ${modelInfo} (${llmConfig.source})`
          );
        } else {
          debug(
            `[Entelechy] ✓ Using provider: ${llmConfig.provider}, model: ${modelInfo} (${llmConfig.source}${baseUrlInfo})`
          );
        }
        if (pluginConfig.bankMission) {
          debug(
            `[Entelechy] Custom bank mission configured: "${pluginConfig.bankMission.substring(0, 50)}..."`
          );
        }
        ...
        if (pluginConfig.dynamicBankId) {
          const prefixInfo = pluginConfig.bankIdPrefix
            ...
            : "";
          debug(
            `[Entelechy] ✓ Dynamic bank IDs enabled${prefixInfo} - each channel gets isolated memory`
          );
        } else {
          const sourceInfo = getConfiguredBankId(pluginConfig) ? "configured" : "default";
          debug(
            `[Entelechy] Dynamic bank IDs disabled - using ${sourceInfo} static bank: ${getStaticBankId(pluginConfig)}`
          );
        }
        ...
        if (externalApi.apiUrl) {
          ...
          usingExternalApi = true;
          debug(`[Entelechy] ✓ Using external API: ${externalApi.apiUrl}`);
          ...
          try {
            ...
            const queuePath =
              pluginConfig.retainQueuePath || join(queueDir, "entelechy-retain-queue.jsonl");
            ...
            debug(`[Entelechy] Retain queue initialized: ${queuePath}`);
          ...
          if (externalApi.apiToken) {
            debug("[Entelechy] API token configured");
          }
        } else {
          debug(
            `[Entelechy] Daemon idle timeout: ${pluginConfig.daemonIdleTimeout}s (0 = never timeout)`
          );
          debug(`[Entelechy] API Port: ${apiPort}`);
        }

        // Initialize (runs synchronously in service.start())
        debug("[Entelechy] Starting initialization...");
        initPromise = (async () => {
          try {
            if (usingExternalApi && externalApi.apiUrl) {
              // External API mode - check health, skip daemon startup
              debug("[Entelechy] External API mode - skipping local daemon...");
              await checkExternalApiHealth(externalApi.apiUrl, externalApi.apiToken);
              await detectAppendCapability(externalApi.apiUrl, externalApi.apiToken);

              // Initialize client for external API
              debug("[Entelechy] Creating EntelechyClient (external API)...");
              ...
              debug(`[Entelechy] Default bank: ${defaultBankId}`);
              ...
              if (pluginConfig.bankMission && usesStaticBank(pluginConfig)) {
                debug(`[Entelechy] Setting bank mission...`);
                try {
              ...
              isInitialized = true;
              debug("[Entelechy] ✓ Ready (external API mode)");
            } else {
              // Local daemon mode - start entelechy-embed daemon
              debug("[Entelechy] Creating EntelechyServer...");
              entelechyServer = new EntelechyServer({
                profile: "openclaw",
                port: apiPort,
                embedVersion: pluginConfig.embedVersion,
                embedPackagePath: pluginConfig.embedPackagePath,
                env: {
                  ENTELECHY_API_LLM_PROVIDER: llmConfig.provider || "",
                  ENTELECHY_API_LLM_API_KEY: llmConfig.apiKey || "",
                  ENTELECHY_API_LLM_MODEL: llmConfig.model,
                  ENTELECHY_API_LLM_BASE_URL: llmConfig.baseUrl,
                  ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT: String(pluginConfig.daemonIdleTimeout ?? 0),
                },
                logger: embedLogger,
              });

              // Start the embedded server
              debug("[Entelechy] Starting embedded server...");
              await entelechyServer.start();

              // Initialize client pointed at the local daemon URL
              debug("[Entelechy] Creating EntelechyClient (local daemon)...");
              clientOptions = { baseUrl: entelechyServer.getBaseUrl() };
              banksWithMissionSet.clear();
              client = new EntelechyClient(clientOptions);
        ...
        if (usingExternalApi) {
          ...
          if (externalApi.apiUrl && isInitialized) {
            try {
              await checkExternalApiHealth(externalApi.apiUrl, externalApi.apiToken);
              await detectAppendCapability(externalApi.apiUrl, externalApi.apiToken);
              debug("[Entelechy] External API is healthy");
              return;
            } catch (error) {
          ...
          if (entelechyServer && isInitialized) {
            const healthy = await entelechyServer.checkHealth();
            if (healthy) {
              debug("[Entelechy] Daemon is healthy");
              return;
            }

            debug("[Entelechy] Daemon is not responding - reinitializing...");
            // Reset state for reinitialization
            entelechyServer = null;
            client = null;
            clientOptions = null;
            banksWithMissionSet.clear();
            isInitialized = false;
        ...
        if (!isInitialized) {
          debug("[Entelechy] Reinitializing...");
          ...
          if (externalApi.apiUrl) {
            ...
            isInitialized = true;
            debug("[Entelechy] Reinitialization complete (external API mode)");
          } else {
            // Local daemon mode
            entelechyServer = new EntelechyServer({
              profile: "openclaw",
              ...
              env: {
                ...
                ENTELECHY_API_LLM_MODEL: llmConfig.model,
                ENTELECHY_API_LLM_BASE_URL: llmConfig.baseUrl,
                ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT: String(
            ...
            isInitialized = true;
            debug("[Entelechy] Reinitialization complete");
          }
        }
      },
      ...
      async stop() {
        try {
          debug("[Entelechy] Service stopping...");

          // Only stop daemon if in local mode
          if (!usingExternalApi && entelechyServer) {
            await entelechyServer.stop();
            entelechyServer = null;
          }
          ...
          if (retainQueue) {
            ...
            if (pending > 0) {
              debug(
                `[Entelechy] Service stopping with ${pending} queued retains (will resume on next start)`
              );
            }
            retainQueue.close();
            retainQueue = null;
          ...
          stopLogger();
          debug("[Entelechy] Service stopped");
        } catch (error) {
    ...
    debug("[Entelechy] Plugin loaded successfully");
    ...
    // "win" and leave subsequent registries with zero entelechy hooks — which is
    // exactly how auto-recall/auto-retain silently stopped firing in 0.6.x.
    debug("[Entelechy] Registering agent hooks...");
    ...
    api.on("before_dispatch", async (event: any, ctx?: PluginHookAgentContext) => {
      try {
        ...
        if (skipReason) {
          debug(
            `[Entelechy] before_dispatch marked session ${sessionKey} to skip this turn: ${formatIdentitySkipReason(skipReason)}`
          );
        ...
        debug(
          `[Entelechy] before_dispatch cached identity for ${sessionKey}: ${resolvedCtx.messageProvider}/${resolvedCtx.channelId} sender=${resolvedCtx.senderId}`
        );
      } catch (error) {
    ...
    api.on("before_agent_start", async (event: any, ctx?: PluginHookAgentContext) => {
      try {
        ...
        if (sessionKey && skipReason) {
          debug(
            `[Entelechy] before_agent_start skipping session ${sessionKey}: ${formatIdentitySkipReason(skipReason)}`
          );
        ...
        debug(
          `[Entelechy] before_agent_start - bank: ${bankId}, channel: ${resolvedCtx.messageProvider}/${resolvedCtx.channelId}`
        );
      } catch (error) {
    ...
    api.on("before_prompt_build", async (event: any, ctx?: PluginHookAgentContext) => {
      try {
        ...
        if (ctx?.messageProvider && pluginConfig.excludeProviders?.includes(ctx.messageProvider)) {
          debug(`[Entelechy] Skipping recall for excluded provider: ${ctx.messageProvider}`);
          return;
        }
        ...
        if (sessionKey) {
          ...
          if (ignorePatterns.length > 0 && matchesSessionPattern(sessionKey, ignorePatterns)) {
            debug(
              `[Entelechy] Skipping recall: session '${sessionKey}' matches ignoreSessionPatterns`
            );
            return;
          }
          ...
          if (skipStateless) {
            ...
            if (
              ...
              debug(
                `[Entelechy] Skipping recall: session '${sessionKey}' matches statelessSessionPatterns (skipStatelessSessions=true)`
              );
              return;
            }
          }
        ...
        if (!pluginConfig.autoRecall) {
          debug("[Entelechy] Auto-recall disabled via config, skipping");
          return;
        }
        ...
        const skipTurnReason = sessionKeyForCache
          ? skipEntelechyTurnBySession.get(sessionKeyForCache)
          : undefined;
        if (skipTurnReason && !isRetryableIdentitySkipReason(skipTurnReason)) {
          debug(
            `[Entelechy] Skipping recall for session ${sessionKeyForCache}: ${formatIdentitySkipReason(skipTurnReason)}`
          );
        ...
        if (identitySkipReason) {
          debug(
            `[Entelechy] Skipping recall for session ${sessionKeyForCache}: ${formatIdentitySkipReason(identitySkipReason)}`
          );
        ...
        debug(
          `[Entelechy] before_prompt_build - bank: ${bankId}, channel: ${resolvedCtxForRecall?.messageProvider}/${resolvedCtxForRecall?.channelId}`
        );
        debug(`[Entelechy] event keys: ${Object.keys(event).join(", ")}`);
        debug(`[Entelechy] event.context keys: ${Object.keys(event.context ?? {}).join(", ")}`);
        ...
        debug(
          `[Entelechy] extractRecallQuery input lengths - raw: ${event.rawMessage?.length ?? 0}, prompt: ${event.prompt?.length ?? 0}`
        );
        const extracted = extractRecallQuery(event.rawMessage, event.prompt);
        if (!extracted) {
          debug("[Entelechy] extractRecallQuery returned null, skipping recall");
          return;
        }
        if (isEphemeralOperationalText(extracted)) {
          debug("[Entelechy] Recall query is operational/ephemeral noise, skipping recall");
          return;
        }
        debug(`[Entelechy] extractRecallQuery result length: ${extracted.length}`);
        ...
        debug(
          `[Entelechy] event.messages count: ${messageCount}, roles: ${sessionMessages.map((m: any) => m.role).join(",")}`
        );
        if (recallContextTurns > 1 && messageCount === 0) {
          debug(
            "[Entelechy] recallContextTurns > 1 but event.messages is empty — prior context unavailable at before_agent_start for this provider"
          );
        }
        ...
        const clientGlobal = (global as any).__entelechyClient;
        if (!clientGlobal) {
          debug("[Entelechy] Client global not available, skipping auto-recall");
          return;
        }
        ...
        if (!client) {
          debug("[Entelechy] Client not initialized, skipping auto-recall");
          return;
        }

        debug(`[Entelechy] Auto-recall for bank ${bankId}, full query:\n---\n${prompt}\n---`);
        ...
        if (existing) {
          debug(`[Entelechy] Reusing in-flight recall for bank ${bankId}`);
        ...
        debug(
          `[Entelechy] After topK (${pluginConfig.recallTopK ?? "unlimited"}): ${results.length} results injected`
        );

        // Format memories as JSON with all fields from recall
        const memoriesFormatted = formatMemories(results);

        const contextMessage = `<entelechy_memories>
...
${memoriesFormatted}
        ...
        debug(`[Entelechy] Auto-recall: Injecting ${results.length} memories from bank ${bankId}`);
        ...
        if (error instanceof DOMException && error.name === "TimeoutError") {
          log.warn(
            `[Entelechy] Auto-recall timed out after ${pluginConfig.recallTimeoutMs ?? DEFAULT_RECALL_TIMEOUT_MS}ms, skipping memory injection`
          );
        } else if (error instanceof Error && error.name === "AbortError") {
          log.warn(
            `[Entelechy] Auto-recall aborted after ${pluginConfig.recallTimeoutMs ?? DEFAULT_RECALL_TIMEOUT_MS}ms, skipping memory injection`
          );
        } else {
    ...
    api.on("agent_end", async (event: any, ctx?: PluginHookAgentContext) => {
      try {
        ...
        if (
          ...
          debug(
            `[Entelechy] Skipping retain for excluded provider: ${effectiveCtx.messageProvider}`
          );
          return;
        }
        ...
        if (agentEndSessionKey) {
          ...
          if (
            ...
            debug(
              `[Entelechy] Skipping retain: session '${agentEndSessionKey}' matches ignoreSessionPatterns`
            );
            return;
          }
          const statelessPatterns = compileSessionPatterns(
          ...
          if (
            ...
            debug(
              `[Entelechy] Skipping retain: session '${agentEndSessionKey}' matches statelessSessionPatterns`
            );
            return;
          }
        }
        ...
        const skipTurnReason = sessionKeyForLookup
          ? skipEntelechyTurnBySession.get(sessionKeyForLookup)
          : undefined;
        if (skipTurnReason && !isRetryableIdentitySkipReason(skipTurnReason)) {
          debug(
            `[Entelechy Hook] Skipping retain for session ${sessionKeyForLookup}: ${formatIdentitySkipReason(skipTurnReason)}`
          );
          logSkipOnce("retain", sessionKeyForLookup, skipTurnReason);
          if (sessionKeyForLookup) {
            skipEntelechyTurnBySession.delete(sessionKeyForLookup);
          }
          return;
        }
        ...
        if (identitySkipReason) {
          debug(
            `[Entelechy Hook] Skipping retain for session ${sessionKeyForLookup}: ${formatIdentitySkipReason(identitySkipReason)}`
          );
        ...
        debug(`[Entelechy Hook] agent_end triggered - bank: ${bankId}`);

        if (event.success === false) {
          debug("[Entelechy Hook] Agent run failed, skipping retention");
          return;
        }
        ...
        if (
          ...
          debug("[Entelechy Hook] No messages in event, skipping retention");
          return;
        }

        if (pluginConfig.autoRetain === false) {
          debug("[Entelechy Hook] autoRetain is disabled, skipping retention");
          return;
        }
        ...
        if (retainEveryN > 1) {
          ...
          if (turnCount % retainEveryN !== 0) {
            ...
            debug(
              `[Entelechy Hook] Turn ${turnCount}/${retainEveryN}, skipping retain (next at turn ${nextRetainAt})`
            );
            return;
          }
          ...
          debug(
            `[Entelechy Hook] Turn ${turnCount}: chunked retain firing (window: ${windowTurns} turns, ${messagesToRetain.length} messages)`
          );
        }
        ...
        if (!retention) {
          debug("[Entelechy Hook] No messages to retain (filtered/short/no-user)");
          return;
        }
        const { transcript, messageCount } = retention;

        if (isEphemeralOperationalText(transcript)) {
          debug("[Entelechy Hook] Transcript is operational/ephemeral noise, skipping retention");
          return;
        }
        ...
        // Retain to Entelechy
        debug(
          `[Entelechy] Retaining to bank ${bankId}, document: ${retainRequest.documentId}, chars: ${transcript.length}\n---\n${transcript.substring(0, 500)}${transcript.length > 500 ? "\n...(truncated)" : ""}\n---`
        );
        ...
        try {
          ...
          debug(
            `[Entelechy] Retained ${messageCount} messages to bank ${bankId} for session ${retainRequest.documentId}`
          );
    ...
    debug("[Entelechy] Hooks registered");
    ...
    if (pluginConfig.enableKnowledgeTools && typeof api.registerTool === "function") {
      try {
        ...
        const apiToken = pluginConfig.entelechyApiToken || undefined;
...
export function buildRetainRequest(
  ...
  options?: {
    ...
    tags?: string[];
    /**
     * Whether the live Entelechy API supports `update_mode: 'append'`. When
...
// loops on Entelechy's own MCP surface. Mirrors the claude-code integration.
 ...
 * expected by Entelechy's Claude Code integration (and downstream consumers)
...
function extractStructuredBlocks(content: any, role: string): any[] {
  ...
  for (const block of content) {
    ...
    if (blockType === "text") {
      ...
      // Skip Entelechy's own MCP operational tools to avoid feedback loops.
