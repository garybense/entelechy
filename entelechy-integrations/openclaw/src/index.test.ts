...
describe("stripMemoryTags", () => {
  it("strips simple entelechy_memories tags", () => {
    const input =
  ...
  it("strips multiple entelechy_memories blocks", () => {
    const input =
      "Start\n<entelechy_memories>\nBlock 1\n</entelechy_memories>\nMiddle\n<entelechy_memories>\nBlock 2\n</entelechy_memories>\nEnd";
  ...
  it("strips tags from a real-world agent conversation with injected memories", () => {
    ...
    expect(result).not.toContain("<entelechy_memories>");
    expect(result).not.toContain("</entelechy_memories>");
...
describe("inline retain tag helpers", () => {
  ...
  it("supports entelechy_retain_tags alias and strips directives from content", () => {
    const input =
      "Keep this.\n<entelechy_retain_tags>scope:user</entelechy_retain_tags>\nNot the directive.";
...
describe("prepareRetentionTranscript", () => {
  ...
  it("strips memory tags from retained content (feedback loop prevention)", () => {
    ...
    const result = prepareRetentionTranscript(messages, baseConfig);
    expect(result).not.toBeNull();
    expect(result?.transcript).not.toContain("<entelechy_memories>");
  ...
  it("filters operational MCP tool calls to avoid feedback loops", () => {
    const messages = [
      { role: "user", content: "recall stuff" },
      {
        role: "assistant",
        content: [
          { type: "toolCall", id: "c1", name: "mcp__entelechy__recall", arguments: { query: "x" } },
          {
            type: "toolCall",
            id: "c2",
    ...
    expect(
      assistantBlocks.some((b: any) => b.type === "tool_use" && b.name === "mcp__entelechy__recall")
    ).toBe(false);
    expect(
  ...
  it("reports accurate messageCount excluding empty messages", () => {
    const messages = [
      ...
      { role: "assistant", content: "<entelechy_memories>\nonly tags\n</entelechy_memories>" },
...
describe("waitForReady (CLI mode)", () => {
  it("returns without error when initPromise is null (service.start not called)", async () => {
    // The module sets up global.__entelechyClient on import.
    // In test context, service.start() is never called so initPromise remains null.
    const entelechy = (global as any).__entelechyClient;
    expect(entelechy).toBeDefined();
    // Should resolve without throwing
    await expect(entelechy.waitForReady()).resolves.toBeUndefined();
  });

  it("getClient returns null when service.start not called", () => {
    const entelechy = (global as any).__entelechyClient;
    expect(entelechy.getClient()).toBeNull();
  });
});
