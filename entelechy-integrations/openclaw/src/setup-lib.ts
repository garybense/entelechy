/**
 * Pure helpers behind the Entelechy OpenClaw setup wizard. Kept separate from
...
export const PLUGIN_ID = "entelechy-openclaw";
...
/**
 * Ensure a `plugins.entries["entelechy-openclaw"].config` object exists, set
...
export function clearCloudFields(pluginConfig: Record<string, unknown>): void {
  delete pluginConfig.entelechyApiUrl;
  delete pluginConfig.entelechyApiToken;
}
...
/**
 * Apply the Cloud mode to a plugin config in place: sets `entelechyApiUrl` and
 * `entelechyApiToken` (either as a plaintext string or as a SecretRef —
...
export function applyCloudMode(
  ...
  pluginConfig.entelechyApiUrl = (input.apiUrl ?? ENTELECHY_CLOUD_URL).trim();
  pluginConfig.entelechyApiToken = token;
}

/**
 * Apply the external-API mode to a plugin config in place: sets a required
 * `entelechyApiUrl`, optional `entelechyApiToken` (plaintext or SecretRef),
...
export function applyApiMode(pluginConfig: Record<string, unknown>, input: ApiSetupInput): void {
  const token = pickCredential(input.token, input.tokenEnvVar);
  clearLocalLlmFields(pluginConfig);
  pluginConfig.entelechyApiUrl = input.apiUrl.trim();
  if (token !== undefined) {
    pluginConfig.entelechyApiToken = token;
  } else {
    delete pluginConfig.entelechyApiToken;
...
export function summarizeCloud(input: CloudSetupInput): string {
  const url = (input.apiUrl ?? ENTELECHY_CLOUD_URL).trim();
