# Entelechy Memory Plugin for OpenClaw

Biomimetic long-term memory for [OpenClaw](https://openclaw.ai) using [Entelechy](https://vectorize.io/entelechy). Automatically captures conversations and intelligently recalls relevant context.
...
`entelechy-openclaw-setup` walks you through picking one of three modes:

- **Cloud** — managed Entelechy. Paste your cloud API token, done.
- **External API** — your own running Entelechy deployment. Prompts for the URL and optional token.
- **Embedded daemon** — spawns a local `entelechy-embed` daemon on this machine. Prompts for the LLM provider (OpenAI / Anthropic / Gemini / Groq / Claude Code / Codex / Ollama) and its API key.
...
openclaw config set plugins.entries.entelechy-openclaw.config.llmProvider claude-code

# Or: point at an external Entelechy API
openclaw config set plugins.entries.entelechy-openclaw.config.entelechyApiUrl https://mcp.entelechy.example.com
openclaw config set plugins.entries.entelechy-openclaw.config.entelechyApiToken \
    --ref-source env --ref-id ENTELECHY_API_TOKEN
```
...
| Old (0.5.x)                              | New (0.6.0)                                                                                                                                                                                                |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENAI_API_KEY=…` (auto-detected)       | `openclaw config set plugins.entries.entelechy-openclaw.config.llmProvider openai` <br> `openclaw config set plugins.entries.entelechy-openclaw.config.llmApiKey --ref-source env --ref-id OPENAI_API_KEY` |
| `ENTELECHY_API_LLM_PROVIDER=…`           | `openclaw config set plugins.entries.entelechy-openclaw.config.llmProvider …`                                                                                                                              |
| `ENTELECHY_API_LLM_MODEL=…`              | `openclaw config set plugins.entries.entelechy-openclaw.config.llmModel …`                                                                                                                                 |
| `ENTELECHY_API_LLM_API_KEY=…`            | `openclaw config set plugins.entries.entelechy-openclaw.config.llmApiKey --ref-source env --ref-id …`                                                                                                      |
| `ENTELECHY_API_LLM_BASE_URL=…`           | `openclaw config set plugins.entries.entelechy-openclaw.config.llmBaseUrl …`                                                                                                                               |
| `ENTELECHY_EMBED_API_URL=…`              | `openclaw config set plugins.entries.entelechy-openclaw.config.entelechyApiUrl …`                                                                                                                          |
| `ENTELECHY_EMBED_API_TOKEN=…`            | `openclaw config set plugins.entries.entelechy-openclaw.config.entelechyApiToken --ref-source env --ref-id …`                                                                                              |
| `ENTELECHY_BANK_ID=…`                    | `openclaw config set plugins.entries.entelechy-openclaw.config.bankId …`                                                                                                                                   |
...
- **Historical backfill CLI** — import prior OpenClaw session history into Entelechy using the active plugin bank-routing config by default
...
Optional settings in `~/.openclaw/openclaw.json` under `plugins.entries.entelechy-openclaw.config`:

| Option                     | Default                        | Description                                                                                                                                                                                                                                                                                                 |
| -------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apiPort`                  | `9077`                         | Port for the local Entelechy daemon                                                                                                                                                                                                                                                                         |
| `daemonIdleTimeout`        | `0`                            | Seconds before daemon shuts down from inactivity (0 = never)                                                                                                                                                                                                                                                |
| `embedPort`                | `0`                            | Port for `entelechy-embed` server (`0` = auto-assign)                                                                                                                                                                                                                                                       |
| `embedVersion`             | `"latest"`                     | entelechy-embed version                                                                                                                                                                                                                                                                                     |
| `embedPackagePath`         | —                              | Local path to `entelechy-embed` package for development                                                                                                                                                                                                                                                     |
...
| `llmProvider`              | —                              | LLM provider for memory extraction (`openai`, `anthropic`, `gemini`, `groq`, `ollama`, `openai-codex`, `claude-code`). Required unless `entelechyApiUrl` is set.                                                                                                                                            |
...
| `retainTags`               | `[]`                           | Tags applied to every retained document, useful for cross-agent/source labeling (e.g. `source_system:openclaw`, `agent:agentname`). Auto-retain also merges inline per-message tags from `<retain_tags>...</retain_tags>` or `<entelechy_retain_tags>...</entelechy_retain_tags>` blocks in user messages.  |
...
| `retainToolCalls`          | `true`                         | With `retainFormat: "json"`, each message's content is an Anthropic-shaped block array (`text` / `tool_use` / `tool_result`). Tool results are truncated at 2000 chars. Entelechy's own MCP tools (recall/retain/search/…) are filtered to prevent feedback loops. Set `false` to retain text-only content. |
...
| `recallPromptPreamble`     | built-in string                | Prompt text placed above recalled memories in the injected `<entelechy_memories>` system-context block.                                                                                                                                                                                                     |
| `entelechyApiUrl`          | —                              | External Entelechy API URL (skips local daemon)                                                                                                                                                                                                                                                             |
| `entelechyApiToken`        | —                              | Auth token for external API. **Sensitive** — set via `openclaw config set ... --ref-source env --ref-id ENTELECHY_API_TOKEN`.                                                                                                                                                                               |
  ...
  "plugins": {
    "entries": {
      "entelechy-openclaw": {
        "enabled": true,
        "config": {
          "embedPackagePath": "/path/to/entelechy-wt3/entelechy-embed"
        }
      }
    }
  }
...
2. The plugin will use `uv run --directory <path> entelechy-embed` instead of `uvx entelechy-embed@latest`

3. To use a specific profile for testing:

```bash
# Check daemon status
uvx entelechy-embed@latest -p openclaw daemon status

# View logs
tail -f ~/.entelechy/profiles/openclaw.log
...
The package includes a config-aware backfill CLI for importing historical OpenClaw sessions into Entelechy.
...
- `bankIdPrefix`
- local daemon vs external `entelechyApiUrl`
