 ...
 *
 * Requirements:
 *   Running Entelechy API at ENTELECHY_API_URL (default: http://localhost:8888)
 *
 * Run:
...
function createMockApi(pluginConfig: Partial<PluginConfig> = {}): MockApiHandle {
  ...
  const api: MoltbotPluginAPI = {
    config: {
      plugins: {
        entries: {
          "entelechy-openclaw": { enabled: true, config: pluginConfig as PluginConfig },
        },
      },
    },
    logger: {
...
// play nicely with the entelechy-client class shape (method overloads).
let recallSpy: MockInstance;
let retainSpy: MockInstance;

beforeAll(async () => {
  apiReachable = await waitForApi(ENTELECHY_API_URL, 8000);
  if (!apiReachable) {
    console.warn(
      `[Hooks Integration] Entelechy API not reachable at ${ENTELECHY_API_URL} – skipping hook tests.`
    );
    return;
  }
  ...
  const handle = createMockApi({
    entelechyApiUrl: ENTELECHY_API_URL,
    dynamicBankId: true,
  ...
  // Spy on the EntelechyClient prototype so all calls go through the spy.
  // The plugin's scopeClient() wrapper calls through these prototype methods.
  recallSpy = vi.spyOn(EntelechyClient.prototype, "recall");
  retainSpy = vi.spyOn(EntelechyClient.prototype, "retain");
}, 30_000);
...
describe("before_prompt_build hook", () => {
  ...
  it("returns { prependSystemContext } with <entelechy_memories> when recall returns results", async () => {
    if (!apiReachable) return;
    ...
    expect(result.prependSystemContext).toContain("<entelechy_memories>");
  ...
  it("extracts the inner query from an envelope-formatted prompt when rawMessage is absent", async () => {
    ...
    expect(recallSpy).toHaveBeenCalledOnce();
    // EntelechyClient.recall signature: (bankId, query, options?)
...
describe("agent_end hook", () => {
  ...
  it("calls retain with correctly formatted transcript for string content", async () => {
    ...
    // EntelechyClient.retain signature: (bankId, content, options?)
  ...
  it("strips <entelechy_memories> tags from content before retaining", async () => {
    if (!apiReachable) return;
    retainSpy.mockResolvedValue(OK_RETAIN);
    ...
    expect(retainSpy).toHaveBeenCalledOnce();
    const [, content] = retainSpy.mock.calls[0];
    expect(content).not.toContain("<entelechy_memories>");
    expect(content).not.toContain("</entelechy_memories>");
