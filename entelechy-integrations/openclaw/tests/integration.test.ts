/**
 * Integration tests for the Entelechy OpenClaw integration.
 *
 ...
 *   Embed mode: entelechy-embed package at ENTELECHY_EMBED_PACKAGE_PATH
 *               + LLM credentials (ENTELECHY_API_LLM_PROVIDER / ENTELECHY_API_LLM_API_KEY)
 *
 * Run:
...
const LLM_PROVIDER = process.env.ENTELECHY_API_LLM_PROVIDER || "";
const LLM_API_KEY = process.env.ENTELECHY_API_LLM_API_KEY || "";
const LLM_MODEL = process.env.ENTELECHY_API_LLM_MODEL || "";

// Embed package path – defaults to the sibling entelechy-embed directory in the repo
const EMBED_PACKAGE_PATH =
  process.env.ENTELECHY_EMBED_PACKAGE_PATH || join(__dirname, "..", "..", "..", "entelechy-embed");
...
describe("openclaw integration — HTTP mode", () => {
  let client: EntelechyClient;

  beforeAll(async () => {
    const reachable = await waitForApi(ENTELECHY_API_URL);
    if (!reachable) {
      throw new Error(
        `Entelechy API not reachable at ${ENTELECHY_API_URL}. ` +
          "Start the server before running integration tests."
      );
    }

    client = new EntelechyClient({ baseUrl: ENTELECHY_API_URL });
  });
...
// ---------------------------------------------------------------------------
// Embed Mode Tests (local daemon spawned by EntelechyServer)
// ---------------------------------------------------------------------------

describe("openclaw integration — embed mode", () => {
  let client: EntelechyClient;
  let server: EntelechyServer;
  ...
  beforeAll(async () => {
    if (!hasEmbedCredentials) {
      console.warn(
        "[Integration] Skipping embed mode tests: " +
          "ENTELECHY_API_LLM_PROVIDER and ENTELECHY_API_LLM_API_KEY must both be set."
      );
      return;
    }

    server = new EntelechyServer({
      ...
      env: {
        ...
        ENTELECHY_API_LLM_MODEL: LLM_MODEL || undefined,
    ...
    client = new EntelechyClient({ baseUrl: server.getBaseUrl() });
