#!/usr/bin/env bash
#
# End-to-end smoke test for the Entelechy OpenClaw plugin.
#
# What it verifies:
...
#     the plugin's hook handlers directly against a real Entelechy API via a
...
#   - Removes any pre-existing ~/.openclaw/extensions/entelechy-openclaw dir
...
EXT_DIR="$OPENCLAW_HOME/extensions/entelechy-openclaw"
ENTELECHY_API_URL="${ENTELECHY_API_URL:-http://127.0.0.1:7777}"
CONFIG_BACKUP=""
EXT_BACKUP=""
TARBALL=""
...
cleanup() {
  ...
  yes 2>/dev/null | openclaw plugins uninstall entelechy-openclaw >/dev/null 2>&1 || true
  rm -rf "$EXT_DIR"
  ...
  if [[ -n "$TARBALL" && "$TARBALL" == "$PLUGIN_DIR"/vectorize-io-entelechy-openclaw-*.tgz ]]; then
    rm -f "$TARBALL"
  fi
  exit "$rc"
}
...
run_setup_mode() {
  ...
  if ! node "$EXT_DIR/dist/setup.js" --config-path "$CONFIG_PATH" "$@" >/dev/null; then
    fail "entelechy-openclaw-setup --mode failed for: $label"
  fi
  ...
  # doctor surfaces something that specifically names entelechy, or if the
  ...
  if printf '%s' "$doctor_out" | grep -iE 'entelechy.*(fail|error|not loaded)|(fail|error).*entelechy' >/dev/null; then
    printf '%s\n' "$doctor_out" >&2
    fail "plugins doctor reported entelechy-specific issues after: $label"
  fi
...
main() {
  ...
  log "clearing any pre-existing entelechy-openclaw install"
  yes 2>/dev/null | openclaw plugins uninstall entelechy-openclaw >/dev/null 2>&1 || true
  rm -rf "$EXT_DIR"
  ...
  if [[ $# -gt 0 && -n "$1" ]]; then
    ...
    TARBALL="$(ls -t "$PLUGIN_DIR"/vectorize-io-entelechy-openclaw-*.tgz | head -1)"
    log "packed: $TARBALL"
  fi
  ...
  if ! grep -q "Installed plugin: entelechy-openclaw" "$install_log"; then
    cat "$install_log" >&2
  ...
  [[ "$(get_config_value plugins.entries.entelechy-openclaw.config.entelechyApiUrl)" == "https://api.entelechy.vectorize.io" ]] \
    || fail "cloud mode: entelechyApiUrl not set to the default URL"
  ...
  [[ "$(get_raw_config_value plugins.entries.entelechy-openclaw.config.entelechyApiToken)" == "hsk_smoke_test_inline_value" ]] \
    || fail "cloud mode: --token did not roundtrip as inline string"

  run_setup_mode "external API (no auth)" \
    --mode api --api-url "$ENTELECHY_API_URL" --no-token
  [[ "$(get_config_value plugins.entries.entelechy-openclaw.config.entelechyApiUrl)" == "$ENTELECHY_API_URL" ]] \
    || fail "api mode: entelechyApiUrl did not roundtrip"

  run_setup_mode "external API (inline --token)" \
    --mode api --api-url "$ENTELECHY_API_URL" --token api_smoke_test_inline
  [[ "$(get_raw_config_value plugins.entries.entelechy-openclaw.config.entelechyApiToken)" == "api_smoke_test_inline" ]] \
  ...
  [[ "$(get_config_value plugins.entries.entelechy-openclaw.config.llmProvider)" == "openai" ]] \
    || fail "embedded mode: llmProvider not set to openai"
  [[ "$(get_config_value plugins.entries.entelechy-openclaw.config.llmModel)" == "gpt-4o-mini" ]] \
  ...
  [[ "$(get_raw_config_value plugins.entries.entelechy-openclaw.config.llmApiKey)" == "sk-smoke-test-inline" ]] \
  ...
  if node "$EXT_DIR/dist/setup.js" --config-path "$CONFIG_PATH" --mode api --api-url "$ENTELECHY_API_URL" --token-env TOK --no-token 2>/dev/null; then
