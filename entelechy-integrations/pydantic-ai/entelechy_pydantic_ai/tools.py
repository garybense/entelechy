"""Pydantic AI tools for Entelechy memory operations.

Provides factory functions that create Pydantic AI ``Tool`` instances
backed by Entelechy's retain/recall/reflect APIs.
"""
...
try:
    _VERSION = metadata.version("entelechy-pydantic-ai")
except metadata.PackageNotFoundError:
    _VERSION = "0.0.0"
_USER_AGENT = f"entelechy-pydantic-ai/{_VERSION}"
...
def create_entelechy_tools(
    *,
    ...
    if include_retain:
        ...
        async def entelechy_retain(content: str) -> str:
        ...
        tools.append(Tool(entelechy_retain, takes_ctx=False))

    if include_recall:

        async def entelechy_recall(query: str) -> str:
        ...
        tools.append(Tool(entelechy_recall, takes_ctx=False))

    if include_reflect:

        async def entelechy_reflect(query: str) -> str:
        ...
        tools.append(Tool(entelechy_reflect, takes_ctx=False))
