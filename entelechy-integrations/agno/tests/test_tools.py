"""Unit tests for Entelechy Agno tools."""
...
from entelechy_agno import (
    EntelechyTools,
    configure,
    memory_instructions,
    reset_config,
)
from entelechy_agno.errors import EntelechyError
from entelechy_agno.tools import _TOOL_INSTRUCTIONS, _resolve_client
...
def _mock_client():
    """Create a mock Entelechy client."""
    client = MagicMock()
...
class TestResolveClient:
    ...
    def test_creates_client_from_url(self):
        with patch("entelechy_agno.tools.Entelechy") as mock_cls:
    ...
    def test_falls_back_to_global_config_url(self):
        configure(entelechy_api_url="http://config:8888")
        with patch("entelechy_agno.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
    ...
    def test_falls_back_to_global_config_api_key(self):
        configure(entelechy_api_url="http://config:8888", api_key="config-key")
        with patch("entelechy_agno.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            _resolve_client(None, None, None)
            mock_cls.assert_called_once_with(
    ...
    def test_raises_without_url_or_config(self):
        with pytest.raises(EntelechyError, match="No Entelechy API URL"):
            _resolve_client(None, None, None)
...
# ---------------------------------------------------------------------------
# EntelechyTools initialization
# ---------------------------------------------------------------------------


class TestEntelechyToolsInit:
    def setup_method(self):
        reset_config()
    ...
    def test_creates_three_tools_by_default(self):
        client = _mock_client()
        toolkit = EntelechyTools(bank_id="test", client=client)
    ...
    def test_explicit_url_overrides_config(self):
        ...
        with patch("entelechy_agno.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            EntelechyTools(bank_id="test", entelechy_api_url="http://explicit:9999")
            mock_cls.assert_called_once_with(
                base_url="http://explicit:9999", timeout=30.0
            )
    ...
    def test_toolkit_name(self):
        client = _mock_client()
        toolkit = EntelechyTools(bank_id="test", client=client)
        assert toolkit.name == "entelechy_tools"
    ...
    def test_constructor_defaults_override_config_for_budget(self):
        ...
        configure(entelechy_api_url="http://localhost:8888", budget="low")
        with patch("entelechy_agno.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            toolkit = EntelechyTools(bank_id="test")
    ...
    def test_explicit_budget_overrides_default(self):
        ...
        with patch("entelechy_agno.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            toolkit = EntelechyTools(bank_id="test", budget="low")
        client = mock_cls.return_value
        client.recall.return_value = _mock_recall_response(["fact"])
        ctx = _mock_run_context()
        toolkit.recall_memory(ctx, "q")
...
class TestBankIdResolution:
    def test_static_bank_id(self):
        client = _mock_client()
        client.recall.return_value = _mock_recall_response(["fact"])
        toolkit = EntelechyTools(bank_id="my-bank", client=client)
        ctx = _mock_run_context()
    ...
    def test_bank_id_from_run_context_user_id(self):
        client = _mock_client()
        client.recall.return_value = _mock_recall_response(["fact"])
        toolkit = EntelechyTools(client=client)
    ...
    def test_custom_bank_resolver(self):
        ...
        toolkit = EntelechyTools(bank_resolver=resolver, client=client)
    ...
    def test_static_bank_id_takes_priority_over_user_id(self):
        client = _mock_client()
        client.recall.return_value = _mock_recall_response(["fact"])
        toolkit = EntelechyTools(bank_id="static-bank", client=client)
    ...
    def test_missing_bank_id_raises_error(self):
        ...
        with pytest.raises(EntelechyError, match="No bank_id available"):
            toolkit.recall_memory(ctx, "query")
    ...
    def test_bank_resolver_error_propagates(self):
        ...
        toolkit = EntelechyTools(bank_resolver=bad_resolver, client=client)
        ctx = _mock_run_context()

        with pytest.raises(EntelechyError, match="resolver broke"):
            toolkit.retain_memory(ctx, "content")
...
class TestEnsureBank:
    def test_creates_bank_on_first_tool_use(self):
        client = _mock_client()
        toolkit = EntelechyTools(bank_id="new-bank", client=client)
        ctx = _mock_run_context()
    ...
    def test_bank_creation_failure_is_swallowed(self):
        ...
        toolkit = EntelechyTools(bank_id="existing-bank", client=client)
        ctx = _mock_run_context()
...
class TestRetainTool:
    def test_retain_stores_memory(self):
        client = _mock_client()
        toolkit = EntelechyTools(bank_id="test-bank", client=client)
        ctx = _mock_run_context()
    ...
    def test_retain_raises_entelechy_error(self):
        client = _mock_client()
        client.retain.side_effect = RuntimeError("connection refused")
        toolkit = EntelechyTools(bank_id="test", client=client)
        ctx = _mock_run_context()

        with pytest.raises(EntelechyError, match="Retain failed"):
            toolkit.retain_memory(ctx, "content")

    def test_retain_preserves_entelechy_error(self):
        """EntelechyError from bank resolution is not double-wrapped."""
        client = _mock_client()
        toolkit = EntelechyTools(client=client)
    ...
    def test_retain_error_chains_original_exception(self):
        ...
        with pytest.raises(EntelechyError) as exc_info:
            toolkit.retain_memory(ctx, "content")
...
class TestRecallTool:
    ...
    def test_recall_raises_entelechy_error(self):
        client = _mock_client()
        client.recall.side_effect = RuntimeError("timeout")
        toolkit = EntelechyTools(bank_id="test", client=client)
        ctx = _mock_run_context()

        with pytest.raises(EntelechyError, match="Recall failed"):
            toolkit.recall_memory(ctx, "query")

    def test_recall_preserves_entelechy_error(self):
        client = _mock_client()
        toolkit = EntelechyTools(client=client)
        ctx = _mock_run_context()
...
class TestReflectTool:
    ...
    def test_reflect_passes_budget(self):
        client = _mock_client()
        client.reflect.return_value = _mock_reflect_response("answer")
        toolkit = EntelechyTools(bank_id="test", client=client, budget="high")
        ctx = _mock_run_context()
    ...
    def test_reflect_raises_entelechy_error(self):
        client = _mock_client()
        client.reflect.side_effect = RuntimeError("timeout")
        toolkit = EntelechyTools(bank_id="test", client=client)
        ctx = _mock_run_context()

        with pytest.raises(EntelechyError, match="Reflect failed"):
            toolkit.reflect_on_memory(ctx, "query")

    def test_reflect_preserves_entelechy_error(self):
        client = _mock_client()
        toolkit = EntelechyTools(client=client)
        ctx = _mock_run_context()
...
class TestExports:
    def test_all_exports_importable(self):
        import entelechy_agno

        for name in entelechy_agno.__all__:
            assert hasattr(entelechy_agno, name), (
    ...
    def test_version(self):
        ...
        assert entelechy_agno.__version__ == "0.1.0"

    def test_entelechy_error_importable_from_top_level(self):
        from entelechy_agno import EntelechyError

        assert issubclass(EntelechyError, Exception)

    def test_config_class_importable(self):
        from entelechy_agno import EntelechyAgnoConfig

        config = EntelechyAgnoConfig()
...
class TestEntelechyError:
    def test_is_exception(self):
        assert issubclass(EntelechyError, Exception)

    def test_can_be_raised_and_caught(self):
        with pytest.raises(EntelechyError, match="test error"):
            raise EntelechyError("test error")

    def test_str_representation(self):
        err = EntelechyError("something went wrong")
...
class TestMultipleToolkits:
    def test_separate_created_banks_sets(self):
        ...
        client = _mock_client()
        toolkit1 = EntelechyTools(bank_id="bank-1", client=client)
        toolkit2 = EntelechyTools(bank_id="bank-2", client=client)
        ctx = _mock_run_context()
    ...
    def test_separate_config_per_toolkit(self):
        ...
        toolkit_low = EntelechyTools(bank_id="test", client=client, budget="low")
        toolkit_high = EntelechyTools(bank_id="test", client=client, budget="high")
        ctx = _mock_run_context()
...
class TestLogging:
    def test_retain_logs_error(self, caplog):
        ...
        with caplog.at_level(logging.ERROR, logger="entelechy_agno.tools"):
            with pytest.raises(EntelechyError):
                toolkit.retain_memory(ctx, "content")
    ...
    def test_entelechy_error_not_logged(self, caplog):
        """EntelechyError (e.g. missing bank_id) should not log — it's re-raised directly."""
        client = _mock_client()
        toolkit = EntelechyTools(client=client)
        ctx = _mock_run_context()  # No user_id

        with caplog.at_level(logging.ERROR, logger="entelechy_agno.tools"):
            with pytest.raises(EntelechyError, match="No bank_id"):
                toolkit.retain_memory(ctx, "content")
...
class TestEdgeCases:
    def test_empty_string_bank_id_is_falsy(self):
        ...
        client = _mock_client()
        client.recall.return_value = _mock_recall_response(["fact"])
        toolkit = EntelechyTools(bank_id="", client=client)
    ...
    def test_special_chars_in_bank_id(self):
        client = _mock_client()
        client.recall.return_value = _mock_recall_response(["fact"])
        toolkit = EntelechyTools(bank_id="org/user-123_v2", client=client)
        ctx = _mock_run_context()
    ...
    def test_empty_tags_list_not_sent(self):
        ...
        client = _mock_client()
        toolkit = EntelechyTools(bank_id="test", client=client, tags=[])
        ctx = _mock_run_context()
    ...
    def test_empty_recall_tags_list_not_sent(self):
        ...
        client = _mock_client()
        client.recall.return_value = _mock_recall_response(["fact"])
        toolkit = EntelechyTools(bank_id="test", client=client, recall_tags=[])
        ctx = _mock_run_context()
...
class TestErrorTypes:
    ...
    def test_retain_wraps_type_error(self):
        ...
        with pytest.raises(EntelechyError, match="Retain failed.*bad type"):
            toolkit.retain_memory(ctx, "content")
    ...
    def test_reflect_wraps_os_error(self):
        ...
        with pytest.raises(EntelechyError, match="Reflect failed.*disk full"):
            toolkit.reflect_on_memory(ctx, "query")
    ...
    def test_retain_wraps_value_error(self):
        ...
        with pytest.raises(EntelechyError, match="Retain failed.*invalid input"):
            toolkit.retain_memory(ctx, "content")
...
class TestBankCreationCrossTool:
    def test_retain_then_recall_creates_bank_once(self):
        ...
        client = _mock_client()
        client.recall.return_value = _mock_recall_response(["fact"])
        toolkit = EntelechyTools(bank_id="shared-bank", client=client)
        ctx = _mock_run_context()
...
class TestResolveClientEdgeCases:
    ...
    def test_empty_string_api_key_not_passed(self):
        """api_key='' is falsy, so it should not be passed to Entelechy."""
        with patch("entelechy_agno.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
    ...
    def test_config_none_api_key_not_passed(self):
        configure(entelechy_api_url="http://config:8888", api_key=None)
        with patch("entelechy_agno.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            _resolve_client(None, None, None)
...
class TestConfigToolkitInteraction:
    ...
    def test_config_set_after_toolkit_creation_not_used(self):
        ...
        client = _mock_client()
        client.recall.return_value = _mock_recall_response(["fact"])
        toolkit = EntelechyTools(bank_id="test", client=client, tags=["original"])
        ctx = _mock_run_context()

        # Change config after construction
        configure(entelechy_api_url="http://new:8888", tags=["updated"])
    ...
    def test_toolkit_without_config_then_configure(self):
        ...
        configure(entelechy_api_url="http://new:8888", budget="high")
...
class TestConstructorAPI:
    def test_entelechy_tools_requires_keyword_args(self):
        """All EntelechyTools args are keyword-only (due to *)."""
        client = _mock_client()
        with pytest.raises(TypeError):
            EntelechyTools("test", client)  # type: ignore[misc]
