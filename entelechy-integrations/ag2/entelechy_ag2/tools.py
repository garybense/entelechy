"""AG2 tool definitions for Entelechy memory operations.
...
def create_entelechy_tools(
    *,
    ...
    """Create Entelechy memory tools for AG2 agents.
    ...
    Usage::
        ...
        tools = create_entelechy_tools(bank_id="my-bank", client=client)
        for tool_fn in tools:
            agent.register_for_llm(description=tool_fn.__doc__)(tool_fn)
            executor.register_for_execution()(tool_fn)
    """
    resolved_client = resolve_client(client, entelechy_api_url, api_key)
...
def register_entelechy_tools(
    ...
    **kwargs,
    ...
    Creates Entelechy memory tools and registers them on the given AG2
    ...
    Args:
        ...
        bank_id: Entelechy memory bank ID.
        **kwargs: All other args passed to ``create_entelechy_tools()``.
    ...
    """
    tools = create_entelechy_tools(bank_id=bank_id, **kwargs)
    for tool_fn in tools:
        agent.register_for_llm(description=tool_fn.__doc__)(tool_fn)
        executor.register_for_execution()(tool_fn)
    return tools
