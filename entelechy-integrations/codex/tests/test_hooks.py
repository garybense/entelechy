...
Mocks the Codex hook runtime:
  ...
  - HOME → tmp_path (isolates ~/.entelechy/codex.json and state)
"""
...
def _run_hook(module_name, hook_input, monkeypatch, tmp_path, urlopen_side_effect=None, user_config=None):
    ...
    # Isolate HOME so ~/.entelechy/codex.json and state land in tmp_path
    monkeypatch.setenv("HOME", str(tmp_path))

    # Strip real ENTELECHY_* env vars
    for k in list(os.environ):
        if k.startswith("ENTELECHY_"):
            monkeypatch.delenv(k, raising=False)

    # Set required API URL via env var
    monkeypatch.setenv("ENTELECHY_API_URL", "http://fake:9077")
