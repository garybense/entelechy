"""Shared fixtures for Entelechy Codex plugin tests."""
...
def make_user_config(tmp_path, overrides=None):
    """Write a ~/.entelechy/codex.json in tmp_path with test defaults."""
    entelechy_dir = tmp_path / ".entelechy"
    entelechy_dir.mkdir(exist_ok=True)
    config = {"retainEveryNTurns": 1}
    if overrides:
        config.update(overrides)
    (entelechy_dir / "codex.json").write_text(json.dumps(config))
