...
Fires once when a Codex session begins. Verifies the Entelechy server is
...
def main():
    ...
    try:
        ...
        EntelechyClient(api_url, config.get("entelechyApiToken"))
        debug_log(config, f"Entelechy server reachable at {api_url}")
    except (RuntimeError, ValueError) as e:
        debug_log(config, f"Entelechy not running, initiating background pre-start: {e}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"[Entelechy] SessionStart error: {e}", file=sys.stderr)
        sys.exit(0)
