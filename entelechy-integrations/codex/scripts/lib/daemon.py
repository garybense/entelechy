"""Entelechy-embed daemon lifecycle management for Codex.

Manages three connection modes:
  1. External API — user provides entelechyApiUrl (skip daemon entirely)
  2. Existing local server — user already has entelechy running
  3. Auto-managed daemon — plugin starts/stops entelechy-embed

Daemon state is tracked via files in ~/.entelechy/codex/state/.
"""
...
def _get_embed_command(config: dict) -> list:
    ...
    if embed_path:
        return ["uv", "run", "--directory", embed_path, "entelechy-embed"]

    version = config.get("embedVersion", "latest")
    package = f"entelechy-embed@{version}" if version else "entelechy-embed@latest"
    return ["uvx", package]


def _run_embed(config: dict, args: list, env: dict = None, timeout: int = 10) -> subprocess.CompletedProcess:
    """Run a entelechy-embed command and return the result."""
...
def _is_embed_available(config: dict) -> bool:
    """Quick check if entelechy-embed is available on PATH."""
    import shutil

    embed_path = config.get("embedPackagePath")
    if embed_path:
        return os.path.isdir(embed_path)
    return shutil.which("uvx") is not None or shutil.which("entelechy-embed") is not None


def _check_health(base_url: str, timeout: int = 2) -> bool:
    """Quick health check against a Entelechy server."""
    try:
...
def get_api_url(config: dict, debug_fn=None, allow_daemon_start: bool = False) -> str:
    ...
    if not allow_daemon_start:
        raise RuntimeError(
            f"No Entelechy server on port {port}. Set entelechyApiUrl for external "
            f"API, start entelechy-embed manually, or wait for the retain hook to "
    ...
    try:
        ...
        raise RuntimeError(
            "No Entelechy server available. Set entelechyApiUrl for external API, "
            "or ensure entelechy-embed is installed for local daemon mode."
        ) from e
...
def _ensure_daemon_running(config: dict, port: int, debug_fn=None):
    ...
    if not _is_embed_available(config):
        raise RuntimeError(
            "entelechy-embed not found (uvx not on PATH). "
            "Install with: pip install entelechy-embed, or set entelechyApiUrl."
        )
    ...
    daemon_env["ENTELECHY_EMBED_DAEMON_IDLE_TIMEOUT"] = str(idle_timeout)

    if platform.system() == "Darwin":
        daemon_env["ENTELECHY_API_EMBEDDINGS_LOCAL_FORCE_CPU"] = "1"
        daemon_env["ENTELECHY_API_RERANKER_LOCAL_FORCE_CPU"] = "1"
    ...
    try:
        ...
        raise RuntimeError(
            "entelechy-embed not found. Install with: pip install entelechy-embed "
            "or set entelechyApiUrl for external API mode."
        )
...
def prestart_daemon_background(config: dict, debug_fn=None):
    ...
    if not _is_embed_available(config):
        if debug_fn:
            debug_fn("entelechy-embed not available, skipping pre-start")
        return
