"""SmolAgents Tool subclasses for Entelechy memory operations.

Provides three ``Tool`` subclasses (retain, recall, reflect) that give
SmolAgents agents persistent long-term memory via the Entelechy API.
"""
...
class EntelechyRetainTool(Tool):
...
class EntelechyRecallTool(Tool):
...
class EntelechyReflectTool(Tool):
...
def create_entelechy_tools(
    *,
    ...
    """Create all Entelechy memory tools for use with SmolAgents.
