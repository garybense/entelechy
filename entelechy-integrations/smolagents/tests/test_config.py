"""Unit tests for entelechy_smolagents configuration."""

import os
from unittest.mock import patch

from entelechy_smolagents import configure, get_config, reset_config
from entelechy_smolagents.config import (
...
class TestEntelechySmolAgentsConfigDataclass:
    def test_default_values(self):
        config = EntelechySmolAgentsConfig()
...
class TestConfigure:
    ...
    def test_configure_all_options(self):
        config = configure(
            entelechy_api_url="http://custom:8888",
            api_key="my-key",
            budget="high",
            max_tokens=2048,
            tags=["env:test"],
        ...
        assert config.entelechy_api_url == "http://custom:8888"
    ...
    def test_configure_returns_config_instance(self):
        config = configure()
        assert isinstance(config, EntelechySmolAgentsConfig)
    ...
    def test_configure_url_defaults_when_none(self):
        config = configure(entelechy_api_url=None)
        assert config.entelechy_api_url == DEFAULT_ENTELECHY_API_URL
