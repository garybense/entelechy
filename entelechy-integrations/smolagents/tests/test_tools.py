"""Unit tests for Entelechy SmolAgents tools."""
...
from entelechy_smolagents import (
    EntelechyRecallTool,
...
from entelechy_smolagents.errors import EntelechyError
from entelechy_smolagents.tools import _resolve_client
...
class TestResolveClient:
    ...
    def test_creates_client_from_url(self):
        with patch("entelechy_smolagents.tools.Entelechy") as mock_cls:
...
class TestToolConstruction:
    ...
    def test_retain_tool_attributes(self):
        client = _mock_client()
        tool = EntelechyRetainTool(bank_id="test", client=client)
        assert tool.name == "entelechy_retain"
    ...
    def test_recall_tool_attributes(self):
        client = _mock_client()
        tool = EntelechyRecallTool(bank_id="test", client=client)
        assert tool.name == "entelechy_recall"
    ...
    def test_reflect_tool_attributes(self):
        client = _mock_client()
        tool = EntelechyReflectTool(bank_id="test", client=client)
        assert tool.name == "entelechy_reflect"
...
class TestRetainTool:
    def test_retain_success(self):
        client = _mock_client()
        tool = EntelechyRetainTool(bank_id="my-bank", client=client)
    ...
    def test_retain_with_tags(self):
        client = _mock_client()
        tool = EntelechyRetainTool(bank_id="my-bank", client=client, tags=["env:test"])
    ...
    def test_retain_config_tags(self):
        configure(entelechy_api_url="http://localhost:8888", tags=["config-tag"])
        with patch("entelechy_smolagents.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            tool = EntelechyRetainTool(bank_id="test")
    ...
    def test_retain_explicit_tags_override_config(self):
        ...
        with patch("entelechy_smolagents.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            tool = EntelechyRetainTool(bank_id="test", tags=["explicit-tag"])
        client = mock_cls.return_value
        tool.forward("content")
    ...
    def test_retain_creates_bank(self):
        client = _mock_client()
        tool = EntelechyRetainTool(bank_id="new-bank", client=client)
        tool.forward("content")
    ...
    def test_retain_bank_already_exists(self):
        ...
        tool = EntelechyRetainTool(bank_id="existing-bank", client=client)
        result = tool.forward("content")
        assert result == "Memory stored successfully."

    def test_retain_failure_raises_entelechy_error(self):
        client = _mock_client()
    ...
    def test_retain_entelechy_error_not_wrapped(self):
        client = _mock_client()
        client.retain.side_effect = EntelechyError("original error")
        tool = EntelechyRetainTool(bank_id="my-bank", client=client)
        with pytest.raises(EntelechyError, match="original error"):
            tool.forward("content")
    ...
    def test_retain_failure_logs_error(self, caplog):
        ...
        client.retain.side_effect = RuntimeError("network error")
        tool = EntelechyRetainTool(bank_id="my-bank", client=client)
        with caplog.at_level(logging.ERROR), pytest.raises(EntelechyError):
            tool.forward("content")
...
class TestRecallTool:
    def test_recall_returns_numbered_results(self):
        ...
        tool = EntelechyRecallTool(bank_id="my-bank", client=client)
    ...
    def test_recall_failure_raises_entelechy_error(self):
        client = _mock_client()
    ...
    def test_recall_entelechy_error_not_wrapped(self):
        client = _mock_client()
        client.recall.side_effect = EntelechyError("original error")
        tool = EntelechyRecallTool(bank_id="my-bank", client=client)
        with pytest.raises(EntelechyError, match="original error"):
            tool.forward("query")
...
class TestReflectTool:
    def test_reflect_returns_text(self):
        ...
        tool = EntelechyReflectTool(bank_id="my-bank", client=client)
    ...
    def test_reflect_passes_budget(self):
        ...
        tool = EntelechyReflectTool(bank_id="my-bank", client=client, budget="high")
        tool.forward("query")
    ...
    def test_reflect_failure_raises_entelechy_error(self):
        client = _mock_client()
    ...
    def test_reflect_entelechy_error_not_wrapped(self):
        client = _mock_client()
        client.reflect.side_effect = EntelechyError("original error")
        tool = EntelechyReflectTool(bank_id="my-bank", client=client)
        with pytest.raises(EntelechyError, match="original error"):
            tool.forward("query")
...
class TestCreateEntelechyTools:
    def setup_method(self):
        reset_config()
    ...
    def test_creates_three_tools_by_default(self):
        client = _mock_client()
        tools = create_entelechy_tools(bank_id="test", client=client)
        assert len(tools) == 3
        names = {t.name for t in tools}
        assert names == {"entelechy_retain", "entelechy_recall", "entelechy_reflect"}
    ...
    def test_enable_retain_only(self):
        ...
        assert len(tools) == 1
        assert tools[0].name == "entelechy_retain"
    ...
    def test_enable_recall_only(self):
        ...
        assert len(tools) == 1
        assert tools[0].name == "entelechy_recall"
    ...
    def test_enable_reflect_only(self):
        ...
        assert len(tools) == 1
        assert tools[0].name == "entelechy_reflect"
    ...
    def test_raises_without_client_or_config(self):
        with pytest.raises(EntelechyError, match="No Entelechy API URL"):
            create_entelechy_tools(bank_id="test")
    ...
    def test_passes_tags_to_retain(self):
        client = _mock_client()
        tools = create_entelechy_tools(bank_id="test", client=client, tags=["env:test"])
        retain_tool = [t for t in tools if t.name == "entelechy_retain"][0]
    ...
    def test_passes_recall_options(self):
        ...
        recall_tool = [t for t in tools if t.name == "entelechy_recall"][0]
