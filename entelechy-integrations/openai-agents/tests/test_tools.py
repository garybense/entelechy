"""Unit tests for Entelechy OpenAI Agents SDK tools."""

import json
from unittest.mock import AsyncMock, MagicMock, patch

import pytest
from entelechy_openai_agents import (
    configure,
...
class TestCreateEntelechyTools:
    ...
    def test_falls_back_to_global_config(self):
        ...
        with patch("entelechy_openai_agents._client.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            tools = create_entelechy_tools(bank_id="test")
            assert len(tools) == 3
            mock_cls.assert_called_once_with(
                base_url="http://localhost:8888", timeout=30.0, user_agent="entelechy-openai-agents/0.1.0"
            )
    ...
    def test_explicit_url_overrides_config(self):
        ...
        with patch("entelechy_openai_agents._client.Entelechy") as mock_cls:
            ...
            mock_cls.assert_called_once_with(
                base_url="http://explicit:9999", timeout=30.0, user_agent="entelechy-openai-agents/0.1.0"
            )
