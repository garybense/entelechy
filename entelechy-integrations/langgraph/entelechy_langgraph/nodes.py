"""Pre-built LangGraph nodes for Entelechy memory operations.
...
def create_recall_node(
    *,
    ...
    from Entelechy, and returns them either as a SystemMessage in the
    ...
    **Message ordering:** When using the default ``messages`` output,
    ...
    Args:
        bank_id: Static Entelechy memory bank ID.
    ...
    async def recall_node(state: MessagesState, config: Optional[RunnableConfig] = None) -> dict[str, Any]:
        ...
        try:
            ...
            return {"messages": [SystemMessage(content=memory_text, id="entelechy_memory_context")]}
...
def create_retain_node(
    *,
    ...
    via Entelechy retain. It should be placed after the LLM response
    node in your graph.
