"""LangGraph BaseStore adapter backed by Entelechy.

Maps LangGraph's key-value store interface to Entelechy's memory operations.
...
def _namespace_to_bank_id(namespace: tuple[str, ...]) -> str:
    """Convert a namespace tuple to a Entelechy bank ID.

    Uses "." as separator since "/" is not valid in Entelechy bank IDs
...
def _make_item(
    ...
    """Create a LangGraph Item from Entelechy data."""
...
def _make_search_item(
    ...
    """Create a LangGraph SearchItem from Entelechy recall results."""
    now = datetime.now(timezone.utc)
    return SearchItem(
        namespace=namespace,
        key=key,
...
class EntelechyStore(BaseStore):
    """LangGraph BaseStore implementation backed by Entelechy.

    Maps LangGraph's namespace/key-value model to Entelechy memory banks:
    - Namespace tuples are joined with "." to form bank IDs
    - ``put()`` stores values via Entelechy retain with the key as document_id
    - ``search()`` uses Entelechy recall for semantic search
    - ``get()`` uses recall with the key as a targeted query, returning only
      ...
      Entelechy does not currently expose a direct document-lookup endpoint.
    ...
    **Known limitations:**
    ...
    - **``list_namespaces`` is session-scoped.** It only tracks namespaces that
      ...
      in Entelechy. Entelechy does not currently provide a bank-listing API.
    - **``delete`` is a no-op.** Calling ``adelete()`` logs a debug message but
      does not remove data. Entelechy's memory model is append-oriented; fact
    ...
    Example::
        ...
        store = EntelechyStore(client=Entelechy(base_url="http://localhost:8888"))
    ...
    def __init__(
        self,
        *,
        client: Optional[Entelechy] = None,
        entelechy_api_url: Optional[str] = None,
        api_key: Optional[str] = None,
        tags: Optional[list[str]] = None,
    ):
        self._client = resolve_client(client, entelechy_api_url, api_key)
        self._tags = tags
    ...
    async def _handle_put(self, op: PutOp) -> None:
        ...
        try:
            ...
            raise EntelechyError(f"Store put failed: {e}") from e

    async def _handle_search(self, op: SearchOp) -> list[SearchItem]:
        """Handle a search operation via Entelechy recall."""
    ...
    async def aput(
        ...
        # Entelechy does not support TTL-based expiration natively.
