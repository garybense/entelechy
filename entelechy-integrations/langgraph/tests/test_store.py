"""Unit tests for Entelechy LangGraph BaseStore adapter."""
...
from entelechy_langgraph.store import (
    EntelechyStore,
...
class TestEntelechyStorePut:
    ...
    async def test_put_passes_tags(self):
        client = _mock_client()
        store = EntelechyStore(client=client, tags=["source:langgraph"])
    ...
    async def test_put_raises_on_error(self):
        ...
        with pytest.raises(EntelechyError, match="Store put failed"):
            await store.aput(("user", "123"), "key", {"value": 1})


class TestEntelechyStoreGet:
    @pytest.mark.asyncio
...
class TestEntelechyStoreSearch:
    @pytest.mark.asyncio
...
class TestEntelechyStoreListNamespaces:
    @pytest.mark.asyncio
