...
import type {
  EntelechyChatOptions,
  EntelechyChatContext,
  EntelechyChatHandler,
  ChatThread,
  ChatMessage,
  RecallResult,
  EntityState,
...
/**
 * Wraps a Chat SDK handler to automatically provide Entelechy memory context.
 *
 ...
 * 4. Builds a EntelechyChatContext and passes it to the handler
 *
 ...
 * ```
 */
export function withEntelechyChat<TState = unknown>(
  options: EntelechyChatOptions,
  handler: EntelechyChatHandler<TState>
  ...
  return async (thread: ChatThread<TState>, message: ChatMessage): Promise<void> => {
    ...
    if (retainEnabled && message.text && !message.author.isMe) {
      const retainPromise = client
        ...
        .catch((err) => {
          console.warn("[entelechy-chat] Auto-retain failed:", err);
        });
    ...
    if (recallEnabled && message.text) {
      try {
        ...
        console.warn("[entelechy-chat] Auto-recall failed:", err);
      }
    }

    // 4. Build context
    const ctx: EntelechyChatContext = {
