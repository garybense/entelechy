...
import { withEntelechyChat } from "./wrapper.js";
import type { EntelechyClient, ChatThread, ChatMessage, EntelechyChatContext } from "./types.js";

// --- Mocks ---

function mockClient(overrides?: Partial<EntelechyClient>): EntelechyClient {
  return {
...
describe("withEntelechyChat", () => {
  ...
  describe("bankId resolution", () => {
    it("uses static bankId", async () => {
      const handler = vi.fn();
      const wrapped = withEntelechyChat({ client, bankId: "static-bank" }, handler);
    ...
    it("uses dynamic bankId from message", async () => {
      const handler = vi.fn();
      const wrapped = withEntelechyChat(
  ...
  describe("auto-recall", () => {
    it("recalls by default", async () => {
      const handler = vi.fn();
      const wrapped = withEntelechyChat({ client, bankId: "bank" }, handler);
      ...
      const ctx: EntelechyChatContext = handler.mock.calls[0][2];
    ...
    it("handles recall errors gracefully", async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        "[entelechy-chat] Auto-recall failed:",
        expect.any(Error)
      );
  ...
  describe("auto-retain", () => {
    ...
    it("handles retain errors gracefully", async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        "[entelechy-chat] Auto-retain failed:",
        expect.any(Error)
      );
      // Handler still runs
