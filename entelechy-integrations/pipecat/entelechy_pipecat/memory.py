"""Entelechy memory service for Pipecat voice AI pipelines.

Provides a FrameProcessor that slots between the user context aggregator
and the LLM service to add persistent memory via Entelechy.

Placement::

    user_aggregator → EntelechyMemoryService → LLM service
...
from .errors import EntelechyPipecatError

logger = logging.getLogger(__name__)

_MEMORY_MARKER = "<entelechy_memories>"
...
class EntelechyMemoryService(FrameProcessor):
    """Pipecat frame processor that adds Entelechy persistent memory.
    ...
    Place this between the user context aggregator and the LLM service::
        ...
        pipeline = Pipeline([
            ...
            stt_service,
            user_aggregator,
            EntelechyMemoryService(bank_id="user-123", entelechy_api_url="..."),
            llm_service,
    ...
    3. Injects memories as a ``<entelechy_memories>`` system message into
       the LLM context before forwarding it downstream.

    Args:
        bank_id: Entelechy memory bank to read from and write to.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._client = _resolve_client(client, entelechy_api_url, api_key)
        config = get_config()
    ...
    async def _recall(self, query: str) -> str | None:
        """Call Entelechy recall and return a formatted string of memories, or None."""
        try:
            ...
            logger.warning(f"Entelechy recall failed (continuing without memories): {e}")
            return None

    async def _retain(self, content: str) -> None:
        """Call Entelechy retain (fire-and-forget — errors are logged and swallowed)."""
        try:
            ...
            logger.warning(f"Entelechy retain failed: {e}")
