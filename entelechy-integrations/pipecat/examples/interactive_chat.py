"""Interactive text chat — test EntelechyMemoryService by hand.
...
Usage:
    ...
    python examples/interactive_chat.py --bank demo-$USER --entelechy-url http://localhost:8888

Each turn shows:
  [RECALL]  — what Entelechy returned for the current user query
  [INJECT]  — the <entelechy_memories> system message added to the LLM context
  [LLM]     — the assistant's response (OpenAI or manual)
  [RETAIN]  — complete user+assistant turn sent to Entelechy

Commands:
  :quit / :q    — exit
  :memories     — dump all memories in the bank
  :reset        — clear context (keeps the Entelechy bank)
...
async def _run_turn(service: EntelechyMemoryService, messages: list[dict]) -> list[dict]:
...
async def main() -> None:
    ...
    parser.add_argument("--entelechy-url", default=os.environ.get("ENTELECHY_API_URL", "http://localhost:8888"))
    parser.add_argument("--entelechy-api-key", default=os.environ.get("ENTELECHY_API_KEY"))
    parser.add_argument(
        "--system-prompt",
    ...
    service = EntelechyMemoryService(
        bank_id=args.bank,
        entelechy_api_url=args.entelechy_url,
        api_key=args.entelechy_api_key,
    )
    ...
    print(f"\n{GREEN}=== Pipecat + Entelechy Interactive Chat ==={RESET}")
    print(f"Bank:        {args.bank}")
    print(f"Entelechy:   {args.entelechy_url}")
    ...
    while True:
        ...
        if user_input == ":memories":
            dump_memories(args.entelechy_url, args.bank, args.entelechy_api_key)
            continue
        if user_input == ":reset":
            messages = [{"role": "system", "content": args.system_prompt}]
            service._last_retained_count = 0
            print(f"{YELLOW}Context reset (Entelechy bank untouched){RESET}")
            continue
        if user_input == ":bank":
        ...
        if memory_msg:
            ...
            inner = content.replace(_MEMORY_MARKER, "").replace("</entelechy_memories>", "").strip()
