"""Basic Pipecat pipeline with Entelechy persistent memory.

This example shows how to add long-term memory to a voice AI pipeline using
EntelechyMemoryService. The processor slots between the user context aggregator
and the LLM service — recalling relevant memories before each turn and retaining
conversation content after.

Requirements:
    pip install pipecat-entelechy pipecat-ai[deepgram,openai,cartesia,silero,daily]
...
Environment variables:
    ...
    ENTELECHY_API_URL    - Entelechy API URL (default: http://localhost:8888)
...
async def main() -> None:
    ...
    # EntelechyMemoryService — add between user aggregator and LLM.
    # bank_id should be stable per user so memories persist across sessions.
    memory = EntelechyMemoryService(
        bank_id="demo-user-001",
        entelechy_api_url=os.environ.get("ENTELECHY_API_URL", "http://localhost:8888"),
