"""Live integration test for EntelechyMemoryService.

Runs conversation turns through the FrameProcessor against a real Entelechy
instance (default: http://localhost:8888) and verifies:

  1. Retain — turns get stored in Entelechy (verified via REST)
  2. Recall — subsequent turns surface stored memories
  3. Inject — <entelechy_memories> system message appears in forwarded frame
  4. Idempotency — memory block is replaced on re-injection (no duplication)

Run:
    ENTELECHY_LIVE_URL=http://localhost:8888 python tests/test_live_integration.py

Or via pytest (also respects ENTELECHY_LIVE_URL):
    ENTELECHY_LIVE_URL=http://localhost:8888 python -m pytest tests/test_live_integration.py -v -s
"""
...
from entelechy_pipecat.memory import _MEMORY_MARKER

LIVE_URL = os.environ.get("ENTELECHY_LIVE_URL", "http://localhost:8888")
LIVE_API_KEY = os.environ.get("ENTELECHY_LIVE_API_KEY")
...
async def _run_turn(service: EntelechyMemoryService, messages: list[dict[str, Any]]) -> MagicMock:
...
async def run_live_test() -> bool:
    print(f"\n=== Entelechy Pipecat Live Integration Test ===")
    ...
    try:
        ...
        print(f"  ❌ Entelechy not reachable: {e}")
        return False

    # 1. Create service
    service = EntelechyMemoryService(
        bank_id=UNIQUE_BANK,
        entelechy_api_url=LIVE_URL,
    ...
    # 5. Verify Inject — <entelechy_memories> system message added
    print("\n[4] Verify Inject (entelechy_memories system message)")
    ...
    if not memory_msgs:
        print("  ❌ No <entelechy_memories> system message injected")
        return False
    ...
    _print("entelechy_memories system messages", len(injected_msgs_after))
...
@pytest.mark.skipif(
    not os.environ.get("ENTELECHY_LIVE_URL"),
    reason="ENTELECHY_LIVE_URL not set — skipping live integration test",
)
@pytest.mark.asyncio
