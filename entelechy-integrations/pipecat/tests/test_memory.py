"""Unit tests for EntelechyMemoryService.

All Pipecat and Entelechy API calls are mocked — no live servers required.
"""
...
from entelechy_pipecat import (
    EntelechyMemoryService,
    EntelechyPipecatError,
    configure,
    reset_config,
)
from entelechy_pipecat.memory import _MEMORY_MARKER, _resolve_client
...
def _mock_client(recall_texts: list[str] | None = None) -> MagicMock:
    """Return a mock Entelechy client with async recall/retain methods."""
    client = MagicMock()
    response = MagicMock()
    if recall_texts:
        results = []
...
class TestResolveClient:
    ...
    def test_creates_client_from_url(self) -> None:
        with patch("entelechy_pipecat.memory.Entelechy") as MockEntelechy:
            _resolve_client(None, "http://localhost:8888", None)
            MockEntelechy.assert_called_once()

    def test_raises_when_no_url(self) -> None:
        with pytest.raises(EntelechyPipecatError, match="No Entelechy API URL"):
            _resolve_client(None, None, None)

    def test_uses_global_config_url(self) -> None:
        configure(entelechy_api_url="http://config-host:8888")
        with patch("entelechy_pipecat.memory.Entelechy") as MockEntelechy:
            _resolve_client(None, None, None)
            MockEntelechy.assert_called_once()
...
class TestRecall:
    async def test_memories_injected_as_system_message(self) -> None:
        ...
        svc = EntelechyMemoryService(bank_id="test", client=client)
    ...
    async def test_no_recall_when_disabled(self) -> None:
        ...
        svc = EntelechyMemoryService(bank_id="test", client=client, enable_recall=False)
        with patch.object(svc, "push_frame", new_callable=AsyncMock):
            await svc._handle_context_frame(frame)
