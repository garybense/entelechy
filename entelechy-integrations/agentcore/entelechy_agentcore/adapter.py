"""
EntelechyRuntimeAdapter — durable memory layer for AgentCore Runtime agents.
...
class RecallPolicy:
    ...
    Attributes:
        ...
        budget: Entelechy search depth — 'low', 'mid', or 'high'.
...
class EntelechyRuntimeAdapter:
    """Durable memory adapter for Amazon Bedrock AgentCore Runtime agents.

    Bridges AgentCore Runtime invocation identity to Entelechy memory banks.
    ...
    def __init__(
        ...
        *,
        ...
        """Create a EntelechyRuntimeAdapter.
        ...
        Args:
            ...
            entelechy_api_url: Entelechy API URL. Falls back to global config,
                then ENTELECHY_API_URL env var, then Entelechy Cloud.
            api_key: API key for Entelechy Cloud. Falls back to global config,
                then ENTELECHY_API_KEY / ENTELECHY_API_TOKEN env vars.
    ...
    async def before_turn(
        ...
        *,
        ...
        if self._verbose:
            logger.info("[Entelechy] Recalling from bank '%s'", bank_id)
        ...
        try:
            ...
            logger.warning("[Entelechy] Recall failed — continuing without memory", exc_info=True)
            return ""
    ...
    def _get_client(self) -> Any:
        """Return a lazily-initialized Entelechy client."""
        if self._client is None:
            from entelechy_client import Entelechy  # type: ignore[import]

            self._client = Entelechy(
    ...
    async def _retain(
        ...
        *,
        ...
        """Internal: call Entelechy retain with full metadata."""
        try:
            bank_id = self._resolve_bank(context)
        except BankResolutionError:
        ...
        if self._verbose:
            logger.info("[Entelechy] Retaining to bank '%s'", bank_id)
        ...
        try:
            ...
            logger.warning("[Entelechy] Retain failed — memory not stored", exc_info=True)
