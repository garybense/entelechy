...
Maps AgentCore invocation identity (tenant, user, agent) to a Entelechy bank ID
...
class BankResolver(Protocol):
    ...
    Implement this to customize how invocation identity maps to Entelechy bank IDs.
    """

    def __call__(self, context: TurnContext) -> str:
        """Resolve and return a Entelechy bank ID for the given context."""
        ...
...
def default_bank_resolver(context: TurnContext) -> str:
    ...
    Returns:
        A stable Entelechy bank ID string.
