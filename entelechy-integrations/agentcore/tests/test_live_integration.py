"""
Live integration test for entelechy-agentcore.

Skipped unless ``ENTELECHY_API_KEY`` is set. Targets the URL in
``ENTELECHY_API_URL`` (default: http://localhost:8888).
...
from entelechy_agentcore import EntelechyRuntimeAdapter, TurnContext, configure, reset_config

_api_key = os.environ.get("ENTELECHY_API_KEY")
_api_url = os.environ.get("ENTELECHY_API_URL", "http://localhost:8888")

pytestmark = pytest.mark.skipif(
    not _api_key,
    reason="ENTELECHY_API_KEY not set — skipping live integration test",
)
...
async def test_run_turn_retain_then_recall() -> None:
    reset_config()
    configure(entelechy_api_url=_api_url, api_key=_api_key)

    user_id = f"agentcore-live-{uuid.uuid4().hex[:8]}"
    adapter = EntelechyRuntimeAdapter(agent_name="agentcore-live-test")
