from entelechy_agentcore.config import (
    configure,
    get_config,
    reset_config,
...
class TestConfigure:
    ...
    def test_returns_config_object(self):
        cfg = configure(entelechy_api_url="http://localhost:8888")
        assert cfg.entelechy_api_url == "http://localhost:8888"
    ...
    def test_get_config_returns_config_after_configure(self):
        ...
        assert get_config().entelechy_api_url == "http://localhost:8888"
    ...
    def test_defaults_to_entelechy_cloud(self):
        cfg = configure()
        assert cfg.entelechy_api_url == DEFAULT_ENTELECHY_API_URL

    def test_reads_api_url_from_env(self, monkeypatch):
        monkeypatch.setenv("ENTELECHY_API_URL", "http://from-env:8888")
        cfg = configure()
        assert cfg.entelechy_api_url == "http://from-env:8888"

    def test_explicit_url_overrides_env(self, monkeypatch):
        monkeypatch.setenv("ENTELECHY_API_URL", "http://from-env:8888")
        cfg = configure(entelechy_api_url="http://explicit:9000")
        assert cfg.entelechy_api_url == "http://explicit:9000"

    def test_reads_api_key_from_env(self, monkeypatch):
        monkeypatch.setenv("ENTELECHY_API_KEY", "test-key-123")
        cfg = configure()
        assert cfg.api_key == "test-key-123"

    def test_reads_api_token_env_as_fallback(self, monkeypatch):
        monkeypatch.delenv("ENTELECHY_API_KEY", raising=False)
        monkeypatch.setenv("ENTELECHY_API_TOKEN", "token-456")
        cfg = configure()
