...
from entelechy_agentcore import (
...
def _make_mock_client() -> MagicMock:
    """Mock Entelechy client with async-native methods (arecall/aretain/areflect)."""
    client = MagicMock()
...
class TestBeforeTurn:
    def setup_method(self):
        reset_config()
        configure(entelechy_api_url="http://fake:9077")

    def teardown_method(self):
        reset_config()

    def _make_adapter(self, **kwargs):
        adapter = EntelechyRuntimeAdapter(**kwargs)
...
class TestAfterTurn:
    def setup_method(self):
        reset_config()
        configure(entelechy_api_url="http://fake:9077", retain_async=False)
...
class TestRetainAsync:
    ...
    def setup_method(self):
        reset_config()
        configure(entelechy_api_url="http://fake:9077", retain_async=True)
...
class TestRunTurn:
    ...
    async def test_default_agent_name_from_adapter(self):
        adapter = EntelechyRuntimeAdapter(agent_name="default-agent")
        mock_client = _make_mock_client()
        mock_client.arecall.return_value = _make_recall_response()
        adapter._client = mock_client
