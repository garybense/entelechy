/**
 * Hook implementations for the Entelechy OpenCode plugin.
 *
 * Hooks:
...
export interface EntelechyHooks {
...
export function createHooks(
  entelechyClient: EntelechyClient,
  bankId: string,
  ...
  async function recallForContext(query: string): Promise<RecallOutcome> {
    try {
      const response = await entelechyClient.recall(bankId, query, {
  ...
  async function retainSession(sessionId: string, messages: Message[]): Promise<void> {
    ...
    await ensureBankMission(entelechyClient, bankId, config, state.missionsSet);
    await entelechyClient.retain(bankId, transcript, {
      documentId,
