 ...
 */
export function deriveBankId(config: EntelechyConfig, directory: string): string {
  ...
  for (const f of fields) {
    if (!VALID_FIELDS.has(f)) {
      console.error(
        `[Entelechy] Unknown dynamicBankGranularity field "${f}" — ` +
          `valid: ${[...VALID_FIELDS].sort().join(", ")}`
      );
    }
  }

  const channelId = process.env.ENTELECHY_CHANNEL_ID || "";
  const userId = process.env.ENTELECHY_USER_ID || "";
