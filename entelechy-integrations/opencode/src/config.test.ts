...
import { loadConfig, type EntelechyConfig } from "./config.js";

describe("loadConfig", () => {
  const originalEnv = { ...process.env };

  beforeEach(() => {
    // Clear all ENTELECHY_ env vars
  ...
  it("returns defaults when no config sources exist", () => {
    ...
    expect(config.entelechyApiUrl).toBeNull();
    expect(config.entelechyApiToken).toBeNull();
    expect(config.bankId).toBeNull();
  });

  it("env vars override defaults", () => {
    process.env.ENTELECHY_API_URL = "https://example.com";
    process.env.ENTELECHY_API_TOKEN = "secret-token";
    process.env.ENTELECHY_BANK_ID = "my-bank";
    process.env.ENTELECHY_AUTO_RECALL = "false";
    process.env.ENTELECHY_AUTO_RETAIN = "0";
    process.env.ENTELECHY_RECALL_MAX_TOKENS = "2048";
    process.env.ENTELECHY_DEBUG = "true";

    const config = loadConfig();
    expect(config.entelechyApiUrl).toBe("https://example.com");
    expect(config.entelechyApiToken).toBe("secret-token");
  ...
  it("env vars override plugin options", () => {
    process.env.ENTELECHY_BANK_ID = "env-bank";
    const config = loadConfig({ bankId: "plugin-bank" });
    expect(config.bankId).toBe("env-bank");
  });

  it("boolean env var parsing", () => {
    process.env.ENTELECHY_AUTO_RECALL = "true";
    expect(loadConfig().autoRecall).toBe(true);

    process.env.ENTELECHY_AUTO_RECALL = "1";
    expect(loadConfig().autoRecall).toBe(true);

    process.env.ENTELECHY_AUTO_RECALL = "yes";
    expect(loadConfig().autoRecall).toBe(true);

    process.env.ENTELECHY_AUTO_RECALL = "false";
    expect(loadConfig().autoRecall).toBe(false);

    process.env.ENTELECHY_AUTO_RECALL = "no";
    expect(loadConfig().autoRecall).toBe(false);
  });

  it("integer env var parsing", () => {
    process.env.ENTELECHY_RECALL_MAX_TOKENS = "4096";
    expect(loadConfig().recallMaxTokens).toBe(4096);

    // Invalid integer keeps default
    process.env.ENTELECHY_RECALL_MAX_TOKENS = "not-a-number";
