...
describe("deriveBankId", () => {
  ...
  it("uses channel/user from env vars", () => {
    process.env.ENTELECHY_CHANNEL_ID = "slack-general";
    process.env.ENTELECHY_USER_ID = "user123";
    const config = makeConfig({
      dynamicBankId: true,
  ...
  it("uses defaults for missing env vars", () => {
    delete process.env.ENTELECHY_CHANNEL_ID;
    delete process.env.ENTELECHY_USER_ID;
    const config = makeConfig({
      dynamicBankId: true,
