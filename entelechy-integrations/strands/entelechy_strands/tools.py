"""Strands Agents tool factory for Entelechy memory operations.

Provides a factory function that creates Strands-compatible tool functions
backed by Entelechy's retain/recall/reflect APIs. Tools are plain Python
...
try:
    _VERSION = metadata.version("entelechy-strands")
except metadata.PackageNotFoundError:
    _VERSION = "0.0.0"
_USER_AGENT = f"entelechy-strands/{_VERSION}"
...
def _run_in_thread(fn: Any, *args: Any, **kwargs: Any) -> Any:
    ...
    Strands runs tools inside its own asyncio event loop. The entelechy client
...
def create_entelechy_tools(
    *,
    bank_id: str,
    client: Entelechy | None = None,
    ...
    """Create Entelechy memory tools for a Strands agent.

    Returns a list of ``@tool``-decorated functions that can be passed
    directly to ``Agent(tools=...)``. Each function captures ``bank_id``
    and the Entelechy client in its closure — no context modification needed.

    Args:
        bank_id: The Entelechy memory bank to operate on.
        client: Pre-configured Entelechy client (preferred).
        entelechy_api_url: API URL (used if no client provided).
    ...
    Raises:
        EntelechyError: If no client or API URL can be resolved.
    """
    resolved_client = _resolve_client(client, entelechy_api_url, api_key)
    ...
    if enable_retain:
        ...
        @tool
        def entelechy_retain(content: str) -> str:
            ...
            try:
                ...
                raise EntelechyError(f"Retain failed: {e}") from e

        tools.append(entelechy_retain)

    if enable_recall:

        @tool
        def entelechy_recall(query: str) -> str:
            ...
            try:
                ...
                raise EntelechyError(f"Recall failed: {e}") from e

        tools.append(entelechy_recall)

    if enable_reflect:

        @tool
        def entelechy_reflect(query: str) -> str:
        ...
        tools.append(entelechy_reflect)
...
def memory_instructions(
    *,
    ...
    Args:
        bank_id: The Entelechy memory bank to recall from.
        client: Pre-configured Entelechy client (preferred).
        entelechy_api_url: API URL (used if no client provided).
        api_key: API key (used if no client provided).
