"""Unit tests for Entelechy Strands tools."""
...
from entelechy_strands import (
    configure,
    create_entelechy_tools,
    memory_instructions,
    reset_config,
)
from entelechy_strands.errors import EntelechyError
from entelechy_strands.tools import _resolve_client
...
class TestResolveClient:
    ...
    def test_creates_client_from_url(self):
        with patch("entelechy_strands.tools.Entelechy") as mock_cls:
...
# ---------------------------------------------------------------------------
# create_entelechy_tools — factory
# ---------------------------------------------------------------------------
...
class TestCreateEntelechyTools:
    ...
    def test_enable_retain_only(self):
        ...
        assert len(tools) == 1
        assert tools[0].__name__ == "entelechy_retain"
    ...
    def test_enable_recall_only(self):
        ...
        assert len(tools) == 1
        assert tools[0].__name__ == "entelechy_recall"
    ...
    def test_enable_reflect_only(self):
        ...
        assert len(tools) == 1
        assert tools[0].__name__ == "entelechy_reflect"
    ...
    def test_falls_back_to_global_config(self):
        ...
        with patch("entelechy_strands.tools.Entelechy") as mock_cls:
            mock_cls.return_value = _mock_client()
            tools = create_entelechy_tools(bank_id="test")
            assert len(tools) == 3
            mock_cls.assert_called_once_with(
                base_url="http://localhost:8888", timeout=30.0
            )
