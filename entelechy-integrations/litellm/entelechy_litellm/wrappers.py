"""Native client wrappers for Entelechy memory integration.

This module provides wrappers for native LLM client SDKs (OpenAI, Anthropic)
that automatically integrate with Entelechy for memory injection and storage.
...
def _get_client(api_url: str, api_key: Optional[str] = None):
    """Create a fresh Entelechy client for the given URL.

    Note: We create a fresh client each time because the entelechy_client
    ...
    return Entelechy(base_url=api_url, api_key=api_key, timeout=30.0, user_agent=USER_AGENT)
...
def recall(
    ...
    """Recall memories from Entelechy.
    ...
    Args:
        ...
        entelechy_api_url: Override the configured API URL
    ...
    Raises:
        RuntimeError: If Entelechy is not configured and no overrides provided

    Example:
        >>> from entelechy_litellm import configure, recall
        >>> configure(bank_id="my-agent", entelechy_api_url="http://localhost:8888")
        >>>
        >>> # Query memories
    ...
    api_url = entelechy_api_url or (config.entelechy_api_url if config else None)
    ...
    if not api_url or not target_bank_id:
        raise RuntimeError("Entelechy not configured. Call configure() or provide bank_id and entelechy_api_url.")
    ...
    try:
        ...
        raise RuntimeError(f"entelechy-client not installed: {e}")
...
def reflect(
    ...
    Example:
        >>> from entelechy_litellm import configure, reflect
        >>> configure(bank_id="my-agent", entelechy_api_url="http://localhost:8888")
        >>>
...
def _retain_sync(
    ...
    try:
        ...
        debug_info = None
        if verbose:
            logger.info(f"Stored content to Entelechy bank: {target_bank_id}")
...
def retain(
    ...
    """Store content to Entelechy memory.
    ...
    Example:
        >>> from entelechy_litellm import configure, retain
        >>> configure(bank_id="my-agent", entelechy_api_url="http://localhost:8888")
        >>>
...
class _StreamWrapper:
    ...
    def __init__(
        ...
        user_query: str,
        model: str,
        wrapper: "EntelechyOpenAI",
        settings: EntelechyCallSettings,
    ):
        self._stream = stream
...
class EntelechyOpenAI:
    """Wrapper for OpenAI client with Entelechy memory integration.
    ...
    Example:
        ...
        ...     entelechy_bank_id="other-user",  # Different bank for this call
        ...     entelechy_budget="high",          # Higher recall budget
        ...     entelechy_inject_memories=False,  # Skip memory injection
        ... )
    """
    ...
    def __init__(
        ...
        api_key: Optional[str] = None,
        default_settings: Optional[EntelechyCallSettings] = None,
        **setting_kwargs,
    ):
        ...
        Args:
            ...
            entelechy_api_url: URL of the Entelechy API server (required, connection-level)
            api_key: API key for Entelechy authentication (connection-level)
            default_settings: Default settings for all calls (alternative to kwargs)
            **setting_kwargs: Default values for any EntelechyCallSettings field
                (e.g., bank_id="my-bank", budget="high", verbose=True)
        """
        self._client = client
        self._api_url = entelechy_api_url
        self._api_key = api_key
        self._entelechy_client = None

        # Build default settings from kwargs or use provided settings
        if default_settings is not None:
            self._default_settings = default_settings
        else:
            # Filter kwargs to only valid EntelechyCallSettings fields
            ...
            self._default_settings = EntelechyCallSettings(**settings_kwargs)

        # Create wrapped chat.completions interface
        self.chat = _WrappedChat(self)

    def _get_entelechy_client(self):
        """Get or create the Entelechy client."""
        if self._entelechy_client is None:
            from entelechy_client import Entelechy

            self._entelechy_client = Entelechy(
                base_url=self._api_url,
                api_key=self._api_key,
                timeout=30.0,
                user_agent=USER_AGENT,
            )
        return self._entelechy_client

    def _recall_memories(self, query: str, settings: EntelechyCallSettings) -> str:
        ...
        try:
            client = self._get_entelechy_client()
    ...
    def _reflect_memories(self, query: str, settings: EntelechyCallSettings) -> str:
    ...
    def _store_conversation(
        ...
        """Store the conversation to Entelechy.
        ...
        try:
            ...
            if settings.verbose:
                logger.info(f"Stored conversation to Entelechy bank: {settings.bank_id}")
...
class _WrappedChat:
    ...
    def __init__(self, wrapper: EntelechyOpenAI):
        self._wrapper = wrapper
...
class _WrappedCompletions:
    ...
    def create(self, **kwargs) -> Any:
        ...
        Supports per-call overrides via entelechy_* kwargs:
        - entelechy_bank_id: Override memory bank
        - entelechy_budget: Override recall budget (low/mid/high)
        - entelechy_inject_memories: Override whether to inject memories
        - entelechy_store_conversations: Override whether to store
        - entelechy_query: Custom query for memory recall
        - entelechy_use_reflect: Use reflect API instead of recall
        - And all other EntelechyCallSettings fields...
        """
        # Merge default settings with per-call overrides
        settings = _merge_settings(self._wrapper._default_settings, kwargs)

        # Remove entelechy_* kwargs before passing to OpenAI
        openai_kwargs = {k: v for k, v in kwargs.items() if not k.startswith("entelechy_")}
...
class EntelechyAnthropic:
    """Wrapper for Anthropic client with Entelechy memory integration.
    ...
    Example:
        ...
        ...     max_tokens=1024,
        ...     messages=[{"role": "user", "content": "Hello"}],
        ...     entelechy_bank_id="other-user",  # Different bank
        ...     entelechy_budget="high",          # More memories
        ... )
    """
...
class _WrappedAnthropicMessages:
    ...
    def __init__(self, wrapper: EntelechyAnthropic):
        self._wrapper = wrapper
    ...
    def create(self, **kwargs) -> Any:
        ...
        # Remove entelechy_* kwargs before passing to Anthropic
        anthropic_kwargs = {k: v for k, v in kwargs.items() if not k.startswith("entelechy_")}
...
def wrap_openai(
    ...
    **settings_kwargs,
    ...
    """Wrap an OpenAI client with Entelechy memory integration.
    ...
    All settings can be overridden per-call using entelechy_* kwargs.
    ...
    Args:
        ...
        **settings_kwargs: Default values for any EntelechyCallSettings field:
    ...
    if mission or bank_name:
        try:
            ...
            hs_client = Entelechy(base_url=resolved_api_url, api_key=resolved_api_key, user_agent=USER_AGENT)
...
def wrap_anthropic(
    ...
    **settings_kwargs,
    ...
    """Wrap an Anthropic client with Entelechy memory integration.
