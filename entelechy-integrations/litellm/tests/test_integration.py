"""Integration tests for entelechy-litellm."""

import pytest

from entelechy_litellm import (
    configure,
    set_defaults,
    get_defaults,
    enable,
...
from entelechy_litellm.callbacks import EntelechyCallback
...
class TestConfiguration:
    ...
    def test_configure_with_all_options(self):
        ...
        assert config.entelechy_api_url == "http://custom:9999"
...
class TestCallback:
    """Tests for the EntelechyCallback class."""
    ...
    def test_format_memories(self):
        ...
        configure(entelechy_api_url="http://localhost:8888", verbose=False)
    ...
    def test_inject_memories_uses_last_user_message_when_no_entelechy_query(self):
        """Regression test: inject_memories=True should not require entelechy_query.

        The documented Quick Start example does not pass entelechy_query; the
        ...
        kwargs = {}  # No entelechy_query provided — this is the regression scenario
    ...
    def test_inject_memories_entelechy_query_takes_precedence(self):
        """When entelechy_query is provided it should be used over the last user message."""
        from unittest.mock import MagicMock, patch
        ...
        kwargs = {"entelechy_query": "What do I know about Alice?"}
...
class TestContextManager:
    """Tests for the entelechy_memory context manager."""
    ...
    def test_context_manager_enables_and_disables(self):
        ...
        with entelechy_memory(bank_id="test-agent"):
            assert is_enabled() is True
            defaults = get_defaults()
            assert defaults.bank_id == "test-agent"
    ...
    def test_context_manager_restores_previous_config(self):
        ...
        with entelechy_memory(bank_id="temporary-agent"):
    ...
    def test_context_manager_with_fact_types(self):
        ...
        with entelechy_memory(bank_id="test-agent", fact_types=["world", "opinion"]):
            defaults = get_defaults()
            assert defaults.fact_types == ["world", "opinion"]
...
class TestStreamingResponseHandling:
    ...
    def test_format_conversation_for_storage_returns_empty_for_stream(self):
        ...
        """
        from entelechy_litellm import _format_conversation_for_storage
    ...
    def test_store_conversation_skips_stream_response(self):
        ...
        from entelechy_litellm import _store_conversation
    ...
    def test_wrapped_completion_returns_stream_wrapper(self):
        ...
        from entelechy_litellm import _LiteLLMStreamWrapper

        configure(entelechy_api_url="http://localhost:8888", store_conversations=True)
        set_defaults(bank_id="test-agent")
        enable()
    ...
    def test_stream_wrapper_yields_all_chunks(self):
        ...
        configure(entelechy_api_url="http://localhost:8888", store_conversations=False)
        set_defaults(bank_id="test-agent")
    ...
    def test_stream_wrapper_stores_conversation_on_exhaustion(self):
        ...
        with patch("entelechy_litellm._store_conversation_from_text") as mock_store:
            # Consume all chunks
            list(wrapper)
    ...
    def test_callback_log_success_with_stream_response(self):
        """Test that EntelechyCallback.log_success_event handles stream responses."""
        from entelechy_litellm.callbacks import EntelechyCallback
...
class TestStreamingSupport:
    ...
    def test_wrap_openai_with_stream_no_error(self):
        ...
        from entelechy_litellm.wrappers import wrap_openai
    ...
    def test_wrap_anthropic_with_stream_no_error(self):
        ...
        from unittest.mock import Mock, MagicMock
        from entelechy_litellm.wrappers import wrap_anthropic
    ...
    def test_wrap_openai_stream_stores_conversation(self):
        ...
        # Mock the entelechy client
        mock_entelechy_client = MagicMock()
        with patch.object(wrapped, "_get_entelechy_client", return_value=mock_entelechy_client):
            # Call with stream=True
            result = wrapped.chat.completions.create(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": "Hello"}],
            ...
            mock_entelechy_client.retain.assert_called_once()
            call_kwargs = mock_entelechy_client.retain.call_args[1]
