...
const CLOUD_TAG = 'entelechy-cloud';
const CLOUD_PREVIEW_COUNT = 3;
const ENTELECHY_PAGE_SIZE = 9;
...
export default function BlogListPage({items, metadata}: Props): React.ReactElement {
  ...
  const entelechyPosts = items.filter(({content}) =>
    !(content.metadata.tags ?? []).some((t) => t.label === CLOUD_TAG),
  );

  const totalEntelechyPages = Math.max(1, Math.ceil(entelechyPosts.length / ENTELECHY_PAGE_SIZE));
  const entelechyPagePosts = entelechyPosts.slice(
    (currentPage - 1) * ENTELECHY_PAGE_SIZE,
    currentPage * ENTELECHY_PAGE_SIZE,
  );
      ...
      <main className={styles.blogPage}>
        ...
        {currentPage === 1 && cloudPosts.length > 0 && (
          <section className={styles.section}>
            <SectionHeader
              title="Entelechy Cloud"
              subtitle="News and updates from Entelechy Cloud"
        ...
        {entelechyPagePosts.length > 0 && (
          <section className={styles.section}>
            ...
            />
            <div className={styles.grid}>
              {entelechyPagePosts.map(({content: BlogPostContent}) => (
                <BlogCard key={BlogPostContent.metadata.permalink} content={BlogPostContent} />
              ))}
            </div>
          </section>
        ...
        {totalEntelechyPages > 1 && (
          <nav className={styles.pagination}>
            ...
            <span className={styles.paginationInfo}>
              Page {currentPage} of {totalEntelechyPages}
            </span>
            {currentPage < totalEntelechyPages && (
              <button onClick={() => goToPage(currentPage + 1)} className={styles.paginationButton}>
                Next →
              </button>
            )}
