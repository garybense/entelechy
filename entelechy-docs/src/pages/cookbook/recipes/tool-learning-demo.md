...
This notebook demonstrates how Entelechy helps an LLM learn which tool to use when tool names are ambiguous. Without memory, the LLM might randomly select between similarly-named tools. With Entelechy, it learns from past interactions and consistently makes the correct choice.
...
**Without Entelechy:** The LLM guesses randomly based on vague descriptions  
**With Entelechy:** The LLM learns from feedback which channel handles what
...
```python
!pip install entelechy-litellm entelechy-client litellm nest_asyncio python-dotenv -U -q
```
...
def make_routing_request(user_request: str, use_entelechy: bool, bank_id: Optional[str] = None):
...
def store_feedback(bank_id: str, request: str, correct_tool: str, request_type: str):
    ...
    client = Entelechy(base_url=ENTELECHY_API_URL, timeout=60.0)
...
## Phase 1: Without Entelechy (No Memory)
...
```python
print("=" * 60)
print("PHASE 1: WITHOUT ENTELECHY (No Memory)")
print("=" * 60)
...
for i, scenario in enumerate(TEST_SCENARIOS[:3], 1):
    ...
    tool_name = make_routing_request(scenario['request'], use_entelechy=False)
...
print("\nWaiting 15 seconds for Entelechy to process memories...")
time.sleep(15)
print("Done!")
```

## Phase 3: With Entelechy (Memory-Augmented)

The LLM now has access to learned routing knowledge via Entelechy. It should route requests correctly based on past feedback.


```python
print("=" * 60)
print("PHASE 3: WITH ENTELECHY (Memory-Augmented)")
print("=" * 60)
...
print(f"Phase 3 (With Entelechy): {phase3_accuracy:.0f}% accuracy")

improvement = phase3_accuracy - phase1_accuracy
if improvement > 0:
    print(f"\n🎉 Improvement: +{improvement:.0f}% accuracy with Entelechy!")
elif improvement == 0:
...
print("KEY INSIGHT")
print("=" * 60)
print("Entelechy allows the LLM to learn from experience which tool")
