...
# Movie Recommendation Assistant with Entelechy Memory


:::tip Run this notebook
This recipe is available as an interactive Jupyter notebook.
[**Open in GitHub →**](https://github.com/vectorize-io/entelechy-cookbook/blob/main/notebooks/movie_recommendation.ipynb)
:::
...
## Prerequisites
- OpenAI API key
- Entelechy running locally via Docker (see setup below)

## Start Entelechy Locally

Before running this notebook, start Entelechy in a terminal:
...
```python
!pip install -q entelechy-client openai nest-asyncio
```

## 2. Configure OpenAI API Key

Enter your OpenAI API key when prompted (used by both Entelechy and the demo).
...
# Set OpenAI API key (used by both Entelechy and the demo)
...
# Initialize Entelechy client (connects to local Docker instance)
entelechy = Entelechy(
    base_url=os.getenv("ENTELECHY_BASE_URL", "http://localhost:8888"),
)
...
These functions demonstrate the three core Entelechy operations:
...
Use `reflect()` to synthesize what Entelechy has learned about your movie tastes.
...
Close the Entelechy client connection.
