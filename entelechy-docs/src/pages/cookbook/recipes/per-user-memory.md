...
```python
!pip install entelechy-client nest_asyncio openai python-dotenv -U
```
...
def on_user_signup(user_id: str):
    ...
    print(f"View bank: {ENTELECHY_UI_URL}/banks/user-{user_id}?view=documents")
```

## 2. Manage Conversation Sessions

Use `document_id` to group messages belonging to the same conversation. When you retain with the same `document_id`, Entelechy replaces the previous version (upsert behavior), keeping the memory up-to-date as the conversation evolves.
...
class ConversationSession:
    ...
    def save(self, client: Entelechy):
...
print(f"\nView documents: {ENTELECHY_UI_URL}/banks/user-alice?view=documents")
```
...
Entelechy automatically extracts and connects:
...
response = requests.delete(f"{ENTELECHY_API_URL}/v1/default/banks/user-alice")
