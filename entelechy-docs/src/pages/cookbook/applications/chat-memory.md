...
A demo chat application that uses Groq's `qwen/qwen3-32b` model with Entelechy for persistent per-user memory.
...
### 1. Start Entelechy API

First, start the Entelechy API server using Docker:

```bash
export GROQ_API_KEY=your_groq_api_key_here

# Start Entelechy with Groq as the LLM provider
docker run --rm -it --pull always -p 8888:8888 -p 9999:9999 \
  -e ENTELECHY_API_LLM_PROVIDER=groq \
...
echo "ENTELECHY_API_URL=http://localhost:8888" >> .env.local
```
...
2. **Memory Bank Creation**: First message creates a personal memory bank in Entelechy
...
Next.js API Route (/api/chat)
     ↓
Entelechy.recall() → Get relevant memories
     ↓
Groq API → Generate response with memory context
     ↓
Entelechy.retain() → Store conversation
     ↓
Response to User
```
...
4. **Memory Verification**: Visit the Entelechy Control Plane at http://localhost:9999 to see stored memories
