...
# OpenAI Agent + Entelechy Memory Integration


:::info Complete Application
This is a complete, runnable application demonstrating Entelechy integration.
[**View source on GitHub →**](https://github.com/vectorize-io/entelechy-cookbook/tree/main/applications/openai-fitness-coach)
:::


A fitness coach example demonstrating how to use **OpenAI Agents** with **Entelechy as a memory backend**.
...
- **Entelechy** providing sophisticated memory storage & retrieval
...
|-----------|---------------|-------------------|
| **Conversation** | Entelechy `/think` endpoint | OpenAI Assistant API |
| **Memory** | Entelechy (built-in) | Entelechy (via function calling) |
| **LLM** | Configured in Entelechy | OpenAI GPT-4 |
| **Opinion Formation** | Automatic in `/think` | Explicit via `store_memory(type="opinion")` |
| **Best For** | Entelechy-native apps | Integrating memory into existing OpenAI agents |
...
2. **Entelechy API running**
   ```bash
   # Follow Entelechy setup instructions to start the API
...
Each function makes API calls to Entelechy to fetch relevant memories.
...
3. Functions fetch data from Entelechy
...
# Function retrieves from Entelechy:
{
  "results": [
...
3. Retrieve data from Entelechy
4. Generate a personalized response

## Memory Types Retrieved

The OpenAI Agent can retrieve different memory types from Entelechy:
...
**To add Entelechy memory to your own OpenAI Agent:**

1. Define function tools that call Entelechy API
2. Register them with your OpenAI Assistant
3. Implement function handlers to execute Entelechy queries
4. Let OpenAI Assistant decide when to retrieve memories

The key benefit: **Separation of concerns**
- OpenAI = Conversation logic
- Entelechy = Memory storage, retrieval, temporal queries, entity linking

## When to Use This vs. Standard Entelechy

**Use OpenAI + Entelechy (this example) when:**
...
- You want to combine Entelechy with other OpenAI features

**Use Entelechy directly when:**
...
- `memory_tools.py` - Function calling tools that bridge to Entelechy API
...
**"Agent not found"**
- Make sure the Entelechy fitness-coach agent exists

**"Connection refused"**
- Make sure Entelechy API is running on localhost:8888
...
- Entelechy (temporal-semantic memory)
