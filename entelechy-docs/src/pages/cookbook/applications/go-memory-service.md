...
[**View source on GitHub →**](https://github.com/vectorize-io/entelechy-cookbook/tree/main/applications/go-memory-service)
:::


A Go HTTP microservice demonstrating per-user memory isolation with Entelechy. Remembers each user's tech stack, problems solved, and preferences to provide personalized assistance.
...
go func() {
    ...
    retainReq := entelechy.RetainRequest{
        Items: []entelechy.MemoryItem{{
            Content: interaction,
            Context: *entelechy.NewNullableString(entelechy.PtrString("Q&A interaction")),
        }},
    }
