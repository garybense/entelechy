...
Run 1: "Research Rust benefits"
    │
    ├─ Researcher agent ──► Entelechy reflect (no prior memories)
    │                        ──► produces research findings
    ├─ Writer agent ────────► summarizes findings
    │
    └─ CrewAI auto-stores task outputs to Entelechy
                    │
Run 2: "Compare Rust with Go"
    │
    ├─ Researcher agent ──► Entelechy reflect (recalls Rust research!)
...
The `EntelechyReflectTool` lets agents explicitly query their memories with disposition-aware synthesis:
...
researcher = Agent(
    ...
    backstory="Always use entelechy_reflect to check what you already know.",
    tools=[reflect_tool],
)
```
...
| `mission` | `None` | Guide how Entelechy organizes memories |
...
**"Connection refused"**
- Make sure Entelechy is running on `localhost:8888`
...
**"No module named 'entelechy_crewai'"**
```bash
pip install -r requirements.txt
```
