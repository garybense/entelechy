...
An AI-powered application that tracks political candidates' stances on issues over time using Entelechy memory system and web scraping.
...
Verify Entelechy is running at `http://localhost:8888`
...
## Entelechy Integration Examples

### 1. Storing Memories

```typescript
// Store web scraping results
await entelechyClient.retain(bankId, articleContent, {
...
const results = await entelechyClient.recall(bankId, query, {
  budget: 'high',
  maxTokens: 8192
});
```
...
const response = await entelechyClient.reflect(bankId,
...
│   ├── entelechy-client.ts    # Entelechy API client
...
- **Rate Limits**: Respect API rate limits for Tavily, Entelechy, and LLM providers
