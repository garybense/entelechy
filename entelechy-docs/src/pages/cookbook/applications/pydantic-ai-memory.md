...
- **Memory tools** — retain, recall, and reflect via `create_entelechy_tools()`
...
Session 1:
    ...
    ├─ Agent decides to call entelechy_retain ──► stores the fact
...
Session 2:
    ...
    ├─ Agent calls entelechy_recall ──► finds stored facts
...
`create_entelechy_tools()` returns Pydantic AI `Tool` instances the agent can call:

```python
from entelechy_pydantic_ai import create_entelechy_tools

tools = create_entelechy_tools(client=client, bank_id="personal-assistant")
# Returns: [entelechy_retain, entelechy_recall, entelechy_reflect]
```
...
```python
from entelechy_pydantic_ai import memory_instructions
...
agent = Agent(
    "openai:gpt-4o-mini",
    tools=create_entelechy_tools(client=client, bank_id="my-bank"),
)
```
...
**"No module named 'entelechy_pydantic_ai'"**
```bash
pip install -r requirements.txt
```
