---
title: "What's new in Entelechy 0.4.18"
description: New features and improvements in Entelechy 0.4.18
...
Entelechy 0.4.18 adds compound tag filtering with boolean logic, slimmer Python packages, MiniMax and Jina MLX (Apple Silicon) provider support, per-bank HNSW vector indexes, and a configurable recall query token limit.
...
By default, `entelechy-api` bundles everything needed to run embeddings and reranking **locally**—PyTorch, sentence-transformers, transformers, and MLX. These local ML libraries are what make zero-config deployments work out of the box, but they're heavy (several GB of dependencies). If you're already pointing Entelechy at an external embedding or reranking service (TEI, Cohere, ZeroEntropy, LiteLLM), you don't need any of that.
...
|---------|-------------|
| `pip install entelechy-api-slim` | Core API server; no local ML models |
| `pip install "entelechy-api-slim[local-ml]"` | Adds PyTorch, sentence-transformers, MLX (Apple Silicon) |
| `pip install "entelechy-api-slim[embedded-db]"` | Adds pg0-embedded (local PostgreSQL) |
| `pip install "entelechy-api-slim[all]"` | Local ML + embedded DB; equivalent to the old `entelechy-api` |
| `pip install entelechy-api` | Unchanged; still installs the full stack for backward compatibility |

The base `entelechy-api-slim` package includes the FastAPI server, all LLM provider integrations (OpenAI, Anthropic, Gemini, Groq, MiniMax, and more), Cohere cloud embeddings/reranking, LiteLLM, MCP, and all API functionality. Only the local ML model dependencies are gated behind the `[local-ml]` extra.

If you use external embedding and reranking providers, you can now get a significantly leaner Python environment with `pip install entelechy-api-slim`.
...
```bash
ENTELECHY_API_LLM_PROVIDER=minimax
ENTELECHY_API_LLM_API_KEY=your_minimax_key
ENTELECHY_API_LLM_MODEL=MiniMax-M2.5   # optional; this is the default
```
...
```bash
ENTELECHY_API_RERANKER_PROVIDER=jina-mlx
```
...
**Improvements**
- Recall query token limit is now configurable via `ENTELECHY_API_RECALL_MAX_QUERY_TOKENS` (default: 500). Useful when you want to allow longer multi-turn context as the recall query.
...
Entelechy 0.4.18 is a drop-in replacement for 0.4.x with no breaking changes.
