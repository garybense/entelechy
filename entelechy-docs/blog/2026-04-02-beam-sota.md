---
title: "Entelechy Is #1 on BEAM — the Benchmark That Tests Memory at 10 Million Tokens"
authors: [benfrank241]
date: 2026-04-02
tags: [benchmark, memory, beam, evaluation, open-source]
image: /img/blog/beam-benchmark-chart.png
description: "Entelechy is #1 on BEAM — the memory benchmark that tests at 10M tokens where context stuffing is impossible. See every published result and what drives the 58% margin."
hide_table_of_contents: true
---

![Entelechy leads the BEAM 10M benchmark with 64.1% vs the next-best 40.6%](/img/blog/beam-benchmark-chart.png)

<!-- truncate -->

[Entelechy](https://github.com/vectorize-io/entelechy) is SOTA on BEAM, the benchmark that tests memory at 10 million tokens, where context stuffing is physically impossible and only a real memory architecture survives.
...
Entelechy scores 64.1% at 10M. The next-best published result is 40.6%. That's a 58% margin. Against the paper baselines, it's more than 2.4x.

The full picture across all BEAM tiers:

| Tier | Entelechy | Honcho | LIGHT baseline | RAG baseline |
|---|---|---|---|---|
...
One detail worth noting: Entelechy's 1M score (73.9%) is higher than its 500K score (71.1%). Performance doesn't degrade as token volume increases; it improves. Most systems show the opposite. That's the architecture working as intended, and it's where the gap versus other approaches becomes most visible.
...
Entelechy runs fully locally. No account, no API key, no data leaving your machine:
...
For memory that follows you across machines, or that multiple agents can share, connect to Entelechy Cloud instead. One config change:
...
- **Quick start**: [entelechy.vectorize.io/developer/api/quickstart](/developer/api/quickstart)
