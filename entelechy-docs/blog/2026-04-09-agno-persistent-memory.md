...
description: "Agno agents start each run fresh. entelechy-agno adds EntelechyTools and memory_instructions so they can retain, recall, and reflect across sessions."
...
Adding persistent memory does not require building a custom RAG pipeline or maintaining your own vector store. The `entelechy-agno` package extends Agno's native `Toolkit` pattern to give your agents long-term memory: retain facts, recall them by semantic search, and synthesize coherent answers from accumulated knowledge.
...
- `entelechy-agno` adds `EntelechyTools`, a native Agno `Toolkit` with retain, recall, and reflect tools.
...
The `entelechy-agno` integration connects to Agno at two points: tools and instructions.
...
`EntelechyTools` extends Agno's `Toolkit` base class directly, the same pattern used by `Mem0Tools` in the Agno ecosystem. You add it to `tools=[...]` exactly like any other Agno toolkit.
...
```bash
pip install entelechy-agno agno
```

### Step 3: Add `EntelechyTools` to your agent
...
Restart the process and run the second call again. The agent still knows. Agno persistent memory is stored in Entelechy, not in the agent's context window.
...
On every `agent.run()`, `memory_instructions` calls Entelechy's recall API and injects relevant memories into the system prompt. The agent starts each conversation with context. No tool call needed.
...
For agents that serve multiple users, `EntelechyTools` resolves the bank ID dynamically. Resolution order:
...
agent1_tools = EntelechyTools(bank_id="user-alice")
agent2_tools = EntelechyTools(bank_id="user-bob")
```

### Entelechy Cloud configuration
...
BANK_ID = "demo-user"
ENTELECHY_URL = "http://localhost:8888"

agent = Agent(
    model=OpenAIChat(id="gpt-4o-mini"),
    tools=[EntelechyTools(
        bank_id=BANK_ID,
        entelechy_api_url=ENTELECHY_URL,
    )],
    instructions=[memory_instructions(
        bank_id=BANK_ID,
        entelechy_api_url=ENTELECHY_URL,
...
Skip the memory layer for one-shot agents that never interact with the same user twice, stateless API handlers where each request is fully independent, or agents where you want complete control over what enters the prompt. In those cases, use the Entelechy Python client directly instead of the Agno integration.

## How It Compares to Alternatives

| Approach | Strengths | Weaknesses | Best For |
|---|---|---|---|
| **Entelechy + Agno** | Multi-strategy retrieval (semantic + BM25 + graph + temporal), structured fact extraction, synthesis | Requires Entelechy server or cloud | Multi-session agents needing deep memory |
...
**Duplicate memories.** Entelechy deduplicates at the fact level, but giving the agent guidance in the system prompt about when to store new facts helps.

## Recap

| | Agno default | With Entelechy |
|---|---|---|
