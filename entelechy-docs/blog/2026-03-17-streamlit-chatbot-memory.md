...
Build a web chatbot with persistent memory using Streamlit and Entelechy. ~80 lines of Python, no frontend framework. Memory survives restarts, and a sidebar shows what the agent remembers.
...
You built a chatbot with memory using [OpenAI and Entelechy](/blog/2026/03/05/add-memory-to-openai-application). It works in a terminal:
...
st.chat_input → user message
     ↓
recall(query)       ← pull relevant memories from Entelechy
     ↓
...
Start with a working chat UI. No memory, no Entelechy.
...
## Step 2 — Add Persistent Memory with Entelechy

Install the dependencies:

```bash
pip install entelechy-all entelechy-client
```

Start the Entelechy server in a separate terminal:
...
if prompt := st.chat_input("Say something"):
    ...
    with st.spinner("Remembering..."):
        memories = entelechy.recall(bank_id=BANK_ID, query=prompt, budget="low")
...
It remembers. Because `recall` pulled the fact from Entelechy and injected it into the system prompt.

**What happens under the hood:** When you call `retain`, Entelechy extracts structured facts from the natural language content — entities, relationships, timestamps — and stores them in a knowledge graph backed by embedded Postgres with [pgvector](https://github.com/pgvector/pgvector). When you call `recall`, it runs semantic search over those facts and returns the most relevant ones for your query. You don't write schemas, queries, or extraction logic. The [OpenAI API](https://platform.openai.com/docs/api-reference) handles the LLM calls for both extraction and generation.
...
with st.sidebar:
    ...
    memory_list = entelechy.list_memories(bank_id=BANK_ID, limit=1)
...
- **Last recalled** — the specific facts Entelechy found for the most recent query
...
**6. Conversation history grows without bound.** `st.session_state.messages` accumulates every message. For long sessions, this means increasingly large payloads to OpenAI. Trim the list or switch to a sliding window. Entelechy handles long-term context — you don't need to keep the full conversation in the prompt.
...
- `st.session_state` for ephemeral display state, Entelechy for persistent memory
...
Streamlit handles the UI. Entelechy handles the memory. OpenAI handles the generation. Each does one thing.
