---
title: "What's new in entelechy-openclaw 0.6"
authors: [benfrank241]
...
![Entelechy x OpenClaw](/img/blog/openclaw-plugin-06.png)
...
- [**External API Backend**](#external-api-backend): Point multiple OpenClaw instances at a shared Entelechy server.
...
- [**Backfill CLI**](#backfill-cli): Ingest historical conversations into Entelechy retroactively.
  ...
  "plugins": {
    "entries": {
      "entelechy-openclaw": {
        "enabled": true,
        "config": {
          "entelechyApiUrl": "http://localhost:9077",
          "provider": "openai",
          "model": "gpt-4o-mini",
          "apiKey": "sk-..."
        }
...
You can now point the plugin at a self-hosted Entelechy API server instead of running a local embedded daemon. This is the right setup when multiple OpenClaw instances should share the same memory store, or when you want to separate the memory service from the gateway machine. See the [self-hosting quickstart](/developer/api/quickstart) for infrastructure requirements.
