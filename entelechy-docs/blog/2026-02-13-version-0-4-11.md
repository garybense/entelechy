---
title: "What's new in Entelechy 0.4.11"
description: New features and improvements in Entelechy 0.4.11
authors: [nicoloboschi]
date: 2026-02-13T12:00
hide_table_of_contents: true
---

Entelechy 0.4.11 focuses on production-ready deployments with improved flexibility and observability.
...
Configuration cascades from system defaults (env vars) → tenant overrides → bank-specific settings. The bank config API is enabled by default and can be disabled with `ENTELECHY_API_ENABLE_BANK_CONFIG_API=false`.
...
Entelechy already supported LiteLLM via proxy mode (routing requests through a LiteLLM proxy server). Now you can use LiteLLM directly via the Python SDK for embeddings and reranking—no proxy server needed.
...
export ENTELECHY_API_EMBEDDINGS_LITELLM_SDK_API_KEY=your-api-key
export ENTELECHY_API_EMBEDDINGS_LITELLM_SDK_MODEL=cohere/embed-english-v3.0
```

The same applies to reranking with `ENTELECHY_API_RERANKER_PROVIDER=litellm-sdk`. Use proxy mode when you need centralized rate limiting and caching; use SDK mode for simpler deployments.

## Expanded Database Support

PostgreSQL search support now includes:

- **TimescaleDB pg_textsearch** for better full-text search in time-series workloads ([docker-compose example](https://github.com/vectorize-io/entelechy/tree/main/docker/docker-compose/pg_textsearch))
- **vchord and pgvector** for flexible vector storage options ([docker-compose example](https://github.com/vectorize-io/entelechy/tree/main/docker/docker-compose/vchord))
...
Entelechy now emits OpenTelemetry traces for all operations, providing request-level observability across distributed systems. Combined with actual LLM token usage (not estimates) and improved user-initiated attribution in request context, you get complete visibility into costs and performance.
...
The Model Context Protocol server now supports full mental model lifecycle management. Agents using Entelechy via MCP can create, read, update, and delete mental models—not just query them.
...
Running Entelechy behind a reverse proxy or at a non-root path? Configure your base path and Entelechy handles routing correctly, making it easier to integrate with existing infrastructure.

See the [nginx docker-compose example](https://github.com/vectorize-io/entelechy/tree/main/docker/docker-compose/nginx) for a ready-to-use setup.
...
Entelechy 0.4.11 is a drop-in replacement for 0.4.x with no breaking changes.
