...
We're building Entelechy to be the best AI memory system in the world. "Best" doesn't mean winning every benchmark. It means building something that genuinely works — and being honest about where it does and doesn't.
...
When we say we want Entelechy to be the best AI memory system, we're not talking about a leaderboard position. We're talking about a complete system that performs across dimensions that actually matter in production:
...
- The evaluation harness ([github.com/garybense/agent-memory-benchmark](https://github.com/garybense/agent-memory-benchmark))
...
The evaluation harness is not coupled to Entelechy's internals. The methodology doesn't assume any specific retrieval strategy. Anyone can plug in a different memory backend, run the same harness, and get a comparable result.
...
## Entelechy on the new baseline

To establish a reference point for AMB, we re-ran Entelechy against the datasets using the same harness. The last published results came from [our paper](https://arxiv.org/abs/2512.12818), which used version 0.1.0. We've shipped dozens of features and improvements since then. Here's where v0.4.19 lands in single-query mode:
...
These results will serve as the reference point for AMB going forward. Every future Entelechy release will be measured against them.

<ImageCarousel images={[
  { src: '/img/blog/amb/results-locomo.png', alt: 'Entelechy results on LoComo', caption: 'LoComo — 92.0%' },
  { src: '/img/blog/amb/results-longmemeval.png', alt: 'Entelechy results on LongMemEval', caption: 'LongMemEval — 94.6%' },
  { src: '/img/blog/amb/results-lifebench.png', alt: 'Entelechy results on LifeBench', caption: 'LifeBench — 71.5%' },
  { src: '/img/blog/amb/results-personamem.png', alt: 'Entelechy results on PersonaMem', caption: 'PersonaMem — 86.6%' },
]} />
...
The repo is at **[github.com/garybense/agent-memory-benchmark](https://github.com/garybense/agent-memory-benchmark)** — follow the instructions there to run the benchmarks against your own system and upload your results to the leaderboard.
