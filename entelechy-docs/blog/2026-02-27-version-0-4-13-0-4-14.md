---
title: "What's new in Entelechy 0.4.13 and 0.4.14"
description: New features and improvements in Entelechy 0.4.13 and 0.4.14
authors: [nicoloboschi]
date: 2026-02-27T12:00
hide_table_of_contents: true
---

Entelechy 0.4.13 and 0.4.14 add Chat SDK support, CrewAI integration, configurable MCP tools, graph retrieval tag filtering, and a new reranker provider—plus a set of reliability and performance improvements across the board.
...
We launched a public [Model Leaderboard](https://benchmarks.mindmods.org/) that ranks every supported LLM on both the `retain` and `reflect` operations—measuring fact extraction quality, speed, and cost side by side. Whether you're optimizing for accuracy or trying to cut inference costs, the leaderboard gives you real data to pick the right model for your workload.

![Entelechy Model Leaderboard](/img/blog/release-0414/leaderboard.png)

## Chat SDK Integration

Entelechy now integrates with the [AI SDK](https://sdk.vercel.ai/) `useChat` hook, making it straightforward to give any chatbot persistent memory across sessions.

```typescript
import { useChat } from "ai/react";
import { createEntelechyMiddleware } from "entelechy-sdk/chat";

const { messages, input, handleSubmit } = useChat({
  middleware: createEntelechyMiddleware({
    bankId: "user-123",
...
CrewAI agents can now use Entelechy as a persistent memory backend. The integration plugs into CrewAI's memory interface and routes storage and retrieval through a Entelechy bank.

```python
from crewai import Agent, Crew
from entelechy_integrations.crewai import EntelechyMemory
...
This gives CrewAI agents access to the full Entelechy retrieval stack—semantic search, BM25, graph traversal, and reranking—rather than the default in-process storage.
...
```bash
export ENTELECHY_API_MCP_ENABLED_TOOLS="recall,retain"
```
...
export ENTELECHY_API_ZEROENTROPY_API_KEY=your-key
```
...
**0.4.14**
- **Bank configuration API enabled by default**: The per-bank config API is now on by default; set `ENTELECHY_API_ENABLE_BANK_CONFIG_API=false` to disable it.
...
Entelechy 0.4.13 and 0.4.14 are drop-in replacements for 0.4.x with no breaking changes.
