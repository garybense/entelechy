...
- AI agents forget everything between sessions. Entelechy gives them persistent, structured memory via MCP.
...
Entelechy is an open-source memory system designed for exactly this. It connects to any MCP-compatible agent and gives it persistent, structured long-term memory.
...
```
Your MCP Client  ──MCP (HTTP)──>  Entelechy API
(Claude, Cursor,                       │
...
Your agent connects to Entelechy over MCP (Model Context Protocol). MCP is an open standard — any client that speaks it can use Entelechy as a memory backend.

When your agent stores a memory via `retain`, Entelechy doesn't just dump raw text into a vector database. It extracts structured facts, resolves entities ("Alice" and "my coworker Alice" are the same person), generates embeddings, and indexes everything for retrieval.

When your agent needs context via `recall`, Entelechy runs four retrieval strategies in parallel — semantic search, BM25 keyword matching, entity graph traversal, and temporal filtering — then reranks results with a cross-encoder. What comes back is the most relevant subset of your memories, not a raw dump.

This matters because naive RAG (embed text, cosine similarity, return top-k) breaks down when you have hundreds of memories spanning different topics and time periods. Entelechy's multi-strategy approach ensures that a question like "what did we decide about caching?" finds the right answer even when the memory uses different terminology.
...
The quickest way to run Entelechy is with Docker. One command gives you the full stack — API server, embedded PostgreSQL, local embedding models, and MCP endpoints:
...
```bash
docker run --rm -it --pull always -p 8888:8888 -p 9999:9999 \
  -e ENTELECHY_API_LLM_PROVIDER=gemini \
...
Entelechy works with any MCP-compatible client. Add the following JSON to your client's config file:
...
```bash
claude mcp add --transport http entelechy http://localhost:8888/mcp/your_bank_id/
```
...
It should list Entelechy's memory tools, including the three core operations (`retain`, `recall`, `reflect`), six mental model tools, and additional tools for browsing memories, managing documents, and bank administration.
...
Behind the scenes, Entelechy extracts structured facts, resolves entities, and indexes the memory for later retrieval. A single `retain` call on "Alice from engineering recommended we switch to Postgres 16 for the new JSONB features" produces:
...
### LLM key is for Entelechy, not your agent

The `ENTELECHY_API_LLM_API_KEY` is used by Entelechy internally for fact extraction, entity resolution, and reflect operations. It's separate from whatever LLM your agent uses. You can use a cheap, fast model here (Gemini Flash, Groq, etc.) — it doesn't need to be the same model powering your agent.

---

## When Entelechy Works Well
...
Entelechy gives any MCP-compatible agent persistent long-term memory. One Docker command to start, a few lines of JSON to connect.

The key insight is that memory isn't just storage and retrieval. Entelechy extracts structured facts from raw input, links entities, tracks temporal data, and uses cross-encoder reranking to surface the most relevant memories. That's what separates it from stuffing conversation logs into a vector database.
