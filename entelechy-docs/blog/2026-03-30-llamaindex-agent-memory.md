...
description: LlamaIndex agents reset memory every session. Learn how to add persistent cross-session memory using entelechy-llamaindex in 3 steps. Full code examples included.
authors: [DK09876]
...
`entelechy-llamaindex` provides this via two patterns. The **tools pattern** (`EntelechyToolSpec`) exposes three native LlamaIndex tools: `retain_memory` (store facts), `recall_memory` (search memory), and `reflect_on_memory` (synthesize a reasoned answer from everything stored). The **memory pattern** (`EntelechyMemory`) implements LlamaIndex's `BaseMemory` interface to automatically store messages and recall relevant context on every turn -- no explicit tool calls needed.
...
That's what Entelechy does. And `entelechy-llamaindex` wires it into LlamaIndex's standard tool system without replacing anything you already have.

## How LlamaIndex Persistent Memory Works

```
LlamaIndex Agent (ReAct, FunctionCalling, etc.)
  ├─ EntelechyToolSpec (extends BaseToolSpec) — agent-driven
  │    ├─ retain_memory()     → Entelechy retain
  │    ├─ recall_memory()     → Entelechy recall
  │    └─ reflect_on_memory() → Entelechy reflect
  │
  └─ EntelechyMemory (extends BaseMemory) — automatic
       ├─ put()  → auto-retains user/assistant messages
       └─ get()  → auto-recalls relevant memories as context
```

`EntelechyToolSpec` extends LlamaIndex's [`BaseToolSpec`](https://docs.llamaindex.ai/en/stable/api_reference/tools/). Call `to_tool_list()` and you get standard `FunctionTool` instances. `EntelechyMemory` extends `BaseMemory` for transparent, automatic memory. No monkey-patching, no custom wrappers. Any LlamaIndex agent that accepts tools or a memory parameter can use it.
...
Pulls in `llama-index-core` and `entelechy-client`.
...
Three tools, one bank. Memory persists across agent instances because it's stored in Entelechy, not in the agent object.
...
```python
from entelechy_llamaindex import create_entelechy_tools
...
The factory wraps `EntelechyToolSpec` and returns a filtered list. Use `include_retain`, `include_recall`, and `include_reflect` to control which tools are exposed.

## Automatic Memory (BaseMemory)

If you want memory to happen transparently -- no tool calls, no agent prompting -- use `EntelechyMemory`. It implements LlamaIndex's `BaseMemory` interface: messages are retained on `put()`, relevant memories are recalled on `get()`.
...
You can also combine both patterns: use `EntelechyMemory` for automatic context enrichment, and expose only `reflect_on_memory` as an explicit tool for when the agent needs to synthesize a reasoned answer.
...
```python
from entelechy_llamaindex import create_entelechy_tools, EntelechyMemory

# Automatic memory for context enrichment
memory = EntelechyMemory.from_client(client=client, bank_id="user-123")
...
**Customer support agent.** A support bot serves a user who contacts it five times over two months. Without persistent memory, each interaction starts with identity verification and problem re-explanation. With Entelechy, the agent recalls the user's account tier, their previous issue history, and the resolution steps already tried. Resolution time drops because the agent skips the orientation phase.
...
**Bank auto-creation with mission.** Pass `mission=` to `EntelechyToolSpec` or `EntelechyMemory` and the bank is created automatically on first use. If you prefer explicit control, call `client.create_bank(bank_id, name=...)` before the agent starts.

**Async processing delay.** After `retain_memory`, Entelechy processes content asynchronously. Extracting facts, entities, and embeddings takes 1-3 seconds. If you retain and immediately recall in the same session, the new memories may not be searchable yet.
...
**LlamaIndex agents are async.** `EntelechyToolSpec` provides both sync and async tool methods. Async agents like [`ReActAgent`](https://docs.llamaindex.ai/en/stable/api_reference/agent/react/) use the async variants (`aretain`, `arecall`, `areflect`) directly.

## When NOT to Use This

Persistent memory isn't always the right tool.

- **In-session context only.** If your agent only needs to remember things within a single conversation, LlamaIndex's `ChatMemoryBuffer` is simpler and has zero latency overhead. Don't add Entelechy just because you can.
- **Document search (RAG).** If you need vector search over a document corpus, use LlamaIndex's built-in `VectorStoreIndex`. Entelechy is a memory system for facts learned over time, not a document store.
...
| | `ChatMemoryBuffer` | LlamaIndex `Memory` (SQLite) | Raw vector store | Entelechy |
|---|---|---|---|---|
...
Entelechy processes retained content asynchronously. After calling `retain_memory`, wait 1-3 seconds before calling `recall_memory`. In production, architect your workflow so retain and recall happen in separate sessions rather than back-to-back in the same one.

**What happens if I call `recall_memory` before the bank exists?**
The call will fail. The easiest fix is to pass `mission=` when creating `EntelechyToolSpec` or `EntelechyMemory` -- the bank is auto-created on first use. Alternatively, call `client.create_bank(bank_id)` before initializing the spec.
...
**Can I use Entelechy with a non-ReAct LlamaIndex agent?**
Yes. `EntelechyToolSpec` returns standard `FunctionTool` instances via `to_tool_list()`. Any LlamaIndex agent that accepts a tool list supports it, including `FunctionCallingAgent` and custom agent implementations that use LlamaIndex's tool abstractions.

## Recap

Session memory resets. Long-term memory doesn't have to.

`entelechy-llamaindex` fits into any LlamaIndex agent in three steps: install the package, create a bank, pass the tools. From that point, every fact your agent learns is stored, indexed, and retrievable in the current session and in every session that follows. The agent that served a user in January knows what it learned in January when that user returns in March.
