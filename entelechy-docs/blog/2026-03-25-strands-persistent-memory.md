...
AWS Strands agents are stateless by default — every session starts cold. Here's how to add persistent long-term memory using `entelechy-strands`, so your agent remembers what matters across runs.
...
- `entelechy-strands` adds three native memory tools to any Strands agent: retain, recall, and reflect
...
- **`entelechy_retain`** — stores a piece of information to the memory bank
- **`entelechy_recall`** — searches memory semantically and returns relevant facts
- **`entelechy_reflect`** — synthesizes a reasoned answer from everything in memory
...
You'll also need a running Entelechy instance. Two options:
...
`bank_id` is how Entelechy partitions memory. Use something meaningful: a user ID, a session namespace, a project slug. The bank is created automatically on first write.
...
The agent will call `entelechy_retain` with the relevant facts. Next session:

```python
agent("What model and region should I target for this deployment?")
```

It will call `entelechy_recall`, retrieve the stored preferences, and answer correctly — even in a fresh Python process.
...
```python
from strands import Agent
from entelechy_strands import create_entelechy_tools, memory_instructions
...
memories = memory_instructions(
    bank_id=bank_id,
    entelechy_api_url=api_url,
    query="user preferences and environment setup",
)

agent = Agent(
    tools=create_entelechy_tools(bank_id=bank_id, entelechy_api_url=api_url),
...
If you're instantiating agents across multiple modules, threading the `entelechy_api_url` and other params everywhere gets tedious. Use `configure()` once at startup:

```python
from entelechy_strands import configure, create_entelechy_tools
...
Strands runs its own event loop. The `entelechy-strands` package handles this by executing Entelechy client calls in a `ThreadPoolExecutor` rather than `asyncio.run()`. If you're building on top of this or writing custom tooling, don't try to `await` Entelechy calls directly inside a Strands tool — you'll get "event loop already running" errors.
...
`entelechy_reflect` synthesizes an answer from memory rather than just retrieving facts. It's more useful for open-ended queries but will use more tokens. If cost is a concern, leave it disabled and use recall instead.
...
Strands manages within-session context automatically via its message history. `entelechy-strands` is for cross-session memory — facts that need to survive beyond a single conversation. Don't use it as a substitute for prompt history.
...
- **LangChain memory primitives**: If you're already on LangChain, its memory abstractions are more mature. `entelechy-strands` exists because Strands has its own tool protocol that LangChain memory doesn't plug into.

---

## Recap

Strands agents are stateless by default. `entelechy-strands` adds three native memory tools — retain, recall, reflect — that let your agent manage its own long-term memory using the same `@tool` protocol it uses for everything else. `memory_instructions()` handles the case where you want context pre-loaded rather than retrieved on demand.
