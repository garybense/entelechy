---
title: "What's new in Entelechy 0.5.5"
description: New features and improvements in Entelechy 0.5.5
...
:::warning
**0.5.5 contains a regression that can cause 0 facts to be extracted during retain.** The JSON schema simplification change was reverted in [0.5.6](https://github.com/vectorize-io/entelechy/releases/tag/v0.5.6). Please upgrade to 0.5.6.
:::

Entelechy 0.5.5 ships a brand‑new **LlamaParse file parser** for PDFs/DOCX/etc., a redesigned **Mental Models page** in the control plane, a new **Pipecat voice‑agent integration**, full **Windows support** for the embedded runtime, and a wave of LLM‑provider compatibility fixes (DeepSeek, Bedrock Converse, Ollama, LiteLLM streaming). It also includes one **small breaking change** worth a 30‑second read before you upgrade.
...
**Enabling it.** Set the parser via configuration (env var `ENTELECHY_API_FILE_PARSER` plus your LlamaParse API key):

```bash
ENTELECHY_API_FILE_PARSER=llama_parse
ENTELECHY_API_FILE_PARSER_LLAMA_PARSE_API_KEY=llx-...
```
...
A new [Pipecat](https://www.pipecat.ai/) integration brings Entelechy memory into the voice‑AI pipeline: store every turn, recall relevant context, and inject it into the LLM prompt. Available as `entelechy-pipecat` on PyPI. Thanks to [@benfrank241](https://github.com/benfrank241) for the contribution.

## Windows support for embedded mode

`entelechy-embed` (and the embedded `entelechy-all`) now run cleanly on Windows. The daemon manager handles Windows process groups, atomic metadata writes, and CI installs the Rust CLI alongside the Python wheel for sibling‑binary verification. If you've been waiting on this to evaluate Entelechy on a Windows dev box, give it a try.

## LLM compatibility fixes

A handful of fixes make Entelechy play nicer with a wider range of LLM providers:
