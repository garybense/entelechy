...
That's what Entelechy adds. Entelechy is a memory layer for LLM applications that automatically extracts facts from conversations, builds entity graphs, and retrieves relevant context using four parallel recall strategies. The `entelechy-langgraph` package brings that to LangGraph — and since the memory tools are standard LangChain `@tool` functions, they work with plain LangChain too.
...
- **`entelechy_retain`** — stores the conversation and extracts facts from it
- **`entelechy_recall`** — searches the memory bank for relevant context
- **`entelechy_reflect`** — synthesizes across multiple memories to produce a summary or answer a question about what the agent knows (useful for questions like "what has this user told me about their stack?")
...
The recall node runs before the LLM, searches Entelechy for memories relevant to the user's message, and injects them as a `SystemMessage`. The retain node runs after, storing the conversation. Both resolve per-user bank IDs from `RunnableConfig` at runtime.
...
Replace LangGraph's `InMemoryStore` with Entelechy as the storage backend. If your team already uses LangGraph's store patterns, this is the lowest-friction path.
...
Namespace tuples map to Entelechy bank IDs (`("user", "123")` → bank `user.123`), banks are auto-created, and `search()` uses Entelechy's full semantic recall instead of basic vector similarity.
...
Here's a full support agent that remembers each user across sessions using the nodes pattern. This is copy-pasteable and runnable against either self-hosted Entelechy or Entelechy Cloud.
...
What Entelechy extracts from Session 1 and stores in `user-42`'s memory bank:
...
In all three cases, the agent gets meaningfully better the longer it runs — not just because of a longer context window, but because Entelechy distills conversations into structured knowledge it can retrieve precisely when relevant.

Full docs: [LangGraph integration](/sdks/integrations/langgraph) | [GitHub](https://github.com/vectorize-io/entelechy/tree/main/entelechy-integrations/langgraph)
