---
title: "Run Entelechy with Ollama: Local AI Memory, No API Keys Needed"
authors: [entelechy]
date: 2026-03-10T12:00
tags: [ollama, tutorial, python, memory, local, privacy, entelechy, llm, open-source]
image: /img/blog/run-entelechy-with-ollama.png
hide_table_of_contents: true
---

Running Entelechy with Ollama gives you a fully local AI memory system. No API keys, no cloud costs, no data leaving your machine. If you want persistent agent memory powered by open-source models on your own hardware, this tutorial walks through the complete setup.

<!-- truncate -->

## TL;DR

- Run Entelechy with Ollama entirely on your machine, with no API keys, no cloud, and no costs
- Ollama provides the LLM. Entelechy provides the memory engine. Two env vars connect them.
...
You want persistent memory for your AI agent. Entelechy handles that: fact extraction, knowledge graphs, semantic search, synthesis.
...
Running Entelechy with Ollama connects a production-grade memory engine to a local LLM. This tutorial shows you how.

---

## How Entelechy with Ollama Works
...
Same Entelechy memory engine. Same API. Just swap the LLM provider from `openai` to `ollama`. Everything else, including the knowledge graph, semantic search, and fact extraction, stays the same.

---

## Setting Up Entelechy with Ollama Step by Step

The full setup takes about ten minutes. You will install Ollama, pull a model, install Entelechy, and connect them with two environment variables.
...
### Step 3: Start Entelechy with Ollama

Two environment variables:

```bash
export ENTELECHY_API_LLM_PROVIDER=ollama
export ENTELECHY_API_LLM_MODEL=gpt-oss:20b
export ENTELECHY_API_LLM_MAX_CONCURRENT=1
export ENTELECHY_API_ENABLE_OBSERVATIONS=false
```
...
That's it. Entelechy detects Ollama at `localhost:11434` and uses `gpt-oss:20b` for fact extraction.
...
entelechy = Entelechy(base_url="http://localhost:8888")

entelechy.create_bank(
    bank_id="local-agent",
...
Ollama extracts facts from this locally. No data leaves your machine. Entelechy parses the conversation, identifies entities like "Alice" and "Portland," extracts relationships like "Alice is a software engineer," and stores them in the knowledge graph. All of this happens through Ollama running on your local hardware.
...
Recall uses local embeddings by default, so no LLM call is needed for basic semantic search. This makes recall fast even when running Entelechy with Ollama on modest hardware. If you want to connect this to an [MCP-compatible AI client](/blog/2026/03/04/mcp-agent-memory), the recall results work the same way.

### Step 6: Reflect (With a Caveat)

```python
reflection = entelechy.reflect(
    bank_id="local-agent",
...
## Full Entelechy with Ollama Example

Save as `local_memory.py` to test the complete Entelechy with Ollama workflow:
...
## Entelechy with Ollama: Pitfalls and Edge Cases
...
**2. Concurrent requests will queue.** Set `ENTELECHY_API_LLM_MAX_CONCURRENT=1`. Local models can't parallelize like cloud APIs. Multiple concurrent retain calls will queue and process sequentially.
...
**4. RAM matters.** `gpt-oss:20b` needs ~16GB of RAM. Add Entelechy's embedded Postgres and local embeddings, and you want 24GB+ total. For the full `gpt-oss:120b`, you need ~80GB (dedicated GPU). Use a smaller model on constrained hardware.
...
You can also mix both approaches: develop locally with Entelechy and Ollama, then deploy to production with OpenAI or Anthropic. Same Entelechy API, same code, just change the env vars. Many teams use this pattern to keep development costs at zero while getting the best quality in production.

---

## Override Entelechy Ollama Defaults

Entelechy picks sensible defaults when running with Ollama, but you can override everything:

```bash
export ENTELECHY_API_LLM_PROVIDER=ollama
export ENTELECHY_API_LLM_MODEL=gpt-oss:120b       # upgrade from 20b
export ENTELECHY_API_LLM_BASE_URL=http://192.168.1.50:11434/v1  # remote Ollama
export ENTELECHY_API_LLM_MAX_CONCURRENT=2         # if your GPU can handle it
export ENTELECHY_API_ENABLE_OBSERVATIONS=true      # re-enable once tuned
```

This also means you can run Ollama on a different machine (a GPU server, for example) and point Entelechy at it over the network. This is a common pattern for teams that want local privacy but better performance: run Ollama on a dedicated GPU box in your network, and run Entelechy on your application server.

---

## Recap: Running Entelechy with Ollama

Running Entelechy with Ollama takes two environment variables and about ten minutes of setup. Once connected, you get the same memory engine, the same knowledge graph, and the same retain/recall/reflect API that the cloud version provides, all running on your own hardware.

The key points to remember:

- `ENTELECHY_API_LLM_PROVIDER=ollama` is the only switch you need
...
For teams that need data privacy, air-gapped deployments, or zero-cost development environments, Entelechy with Ollama is the fastest way to get persistent agent memory running locally.
...
- **Try MCP integration** -- connect Entelechy to [Claude, Cursor, or VS Code via MCP](/blog/2026/03/04/mcp-agent-memory)
...
- **Go to production with cloud** -- switch to `ENTELECHY_API_LLM_PROVIDER=openai` when ready
