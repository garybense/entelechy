---
title: "What's new in Entelechy 0.4.19"
description: New features and improvements in Entelechy 0.4.19
...
Entelechy 0.4.19 adds Agno and Hermes Agent integrations, three new retain extraction modes with named per-call strategies, Deno support for the TypeScript client, and a recovery mechanism for consolidation failures.
...
`entelechy-agno` is a new integration package that adds persistent memory to [Agno](https://github.com/agno-agi/agno) agents using Entelechy's native Toolkit pattern—the same pattern as Agno's built-in `Mem0Tools`.

```bash
pip install entelechy-agno
```

Pass `EntelechyTools` directly to your agent's `tools` list:
...
```python
tools=[EntelechyTools(bank_resolver=lambda ctx: f"team-{ctx.user_id}")]
```
...
agent = Agent(
    model=OpenAIChat(id="gpt-4o-mini"),
    tools=[EntelechyTools(bank_id="user-123", entelechy_api_url="http://localhost:8888")],
...
`entelechy-hermes` is a new plugin package for [Hermes Agent](https://github.com/NousResearch/hermes-agent) (NousResearch). It uses Hermes's plugin discovery system—no code changes required—and registers three tools under a `[entelechy]` toolset.
...
Hermes has its own built-in `memory` tool that writes to local files. Since the LLM will prefer the one it's most familiar with, disable it so Hermes uses Entelechy instead:

```bash
hermes tools disable memory
```

If neither `ENTELECHY_API_URL` nor `ENTELECHY_API_KEY` is set, the plugin silently skips registration and Hermes starts normally.
...
```bash
ENTELECHY_API_RETAIN_EXTRACTION_MODE=verbatim
```
...
```bash
ENTELECHY_API_RETAIN_EXTRACTION_MODE=chunks
```
...
The TypeScript client (`entelechy-client`) now works in Deno environments. The build was migrated from `tsc` to `tsup`, producing dual CJS + ESM output with a proper `exports` field. The AI SDK and chat integrations were updated in the same pass.
...
**Improvements**
- Local reranker performance: FP16 inference (`ENTELECHY_API_RERANKER_LOCAL_FP16=true`) and length-sorted bucket batching (`ENTELECHY_API_RERANKER_LOCAL_BUCKET_BATCHING=true`) are now available as opt-in flags. Both default to off to preserve existing behavior. FP16 benefits GPU/MPS hardware; bucket batching reduces padding overhead and produces a 36–54% speedup in benchmarks. Also fixes XLM-RoBERTa model loading under Transformers 5.x.
...
- Fixed Docker control-plane startup to respect `ENTELECHY_CP_HOSTNAME` when constructing the API URL.
...
Entelechy 0.4.19 is a drop-in replacement for 0.4.x with no breaking changes.
