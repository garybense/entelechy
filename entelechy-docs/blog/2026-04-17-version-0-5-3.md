...
The new `entelechy-openai-agents` package provides `FunctionTool` instances for retain, recall, and reflect that plug directly into the OpenAI Agents SDK `Agent`. Configure once with `configure()`, create tools anywhere, and the SDK's async runtime handles the rest. Selective tool inclusion, tag-based scoping, and global configuration are all supported out of the box.

```python
from entelechy_openai_agents import configure, create_tools
...
The CLI now supports named connection profiles via `-p`/`--profile`. Define profiles in `~/.entelechy/profiles.toml` with different base URLs, API keys, and default banks, then switch between local, staging, and production with a single flag. No more juggling environment variables.
...
- **TEI reranker timeout.** The TEI reranker timeout is now configurable (`ENTELECHY_API_RERANKER_TEI_HTTP_TIMEOUT`), with better error messages when it fires.
...
Entelechy 0.5.3 is a drop-in replacement for 0.5.2 with no breaking changes to the core API.
