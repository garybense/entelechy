---
title: "Your Vercel Chat SDK bot forgets everything. Entelechy fixes that."
...
![`@vectorize-io/entelechy-chat` adds persistent memory with one wrapper function](/img/blog/vercel-chat.png)

## TL;DR

- Chat bots built with Vercel's Chat SDK forget everything between messages
- `@vectorize-io/entelechy-chat` adds persistent memory with one wrapper function
...
[`@vectorize-io/entelechy-chat`](https://entelechy.vectorize.io/sdks/integrations/chat) adds a `withEntelechyChat()` wrapper to any Chat SDK handler. It automatically recalls relevant memories before your handler runs and retains conversations after.

```
Slack message ─────┐
                    ├─→ withEntelechyChat() ─→ recall ─→ your handler ─→ retain
Discord message ───┘                            │
                                                ▼
                                         Entelechy API
                                         (shared memory)
...
That's the whole handler. The wrapper takes care of recalling memories, formatting them for the LLM, and retaining conversations. You just write the logic. See the [full API reference](https://entelechy.vectorize.io/sdks/integrations/chat) for all configuration options.
...
Entelechy isn't a simple vector store. When the bot retains a conversation, Entelechy's pipeline:
...
The Vercel Chat SDK supports Slack, Discord, Microsoft Teams, Google Chat, GitHub, and Linear. The `withEntelechyChat()` wrapper works with all of them -- same code, same memory, any combination of platforms.

---

## Try It

The full working example is in the [Entelechy Cookbook](https://github.com/vectorize-io/entelechy-cookbook/tree/main/applications/chat-sdk-multi-platform). It includes:

- Complete bot with Slack + Discord adapters
- LLM-powered responses with Vercel AI SDK
- Shared Entelechy memory bank
- Discord Gateway route setup
- Environment configuration and setup instructions

Start Entelechy locally or use [Entelechy Cloud](https://ui.entelechy.vectorize.io/signup), clone the cookbook, and you'll have a working cross-platform bot with memory in minutes.

---

## Next Steps

- **Read the docs**: [Chat SDK integration guide](https://entelechy.vectorize.io/sdks/integrations/chat) covers the full API, configuration options, and advanced patterns
- **Clone the cookbook**: [chat-sdk-multi-platform](https://github.com/vectorize-io/entelechy-cookbook/tree/main/applications/chat-sdk-multi-platform) has the full working example
- **Try Entelechy Cloud**: Skip self-hosting with a [free account](https://ui.entelechy.vectorize.io/signup)
