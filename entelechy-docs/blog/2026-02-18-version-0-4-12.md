---
title: "What's new in Entelechy 0.4.12"
description: New features and improvements in Entelechy 0.4.12
authors: [nicoloboschi]
date: 2026-02-18T12:00
hide_table_of_contents: true
---

Entelechy 0.4.12 expands what you can ingest, cuts ingestion costs, and broadens where you can run it.
...
Entelechy can now retain PDFs, images, and common Office documents (DOCX, PPTX, XLSX) directly—without you needing to extract text first.

```python
from entelechy_sdk import EntelechyClient
...
```bash
export ENTELECHY_API_FILE_PARSER=iris  # or markitdown (default)
```
...
This requires async retain (`async=true` in your request). Entelechy submits fact extraction calls as a batch job to the provider, polls for completion, and processes results automatically. Because retain runs in the background anyway, the delayed processing window is typically invisible to end users.
...
Entelechy now has a Go client, generated from the OpenAPI 3.1 spec using [OpenAPI Generator](https://github.com/OpenAPITools/openapi-generator).
...
// Recall memories
recallReq := entelechy.RecallRequest{Query: "recent deployments"}
...
```bash
export ENTELECHY_API_VECTOR_INDEX_BACKEND=diskann        # pgvectorscale
export ENTELECHY_API_VECTOR_INDEX_BACKEND=azure_diskann  # Azure pg_diskann
```
...
Entelechy 0.4.12 is a drop-in replacement for 0.4.x with no breaking changes.
