...
Adding **Pipecat persistent memory** doesn't require building a custom RAG system or managing separate vector databases. With the `entelechy-pipecat` integration, you can wire long-term memory into any voice pipeline with a single `FrameProcessor` that recalls context before responding and retains conversation content after each turn.
...
- `entelechy-pipecat` adds automatic recall (before LLM) and retain (after each turn) without code changes to the pipeline.
- One-line setup: add `EntelechyMemoryService` between the user aggregator and LLM service.
- On each turn, Entelechy recalls relevant past conversations and injects them as system messages.
...
Entelechy is a memory engine that runs locally or in the cloud. It doesn't just store raw conversation text. Instead, it extracts structured entities and relationships, builds a knowledge graph, and indexes everything for multi-strategy retrieval. Semantic search, BM25 keyword matching, graph traversal, and temporal ranking all work together to surface the most relevant memories.

The `entelechy-pipecat` integration connects this engine to your Pipecat pipeline through a `FrameProcessor` that sits between your user context aggregator and LLM service.
...
EntelechyMemoryService ← Memory goes here
...
2. **Recall.** The EntelechyMemoryService extracts the latest user message and searches Entelechy for relevant past conversations. Results are injected as a system message before the LLM sees the context.
3. **Retain.** Any new complete user+assistant exchange is sent to Entelechy asynchronously (non-blocking), so the pipeline stays responsive.
...
The setup process is minimal. Install the integration, choose your Entelechy deployment, and add one line to your pipeline.
...
### Step 2: Choose Your Entelechy Deployment

Pick either **Entelechy Cloud** (recommended, no self-hosting) or **Local** (run your own daemon).
...
Create a Entelechy memory service and add it to your pipeline:
...
The `EntelechyMemoryService` accepts several parameters to control memory behavior:
...
If you have multiple pipelines sharing the same Entelechy instance, use global configuration:
...
# Setup Entelechy memory with customer ID as bank_id
...
On the first call, the agent knows nothing about the customer. By the third or fourth call, Entelechy has extracted:
...
support_memory = EntelechyMemoryService(bank_id=f"support-{customer_id}")

# Research agent per project
research_memory = EntelechyMemoryService(bank_id=f"research-{project_id}")

# Agent-specific context
agent_memory = EntelechyMemoryService(bank_id=f"agent-{agent_role}")
```
...
Or switch to a local test Entelechy instance that's separate from production.
...
If you need to delete all memories for a user (GDPR, user request), use the Entelechy Cloud dashboard or API to clear the bank:
...
The EntelechyMemoryService processes frames asynchronously. If you need synchronous behavior (rare), you must wait for the retain operation. In practice, async is better—it keeps your pipeline responsive.
...
|---|---|---|---|
| **Entelechy + Pipecat** | Multi-strategy retrieval (semantic + BM25 + graph + temporal), structured entity extraction, low-latency recall | Requires Entelechy server or cloud account | Voice agents with multi-session context needs |
...
For most voice pipelines that need persistent memory across sessions with minimal changes, the Entelechy integration is the fastest path to production.

---

## Recap: Pipecat Persistent Memory Integration

Adding persistent memory to Pipecat voice pipelines doesn't require rebuilding your architecture. The `entelechy-pipecat` integration provides a single `FrameProcessor` that:
...
- **Inspect memories**: Use the Entelechy Cloud dashboard to browse extracted facts and entities from conversations
