---
title: "Your Agent Memory Setup Keeps Drifting. Entelechy 0.5.0 Adds a Templates Hub."
authors: [benfrank241]
date: 2026-04-10T09:00
tags: [entelechy, templates, memory, release, agents]
description: "Entelechy 0.5.0 adds a Bank Templates Hub. Browse starter templates, import a manifest to configure any bank, and export setups to reuse across environments."
image: /img/blog/templates-hub-entelechy-050.png
hide_table_of_contents: true
---

![Your Agent Memory Setup Keeps Drifting. Entelechy 0.5.0 Adds a Templates Hub.](/img/blog/templates-hub-entelechy-050.png)

Every team that uses long-term memory eventually runs into the same problem: the first bank gets tuned carefully, then every new bank becomes a manual recreation job. [Entelechy 0.5.0](/blog/2026/04/07/version-0-5-0) adds a fix for that: the *Bank Templates Hub*.
...
Entelechy 0.5.0 turns bank setup into a reusable artifact.
...
```bash
curl -X POST "$ENTELECHY_URL/v1/default/banks/project-memory/import" \
...
Entelechy also supports dry-run validation for imports:
...
If you casually rename those values between template versions, Entelechy will treat them as new entries instead of updates. For teams that plan to evolve templates over time, identifier discipline matters.
...
The Bank Templates Hub in Entelechy 0.5.0 solves a real operational problem: memory behavior is too important to rebuild manually every time.
