---
title: "Adding Persistent Memory to OpenAI Codex with Entelechy"
authors: [benfrank241]
date: 2026-04-08T09:00
tags: [codex, openai, memory, persistent-memory, entelechy, tutorial, coding-agents, python]
image: /img/blog/adding-memory-to-codex-with-entelechy.png
description: "Give OpenAI Codex persistent memory across sessions with Entelechy. Auto-recall injects context before every prompt. Auto-retain extracts facts when sessions end."
hide_table_of_contents: true
---

![Adding Persistent Memory to OpenAI Codex with Entelechy](/img/blog/adding-memory-to-codex-with-entelechy.png)
...
**Minimal dependencies.** The hook scripts use Python stdlib only — no pip install, no virtualenv, no version conflicts. Local daemon mode requires [`uvx`](https://docs.astral.sh/uv/) to run `entelechy-embed`; Cloud mode has no local prerequisites at all.

```
┌──────────────┐     UserPromptSubmit      ┌─────────────────────────┐
│    Codex     │ ─────────────────────────▶ │  recall.py              │
│    CLI       │ ◀───────────────────────── │  queries Entelechy,     │
│              │   additionalContext inject  │  injects as context     │
│              │                            └─────────────────────────┘
│              │     Stop                  ┌─────────────────────────┐
│              │ ─────────────────────────▶ │  retain.py              │
└──────────────┘                            │  strips tags, sends     │
                                            │  transcript to Entelechy│
                                            └─────────────────────────┘
```
...
If you choose Entelechy Cloud during setup, there's nothing else to install — no local daemon, no `uvx` required.
...
For most users, Entelechy Cloud is the easier option: no daemon to manage, memory syncs across machines, and setup is two lines of JSON. The default setup runs a local `entelechy-embed` daemon, but switching to Cloud means you never have to think about it again. Edit `~/.entelechy/codex.json`:
...
The recall hook fires on every `UserPromptSubmit` event. It takes your prompt (and optionally the previous turn for context), queries Entelechy, and injects the most relevant memories as a block at the top of the conversation:
...
With Entelechy, the same session opens with recalled facts already in context: which payment provider you're using, that you decided against webhooks in favor of polling, and that the staging environment has a known issue with idempotency keys. Codex starts from where you left off, not from zero.
...
Settings live in `~/.entelechy/codex.json`. Every setting can also be set via environment variable.
...
| `debug` | `false` | Logs all Entelechy activity to stderr |
...
**Nothing happening?** Check `~/.entelechy/codex/error.log` first — hook failures are written there. For a full trace of what each hook is doing, enable debug logging:

**Debug mode.** Add `"debug": true` to `~/.entelechy/codex.json` to log detailed activity to `~/.entelechy/codex/debug.log`:
...
**Recall adds latency.** Every prompt triggers a Entelechy query before Codex sees it. In practice this is 100–300ms with Entelechy Cloud on a fast connection. For interactive sessions it's imperceptible; for automated scripts it may matter. Use `"recallBudget": "low"` or `"autoRecall": false` if you need to skip it.
...
**Extraction quality depends on conversation quality.** Entelechy extracts facts from what's actually in the transcript. If you work through a problem entirely in file edits without narrating what you're doing, there may be little for the extraction model to work with. Brief explanations in your prompts help.

## Recap

| | Codex default | With Entelechy |
|---|---|---|
...
| Team shared memory | No | Shared bank via Entelechy Cloud |
