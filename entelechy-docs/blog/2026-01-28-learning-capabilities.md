...
Today we're releasing Entelechy 0.4.0, which introduces two powerful learning capabilities for AI agents: **Observations** for automatic knowledge consolidation, and **Mental Models** for user-curated summaries.

<!-- truncate -->

## Two Levels of Learning

Entelechy 0.4.0 introduces a hierarchical learning system:
...
In Entelechy 0.3.0, we had two separate systems for synthesized knowledge:
...
After every `retain()` call, Entelechy's consolidation engine runs automatically:
...
Mental models are **saved reflect responses** that you curate for your memory bank. When you create a mental model, Entelechy runs a reflect operation with your source query and stores the result. During future reflect calls, these pre-computed summaries are checked first.
