...
- [**Daemon Lifecycle Package**](#daemon-lifecycle-package): `@garybense/entelechy-all` for programmatic daemon start/stop.
...
`@garybense/entelechy-all` is a new npm package that manages the lifecycle of the all-in-one daemon — start, stop, health checks — so Node-based applications can boot and shut down Entelechy locally without shelling out. Useful for local development, embedded deployments, and test harnesses that need a fresh Entelechy instance per run.
