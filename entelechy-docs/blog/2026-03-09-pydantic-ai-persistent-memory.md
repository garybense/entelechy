...
- `entelechy-pydantic-ai` adds retain, recall, and reflect tools plus auto-injected memory instructions.
- Five lines of setup: create a client, call `create_entelechy_tools()`, and pass them to your Agent.
...
## How Pydantic AI Persistent Memory Works with Entelechy

Before diving into code, it helps to understand what happens under the hood when you add Pydantic AI persistent memory through the Entelechy integration.

Entelechy is a memory engine that runs locally or in the cloud. When your agent stores a fact, Entelechy does not just save raw text. Instead, it extracts structured entities and relationships, builds a knowledge graph, and indexes everything for multi-strategy retrieval. That means when your agent searches memory later, it can find relevant information through semantic search, BM25 keyword matching, graph traversal, and temporal ranking, all combined.

The `entelechy-pydantic-ai` package connects this engine to Pydantic AI through two integration points:

```
Pydantic AI Agent
  |-- tools=[create_entelechy_tools(...)]
  |     |-- entelechy_retain  -> store facts to memory
  |     |-- entelechy_recall  -> search memory for relevant info
  |     |-- entelechy_reflect -> synthesize an answer from all memories
  |
...
The tools are async functions that call Entelechy's API directly. Since Pydantic AI is async-native, the closures use `aretain()`, `arecall()`, and `areflect()`. There are no thread-pool hacks or compatibility layers needed. If you prefer a protocol-based approach, you can also use [Entelechy's MCP memory server](/blog/2026/03/04/mcp-agent-memory) instead of direct tool integration.

---

## Setting Up Pydantic AI Persistent Memory in 5 Steps

The setup process takes about five minutes. You will install Entelechy, add the Pydantic AI memory integration package, wire the tools into your agent, and test that memories persist across sessions.

### Step 1: Install and Start Entelechy

First, install the Entelechy server and start it locally:
...
```bash
pip install entelechy-pydantic-ai
```
...
Other providers work the same way. Swap in `[anthropic]` or `[google]` as needed. The Pydantic AI persistent memory integration is model-agnostic, so the memory layer works identically regardless of which LLM you choose. Entelechy supports [100+ LLM providers through LiteLLM](/blog/2026/03/03/litellm).

### Step 3: Add Persistent Memory Tools to Your Pydantic AI Agent

Here is where the five lines come in. Create a Entelechy client, generate the Pydantic AI persistent memory tools, and pass them to your agent:

```python
from entelechy_client import Entelechy
from entelechy_pydantic_ai import create_entelechy_tools
...
- `entelechy_retain(content)` stores information to long-term memory
- `entelechy_recall(query)` searches memory and returns matching facts
- `entelechy_reflect(query)` synthesizes a reasoned answer from all relevant memories
...
In the first run, the agent stores the preferences via `entelechy_retain`. In the second run, the agent calls `entelechy_recall` to find relevant context. It then gives advice grounded in what it knows: functional patterns, Python, data pipelines.

This is the core benefit of Pydantic AI persistent memory: it works across runs, across days, and across process restarts. The memories live in Entelechy's knowledge graph, not in the agent's context window. Even if you restart your Python process completely, the agent picks up right where it left off.
...
Now on every run, `memory_instructions` calls Entelechy's recall API and injects relevant memories into the system prompt. The agent starts every conversation with context about the user, without needing a tool call.
...
You do not always need all three persistent memory tools. `create_entelechy_tools` lets you pick which ones to include:
...
If you have multiple Pydantic AI agents sharing the same Entelechy persistent memory instance, use the global config instead of passing `client` everywhere:

```python
from entelechy_pydantic_ai import configure, create_entelechy_tools

configure(entelechy_api_url="http://localhost:8888", api_key="YOUR_KEY")

# No client needed: tools use the global config
agent1_tools = create_entelechy_tools(bank_id="agent-1")
agent2_tools = create_entelechy_tools(bank_id="agent-2")
```
...
async def main():
    ...
    agent = Agent(
        "openai:gpt-4o-mini",
        tools=create_entelechy_tools(client=client, bank_id=BANK_ID),
...
Run it again after the first execution finishes. The agent remembers everything from the first session because Pydantic AI persistent memory stores facts in Entelechy, not in the process.
...
**Duplicate memories.** If the agent stores the same information multiple times, Entelechy deduplicates at the fact level. However, it is still better to give the agent clear guidance in the system prompt about when to store new facts versus when to skip.

**Async event loop conflicts.** The sync `create_bank()` method does not work inside `asyncio.run()` because it tries to create a nested event loop. Always use `await client.acreate_bank()` in async code. This is a common Python async pitfall, not specific to Entelechy or Pydantic AI persistent memory.
...
Pydantic AI persistent memory with Entelechy works best for agents that interact with the same user or context across multiple sessions. Good use cases include personal assistants, support bots, research agents that accumulate knowledge, and any Python agent where remembering past interactions improves quality over time.

### When Not to Use It

Skip agent persistent memory for one-shot agents that never run again, stateless API handlers where each request is independent, or agents where you want full control over what goes into the prompt. In the last case, use the Entelechy Python client directly instead of the Pydantic AI integration.

### How It Compares to Alternatives

| Approach | Strengths | Weaknesses | Best For |
|---|---|---|---|
| **Entelechy + Pydantic AI** | Multi-strategy retrieval (semantic + BM25 + graph + temporal), structured fact extraction, synthesis engine | Requires running Entelechy server or using cloud | Multi-session agents needing deep memory |
...
The right choice depends on your requirements. For most Pydantic AI agents that need persistent memory across sessions with minimal setup, the Entelechy integration offers the fastest path to production.

---

## Recap: Pydantic AI Persistent Memory Integration

Adding Pydantic AI persistent memory does not require complex infrastructure or custom retrieval pipelines. The `entelechy-pydantic-ai` integration provides two functions that cover the full agent memory lifecycle:

- **`create_entelechy_tools()`** returns async tools that the agent calls to store and retrieve knowledge across sessions
