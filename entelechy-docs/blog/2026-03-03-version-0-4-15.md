---
title: "What's new in Entelechy 0.4.15"
description: New features and improvements in Entelechy 0.4.15
...
Entelechy 0.4.15 adds PydanticAI integration, observation scopes, richer entity labels, and several performance improvements—alongside a set of reliability and stability fixes.
...
Entelechy now integrates with [PydanticAI](https://ai.pydantic.dev/), letting you attach persistent memory to any PydanticAI agent with a few lines of code.
...
`create_entelechy_tools` registers three async tools on the agent:

- **`entelechy_retain`** — stores information to long-term memory
- **`entelechy_recall`** — searches memory for relevant facts
- **`entelechy_reflect`** — synthesizes a reasoned answer from stored memories
...
Both accept the full set of Entelechy retrieval parameters—`budget`, `max_tokens`, `tags`, `recall_tags`, and `recall_tags_match`—for fine-grained control over what gets stored and retrieved.
...
Observations are Entelechy's higher-level summaries derived from raw facts. Previously, observations were always consolidated across all tags at once. Now you can control the _granularity_ of each consolidation pass with `observation_scopes`.
...
When retaining reference material—documentation, books, or any content without a meaningful event date—you can now pass `timestamp="unset"` to tell Entelechy there is no real date to associate with the content.
...
Entelechy 0.4.15 is a drop-in replacement for 0.4.x with no breaking changes.
