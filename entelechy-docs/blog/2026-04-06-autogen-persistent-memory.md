---
title: "Persistent Memory for AutoGen Agents with Entelechy"
authors: [DK09876]
date: 2026-04-06
tags: [autogen, integrations, agents, memory, python, microsoft]
description: "AutoGen agents lose all state when a session ends. entelechy-autogen adds three tools — retain, recall, reflect — that give your agents persistent memory across sessions."
image: /img/blog/autogen-persistent-memory.png
hide_table_of_contents: true
---

![Persistent Memory for AutoGen Agents with Entelechy](/img/blog/autogen-persistent-memory.png)

AutoGen is Microsoft's open-source framework for building multi-agent systems: conversable agents, group chats, tool use, code execution. But when a session ends, every agent in the conversation forgets everything. `entelechy-autogen` fixes that by giving AutoGen agents persistent memory through three callable tools.
...
That's what Entelechy does. And `entelechy-autogen` wires it into AutoGen's tool system.
...
`entelechy-autogen` pulls in `autogen-core` and `entelechy-client`. You also need `autogen-agentchat` for `AssistantAgent` and `autogen-ext[openai]` for the model client.
...
async def main():
    ...
    # Wait for Entelechy to finish processing (fact extraction is async)
...
Three tools, one bank. Memory persists across conversations because it's stored in Entelechy, not in the agent.
...
- **In-session context only** — If your agent only needs to remember things within a single conversation, AutoGen's built-in chat history is simpler and has zero latency overhead. Don't add Entelechy just because you can.
- **Document search (RAG)** — If you need vector search over a document corpus, use a dedicated vector store. Entelechy is a memory system for facts learned over time, not a document store.
...
- `entelechy-autogen` gives AutoGen agents persistent memory via `FunctionTool` instances passed to `AssistantAgent(tools=[...])`
