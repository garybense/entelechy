...
CrewAI agents lose all memory when a crew finishes. `entelechy-crewai` plugs into CrewAI's ExternalMemory to persist knowledge across runs -- three lines of setup, and your agents automatically store task outputs and recall relevant context.
...
That's what the `entelechy-crewai` package does. It implements CrewAI's `Storage` interface using Entelechy's memory engine, so your crews remember everything -- across runs, across days, across weeks.
...
CrewAI Crew
  └─ ExternalMemory
       └─ EntelechyStorage (implements Storage interface)
            ├─ save()   → Entelechy retain (extract facts, entities, relationships)
            ├─ search() → Entelechy recall (semantic + graph + temporal retrieval)
            └─ reset()  → Entelechy delete_bank + recreate
```

CrewAI calls `save()` after each task completes and `search()` before each task starts. You don't manage the lifecycle -- CrewAI drives it, Entelechy stores it.

Under the hood, Entelechy does more than store text. It extracts structured facts, identifies entities, builds a knowledge graph, and runs multi-strategy retrieval (semantic search, BM25, graph traversal, temporal ranking) with cross-encoder reranking.
...
> **Note:** You can also use [Entelechy Cloud](https://ui.entelechy.vectorize.io/signup) and skip the self-hosted setup entirely.
...
This pulls in `entelechy-client` and `crewai` as dependencies.
...
# Point at your Entelechy instance
...
That's it. After `kickoff()`, every task output is retained in Entelechy. Next time you run this crew, it recalls relevant prior work before starting each task.
...
CrewAI's Storage interface has `save` and `search`. But Entelechy also supports `reflect` -- a synthesis operation that reasons across all relevant memories instead of returning raw facts.
...
researcher = Agent(
    ...
    backstory="You are a senior researcher. Use the entelechy_reflect tool to review what you already know before starting new research.",
    tools=[reflect_tool],
    llm="openai/gpt-4o-mini",
)
```

When the agent calls `entelechy_reflect`, it gets a synthesized, reasoned response that draws on the full knowledge graph -- not just the top-k vector matches.
...
**2. Large task outputs.** CrewAI passes the full task output to `save()`. If your tasks produce very long outputs, Entelechy handles the chunking, but retain latency increases. Set a reasonable `expected_output` length in your task definitions.
...
**4. Async event loop conflicts.** CrewAI runs inside an async event loop. The integration handles this transparently via a dedicated thread pool, but if you're also doing async work in custom tools, avoid calling `entelechy-client` directly from the same event loop. Use the `EntelechyStorage` and `EntelechyReflectTool` abstractions instead.

---

## Recap

- `entelechy-crewai` gives CrewAI agents persistent, compounding memory
...
- `EntelechyReflectTool` adds on-demand synthesis for deeper reasoning
...
- **Try it locally**: `pip install entelechy-all entelechy-crewai` and run the example above
...
- **Inspect memories in the web UI**: Run `entelechy-control-plane` locally or use the cloud dashboard to browse facts, entities, and mental models
