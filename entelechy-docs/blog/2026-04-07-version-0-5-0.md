...
Entelechy now ships with a built-in llama.cpp LLM provider, enabling fully local inference without any external API calls. Set the provider to `llama-cpp` and point it at a GGUF model file:

```bash
ENTELECHY_API_LLM_PROVIDER=llama-cpp
ENTELECHY_API_LLM_MODEL=/path/to/model.gguf
```

This is ideal for air-gapped environments, development setups, or anywhere you want to avoid external API costs. The provider uses the `llama-cpp-python` bindings and supports all standard Entelechy LLM operations (fact extraction, consolidation, reflect).

### OpenRouter

Entelechy now supports [OpenRouter](https://openrouter.ai/) as a provider for LLM, embeddings, and reranking. This gives you access to hundreds of models through a single API key:

```bash
ENTELECHY_API_LLM_PROVIDER=openrouter
ENTELECHY_API_LLM_API_KEY=sk-or-...
ENTELECHY_API_LLM_MODEL=anthropic/claude-sonnet-4-20250514
```

OpenRouter is particularly useful for comparing models or accessing providers that don't have a direct Entelechy integration yet.
...
`entelechy-autogen` provides persistent long-term memory for [AutoGen](https://github.com/microsoft/autogen) agents via three `FunctionTool` wrappers: `entelechy_retain`, `entelechy_recall`, and `entelechy_reflect`.

```bash
pip install entelechy-autogen
```
...
- Windows compatibility fix for entelechy-embed.
- Addressed critical and high severity security vulnerabilities in dependencies.

## Feedback and Community

**Note:** Entelechy 0.5.0 contains breaking changes (BFS/MPFP removal, Hermes integration dropped). If you were using the default graph retrieval strategy, no action is needed. If you were using `entelechy-hermes`, switch to the native Hermes memory provider.
