---
title: "What's new in Entelechy 0.4.17"
description: New features and improvements in Entelechy 0.4.17
...
Entelechy 0.4.17 adds history tracking with diff views for mental models and observations, flexible per-request file parsing, manual retry for failed async operations, and tag management for existing documents—plus several reliability fixes.
...
Mental models and observations now track their full change history. Every time Entelechy consolidates new memories and updates a mental model or observation, the previous version is preserved and the diff is recorded.
...
You can also configure a **fallback chain**: if the primary parser fails, Entelechy will try each subsequent parser in order before surfacing an error. This makes ingestion pipelines more resilient when parser availability varies or documents are unpredictable.
...
Entelechy 0.4.17 is a drop-in replacement for 0.4.x with no breaking changes.
