...
```bash
# Store a single fact
entelechy retain my-bank "Alice joined Google in March 2024 as a Senior ML Engineer"

# Store from a file
entelechy retain my-bank --file conversation.txt --context "Daily standup"

# Store multiple files
entelechy retain my-bank --files docs/*.md
```
...
```bash
# Basic search
entelechy recall my-bank "What does Alice do at Google?"

# Search with options
entelechy recall my-bank "What happened last spring?" \
    --budget high \
    --max-tokens 8192 \
    --fact-type world

# Verbose output (shows weights and sources)
entelechy recall my-bank "Tell me about Alice" -v
```
...
```bash
# Basic reflect
entelechy reflect my-bank "Should we adopt TypeScript for our backend?"

# Verbose output (shows sources and opinions)
entelechy reflect my-bank "What are Alice's strengths for the team lead role?" -v

# With higher reasoning budget
entelechy reflect my-bank "Analyze our tech stack" --budget high
```
