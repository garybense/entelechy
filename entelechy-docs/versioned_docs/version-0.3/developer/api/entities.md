...
Entities are the people, organizations, places, and concepts that Entelechy automatically extracts and tracks across your memory bank.

:::info Automatic Feature
You don't need to do anything to use entities—Entelechy extracts them automatically when you call `retain`. However, understanding how entities work is important because they power key features in [recall](./recall) and [reflect](./reflect).
:::
...
1. **Co-occurrence tracking** — When entities appear together in facts, Entelechy builds a graph of relationships. This enables graph-based recall to find indirect connections.

2. **Observations** — Entelechy synthesizes high-level summaries about each entity from multiple facts. Including entity observations in recall provides richer context.
...
When the same entity is mentioned multiple times (possibly with different names), Entelechy resolves them to a single canonical entity using a scoring algorithm:
...
A match requires a combined score above **0.6** (60%). Below this threshold, Entelechy creates a new entity rather than risk merging distinct entities.
...
Observations are **derived state**—high-level summaries that Entelechy automatically synthesizes from the facts associated with an entity. They provide a condensed view of what the system knows about important entities.
...
1. **Top entities selected** — Entelechy identifies the top 5 most-mentioned entities in the batch
