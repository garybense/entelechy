...
Official Python client for the Entelechy API.
...
The `entelechy-all` package includes embedded PostgreSQL, HTTP API server, and client:
...
If you already have a Entelechy server running:
...
`EntelechyEmbedded` provides the simplest way to use Entelechy in Python. It automatically manages a background server for you - no manual setup required:
...
A profile is an isolated Entelechy environment. Each profile gets its own PostgreSQL database (stored in `~/.pg0/instances/entelechy-embed-{profile}/`) and its own API server. Use different profiles to separate environments (dev/prod), applications, or users.

**When to Use EntelechyEmbedded**

Use `EntelechyEmbedded` when you want the server to start automatically and manage itself. Use `EntelechyServer` when you need explicit control over server lifecycle (e.g., testing where you want immediate startup/shutdown).

**Advanced Operations**

`EntelechyEmbedded` provides organized API namespaces for advanced operations. Each method call automatically ensures the daemon is running:
...
```python
from entelechy import EntelechyClient

client = EntelechyClient(
    base_url="http://localhost:8888",  # Entelechy API URL
...
# Organized API access (same as EntelechyEmbedded)
...
Both `EntelechyClient` and `EntelechyEmbedded` provide the same organized API namespaces (`banks`, `mental_models`, `directives`, `memories`) for consistent developer experience.
...
with Entelechy(base_url="http://localhost:8888") as client:
