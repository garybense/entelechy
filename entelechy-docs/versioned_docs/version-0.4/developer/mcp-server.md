...
Entelechy includes a built-in [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that allows AI assistants to store and retrieve memories directly.
...
```bash
export ENTELECHY_API_MCP_ENABLED=false
```
...
export ENTELECHY_API_TENANT_API_KEY=your-secret-key
```
...
3. **Default**: Uses `ENTELECHY_MCP_BANK_ID` env var (default: "default")

## Per-Bank Endpoints

Unlike traditional MCP servers where tools require explicit identifiers, Entelechy uses **per-bank endpoints**. The `bank_id` is part of the URL path, so tools don't need to specify which bank to use—it's implicit from the connection.
