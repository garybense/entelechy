...
# Retain: How Entelechy Stores Memories

When you call `retain()`, Entelechy transforms conversations and documents into structured, searchable memories that preserve meaning and context.
...
Entelechy doesn't just store what was said — it captures **why**, **how**, and **what it means**.

### What Gets Captured

When you retain "Alice joined Google last spring and was thrilled about the research opportunities", Entelechy extracts:
...
Entelechy preserves the full narrative:
...
Entelechy distinguishes between **world** facts (about others) and **experience** (conversations and events):
...
Entelechy automatically identifies and tracks **entities** — the people, organizations, and concepts that matter.
...
If "Alice" appears with "Google" and "Stanford" multiple times, a new "Alice" mentioning those is likely the same person. Entelechy uses co-occurrence patterns to disambiguate common names.
...
Memories aren't isolated — Entelechy creates a **knowledge graph** with four types of connections:
...
Entelechy tracks **two temporal dimensions**:

### When It Happened

For events (meetings, trips, milestones), Entelechy records when they occurred.
...
Entelechy also tracks when you told it each fact.
...
Set `retain_mission` and `retain_extraction_mode` via the [bank config API](/developer/api/memory-banks#retain-configuration) or the [`ENTELECHY_API_RETAIN_MISSION`](/developer/configuration#retain) environment variable.

---

## Observation Consolidation

After `retain()` completes, Entelechy automatically triggers **observation consolidation** in the background. This process:
