...
Set `observations_mission` via the [bank config API](/developer/api/memory-banks#observations-configuration) or the [`ENTELECHY_API_OBSERVATIONS_MISSION`](/developer/configuration#observations) environment variable.
