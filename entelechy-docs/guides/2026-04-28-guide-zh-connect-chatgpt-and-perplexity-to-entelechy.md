---
title: "将 ChatGPT 和 Perplexity 连接到 Entelechy 内存"
authors: [benfrank241]
date: 2026-04-28T14:00:00Z
tags: [integrations, chatgpt, perplexity, guide]
description: "使用 MCP、OAuth 和自定义指令将 ChatGPT 和 Perplexity 连接到 Entelechy，使两个工具都能保留上下文而不是从零开始。"
...
- 两个集成都使用 Entelechy MCP 端点和基于浏览器的 OAuth，因此无需在模型界面中粘贴 API 密钥。
...
Perplexity 在将当前网络搜索与您在早期研究会话中已经学到的内容相结合时最强大。这是 Entelechy 添加的部分。
...
- 一个工具有效，另一个无效。这通常意味着您配置了不同的 URL 或在浏览器中批准了不同的 Entelechy 帐户。
...
不。文档化的集成路径使用针对 Entelechy MCP 服务器的基于浏览器的 OAuth。
