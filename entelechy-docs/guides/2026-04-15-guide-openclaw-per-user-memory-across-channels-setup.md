...
description: "Set up OpenClaw per-user memory across channels with Entelechy, choose the right bank granularity, and keep one user's context consistent everywhere."
...
If you want **OpenClaw per-user memory across channels**, the key is changing how the Entelechy plugin derives bank IDs. By default, `entelechy-openclaw` isolates memory by `agent`, `channel`, and `user`, which is a safe default but also means the same person can feel like a stranger in every new DM, thread, or room. That is great for strict isolation. It is not great if you want a user's preferences and ongoing context to follow them from one conversation to another.
...
> **Quick answer**
>
> 1. Install and configure `entelechy-openclaw` normally.
...
Out of the box, the OpenClaw Entelechy plugin derives bank IDs from:
...
plugin = entries.setdefault('entelechy-openclaw', {'enabled': True, 'config': {}})
plugin['enabled'] = True
...
path = pathlib.Path.home() / '.openclaw' / 'openclaw.json'
config = json.loads(path.read_text())
cfg = config['plugins']['entries']['entelechy-openclaw']['config']
cfg['retainMission'] = (
...
If you need a lower-level check, use the Entelechy log pattern from the main OpenClaw guide:
