---
title: "在 Linux ARM64 上运行 Entelechy CLI 而无需解决方法"
authors: [benfrank241]
date: 2026-04-28T14:00:00Z
tags: [cli, linux, arm64, guide]
description: "使用新的发布资产在 Linux ARM64 上运行 Entelechy CLI，然后在 Pi 或 ARM 主机上配置配置文件、API 访问和日常内存命令。"
...
- 最快的路径是下载 `entelechy-linux-arm64`、标记为可执行并将其移到您的 PATH 上。
...
如果您更喜欢在主目录中保留本地工具，请将二进制移到 `~/.local/bin` 中，并确保该目录在您的 PATH 上。关键检查很简单：`entelechy --help` 应该打印命令树而不是架构或权限错误。
...
环境变量仍然优先于配置文件，因此 CI 作业可以导出 `ENTELECHY_API_URL` 和 `ENTELECHY_API_KEY` 而不会与本地默认值冲突。
...
- **`Exec format error`** 通常意味着您下载了错误的资产。双重检查文件名是 `entelechy-linux-arm64`，而不是 AMD64 构建。
- **`Permission denied`** 意味着二进制文件缺少执行位。重新运行 `chmod +x entelechy`。
...
### 我可以针对 Entelechy Cloud 和本地服务器使用 CLI 吗？
