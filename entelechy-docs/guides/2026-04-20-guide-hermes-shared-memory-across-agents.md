...
description: "Set up Hermes shared memory across agents with Entelechy, reuse one bank on purpose, and test that multiple agents can build on the same context safely."
image: /img/blog/guide-hermes-shared-memory-across-agents.png
hide_table_of_contents: true
---

If you want **Hermes shared memory across agents**, the setup is much simpler than most people expect. Multiple agents do not need a special federation layer. They just need to point at the same Entelechy backend and the same `bank_id`. Once that happens, the agents can retrieve and retain against the same memory space.
...
- Every agent already uses the native Entelechy provider.
...
In the Hermes Entelechy config, set the shared bank explicitly:
...
If Agent A remembers its own facts and Agent B does not, the problem is not Entelechy itself. It is that the two agents are not on the same bank.

## Troubleshooting / common errors

### The second agent does not recall anything

Usually the bank IDs do not match, or one agent is still pointing at a different Entelechy backend.
