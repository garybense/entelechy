---
title: "Hermes Memory Modes with Entelechy, Hybrid, Context, Tools"
authors: [benfrank241]
date: 2026-04-14
tags: [how-to, hermes, memory, configuration]
description: "Choose the right Hermes memory mode for Entelechy. Learn when to use hybrid, context, or tools, how to switch modes, and how to verify recall."
image: /img/blog/guide-hermes-memory-modes-with-entelechy-hybrid-context-tools.png
hide_table_of_contents: true
---

![Hermes Memory Modes with Entelechy, Hybrid, Context, Tools](/img/blog/guide-hermes-memory-modes-with-entelechy-hybrid-context-tools.png)

If you are trying to choose between **Hermes memory modes with Entelechy**, the decision is really about one question: should memory be injected automatically, exposed as tools, or both? Hermes's native Entelechy provider supports three integration modes, `hybrid`, `context`, and `tools`, and each one changes how the model experiences memory during a conversation.
...
> **Quick answer**
>
> 1. Use **`hybrid`** if you want automatic recall and explicit Entelechy tools.
...
- Hermes is already configured to use the native Entelechy provider.
- `hermes memory status` reports a healthy memory setup.
- You know where your Hermes Entelechy config file lives.
- You understand that memory mode affects recall behavior, not whether your bank exists.

Hermes stores the native Entelechy config at `~/.hermes/entelechy/config.json` by default. You can print it with:
...
| Mode | Auto-recall before each turn | Explicit Entelechy tools visible | Best for |
|---|---|---|---|
...
- **`hybrid`** is the default general-purpose mode. Relevant memories are injected before the LLM sees the new user message, and the model can still call `entelechy_recall`, `entelechy_retain`, and `entelechy_reflect` explicitly if needed.
...
- you also want power users or the model itself to call `entelechy_reflect` explicitly for deeper synthesis
- you are still evaluating Entelechy and want the easiest mode to debug
...
- you are experimenting with explicit agent strategies that use `entelechy_recall` and `entelechy_reflect` as first-class reasoning tools
...
- In **`hybrid`**, you should see the Entelechy tools.
...
- **`tools`**: the model must choose to call a Entelechy tool, or it may answer without using memory.
...
- a user chooses `context`, then expects Entelechy tools in `/tools`
...
### Problem: `context` mode works, but I cannot find `entelechy_reflect`
...
Make sure you edited the correct file under `~/.hermes/entelechy/config.json`, then restart Hermes so the new config is loaded.
