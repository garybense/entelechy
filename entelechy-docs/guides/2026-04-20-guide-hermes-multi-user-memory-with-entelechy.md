---
title: "Hermes Multi-User Memory Setup with Entelechy"
authors: [benfrank241]
date: 2026-04-20
tags: [how-to, hermes, memory, multi-user, configuration]
description: "Set up Hermes multi-user memory with Entelechy, map each user to the right bank, and keep recall isolated so one user never inherits another user’s context."
image: /img/blog/guide-hermes-multi-user-memory-with-entelechy.png
hide_table_of_contents: true
---

If you want **Hermes multi-user memory with Entelechy**, the design decision that matters most is not `memory_mode`. It is how you derive `bank_id` for each user. A single static bank is fine for a personal assistant. It is risky in a multi-user deployment, because recall gets better only when the bank contains the right person's history.
...
- Hermes is already using the native Entelechy provider.
...
Hermes stores Entelechy provider config in `~/.hermes/entelechy/config.json` by default. In a multi-user app, you can generate a session-specific config that sets `bank_id` for the current user:
