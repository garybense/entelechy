---
title: "Guide: Migrate entelechy-hermes to Native Hermes Memory"
authors: [benfrank241]
date: 2026-04-14
tags: [how-to, hermes, memory, migration]
description: "Learn how to migrate entelechy-hermes to native Hermes memory without losing your bank, then verify recall, tools, and cross-session memory still work."
image: /img/blog/guide-migrate-entelechy-hermes-to-native-hermes-memory.png
hide_table_of_contents: true
---

![Guide: Migrate entelechy-hermes to Native Hermes Memory](/img/blog/guide-migrate-entelechy-hermes-to-native-hermes-memory.png)

If you need to **migrate `entelechy-hermes` to native Hermes memory**, the good news is that the move is smaller than it looks. Hermes now has a built-in Entelechy provider, so you no longer need the old pip plugin path from the original setup guide. In most cases, the migration is just: uninstall the old plugin package, point Hermes at the native provider, keep the same bank ID, and verify that automatic recall works on the next turn.
...
> **Quick answer**
>
> 1. Back up your current Hermes and Entelechy config.
> 2. Uninstall `entelechy-hermes` from the Hermes Python environment.
> 3. Run `hermes memory setup` and select `entelechy`, or update the native config manually.
...
- **Old plugin setup** from the earlier `entelechy-hermes` guide, where the package was installed into the Hermes virtual environment and registered through Python entry points.
- **Entelechy Cloud backend**, where your memories already live in Entelechy Cloud and Hermes reaches them through an API key.
- **Local Entelechy backend**, where your memories live in a local Entelechy service and Hermes points at it by URL.

You should also have:

- Hermes installed and working.
- A current Entelechy configuration that you can inspect.
...
```bash
$HOME/.hermes/hermes-agent/venv/bin/python -m pip show entelechy-hermes
```
...
Migration is usually straightforward, but the cheapest insurance is a copy of the files you are about to change. Back up the Hermes env file and the Entelechy config directory first:

```bash
mkdir -p ~/hermes-memory-migration-backup
cp -R ~/.hermes ~/hermes-memory-migration-backup/hermes
cp -R ~/.entelechy ~/hermes-memory-migration-backup/entelechy 2>/dev/null || true
```
...
You should also inspect your current Entelechy values before you migrate. In practice, the values that matter most are:

- the Entelechy API URL
- the bank ID
...
path = base / "entelechy" / "config.json"
print(path)
if path.exists():
    print(path.read_text())
else:
    print("No native Hermes Entelechy config yet")
PY
```
...
```bash
grep '^ENTELECHY_' ~/.hermes/.env || true
```
...
The old `entelechy-hermes` path is deprecated. Hermes now ships a native provider, and you do not want both approaches competing for responsibility. Uninstall the plugin from the same Python environment Hermes uses:
...
```bash
$HOME/.hermes/hermes-agent/venv/bin/python -m pip uninstall -y entelechy-hermes
```
...
If you still see an entry for `entelechy`, you are probably uninstalling from the wrong environment.

### 3. Configure the native Entelechy provider
...
When prompted, select **Entelechy** as the provider. If you are using Entelechy Cloud, enter the same API URL and API key you used before. If you are using local mode, point Hermes at the same local backend or choose local mode in the wizard so it can create the expected native config.
...
The native Entelechy provider gives you automatic recall through lifecycle hooks and can also expose explicit Entelechy tools. Hermes still has its built-in `memory` tool, which writes local markdown notes. If both are active, the model may keep choosing the built-in tool out of habit.

If you want Entelechy to be the only memory path, disable the built-in tool:

```bash
hermes tools disable memory
```

This is especially helpful after migration because it removes ambiguity. When you test storage and recall, you know the result came from Entelechy, not from the old markdown path.
...
The next-turn detail matters. Entelechy retention runs asynchronously after the response, and the provider's prefetch happens before the following turn. If you try to retain and recall in the exact same turn, you can conclude the migration failed when the system is actually behaving normally.
...
You want confirmation that Hermes sees the Entelechy provider as active. If it does not, fix that before testing anything else.
...
If you are using `hybrid` or `tools`, launch Hermes and inspect the available tools. You should see `entelechy_retain`, `entelechy_recall`, and `entelechy_reflect` in the tool list.
...
Check `memory_mode`. If it is set to `tools`, auto-recall is disabled by design. The model has to call `entelechy_recall` explicitly. Switch to `hybrid` or `context` if you want automatic memory injection before every turn.
...
Fresh local mode startup can take a while because the embedded Entelechy server and PostgreSQL need to initialize. Check the startup log:
...
Usually, no. If the old plugin and the new provider both point at the same Entelechy backend and the same `bank_id`, the memory bank is the same. Migration changes the integration path, not the underlying stored memories.
...
Not necessarily. Many teams disable it so Entelechy is the single source of truth for persistent memory. That makes testing and debugging much clearer.
