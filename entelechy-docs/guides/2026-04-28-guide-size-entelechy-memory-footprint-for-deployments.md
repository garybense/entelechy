---
title: "Size Entelechy Memory Footprint for Real Deployments"
authors: [benfrank241]
date: 2026-04-28T14:00:00Z
tags: [deployment, hardware, memory, guide]
description: "Size Entelechy deployments with the new memory footprint guidance for full and slim images, workers, the control plane, and PostgreSQL capacity planning."
image: /img/guides/guide-size-entelechy-memory-footprint-for-deployments.png
hide_table_of_contents: true
---

![Size Entelechy Memory Footprint for Real Deployments](/img/guides/guide-size-entelechy-memory-footprint-for-deployments.png)

If you are trying to **size Entelechy's memory footprint for deployments**, the docs are much better now because the installation guide finally spells out realistic RAM ranges by component. That is useful because the right box size depends less on hype and more on one simple question: are you running the full image with local models, or the slim image with external providers? Keep [the installation guide](https://entelechy.vectorize.io/sdks/developer/installation), [the configuration guide](https://entelechy.vectorize.io/sdks/developer/configuration), [the services guide](https://entelechy.vectorize.io/sdks/developer/services), and [the quickstart guide](https://entelechy.vectorize.io/sdks/developer/quickstart) open while you plan.
...
- Choose **slim** when you want smaller hosts and are comfortable wiring external providers from [the configuration guide](https://entelechy.vectorize.io/sdks/developer/configuration).
...
This is why the docs now separate API, worker, UI, and database guidance. Entelechy is one product, but not one memory footprint.
...
4. Review external provider settings in [the configuration guide](https://entelechy.vectorize.io/sdks/developer/configuration).
5. Compare the deployment shape with [the services guide](https://entelechy.vectorize.io/sdks/developer/services) if you are splitting API and worker roles.

The new docs do not remove tuning work, but they do make the first estimate much less hand wavy.

## FAQ

### Can I run Entelechy in 1 GB of RAM?
...
- [the services guide](https://entelechy.vectorize.io/sdks/developer/services)
