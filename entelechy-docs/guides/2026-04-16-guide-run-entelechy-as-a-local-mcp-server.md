---
title: "Guide: Run Entelechy as a Local MCP Server"
authors: [benfrank241]
date: 2026-04-16
tags: [how-to, mcp, local, memory]
description: "Run Entelechy as a local MCP server with embedded PostgreSQL so Claude, Cursor, and other MCP clients get persistent memory without extra infra."
image: /img/blog/guide-run-entelechy-as-a-local-mcp-server.png
hide_table_of_contents: true
---

![Guide: Run Entelechy as a Local MCP Server](/img/blog/guide-run-entelechy-as-a-local-mcp-server.png)

If you want to **run Entelechy as a local MCP server**, the good news is that the setup is smaller than it sounds. Entelechy can run locally with an embedded PostgreSQL database, expose an MCP endpoint on your machine, and give tools like Claude Code, Claude Desktop, Cursor, and Windsurf persistent memory without a separate external database.
...
> **Quick answer**
>
> 1. Start `entelechy-local-mcp` with your preferred LLM provider.
...
```bash
ENTELECHY_API_LLM_PROVIDER=ollama \
ENTELECHY_API_LLM_MODEL=llama3.2 \
uvx --from entelechy-api entelechy-local-mcp
```

That starts the full Entelechy API locally and exposes the MCP endpoint at:

```text
http://localhost:8888/mcp/
```

Behind the scenes, Entelechy also starts an embedded PostgreSQL database so you do not need to provision a separate database service just to test or use memory locally.
...
Entelechy supports both patterns locally.
...
No. The local MCP server can run directly through the Entelechy tooling.
