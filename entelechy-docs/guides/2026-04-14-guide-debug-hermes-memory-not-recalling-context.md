...
> **Quick answer**
>
> 1. Run `hermes memory status` and confirm Entelechy is active.
> 2. Check `memory_mode`, because `tools` disables automatic recall by design.
> 3. Confirm the Entelechy backend is healthy and the bank has real memories.
...
Before you debug anything, make sure you are actually using the native Entelechy provider and not an older mixed setup.

You should have:

- Hermes installed and launching normally.
- A configured Entelechy provider.
...
Hermes supports three Entelechy memory modes:

- `hybrid`, automatic recall plus explicit tools
- `context`, automatic recall only
- `tools`, explicit tools only

If your config says `tools`, then auto-recall is not supposed to happen. The model must call `entelechy_recall` or `entelechy_reflect` explicitly. Many people see missing automatic context and assume recall is broken when the system is just following the chosen mode.
...
If Hermes cannot reach Entelechy, recall obviously cannot work.
...
This next-turn requirement matters. Entelechy retains asynchronously after the assistant response, then makes the new memory available for future retrieval. If you retain and immediately try to verify in the exact same turn, you can mistake normal async behavior for a broken system.

If you want a stricter test, ask Hermes to use explicit memory retrieval when tools are available:

```text
Use entelechy_recall and tell me what you know about the billing freeze.
```
...
The Hermes docs note an important caveat: automatic recall and automatic retain rely on lifecycle hooks such as `pre_llm_call` and `post_llm_call`. On older Hermes builds, the Entelechy tools may still register, but auto-injection can be skipped.

This creates a specific failure pattern:

- `entelechy_recall` exists as a tool
- explicit tool calls work
- automatic context injection does not happen

If that sounds like your situation, you are probably dealing with a Hermes version issue, not bad Entelechy configuration.

A quick symptom-based test is this:

- if explicit `entelechy_recall` works, but memory never shows up automatically in `hybrid` or `context`
...
Hermes still has a built-in `memory` tool that writes markdown notes locally. If that tool stays enabled, the model may keep using it instead of Entelechy. The result is confusing because the assistant looks like it is storing memory, but not in the path you are trying to debug.
...
For runtime issues in Entelechy itself, inspect the active profile logs:

```bash
tail -f ~/.entelechy/profiles/*.log
```
...
This should show Entelechy as active.
...
If tools are visible, ask Hermes to call `entelechy_recall` directly. If that works while auto-recall does not, the problem is almost always mode selection or hook availability.
...
### Pattern: explicit `entelechy_recall` works, but normal replies feel stateless
...
- embedded Entelechy and PostgreSQL are still initializing

Check `~/.hermes/logs/entelechy-embed.log` before assuming the setup is dead.
...
Run a controlled retain test, wait for the response to finish, then query the new fact on the next turn. If explicit `entelechy_recall` also finds nothing, the bank probably does not contain the expected data.
