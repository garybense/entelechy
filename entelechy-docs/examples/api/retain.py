#!/usr/bin/env python3
"""
Retain API examples for Entelechy.
...
client = Entelechy(base_url=ENTELECHY_URL)
...
response = requests.get(f"{ENTELECHY_URL}/v1/default/banks/my-bank/tags")
tags = response.json()
for tag in tags["items"]:
    print(f"{tag['tag']}: {tag['count']} memories")

# Search with wildcards (* matches any characters)
response = requests.get(f"{ENTELECHY_URL}/v1/default/banks/my-bank/tags", params={"q": "user:*"})
user_tags = response.json()
response = requests.get(f"{ENTELECHY_URL}/v1/default/banks/my-bank/tags", params={"q": "*-admin"})
