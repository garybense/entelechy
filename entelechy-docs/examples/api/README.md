...
Requires a running Entelechy server at `http://localhost:8888` (or set `ENTELECHY_API_URL`).
...
| Command | Reason |
|---------|--------|
| `entelechy configure` | Requires interactive user input (prompts for API URL, credentials) |
| `entelechy configure --show` | Displays sensitive configuration, not suitable for automated tests |
