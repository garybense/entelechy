...
func main() {
	...
	client.BanksAPI.CreateOrUpdateBank(ctx, mmBankID).
		CreateBankRequest(entelechy.CreateBankRequest{
			Name: *entelechy.NewNullableString(entelechy.PtrString("Mental Models Demo")),
		}).Execute()
	for _, content := range []string{
		...
		client.MemoryAPI.RetainMemories(ctx, mmBankID).
			RetainRequest(entelechy.RetainRequest{
				Items: []entelechy.MemoryItem{{Content: content}},
			}).Execute()
	}
	...
	result, _, _ := client.MentalModelsAPI.CreateMentalModel(ctx, mmBankID).
		CreateMentalModelRequest(entelechy.CreateMentalModelRequest{
	...
	resultWithID, _, _ := client.MentalModelsAPI.CreateMentalModel(ctx, mmBankID).
		CreateMentalModelRequest(entelechy.CreateMentalModelRequest{
			Id:          *entelechy.NewNullableString(&mmID),
	...
	result2, _, _ := client.MentalModelsAPI.CreateMentalModel(ctx, mmBankID).
		CreateMentalModelRequest(entelechy.CreateMentalModelRequest{
			...
			Trigger: &entelechy.MentalModelTriggerInput{
	...
	updated, _, _ := client.MentalModelsAPI.UpdateMentalModel(ctx, mmBankID, mentalModelID).
		UpdateMentalModelRequest(entelechy.UpdateMentalModelRequest{
			Name: *entelechy.NewNullableString(&newName),
			Trigger: *entelechy.NewNullableMentalModelTriggerInput(&entelechy.MentalModelTriggerInput{
...
func cleanupMentalModels(client *entelechy.APIClient, ctx context.Context, apiURL string) {
