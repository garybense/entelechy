...
func main() {
	...
	sfOpts := entelechy.SourceFactsIncludeOptions{MaxTokens: &maxSFTokens}
	obsResponse, _, _ := client.MemoryAPI.RecallMemories(ctx, "my-bank").
		RecallRequest(entelechy.RecallRequest{
			Query: "What patterns have I learned about Alice?",
			Types: []string{"observation"},
			Include: &entelechy.IncludeOptions{
				SourceFacts: *entelechy.NewNullableSourceFactsIncludeOptions(&sfOpts),
			},
		}).Execute()
