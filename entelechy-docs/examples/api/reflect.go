...
func main() {
	...
	client.MemoryAPI.Reflect(ctx, "my-bank").
		ReflectRequest(entelechy.ReflectRequest{
			...
			Context: *entelechy.NewNullableString(&ctxText),
		}).Execute()
	// [/docs:reflect-with-context]
	...
	client.BanksAPI.CreateOrUpdateBank(ctx, "cautious-advisor").
		CreateBankRequest(entelechy.CreateBankRequest{
			Name:                  *entelechy.NewNullableString(entelechy.PtrString("Cautious Advisor")),
			ReflectMission:        *entelechy.NewNullableString(&mission),
			DispositionSkepticism: *entelechy.NewNullableInt32(&skepticism),
			DispositionLiteralism: *entelechy.NewNullableInt32(&literalism),
			DispositionEmpathy:    *entelechy.NewNullableInt32(&empathy),
		}).Execute()
	...
	sourcesResponse, _, _ := client.MemoryAPI.Reflect(ctx, "my-bank").
		ReflectRequest(entelechy.ReflectRequest{
			...
			Include: &entelechy.ReflectIncludeOptions{
