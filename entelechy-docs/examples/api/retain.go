...
func main() {
			...
			Items: []entelechy.MemoryItem{
				{
					...
					Context:    *entelechy.NewNullableString(&context_),
					DocumentId: *entelechy.NewNullableString(&docID),
					Timestamp: *entelechy.NewNullableTimestamp(&entelechy.Timestamp{
						String: &ts,
					}),
				},
			},
			...
			Items: []entelechy.MemoryItem{
				{
					...
					Context: *entelechy.NewNullableString(&ctxLabel),
					Timestamp: *entelechy.NewNullableTimestamp(&entelechy.Timestamp{
						String: &ts2,
					}),
				},
	...
	client.MemoryAPI.RetainMemories(ctx, "my-bank").
		RetainRequest(entelechy.RetainRequest{
			Items: []entelechy.MemoryItem{
				{Content: "Alice works at Google", Context: *entelechy.NewNullableString(&ctx1), DocumentId: *entelechy.NewNullableString(&doc1)},
				{Content: "Bob is a data scientist at Meta", Context: *entelechy.NewNullableString(&ctx1), DocumentId: *entelechy.NewNullableString(&doc2)},
				{Content: "Alice and Bob are friends", Context: *entelechy.NewNullableString(&ctx2), DocumentId: *entelechy.NewNullableString(&doc3)},
			},
		}).Execute()
	// [/docs:retain-batch]
	...
	retainResp, _, _ := client.MemoryAPI.RetainMemories(ctx, "my-bank").
		RetainRequest(entelechy.RetainRequest{
			Items: []entelechy.MemoryItem{
				{Content: "Large batch item 1", DocumentId: *entelechy.NewNullableString(&largeDoc1)},
				{Content: "Large batch item 2", DocumentId: *entelechy.NewNullableString(&largeDoc2)},
			},
			Async: &asyncTrue,
		}).Execute()
	...
	// [docs:retain-files]
	// Open a file and upload it — Entelechy converts it to text and extracts memories.
	// Supports: PDF, DOCX, PPTX, XLSX, images (OCR), audio (transcription), and text formats.
	f, err := os.Open("../../entelechy-docs/examples/api/sample.pdf")
	if err != nil {
