...
run_cli_examples() {
    ...
    # Check if entelechy CLI is available
    if ! command -v entelechy &> /dev/null; then
        echo -e "  ${YELLOW}SKIPPED (entelechy CLI not installed)${NC}"
...
wait_for_server() {
    echo "Waiting for Entelechy server at $ENTELECHY_URL..."
    for i in {1..30}; do
        if curl -s "$ENTELECHY_URL/health" > /dev/null 2>&1; then
