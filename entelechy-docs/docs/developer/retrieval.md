...
# Recall: How Entelechy Retrieves Memories

When you call `recall()`, Entelechy uses multiple search strategies in parallel to find the most relevant memories, regardless of how you phrase your query.
...
No single search method handles all these well. Entelechy solves this with **TEMPR** — four complementary strategies that run in parallel.
...
Entelechy is built for AI agents, not humans. Traditional search systems return "top-k" results, but agents don't think in terms of result counts—they think in tokens. An agent's context window is measured in tokens, and that's exactly how Entelechy measures results.

**How it works:**
- Top-ranked memories selected first
- Stops when token budget is exhausted
- You specify context budget, Entelechy fills it with the most relevant memories
...
Controls how thoroughly Entelechy explores the memory bank—affecting graph traversal depth, candidate pool size, and cross-encoder re-ranking:
...
This section explains exactly how Entelechy turns raw retrieval results into a final ranked list. The pipeline has three stages: **RRF fusion**, **cross-encoder reranking**, and **combined scoring**.
...
**Pre-filtering:** Before reranking, candidates are trimmed to the top **300** (by RRF score) to limit computational cost. This is configurable via `ENTELECHY_API_RERANKER_MAX_CANDIDATES`.
...
|--------|---------------------------|-----------------|
| **low** | 100 | `ENTELECHY_API_RECALL_BUDGET_FIXED_LOW` |
| **mid** | 300 (default) | `ENTELECHY_API_RECALL_BUDGET_FIXED_MID` |
| **high** | 1000 | `ENTELECHY_API_RECALL_BUDGET_FIXED_HIGH` |
...
Reranking pre-filter (300 candidates) is **independent** of budget — it's a separate knob (`ENTELECHY_API_RERANKER_MAX_CANDIDATES`).
...
|--------|-------|-----------------|
| low | 2.5% of max_tokens | `ENTELECHY_API_RECALL_BUDGET_ADAPTIVE_LOW` |
| mid | 7.5% of max_tokens | `ENTELECHY_API_RECALL_BUDGET_ADAPTIVE_MID` |
| high | 25% of max_tokens | `ENTELECHY_API_RECALL_BUDGET_ADAPTIVE_HIGH` |

The result is clamped to a floor of **20** (`ENTELECHY_API_RECALL_BUDGET_MIN`) and a ceiling of **2000** (`ENTELECHY_API_RECALL_BUDGET_MAX`).

Enable with `ENTELECHY_API_RECALL_BUDGET_FUNCTION=adaptive`.
:::
