...
Extensions allow you to customize and extend Entelechy behavior without modifying core code. They enable multi-tenancy, custom authentication, additional HTTP endpoints, and operation hooks.
...
ENTELECHY_API_TENANT_API_KEY=your-secret-key
```
...
```bash
ENTELECHY_API_TENANT_EXTENSION=entelechy_api.extensions.builtin.supabase_tenant:SupabaseTenantExtension
ENTELECHY_API_TENANT_SUPABASE_URL=https://your-project.supabase.co
...
Adds custom HTTP endpoints under the `/ext/` path prefix. Useful for adding domain-specific APIs that integrate with Entelechy's memory engine.
...
```bash
ENTELECHY_API_HTTP_EXTENSION=mypackage.ext:MyHttpExtension
```
...
Registers additional MCP (Model Context Protocol) tools on the Entelechy MCP server. Enables external packages to add custom tools without modifying core code.

**No built-in implementation** - implement your own to add custom MCP tools.

```bash
ENTELECHY_API_MCP_EXTENSION=mypackage.mcp:MyMCPExtension
```
...
```bash
ENTELECHY_API_<TYPE>_EXTENSION=mypackage.module:MyExtensionClass
```

Configuration is passed via prefixed environment variables:

```bash
ENTELECHY_API_<TYPE>_SOME_CONFIG=value
...
```python
import jwt
from entelechy_api.extensions import TenantExtension, TenantContext, AuthenticationError
...
class JwtTenantExtension(TenantExtension):
    def __init__(self, config: dict[str, str]):
        ...
        if not self.jwt_secret:
            raise ValueError("ENTELECHY_API_TENANT_JWT_SECRET is required")
...
from entelechy_api.extensions import HttpExtension
...
```python
from entelechy_api.extensions import (
    OperationValidatorExtension,
    ValidationResult,
    RetainContext,
    RecallContext,
...
from entelechy_api.extensions import MCPExtension
from entelechy_api.engine import MemoryEngine
...
services:
  entelechy-api:
    image: vectorize/entelechy-api:latest
    volumes:
      - ./my_extensions:/app/my_extensions
    environment:
      - ENTELECHY_API_TENANT_EXTENSION=my_extensions.auth:JwtTenantExtension
      - ENTELECHY_API_TENANT_JWT_SECRET=${JWT_SECRET}
      - PYTHONPATH=/app
```
...
```dockerfile
FROM vectorize/entelechy-api:latest
...
Install your extension package in the same Python environment as Entelechy:
...
# Configure
export ENTELECHY_API_TENANT_EXTENSION=my_extensions.auth:JwtTenantExtension
export ENTELECHY_API_TENANT_JWT_SECRET=your-secret
...
Custom extensions that solve common use cases are welcome contributions to the Entelechy project. If you've built an extension for:
