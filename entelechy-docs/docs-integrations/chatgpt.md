---
sidebar_position: 2
title: "ChatGPT Persistent Memory with Entelechy | Integration"
description: "Add long-term memory to ChatGPT with Entelechy. Use OAuth-secured MCP connectors to store conversations, recall relevant context, and build persistent knowledge across sessions."
---

# ChatGPT

Add persistent, searchable memory to [ChatGPT](https://chatgpt.com) using [Entelechy](https://vectorize.io/entelechy). Store insights from your conversations and automatically recall relevant context in future sessions—all secured with OAuth.

## Overview

ChatGPT's built-in memory helps with preferences, but knowledge from specific conversations (research, code, decisions) is lost when you start a new chat. Entelechy solves this by providing:

- **Cross-session memory** — Knowledge from one conversation persists to the next
- **Smart recall** — Entelechy automatically retrieves relevant memories when you need them
...
- **Intelligent recall** — ChatGPT queries Entelechy before generating each response to include relevant memories as context
...
Later, when you ask ChatGPT *"How should we structure our database?"*, Entelechy recalls these constraints. ChatGPT's answer becomes tailored to your actual situation, not generic advice.
...
**Use consistent terminology.** If you call your project "ProjectX" in one session and "Project X" in another, Entelechy's semantic search may miss the connection. Establish naming conventions and stick to them.
...
- In ChatGPT, explicitly ask Entelechy to store something: *"Entelechy, remember that we use Vue.js for frontend"*
- Check that your bank isn't full (unlikely, but possible with very large memory sets)

## Next Steps

1. Create your Entelechy Cloud account
2. Add the Entelechy connector to ChatGPT
3. Set up your custom instructions
4. Store your first memory — ask ChatGPT to remember something important to you
5. Start a new chat and ask a related question — watch Entelechy retrieve your stored memory
