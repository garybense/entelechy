---
sidebar_position: 4
title: "OpenClaw Persistent Memory with Entelechy | Plugin Integration"
description: "Add persistent, automated memory to your OpenClaw agent with Entelechy. Local-first, open source — one plugin install replaces built-in memory with structured knowledge extraction and auto-recall."
---

# OpenClaw

Local, long term memory for [OpenClaw](https://openclaw.ai) agents using [Entelechy](https://vectorize.io/entelechy).

This plugin integrates [entelechy-embed](https://vectorize.io/entelechy/cli), a standalone daemon that bundles Entelechy's memory engine (API + PostgreSQL) into a single command. Everything runs locally on your machine, reuses the LLM you're already paying for, and costs nothing extra.
...
**Feedback Loop Prevention:** The plugin automatically strips injected memory tags (`<entelechy_memories>`) before storing conversations. This prevents recalled memories from being re-extracted as new facts, which would cause exponential memory growth and duplicate entries.
...
- `embedVersion` - entelechy-embed version (default: `"latest"`)
...
> If you used `entelechy-openclaw-setup` in Quick Start, this section is
...
**Override the model (optional — Entelechy picks a sensible default per provider):**

```bash
openclaw config set plugins.entries.entelechy-openclaw.config.llmModel gpt-4o-mini
```
...
```bash
openclaw config set plugins.entries.entelechy-openclaw.config.llmProvider openai
openclaw config set plugins.entries.entelechy-openclaw.config.llmBaseUrl https://openrouter.ai/api/v1
openclaw config set plugins.entries.entelechy-openclaw.config.llmApiKey \
    --ref-source env --ref-provider default --ref-id OPENROUTER_API_KEY
openclaw config set plugins.entries.entelechy-openclaw.config.llmModel xiaomi/mimo-v2-flash
```
...
> `entelechy-openclaw-setup --mode api --api-url <url>` covers this path
> interactively — this section documents the underlying config fields.

Connect to a remote Entelechy API server instead of running a local daemon. This is useful for:
    ...
    "entries": {
      "entelechy-openclaw": {
        ...
        "config": {
          ...
          "entelechyApiToken": "your-api-token"
        }
      }
    }
  }
...
**Options:**
- `entelechyApiUrl` - Full URL to external Entelechy API (e.g., `https://mcp.entelechy.example.com`)
- `entelechyApiToken` - API token for authentication (optional). **Sensitive** — set as a SecretRef:

  ```bash
  openclaw config set plugins.entries.entelechy-openclaw.config.entelechyApiToken \
      --ref-source env --ref-provider default --ref-id ENTELECHY_API_TOKEN
  ```

#### Behavior

When external API mode is enabled:
- **No local daemon** is started (no entelechy-embed process)
...
# [Entelechy] External API mode enabled: https://your-entelechy-server.com
...
```bash
cat ~/.entelechy/profiles/openclaw.env
```
...
```bash
# Search memories
uvx entelechy-embed@latest -p openclaw memory recall openclaw "user preferences"

# View recent memories
uvx entelechy-embed@latest -p openclaw memory list openclaw --limit 10

# Open web UI (uses openclaw profile's daemon)
uvx entelechy-embed@latest -p openclaw ui
```
...
# Option 2 — Anthropic
openclaw config set plugins.entries.entelechy-openclaw.config.llmProvider anthropic
openclaw config set plugins.entries.entelechy-openclaw.config.llmApiKey \
...
openclaw config set plugins.entries.entelechy-openclaw.config.llmProvider openai-codex

# Verify the config is valid
openclaw config validate

# Inspect the current value
openclaw config get plugins.entries.entelechy-openclaw.config.llmProvider
```
...
# Should see on startup:
# [Entelechy] ✓ Using provider: openai, model: gpt-4o-mini
# or
# [Entelechy] ✓ Using provider: claude-code, model: claude-sonnet-4-20250514

# Should see after conversations:
# [Entelechy] Retained X messages for session ...
# [Entelechy] Auto-recall: Injecting X memories
```
