---
sidebar_position: 3
title: "Perplexity Persistent Memory with Entelechy | Integration"
description: "Add long-term memory to Perplexity with Entelechy. Use OAuth-secured MCP connectors to retain research findings, recall relevant context, and build a persistent knowledge base across searches."
---

# Perplexity

Add persistent, searchable memory to [Perplexity](https://www.perplexity.ai) using [Entelechy](https://vectorize.io/entelechy). Store research findings and automatically recall relevant context in future searches—all secured with OAuth.

## Overview

Perplexity excels at research and fact-checking, but you have to re-discover the same facts across sessions. Entelechy solves this by providing:

- **Research knowledge base** — Discoveries, findings, and sources persist across searches
- **Smart recall** — Entelechy automatically retrieves relevant research from your history
...
- **Intelligent recall** — Perplexity queries Entelechy before each search to include relevant research as context
...
- **Semantic search** — Entelechy understands context, not just keyword matching
...
Later, when you research *"What's the best LLM for my use case?"*, Entelechy recalls your preferences and benchmarks. Perplexity's answer becomes tailored to your situation, not generic.
...
**Use consistent terminology.** If you call a topic "machine learning" in one search and "deep learning" in another, Entelechy's semantic search may miss the connection. Establish naming conventions and stick to them.

**Review and refine.** The Entelechy Cloud dashboard lets you browse your memory bank. Periodically review what you've stored. Delete outdated information (e.g., last year's benchmark data) and consolidate similar insights. A curated memory bank is more valuable than an exhaustive one.
...
**Cross-reference with other tools.** If using both ChatGPT and Perplexity on related tasks, store research context in a shared Entelechy bank (multi-bank mode) so both tools access the same knowledge base. This creates unified context across your workflow.
...
| **Entelechy integration** | Retain reasoning, insights, preferences | Retain research findings, sources |
...
**Recommended use:**
- **ChatGPT + Entelechy** — Build projects, learn complex topics, creative work
- **Perplexity + Entelechy** — Research, fact-checking, competitive analysis, news tracking
...
- In Perplexity, explicitly ask Entelechy to store something: *"Entelechy, remember that Q3 2026 benchmarks show Claude Opus is best for reasoning"*
- Check that your bank isn't full (unlikely, but possible with very large memory sets)

## Next Steps

1. Verify you have Perplexity Pro
2. Create your Entelechy Cloud account
3. Add the Entelechy connector to Perplexity
4. Set up your custom instructions
5. Store your first research finding
6. Start a new search on a related topic — watch Entelechy retrieve your stored research
