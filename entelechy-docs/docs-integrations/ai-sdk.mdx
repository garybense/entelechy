---
sidebar_position: 4
title: "Vercel AI SDK Persistent Memory with Entelechy | Integration"
description: "Add long-term memory to any Vercel AI SDK application with five ready-to-use Entelechy tools. Retain conversations, recall context, and reflect on past interactions — works with any model."
---

# Vercel AI SDK

The `@vectorize-io/entelechy-ai-sdk` package integrates [Entelechy](https://entelechy.vectorize.io) memory with the [Vercel AI SDK](https://ai-sdk.dev). It provides five ready-to-use tools for retaining, recalling, and reflecting on long-term memories.
...
```bash
npm install @vectorize-io/entelechy-ai-sdk @vectorize-io/entelechy-client ai
```

## Setup

Create a Entelechy client and pass it to `createEntelechyTools` along with a `bankId`. The `bankId` identifies the memory store for this session—typically a user ID.
