---
sidebar_position: 13
title: "OpenAI Agents SDK Persistent Memory with Entelechy | Integration"
description: "Add long-term memory to OpenAI Agents SDK agents with Entelechy. Provides FunctionTool instances for retain, recall, and reflect that plug directly into the Agents SDK async runtime."
---

# OpenAI Agents SDK

Persistent long-term memory for [OpenAI Agents SDK](https://github.com/openai/openai-agents-python) agents via Entelechy. Provides `FunctionTool` instances that plug directly into the OpenAI Agents SDK `Agent`.
...
`entelechy-openai-agents` pulls in `openai-agents` and `entelechy-client`.
...
async def main():
    ...
    agent = Agent(
        name="assistant",
        instructions="You are a helpful assistant with long-term memory. Use entelechy_retain to store important facts. Use entelechy_recall to search memory before answering.",
        tools=tools,
    )
...
Instead of relying on the agent to call `entelechy_recall` explicitly, you can auto-inject relevant memories into the system prompt on every turn:
...
```python
from entelechy_openai_agents import configure, create_entelechy_tools
...
```python
from entelechy_openai_agents.errors import EntelechyError
...
| Parameter | Default | Description |
|---|---|---|
| `bank_id` | *required* | Entelechy memory bank to recall from |
