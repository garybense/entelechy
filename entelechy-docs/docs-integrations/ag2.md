---
sidebar_position: 8
title: "AG2 (AutoGen) Persistent Memory with Entelechy | Integration Guide"
description: "Add long-term persistent memory to your AG2 (AutoGen) agents with Entelechy. Automatic fact extraction, entity tracking, and recall tools that persist across conversations."
---
...
- **Drop-in Tools** — `register_entelechy_tools()` registers retain, recall, and reflect in one line
...
# The assistant can now use entelechy_retain, entelechy_recall, entelechy_reflect
...
The integration provides three AG2-compatible tool functions backed by Entelechy's API:

| Tool | Entelechy | What happens |
|------|-----------|--------------|
| `entelechy_retain(content)` | `retain(bank_id, content, ...)` | Content is stored. Entelechy extracts facts, entities, and relationships from the raw text. |
| `entelechy_recall(query)` | `recall(bank_id, query, ...)` | Entelechy runs semantic search, BM25, graph traversal, and reranking. Returns a numbered list of matching memories. |
| `entelechy_reflect(query)` | `reflect(bank_id, query, ...)` | Entelechy synthesizes a reasoned answer from all relevant memories, using the bank's disposition traits. |
...
```python
from entelechy_ag2 import configure

configure(
    entelechy_api_url="http://localhost:8888",
    api_key="your-key",       # or set ENTELECHY_API_KEY env var
...
```python
from entelechy_ag2 import create_entelechy_tools
...
for agent in [researcher, writer]:
    register_entelechy_tools(agent, executor, bank_id="team-memory")
...
| `entelechy_api_url` | from config | Entelechy API URL |
