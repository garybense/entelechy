---
sidebar_position: 8
title: "LlamaIndex Persistent Memory with Entelechy | Integration"
description: "Add long-term memory to LlamaIndex agents with Entelechy. Supports agent-driven tools (EntelechyToolSpec) and automatic memory via the BaseMemory interface."
---

# LlamaIndex

Persistent long-term memory for [LlamaIndex](https://docs.llamaindex.ai/) agents via Entelechy. The `entelechy-llamaindex` package provides two complementary patterns:

- **`EntelechyToolSpec`** — Agent-driven memory tools (retain/recall/reflect)
- **`EntelechyMemory`** — Automatic memory via LlamaIndex's `BaseMemory` interface
...
The simplest way to add Entelechy memory to a LlamaIndex agent. Messages are automatically stored on each turn, and relevant memories are recalled and injected as context.
...
| Event | What Happens |
|-------|-------------|
| Agent receives input | `aget(input)` recalls relevant memories from Entelechy, prepends as system message |
| Agent produces output | `aput(message)` retains the message to Entelechy for future recall |
| New session starts | Previous memories are available via recall; local chat buffer starts empty |

### `EntelechyMemory.from_client()`

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `client` | `Entelechy` | *required* | Entelechy client instance |
...
Also available: `EntelechyMemory.from_url(entelechy_api_url, bank_id, ...)` for creating without a pre-built client.
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `bank_id` | `str` | *required* | Entelechy memory bank to operate on |
| `client` | `Entelechy` | `None` | Pre-configured Entelechy client |
| `entelechy_api_url` | `str` | `None` | API URL (used if no client provided) |
