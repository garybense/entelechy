---
sidebar_position: 14
title: "Amazon Bedrock AgentCore Persistent Memory with Entelechy | Integration"
description: "Add long-term memory to Amazon Bedrock AgentCore Runtime agents with Entelechy. Memory persists across sessions using stable user identity — not the ephemeral runtimeSessionId."
---

# Amazon Bedrock AgentCore Runtime

Persistent memory for [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-sessions.html) agents using Entelechy.

AgentCore Runtime sessions are explicitly ephemeral — they terminate on inactivity and reprovision fresh environments. `entelechy-agentcore` adds durable cross-session memory so agents remember users, decisions, and learned patterns across any number of Runtime sessions.
...
AgentCore Runtime invocation
        │
        ▼
   before_turn()         ← Recall relevant memories from Entelechy
        │
        ▼
  Agent executes          ← Prompt enriched with prior context
        │
        ▼
   after_turn()          ← Retain output to Entelechy (async)
```
...
```python
from entelechy_agentcore import RecallPolicy

adapter = EntelechyRuntimeAdapter(
...
```python
from entelechy_agentcore import TurnContext

def my_resolver(context: TurnContext) -> str:
    return f"acme:{context.user_id}:{context.agent_name}"

adapter = EntelechyRuntimeAdapter(bank_resolver=my_resolver)
```
...
|---|---|---|---|
| `entelechy_api_url` | `ENTELECHY_API_URL` | Entelechy Cloud | Entelechy server URL |
| `api_key` | `ENTELECHY_API_KEY` | — | API key for Entelechy Cloud |
...
| Failure | Behavior |
|---|---|
| Entelechy unavailable | `before_turn()` returns `""`, agent continues |
