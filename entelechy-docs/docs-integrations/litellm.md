---
sidebar_position: 1
title: "LiteLLM Persistent Memory with Entelechy | Integration Guide"
description: "Add persistent memory to any LLM application via LiteLLM and Entelechy. Universal integration — works with any model or provider with just a few lines of code."
---
...
- **Simple Integration** - Just configure, set defaults, enable, and use `entelechy_litellm.completion()`
- **Automatic Memory Injection** - Relevant memories are injected into prompts before LLM calls
- **Automatic Conversation Storage** - Conversations are stored to Entelechy for future recall (async by default for performance)
...
response = entelechy_litellm.completion(
    ...
    entelechy_query="What do I know about AI discussions?",
)
```

**Important:** When `inject_memories=True` (default), you can provide `entelechy_query` to specify what to search for in memory. If omitted, the last user message is used as the query.

## How It Works

When you call `completion()`, the following happens automatically:

1. **Memory Retrieval** - Entelechy is queried for relevant memories based on the conversation
2. **Prompt Injection** - Memories are injected into the system message
3. **LLM Call** - The enriched prompt is sent to the LLM
4. **Conversation Storage** - The conversation is stored to Entelechy for future recall
...
entelechy_litellm.configure(
    # Required
    entelechy_api_url="http://localhost:8888",  # Entelechy API server URL

    # Optional - Authentication
    api_key="your-api-key",        # API key for Entelechy authentication
...
Default values for per-call settings. These can be overridden on individual calls using `entelechy_*` kwargs:
...
Override any default on individual calls using `entelechy_*` kwargs:

```python
response = entelechy_litellm.completion(
    model="gpt-4o-mini",
    messages=[...],
    entelechy_query="Where is Alice located?",      # Custom query for memory lookup
    entelechy_reflect_context="Currently on floor 3",  # Per-call reflect context override
    # entelechy_bank_id="other-bank",               # Override bank_id for this call
)
```
...
```python
entelechy_litellm.set_bank_mission(
...
entelechy_litellm.set_defaults(bank_id="my-agent", use_reflect=True)
...
entelechy_litellm.configure(entelechy_api_url="http://localhost:8888")
...
# OpenAI
entelechy_litellm.completion(model="gpt-4o", messages=messages, entelechy_query="greeting")

# Anthropic
entelechy_litellm.completion(model="claude-sonnet-4-20250514", messages=messages, entelechy_query="greeting")

# Groq
entelechy_litellm.completion(model="groq/llama-3.1-70b-versatile", messages=messages, entelechy_query="greeting")

# Azure OpenAI
entelechy_litellm.completion(model="azure/gpt-4", messages=messages, entelechy_query="greeting")

# AWS Bedrock
entelechy_litellm.completion(model="bedrock/anthropic.claude-3", messages=messages, entelechy_query="greeting")

# Google Vertex AI
entelechy_litellm.completion(model="vertex_ai/gemini-pro", messages=messages, entelechy_query="greeting")
```
...
```python
from entelechy_litellm import configure, set_defaults, reflect
...
```python
from entelechy_litellm import configure, set_defaults, retain, get_pending_retain_errors
...
```python
from entelechy_litellm import arecall, areflect, aretain
...
```python
from entelechy_litellm import configure, set_defaults, enable, completion, get_last_injection_debug

configure(entelechy_api_url="http://localhost:8888", verbose=True)
...
response = completion(
    ...
    entelechy_query="What is the user's favorite color?",
)
...
```python
from entelechy_litellm import disable, cleanup
...
|----------|-------------|
| `configure(...)` | Configure static Entelechy settings (API URL, auth, storage options) |
...
| `is_configured()` | Check if Entelechy is configured with a bank_id |
...
Streaming responses (`stream=True`) are fully supported. When streaming is detected, the response is automatically wrapped to collect chunks as they are consumed. Once the stream is fully consumed (or the context manager exits), the complete conversation is stored to Entelechy.
