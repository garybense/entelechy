...
let apiUrl = process.env.ENTELECHY_CP_DATAPLANE_API_URL;
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === '--port' || args[i] === '-p') {
    ...
    console.log(`
Entelechy Control Plane

Usage: entelechy-control-plane [options]

Options:
  -p, --port <port>       Port to listen on (default: 9999, env: PORT)
  -H, --hostname <host>   Hostname to bind to (default: 0.0.0.0, env: HOSTNAME)
  -a, --api-url <url>     Entelechy API URL (env: ENTELECHY_CP_DATAPLANE_API_URL)
  -h, --help              Show this help message

Environment Variables:
  PORT                              Port to listen on
  HOSTNAME                          Hostname to bind to
  ENTELECHY_CP_DATAPLANE_API_URL    URL of the Entelechy API server
`);
    process.exit(0);
  }
}
...
if (apiUrl) {
  env.ENTELECHY_CP_DATAPLANE_API_URL = apiUrl;
}

console.log(`Starting Entelechy Control Plane on http://${hostname}:${port}`);
if (apiUrl) {
