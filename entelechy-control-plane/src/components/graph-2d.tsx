...
export function convertEntelechyGraphData(entelechyData: {
  ...
  const nodes: GraphNode[] = (entelechyData.nodes || []).map((n) => {
    const tableRow = entelechyData.table_rows?.find((r) => r.id === n.data.id);
  ...
  const links: GraphLink[] = (entelechyData.edges || []).map((e) => ({
