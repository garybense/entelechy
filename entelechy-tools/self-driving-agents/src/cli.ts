...
function enableKnowledgeTools(): void {
  ...
  if (!config) return;
  const pc = config.plugins?.entries?.["entelechy-openclaw"]?.config;
  if (!pc) return;
...
function getInstalledPluginVersion(): string | null {
  try {
    ...
    const extDir = join(homedir(), ".openclaw", "extensions", "entelechy-openclaw");
...
function isPluginInstalled(): boolean {
  ...
  const hasConfig =
    config.plugins?.entries?.["entelechy-openclaw"]?.enabled !== false &&
    config.plugins?.entries?.["entelechy-openclaw"] !== undefined;
...
function isPluginConfigured(): boolean {
  ...
  if (!config) return false;
  const pc = config.plugins?.entries?.["entelechy-openclaw"]?.config || {};
  return !!(pc.entelechyApiUrl || pc.embedVersion || pc.llmProvider);
}
...
function resolveFromPlugin(agentId: string): { apiUrl: string; bankId: string; apiToken?: string } {
  ...
  const apiUrl = pc.entelechyApiUrl || `http://localhost:${pc.apiPort || 9077}`;
  const apiToken = pc.entelechyApiToken || undefined;
...
function getPluginSummary(): string {
  ...
  const pc = config.plugins?.entries?.["entelechy-openclaw"]?.config || {};
  if (pc.entelechyApiUrl) return `External: ${pc.entelechyApiUrl}`;
...
async function ensurePlugin(): Promise<void> {
  ...
  if (needsInstall || needsUpgrade) {
    if (needsUpgrade) {
      p.log.warn(
        `Entelechy plugin v${currentVersion} is outdated (need >=${MIN_PLUGIN_VERSION}). Upgrading...`
      );
    } else {
      p.log.warn("Entelechy plugin not found. Installing...");
    }
    try {
      ...
      if (cfg?.plugins?.slots?.memory === "entelechy-openclaw") {
    ...
    p.log.info(`Entelechy plugin v${currentVersion}`);
  }
  ...
  if (!isPluginConfigured()) {
    ...
    if (process.stdin.isTTY) {
      const ok = await p.confirm({
        message: `Entelechy: ${color.cyan(summary)}. Use this?\n${color.dim("  Changing this will affect all existing agents — one OpenClaw instance shares a single Entelechy instance.")}`,
      });
      if (p.isCancel(ok)) {
        p.cancel("Cancelled.");
        process.exit(0);
      ...
      p.log.info(`Entelechy: ${color.cyan(summary)}`);
    }
  }
}
