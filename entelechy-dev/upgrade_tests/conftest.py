...
def _get_or_create_pg0():
    ...
    if _pg0_instance is None:
        _pg0_instance = EmbeddedPostgres(name="entelechy-upgrade-test", port=5560)
...
def llm_config():
    ...
    """
    provider = os.getenv("ENTELECHY_API_LLM_PROVIDER", "groq")
    api_key = os.getenv("ENTELECHY_API_LLM_API_KEY") or os.getenv("GROQ_API_KEY")
    model = os.getenv("ENTELECHY_API_LLM_MODEL", "llama-3.3-70b-versatile")
    ...
    if provider in providers_unsupported_by_old_versions and not api_key:
        pytest.skip(
            ...
            f"Set ENTELECHY_API_LLM_API_KEY to use a fallback provider."
        )
