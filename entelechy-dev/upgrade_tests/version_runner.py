...
Manages running different git versions of the Entelechy API for upgrade testing.
...
class VersionRunner:
    """
    Manages running a specific git version of the Entelechy API.
    ...
    def __init__(
        ...
        self.db_url = db_url
        self.port = port
        self.llm_provider = llm_provider or os.getenv("ENTELECHY_API_LLM_PROVIDER", "groq")
        self.llm_api_key = llm_api_key or os.getenv("ENTELECHY_API_LLM_API_KEY") or os.getenv("GROQ_API_KEY")
        self.llm_model = llm_model or os.getenv("ENTELECHY_API_LLM_MODEL", "llama-3.3-70b-versatile")
    ...
    def setup(self) -> None:
        ...
        self._temp_dir = tempfile.mkdtemp(prefix=f"entelechy-{self.version}-")
        ...
        api_path = self.work_dir / "entelechy-api"
        logger.info(f"Installing entelechy-api from {api_path}")
    ...
    def _get_venv_path(self) -> Path:
        ...
        if self._is_current:
            ...
            api_venv = self.work_dir / "entelechy-api" / ".venv"

            if (workspace_venv / "bin" / "entelechy-api").exists():
                return workspace_venv
            elif (api_venv / "bin" / "entelechy-api").exists():
                return api_venv
            else:
    ...
    def start(self) -> ServerInfo:
        ...
        entelechy_api_bin = venv_path / "bin" / "entelechy-api"

        if not entelechy_api_bin.exists():
            raise RuntimeError(f"entelechy-api binary not found at {entelechy_api_bin}")

        env = os.environ.copy()
        env.update(
            {
                "ENTELECHY_API_PORT": str(self.port),
                "ENTELECHY_API_DATABASE_URL": self.db_url,
                "ENTELECHY_API_HOST": "127.0.0.1",
                "ENTELECHY_API_LLM_PROVIDER": self.llm_provider,
                "ENTELECHY_API_LLM_API_KEY": self.llm_api_key or "",
                "ENTELECHY_API_LLM_MODEL": self.llm_model,
                "PYTHONUNBUFFERED": "1",
            }
        )
        ...
        # (entelechy-api loads .env with override=True which would override our env vars)
        if self._is_current:
            # Create a temp directory for HEAD to avoid workspace .env
            self._head_cwd = tempfile.mkdtemp(prefix="entelechy-head-cwd-")
            cwd = self._head_cwd
        else:
