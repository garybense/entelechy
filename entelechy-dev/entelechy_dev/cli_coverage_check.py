#!/usr/bin/env python3
"""
Check that the Entelechy CLI covers every operation AND every request-body
parameter in the OpenAPI spec.

Endpoint-level check
--------------------
For each ``operationId`` in ``entelechy-docs/static/openapi.json`` we require
that one of the following is true:
  1. A call of the form ``.<operation_id>(`` appears somewhere in
     ``entelechy-cli/src/**/*.rs``. The progenitor-generated Rust client
     methods are named identically to the OpenAPI ``operationId``, so this is
     a strong signal the CLI wires the endpoint.
  2. The ``operationId`` is listed in ``entelechy-cli/.openapi-coverage.toml``
...
that every property is one of:
  1. Present in ``entelechy-cli/src/main.rs`` as a clap command variant field
...
def get_repo_root() -> Path:
    ...
    for parent in (here, *here.parents):
        if (parent / "entelechy-cli").is_dir() and (parent / "entelechy-docs").is_dir():
            return parent
...
def main() -> None:
    root = get_repo_root()
    spec_path = root / "entelechy-docs" / "static" / "openapi.json"
    manifest_path = root / "entelechy-cli" / ".openapi-coverage.toml"
    cli_src_dir = root / "entelechy-cli" / "src"
    ...
    for op_id in unmapped_ops:
        errors.append(
            f"MISSING OP   {op_id}: not called from entelechy-cli/src/ and not in .openapi-coverage.toml [skip]"
        )
    ...
    print("Entelechy CLI OpenAPI coverage check")
    ...
    if errors:
        ...
        print(
            ...
            "an entry to entelechy-cli/.openapi-coverage.toml with a reason."
        )
        sys.exit(1)
