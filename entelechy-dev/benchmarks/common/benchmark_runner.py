...
def get_model_config() -> Dict[str, Dict[str, str]]:
    ...
    Returns:
        Dict with 'entelechy', 'answer_generation', and 'judge' keys,
        each containing 'provider' and 'model' info.
    """
    # Memory/Entelechy config (base config)
    memory_provider = os.getenv("ENTELECHY_API_LLM_PROVIDER", "groq")
    memory_model = os.getenv("ENTELECHY_API_LLM_MODEL", "openai/gpt-oss-120b")

    # Answer generation config (falls back to memory config)
    answer_provider = os.getenv("ENTELECHY_API_ANSWER_LLM_PROVIDER", memory_provider)
    answer_model = os.getenv("ENTELECHY_API_ANSWER_LLM_MODEL", memory_model)

    # Judge config (falls back to memory config)
    judge_provider = os.getenv("ENTELECHY_API_JUDGE_LLM_PROVIDER", memory_provider)
    judge_model = os.getenv("ENTELECHY_API_JUDGE_LLM_MODEL", memory_model)
...
def print_model_config():
    ...
    console.print(f"  Entelechy:         {config['entelechy']['provider']}/{config['entelechy']['model']}")
    console.print(
...
async def create_memory_engine() -> MemoryEngine:
    ...
    - ENTELECHY_API_DATABASE_URL (default: "pg0")
    - ENTELECHY_API_LLM_PROVIDER (default: "groq")
    - ENTELECHY_API_LLM_API_KEY
    - ENTELECHY_API_LLM_MODEL (default: "openai/gpt-oss-120b")
    - ENTELECHY_API_LLM_BASE_URL (optional)

    Returns:
        Initialized MemoryEngine instance
    """
    memory = MemoryEngine(
        db_url=os.getenv("ENTELECHY_API_DATABASE_URL", "pg0"),
        memory_llm_provider=os.getenv("ENTELECHY_API_LLM_PROVIDER", "groq"),
        memory_llm_api_key=os.getenv("ENTELECHY_API_LLM_API_KEY"),
        memory_llm_model=os.getenv("ENTELECHY_API_LLM_MODEL", "openai/gpt-oss-120b"),
        memory_llm_base_url=os.getenv("ENTELECHY_API_LLM_BASE_URL") or None,  # Use None to get provider defaults
    )
...
class LLMAnswerEvaluator:
    ...
    def __init__(self):
        ...
        Uses ENTELECHY_API_JUDGE_LLM_* env vars with fallback to ENTELECHY_API_LLM_* for
        benchmark-specific LLM configuration (separate from the API config system).
        """
        import os

        from entelechy_api.engine.llm_wrapper import LLMConfig

        self.llm_config = LLMConfig(
            provider=os.getenv("ENTELECHY_API_JUDGE_LLM_PROVIDER", os.getenv("ENTELECHY_API_LLM_PROVIDER", "openai")),
            api_key=os.getenv("ENTELECHY_API_JUDGE_LLM_API_KEY", os.getenv("ENTELECHY_API_LLM_API_KEY", "")),
            base_url=os.getenv("ENTELECHY_API_JUDGE_LLM_BASE_URL", os.getenv("ENTELECHY_API_LLM_BASE_URL", "")),
            model=os.getenv("ENTELECHY_API_JUDGE_LLM_MODEL", os.getenv("ENTELECHY_API_LLM_MODEL", "gpt-4o-mini")),
...
class BenchmarkRunner:
    ...
    def __init__(
        ...
        self.memory = memory or MemoryEngine(
            ...
            memory_llm_provider=os.getenv("ENTELECHY_API_LLM_PROVIDER", "groq"),
            memory_llm_api_key=os.getenv("ENTELECHY_API_LLM_API_KEY"),
            memory_llm_model=os.getenv("ENTELECHY_API_LLM_MODEL", "openai/gpt-oss-20b"),
            memory_llm_base_url=os.getenv("ENTELECHY_API_LLM_BASE_URL") or None,
        )
    ...
    async def apply_template(self, bank_id: str, manifest_path: str) -> None:
        ...
        """
        from entelechy_api.api.http import BankTemplateManifest
        from entelechy_api.models import RequestContext
    ...
    async def run(
        ...
        poller = None
        if wait_consolidation:
            from entelechy_api.worker.poller import WorkerPoller
