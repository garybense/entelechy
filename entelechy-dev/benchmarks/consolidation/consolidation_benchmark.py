...
async def run_consolidation_benchmark(
    ...
    if enable_detailed_logs:
        ...
        consolidation_logger = logging.getLogger("entelechy_api.engine.consolidation.consolidator")
...
async def main():
    ...
    console.print(f"  LLM Provider: {os.getenv('ENTELECHY_API_LLM_PROVIDER', 'not set')}")
    console.print(f"  LLM Model: {os.getenv('ENTELECHY_API_LLM_MODEL', 'not set')}")

    # Check if consolidation is enabled
    config = get_config()
    if not config.enable_observations:
        console.print("\n[red]ERROR: Consolidation is disabled (enable_observations=False)[/red]")
        console.print("Set ENTELECHY_API_ENABLE_OBSERVATIONS=true to enable consolidation")
        return
    ...
    try:
        ...
        results = {
            ...
            "config": {
                ...
                "bank_id": bank_id,
                "llm_provider": os.getenv("ENTELECHY_API_LLM_PROVIDER"),
                "llm_model": os.getenv("ENTELECHY_API_LLM_MODEL"),
            },
            "metrics": metrics,
