...
class LoComoAnswerGenerator(LLMAnswerGenerator):
    ...
    def __init__(self):
        ...
        Uses ENTELECHY_API_ANSWER_LLM_* env vars with fallback to ENTELECHY_API_LLM_* for
        benchmark-specific LLM configuration (separate from the API config system).
        """
        self.llm_config = LLMConfig(
            provider=os.getenv("ENTELECHY_API_ANSWER_LLM_PROVIDER", os.getenv("ENTELECHY_API_LLM_PROVIDER", "openai")),
            api_key=os.getenv("ENTELECHY_API_ANSWER_LLM_API_KEY", os.getenv("ENTELECHY_API_LLM_API_KEY", "")),
            base_url=os.getenv("ENTELECHY_API_ANSWER_LLM_BASE_URL", os.getenv("ENTELECHY_API_LLM_BASE_URL", "")),
            model=os.getenv("ENTELECHY_API_ANSWER_LLM_MODEL", os.getenv("ENTELECHY_API_LLM_MODEL", "gpt-4o-mini")),
...
async def run_benchmark(
    ...
    if api_url:
        from benchmarks.common.benchmark_runner import EntelechyClientAdapter

        memory = EntelechyClientAdapter(base_url=api_url)
...
def generate_markdown_table(results: dict, use_reflect: bool = False):
    ...
    if "model_config" in results:
        ...
        lines.append("")
        lines.append(f"- **Entelechy**: {config['entelechy']['provider']}/{config['entelechy']['model']}")
        lines.append(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Entelechy API URL (default: use local memory, example: http://localhost:8888)",
    )
    parser.add_argument(
