...
Usage (run from entelechy-api/):
    cd entelechy-api

    # Generate a small bank (~10K memory units):
    uv run python ../entelechy-dev/benchmarks/perf/recall_perf.py generate \\
        --bank-id recall-perf-small2 --scale small

    # Benchmark recall:
    uv run python ../entelechy-dev/benchmarks/perf/recall_perf.py benchmark \\
        --bank-id recall-perf-small --query "database migration" --iterations 5

    # Clean up:
    uv run python ../entelechy-dev/benchmarks/perf/recall_perf.py clean \\
...
# Capture DB URL early before entelechy_api imports trigger dotenv override
# (config.py uses load_dotenv(override=True) which stomps env vars)
_EARLY_DB_URL = os.environ.get("ENTELECHY_API_DATABASE_URL")
...
def _build_engine(*, disable_observations: bool = False) -> "Any":
    ...
    db_url = _EARLY_DB_URL or os.getenv("ENTELECHY_API_DATABASE_URL", "pg0")
    if disable_observations:
        os.environ["ENTELECHY_API_ENABLE_OBSERVATIONS"] = "false"
    engine = MemoryEngine(
        db_url=db_url,
...
class _RRFReranker:
    ...
    async def rerank(self, query: str, candidates: list) -> list:
        from entelechy_api.engine.search.types import ScoredResult
