...
Usage:
    # Single file
    uv run python entelechy-dev/benchmarks/perf/retain_perf.py --document <file_path> [options]

    # Directory (batches all files)
    uv run python entelechy-dev/benchmarks/perf/retain_perf.py --document <dir_path> [options]
"""
...
async def stress_test_deadlocks(
    ...
    if max_retain_concurrent is not None:
        os.environ["ENTELECHY_API_RETAIN_MAX_CONCURRENT"] = str(max_retain_concurrent)

    # Enable logging so deadlock retry warnings are visible
    import logging

    logging.basicConfig(level=logging.WARNING, format="%(levelname)s %(name)s: %(message)s")
    logging.getLogger("entelechy_api.engine.db_utils").setLevel(logging.DEBUG)
    ...
    from entelechy_api.engine.retain.fact_storage import ensure_bank_exists
...
Examples:
  ...
  uv run python entelechy-dev/benchmarks/perf/retain_perf.py \\
  ...
  uv run python entelechy-dev/benchmarks/perf/retain_perf.py \\
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override ENTELECHY_API_RETAIN_MAX_CONCURRENT for this run (default: from config)",
    )
    parser.add_argument(
        "--stress",
        action="store_true",
    ...
    if args.max_retain_concurrent is not None:
        os.environ["ENTELECHY_API_RETAIN_MAX_CONCURRENT"] = str(args.max_retain_concurrent)
